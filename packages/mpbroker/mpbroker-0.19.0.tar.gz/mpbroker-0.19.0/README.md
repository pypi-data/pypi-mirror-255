...
- package updates
- added ability to clear sources via update
- removed IngestLog capability (caused more problems than it was worth and increased ingest time), Ingest results (# added, # updated, issues) are now local and not stored in the db
- better/complete implementation of extract-metadata (you can now specify none, new, update or both to only extract metadata for new, update, or both
- general code cleanup: database connection issues are captured showing an issue for the item(s) in question but it no longer stops the complete ingest process
