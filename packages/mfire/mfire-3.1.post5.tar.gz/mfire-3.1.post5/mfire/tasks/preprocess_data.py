...
from mfire.utils import Parallel, exception
from mfire.utils import mfxarray as xr
from mfire.utils import xr_utils
from mfire.utils.date import Datetime, Timedelta
from mfire.utils.xr_utils import compute_grib_step_size
...
class DataPreprocessor:
    ...
    def extract_param_from_grib(
        ...
        try:
            ...
            LOGGER.error(
                "Failed to extract parameter from grib",
                exc_info=True,
                **log_kwargs,
            )
            return None
        ...
        new_attrs = {"units": backend_kwargs["filter_by_keys"].pop("units", None)}
    ...
    def concat_dataarrays(
        ...
        for elt in sorted(dict_das["file"], key=lambda x: x["da"].valid_time.values):
            ...
            if source_grid != preproc_grid:
                ...
                da = xr_utils.interpolate_to_new_grid(elt["da"], preproc_grid)
                das.append(da)
            else:
                das.append(elt["da"])
        LOGGER.debug(
        ...
        if accum is not None:
            ...
            da_step = compute_grib_step_size(das)
            da_out = xr_utils.compute_sum_future(
                concat_da,
                da_step,
                step_out=accum,
    ...
    def preprocess(self, nproc: int) -> None:
        ...
        for file_conf in stack:
            parallel.apply(
                ...
                task_name=file_conf["name"],
                args=(file_conf,),
                callback=append_result,
            )
        parallel.run(timeout=Settings().timeout)
        parallel.clean()
        # solution brutale pour limiter l'utilisation mémoire (en attendant mieux)
