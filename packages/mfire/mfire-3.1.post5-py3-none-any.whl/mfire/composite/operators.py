...
class LogicalOperator(Operator):
    ...
    @staticmethod
    def or_operator(
        first_field: Union[xr.DataArray, xr.Dataset],
        second_field: Union[xr.DataArray, xr.Dataset],
    ) -> bool:
        """
        Takes the maximum of the given fields at each point.
        """
    ...
    def and_operator(
        ...
        """
        Takes the minimum of the given fields at each point.
        """
        if first_field.count().equals(second_field.count()):
...
class ComparisonOperator(Operator):
    ...
    def critical_value(
        self, val: Union[xr.DataArray, xr.Dataset], dim: str
    ) -> Optional[Union[xr.DataArray, xr.Dataset]]:
        if not self.is_order:
            return None
        return val.max(dim) if self.is_increasing_order else val.min(dim)
