...
class Localisation:
    ...
    def _compute_summarized_handler(self):
        ...
        if occurence.sum() == 0:
            raise LocalisationWarning("Spatialy localised occurrence is empty.")
        cover = (
            self.spatial_ingredient.localised_area.sel(id=occurence.id).count()
            / self.spatial_ingredient.domain.count()
        )
        ...
        try:
            ...
            # methode sans complémentaire et donc ne couvrant pas tout l'axe
            # si l'ensemble des zones ne couvrent pas l'axe
            # il faut avoir une zone sans risque
            if cover < GENERALISATION_THRESHOLD:
                if len(occurence.id) > 2:
                    # mettre sans risque dans la dernière zone
                    table_3p = table_3p.copy()
                    table_3p[:, -1] = 0
                    LOGGER.info(
                        f"truncated last zone{occurence.id[-1]}"
                        f" to avoid general axe {self.geo_id}"
                    )
                    occurence = occurence[:-1]
                else:
                    # mettre sans risque dans une zone ajoutée
                    zeroda = xr.DataArray(
                    ...
                    zeroda = zeroda.assign_coords(areaName=("id", ["Zone Zero"]))
                    zeroda = zeroda.assign_coords(areaType=("id", [""]))
                    table_3p = xr.concat([table_3p, zeroda], dim="id")

            # mettre en monozone si toutes les temporalités sont sur les mêmes zones
            # là où il y a un risque
            # et que ces zones recouvrent pratiquement tout l'axe
            period_identique = True
            for zones_idx in range(1, table_3p.sizes["period"]):
                if (table_3p[0, :] != table_3p[zones_idx, :]).any() and not (
                    0 == table_3p[zones_idx, :]
                ).all():
                    period_identique = False
                    break
            if period_identique:
                cover = (
                    self.spatial_ingredient.localised_area.sel(id=occurence.id).count()
                ...
                if cover > GENERALISATION_THRESHOLD:
                    raise LocalisationWarning(
                        "localised zones merely cover axe : shift to monozone"
                    )
            ...
            LOGGER.info(
                f"Failed to summarize localisation: {excpt}",
    ...
    def get_unique_name(self):
        ...
        """
        return self.summarized_handler.unique_name

    def get_unique_table(self):
        """
        Retourne le tableau résumé
        """
        return self.summarized_handler.unique_table
    ...
    def get_summarized_info(self):
        ...
        """
        return (
            self.summarized_handler.unique_table,
            self.summarized_handler.unique_name,
        )
    ...
    def get_level_type(self, risk_level=None):
        ...
        level = self.component.risks_of_level(level=risk_level)
        ...
        for evt in self.spatial_table.evt:
            ...
            if (
                ...
                dtmp["mountain"] = {
                    ...
                    "next_critical": dict_comparison[variable][
                        "mountain"
                    ].next_critical,
                    "value": value,
                    ...
                    "critical_hour": date,
                    "occurrence_event": self.spatial_table.sel(
                        evt=evt
                    ).occurrence_event,
                }
            if (
                ...
                dtmp["plain"] = {
                    ...
                    "next_critical": dict_comparison[variable]["plain"].next_critical,
                    "value": value,
                    "units": self.spatial_table.sel(evt=evt)[
                        "rep_value_plain"
                    ].attrs.get("units", None),
            ...
            # dictionnaire
            if bool(dtmp) and variable not in dout:
                dout[variable] = dtmp
            elif bool(dtmp):
    ...
    def perform_spatial_localisation(self):
        ...
        Args:
            ...
            geo_id ([type]): [description]
            period ([type]): [description]
        return coverage of selected zones over axe
        """
    ...
    def auto_save(self, **kwargs):
        ...
        self.summarized_handler.save(repo)
