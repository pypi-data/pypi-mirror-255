        ...
        sql_latest_requests = f"""
                                ...
                                FROM (
                                    ...
                                    WHERE
                                        ...
                                        AND error = 0
                                        AND not mapContains(parameters, '__tb__semver')
                                        {" AND " + " AND ".join([f"mapContains(params, '{match}')" for match in matches]) if matches and len(matches) > 0 else ''}
                                        {extra_where_clause}
                                    LIMIT {limit}
                                )
...
async def folder_push(
    ...
    def is_new(name, changed, back_dep_map, dep_map):
        ...
        if back_deps := back_dep_map.get(name):
            for dep in back_deps:
                if dep in dep_map and not is_git_new(dep):
                    return False
    ...
    async def push_files(dry_run: bool = False):
        ...
        for group in groups:
            for name in group:
                if not is_new(name, changed, back_dep_map, dep_map) or not fork_downstream:
                    continue
                await push(
