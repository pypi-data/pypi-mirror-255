...
def create_patches_for_type(images_folder_path, patch_size, overlap, crop_start, cond_folder_path=None):
    '''
    ...
    for image in images_folder.iterdir():
        if image.is_file():
            ...
            if "nd2" in image.suffix:
                ...
                tensor_img = transforms.ToTensor()(np.array(img, np.float32))
            patches.append(crop_to_patches(tensor_img, top, left, patch_height, patch_width, overlap)) #maia's function
    patches_all = torch.cat(patches)

    # Load conditioning
    if(cond_folder_path is not None):
        cond_patches = []
        cond_folder = PosixPath(cond_folder_path)

        # Load images
        for cond_image in cond_folder.iterdir():
            if cond_image.is_file():
                image_path = cond_image.__fspath__()
                if "nd2" in cond_image.suffix:
                    nd = ND2Reader(image_path)
                    img_arr = nd.get_frame(0)
                    tensor_img = torch.tensor(np.array([np.int16(img_arr)]))
                else:
                    ...
                    tensor_img = transforms.ToTensor()(np.array(img, np.float32))
                cond_patches.append(crop_to_patches(tensor_img, top, left, patch_height, patch_width, overlap))  # maia's function

        cond_patches_all = torch.cat(cond_patches)
        return patches_all, cond_patches_all
    else:
        return patches_all, torch.tensor([])
