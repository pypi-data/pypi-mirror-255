document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildTooltip(termeEnrichi, classlist) {
        ...
        let ref = ''
        const idref = dataset.idwiki || dataset.idbnf || dataset.idorcid
        if (idref) {
    ...
    const indexes = document
        .querySelector('meta[property="pressoir:indexes"]')
        .getAttribute('content')
    const indexesSelector = JSON.parse(indexes)
        .map((index) => `span.${index}`)
        .join(',')
