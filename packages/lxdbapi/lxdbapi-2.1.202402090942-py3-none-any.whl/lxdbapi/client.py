...
from lxdbapi.network_avatica_connection import HttpAvaticaConnection, TcpAvaticaConnection, NetAvaticaConnection
...
ONLY_CLIENT_PROPERTIES = ('distribute', 'istcp')
...
class AvaticaClient(object):
    ...
    def checkSerialization(self):
        ...
        if isinstance(self.connection, HttpAvaticaConnection):
            self.connection.checkSerialization()
        else:
            res = self.serve_lx_action('SERVER_INFO')
            if res:
                error = res.pop('ERROR', None)
                if error is None:
                    return
                raise RuntimeError('Cannot obtain Server info: ' + error)
            raise IOError('Cannot obtain Server info')
    def close(self):
        if self.connection is not None:
            self.connection.close()

    def _apply(self, request_data, expected_response_type_input=None):
        ...
        if expected_response_type_input is None:
            expected_response_type = request_name.replace('Request', 'Response')
        else:
            expected_response_type = expected_response_type_input

        body = req_message.SerializeToString()
        response_body = self.connection.request(body)
        if response_body is None:
            raise IOError("IO Error on [{}] while expecting a response [{}]".format(self.url, expected_response_type))
        ...
        elif resp_message.name != expected_response_type:
            raise errors.InterfaceError(
                'unexpected response type [{}]. Expecting: [{}]'.format(resp_message.name, expected_response_type))
    ...
    def open_connection(self, connection_id, info=None):
        ...
        if info is not None:
            ...
            request.info["LXJDBCVERS"] = str(versions.MAJOR_VERSION) + "." + str(versions.MINOR_VERSION)
    ...
    def serve_lx_action(self, action, params=None):
        ...
        if params is not None:
            ...
            for k, v in params.items():
                ...
                param.key = k
                param.value = native_2_typed_value(v)
        ...
        if response and response.params:
            for info in response.params:
                result[info.key] = typed_value_2_native(info.value)
        else:
