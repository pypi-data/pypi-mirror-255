...
from nereid.globals import current_app, session
...
class Cart(ModelSQL, ModelView):
    ...
    @classmethod
    @route("/cart.json", methods=["GET"])
    def cart_json(cls):
        """
        Returns a JSON state of the cart
        """
        cart = cls.open_cart()

        # Provide a sensible default to facilitate downstream implementations,
        # e.g. relying on lines, when no cart.sale exists yet
        rv = {
            'cart': {
                #'name': current_user.rec_name,
                'lines': [],
                'empty': True,
                'total_amount': 0,
                'tax_amount': 0,
                'untaxed_amount': 0,
                'size': '%s' % cls.cart_size(),
                },
            }
        if cart.sale:
            ...
            rv['cart']['lines'].extend([
                    line.serialize(purpose='cart')
                    for line in cart.sale.lines
                    ])
            rv['cart']['empty'] = len(cart.sale.lines) == 0
            rv['cart']['total_amount'] = (
                currency_format(cart.sale.total_amount))
            rv['cart']['tax_amount'] = currency_format(cart.sale.tax_amount)
            rv['cart']['untaxed_amount'] = (
