...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import \
    ...
    InvalidInputDatatype, LargeDataAlgorithmsUnsupported, NotSupported, \
...
from azureml.train.automl._environment_utilities import (
    get_curated_environment_label,
    get_curated_environment_scenario,
    validate_non_prod_env_exists
)
...
def _start_execution(
    ...
    log_server.update_custom_dimensions({
        ...
        "workspace_id": experiment.workspace._workspace_id
    })
    ...
    if settings_obj.spark_context is not None:
        ...
        # Only attempt to remove scenario from AutoML settings if it is not set to value used in candidate validation.
        elif settings_obj.scenario not in constants.Scenarios.CANDIDATE_VALIDATION_SET:
            # Check to make sure that the environment with scikit-learn==1.1.3 is released
            latest_env = Environment.get(experiment.workspace, "AzureML-AutoML", label="prod")
            if any(pk in latest_env.docker.base_dockerfile for pk in (
                   'scikit-learn=1.0.2')) and not settings_obj.force_curated_environment:
                settings_obj.scenario = None
                msg = ("This run may take 30 minutes longer than usual "
                       "because it is building a training environment.")
                logger.info(msg)
                print(msg)
        automl_run = _default_execution(experiment, settings_obj, fit_params, False, show_output)
...
class AutoMLConfig(object):
    ...
    def __init__(self,
        ...
        if kwargs.get('use_distributed', False):
            ...
            elif 'node_count' in kwargs:
                # currently we are deprecating node_count in favor of max_nodes. This enables that transition
                max_nodes = kwargs['node_count']
            if max_nodes <= 1:
                logging.warning(
                    "max_nodes will be used to determine cluster size for featurization and training. "
                    "It is recommended to use a value larger than 1 but no larger than max size of the cluster."
                )
                raise ConfigException._with_error(
                    AzureMLError.create(
                        NotSupported, scenario_name="max_nodes<=1"
                    )
                )

            if task == constants.Tasks.REGRESSION and self.is_timeseries:
                param_override_msg = (
                    "Enabling distributed experiment will result in the following parameter overrides:\n"
        ...
        if self.is_timeseries:
            if validation_data is None and X_valid is None and n_cross_validations is None and \
                    not kwargs.get("use_distributed", False):
        ...
        self.user_settings["force_curated_environment"] = kwargs.get('force_curated_environment', False)
        self.user_settings[EnvironmentSettings.SCENARIO] = get_curated_environment_scenario(kwargs, task)
        self.user_settings[EnvironmentSettings.ENVIRONMENT_LABEL] = get_curated_environment_label(kwargs)
    ...
    def as_serializable_dict(self) -> Dict[str, Any]:
        ...
        settings_dict = {k: v for (k, v) in self.user_settings.items() if k not in fit_params}
        return _azureautomlsettings.AzureAutoMLSettings(**settings_dict).as_serializable_dict()
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        if self.user_settings.get('validation_data', None) is not None and \
                (self.user_settings.get('training_data', None) is None
                 or self.user_settings.get(label_column_name_param_name, None) is None):
            whats_none, ref_code = ("training_data", ReferenceCodes._AUTOML_CONFIG_TRAINING_DATA_MISSING) \
                if self.user_settings.get('training_data') is None \
                else ("label_column_name", ReferenceCodes._AUTOML_CONFIG_LABEL_COL_NAME_MISSING)
            raise ConfigException._with_error(
        ...
        if self.user_settings.get('use_distributed') and \
                self.user_settings.get('n_cross_validations') is not None:
            raise ConfigException._with_error(
                AzureMLError.create(
                    ConflictingValueForArguments, target="use_distributed",
                    arguments=', '.join(['use_distributed', 'n_cross_validations'])
                )
            )
    ...
    def _validate_streaming_models(self, is_timeseries: bool, compute_target: str) -> None:
        ...
        if effective_models_all_streaming:
            raise ConfigException._with_error(
                AzureMLError.create(LargeDataAlgorithmsUnsupported))
        ...
        if allowed_models:
            ...
            if allowed_models_some_streaming:
                logger.warning(AutoMLErrorStrings.LARGE_DATA_ALGORITHMS_UNSUPPORTED)
