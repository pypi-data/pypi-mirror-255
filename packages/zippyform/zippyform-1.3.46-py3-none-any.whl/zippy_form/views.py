...
def update_form_recaptcha_details(req, form_id):
    ...
    if serializer.is_valid():
        ...
        msg = "Form ReCaptcha Details Updated Successfully"
...
def form_analytics(req, form_id, year):
    ...
    # Form Views
    form_views_queryset = FormPageViews.objects.filter(form_id=form_id).filter(created_date__year=year).values('created_date__month').annotate(
    ...
    form_submissions_queryset =  FormSubmission.objects.filter(
        form_id=form_id, status=FORM_SUBMISSION_STATUS[1][0]).filter(created_date__year=year).values('created_date__month').annotate(
        views=Count('id')).order_by('created_date__month')
...
def update_form_submission_meta_data_value(req, form_id, submission_id, meta_data_id):
   ...
   try:
       form_submission = FormSubmission.objects.filter(form=form_id).get()
   except FormSubmission.DoesNotExist:
       msg = "Invalid Form Submission ID"
       return Response({"status": "error", "msg": msg}, status=status.HTTP_400_BAD_REQUEST)
