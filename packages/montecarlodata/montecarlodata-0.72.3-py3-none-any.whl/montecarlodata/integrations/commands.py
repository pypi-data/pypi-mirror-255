...
def add_mariadb(ctx, name, **kwargs):
    ...
    """
    TransactionalOnboardingService(config=ctx["config"]).onboard_transactional_db(
        warehouseName=name, dbType=MARIADB_DB_TYPE, **kwargs
    )
...
def add_teradata(ctx, name, **kwargs):
    """
    Onboard a Teradata connection
    """
    TransactionalOnboardingService(config=ctx["config"]).onboard_transactional_db(
        warehouseName=name, dbType=TERADATA_DB_TYPE, **kwargs
    )
...
@add_common_options(DISAMBIGUATE_DC_OPTIONS)
def create_role(ctx, file, aws_profile, dc_id):
    """
    ...
    ).create_role(path_to_policy_doc=file, dc_id=dc_id)
...
@click.option(
    ...
    required=True,
    type=click.Choice(["confluent-cloud", "self-hosted"]),
...
@click.option(
    "--connection-type",
    help="Streaming Connection Type to create. Right now only support ['confluent-kafka', 'confluent-kafka-connect', 'self-hosted-kafka', 'self-hosted-kafka-connect']",
    type=click.Choice(
        [
            "confluent-kafka",
...
def add_streaming_cluster_connection(ctx, **kwargs):
    StreamingOnboardingService(
        config=ctx["config"],
        mc_client=create_mc_client(ctx),
    ).onboard_confluent_cluster_connection(**kwargs)
