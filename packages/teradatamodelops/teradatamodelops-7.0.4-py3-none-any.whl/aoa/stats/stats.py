...
def compute_continuous_stats(features_df: DataFrame, continuous_features: List[str]):
    ...
    reference_edges = _compute_continuous_edges(features, stats, dtypes, bins=bins)
    edges_dict = dict(zip(features, reference_edges))
...
def compute_categorical_stats(features_df: DataFrame, categorical_features: List[str]):
    ...
    features = lowered_features
    for f in features:
        columns_list = list(statistics[f].keys())
