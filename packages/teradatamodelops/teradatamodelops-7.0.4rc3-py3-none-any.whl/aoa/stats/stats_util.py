...
def _compute_continuous_edges(variables: List[str], statistics: pd.DataFrame, dtypes: Dict, bins=10) -> List[float]:
    ...
    for var in variables:
        ...
        if x_max - x_min < bins and dtypes[var].startswith("int"):
            vals = np.linspace(x_min, x_max, int(x_max) - int(x_min) + 1).tolist()
            ...
            # We take precision from min, but reduce it so we can round down a minimum
            p = len(str(x_min).split(".")[1])

            # Rounding all internal edges to the same precision as the minimum
            for i in range(1, bins):
                vals[i] = round(vals[i], p)
            ...
            # does work with VAL)
            # 2024-02-05 This needs to be caught in the wild, I wasn't able to verify this
...
def _capture_stats(df: DataFrame,
    ...
    if len(continuous_vars) > 0:
        ...
        if len(available_continuous_vars) > 0:
            ...
            logger.debug(f"Executing this VAL call {valib.Histogram(data=df,columns=available_continuous_vars,boundaries=hist_boundaries).show_query()}")
            histogram = valib.Histogram(data=df,
                                        columns=available_continuous_vars,
                                        boundaries=hist_boundaries)
            histogram = histogram.result.to_pandas(all_rows=True).reset_index()
