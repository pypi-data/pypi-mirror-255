...
class Base:
    ...
    def auto_iterate(
        self, time_limit: int = 10, precision: float = 10e-3, batch_size: int = 10
    ) -> tuple[int, bool]:
        """Automatically iterates until the algorithm converges or reaches the time limit.

        Args:
            time_limit (int, optional): The maximum time, in seconds, after which no more iterations will be launched.
