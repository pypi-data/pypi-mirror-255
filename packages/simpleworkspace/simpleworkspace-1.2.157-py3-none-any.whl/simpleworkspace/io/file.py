...
def Read(filepath: str, readLimit:int=None, readOffset=0, type:_T=str, encoding:str=None) -> _T:
    """
    :readLimit: Max amount of bytes to read
    :type: specifies return type of str or bytes
    :encoding: only when reading with type str it might be suitable to use a custom encoding
    ...
    with open(filepath, openMode, newline=None, encoding=encoding) as fp:
        fp.seek(readOffset)
        return fp.read(readLimit)
    
def ReadIterator(filepath: str, readSize:int, readLimit:int=None, readOffset=0, type:_T=str, encoding:str=None) -> _Iterator[_T]:
    """
...
def Create(filepath: str, data: bytes | str = None, encoding:str="utf-8"):
...
def Append(filepath: str, data: bytes | str, encoding="utf-8"):
