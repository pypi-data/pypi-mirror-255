...
async def image_search_handler(app, message: Message):
    ...
    try:
        ...
        # start searching
        resp_hentai = await ehentai_image_search(file_bytes)
        resp_ascii = await ascii_image_search(file_bytes)
        if resp_ascii:
            msg_to_send = (
                f"**[{resp_ascii[0]['title']}]({resp_ascii[0]['url']})**\n"
                f"Author: [{resp_ascii[0]['author']}]({resp_ascii[0]['author_url']})\n"
            )
        if resp_hentai:
            # lakukan sync ke telegraph dari ehentai resp_hentai[0]['url']
            msg_to_send += (
                f"\nThe full version is found in :\n**{resp_hentai[0]['title']}**\n"
                "initiating synchronization, Please wait.."
            )

        resp = resp_ascii + resp_hentai
...
def get_handlers():
    pyrogram_handler = {
        ...
        "image_search": MessageHandler(image_search_handler, filters.photo & filters.private & filters.incoming)
    }
