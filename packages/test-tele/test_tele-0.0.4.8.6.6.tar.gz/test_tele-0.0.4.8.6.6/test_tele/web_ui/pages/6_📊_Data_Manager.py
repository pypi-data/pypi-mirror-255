...
def save_datas(table, conn, keys, data_iter):
    ...
    query.delete_data('links', 'type = ?', [data[-1]['type']])
    for x in data:
        try:
...
def update_links(url = None, type = None, notes = None):
    ...
    if url:
        if not query.read_datas('links', None, 'url = ?', [url]):
        ...
        query.delete_data('links', 'url = ?', [' '])
        

@st.cache_resource(show_spinner="Loading channel")
def get_data_channels():
    query = dbh.Query()
    pchannels = query.read_datas('links', None, 'type = "channel"')
    if pchannels:
        list_pchannels = []
        pchannels_dict = {}
...
def get_data_stickers():
    query = dbh.Query()
    stickers = query.read_datas('links', None, 'type = "sticker"')
    if stickers:
        list_stickers = []
        stickers_dict = {}
...
def get_data_addlists():
    query = dbh.Query()
    addlists = query.read_datas('links', None, 'type = "addlist"')
    if addlists:
        list_addlists = []
        addlists_dict = {}
