...
new_open = lambda fp: open(fp, mode="r", encoding="utf-8", errors="ignore").read()
...
def load_documents():
    ...
    for fp, _isfile in valid_files:
        documents.pop(fp)
        documents[Path(fp).stem] = new_open(MAIN_DIR / fp)
    return documents


# XXX Markdown & Document Files
MAIN_DIR = Path(__file__).parents[2].resolve()
VERSION, AUTHOR, CRIGHT, SUMMARY, URL, LICENSE, DOC = __version__, __author__, __copyright__, __summary__, __url__, __license__, __doc__  # type: ignore
(
    README,
    CHANGELOG,
    CLI_OPTIONS_DOC,
...
def cli_parser():
    ...
    if args.source:
        ...
        cli_str, cli_doc = CLI_OPTIONS_DOC
        if not cli_doc:
            for _idx, aparser in enumerate(
                ...
                aparser.print_help(
                    file=open(
                        MAIN_DIR / cli_str, mode="a", encoding="utf-8", errors="ignore"
                    )
                )
            print(new_open(cli_str).read())
        elif len(cli_doc) >= 1:
            print(cli_doc)
        else:
            sys.exit(0)
...
__all__ = ("cli_interface", CHANGELOG, SOURCE_CODE, CLI_OPTIONS_DOC, PYTESTS_DOC)
