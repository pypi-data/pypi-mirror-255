...
def __parse_grid_options(
    gridOptions_parameter, dataframe, default_column_parameters, unsafe_allow_jscode
):
...
def AgGrid(
    ...
    **default_column_parameters:
    ...
    if width:
        warnings.warn(
            DeprecationWarning(
                "Width parameter is deprecated and will be removed on next version."
            )
        )
    ...
    if not isinstance(data_return_mode, (str, DataReturnMode)):
        raise ValueError(
            f"DataReturnMode should be either a DataReturnMode enum value or a string."
        )
    ...
    if not isinstance(update_mode, (str, GridUpdateMode)):
        raise ValueError(
            f"GridUpdateMode should be either a valid GridUpdateMode enum value or string"
        )
    ...
    if not (
        isinstance(excel_export_mode, (str, ExcelExportMode))
        and (excel_export_mode in ExcelExportMode)
    ):
        raise ValueError(
            f"{excel_export_mode} is not valid. Available options: {ExcelExportMode.__members__}"
        )
    else:
