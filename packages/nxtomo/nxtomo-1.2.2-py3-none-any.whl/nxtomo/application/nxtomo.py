...
from nxtomo.utils import get_data, get_data_and_unit
    ...
    def __init__(self, node_name: str = "", parent: Optional[NXobject] = None) -> None:
        if node_name not in (None, ""):
            deprecated_warning(
                type_="parameter",
                name="node_name",
                replacement="None - has been removed",
                since_version="1.1",
                reason="node_name has been removed to simplify usage. Location must be provided directly in the 'save' function using the 'data_path' parameter",
            )
