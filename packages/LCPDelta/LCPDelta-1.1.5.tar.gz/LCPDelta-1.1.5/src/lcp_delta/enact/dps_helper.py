...
class DPSHelper:
    ...
    def initialise_series_subscription_data(self, series_id : str, country_id : str, option_id : list[str], handle_data_method : callable, parse_datetimes : bool):
        ...
        initial_series_data = self.api_helper.get_series_data(series_id, day_start, now, country_id, option_id, parse_datetimes=parse_datetimes)
    ...
    def handle_new_series_data(self, all_data : pd.DataFrame, data_push_holder : list, parse_datetimes : bool) -> pd.DataFrame:
        try:
            ...
            for push in pushes:
                ...
                push_values = push_current['arrayPoint'][1:] if not push['byPoint'] else list(push_current['objectPoint'].values())
    ...
    def subscribe_to_series_updates(self, handle_data_method : callable, series_id : str, option_id : list[str] = None, country_id = 'Gb', parse_datetimes : bool = False) -> None:
        '''
        ...
        (handle_data_old, initial_data_from_series_api, parse_datetimes_old) = self.data_by_subscription_id.get(subscription_id, (None, pd.DataFrame(), False))
        if initial_data_from_series_api.empty:
            self.initialise_series_subscription_data(series_id, country_id, option_id, handle_data_method, parse_datetimes)
        else:
