        ...
        try:
            while urls:
                for url in urls:
                    ...
                    # we get df from parquet files where datetimes are serialized
                    ...
                    # strings are represented as binaries so need to decode it as well
                    for c in [c for c, t in self.column_types.items() if t == String]:
                        df[c] = df[c].str.decode("utf-8")

                    inserted = data_storage.insert_dataset_rows(
                        df, self.shadow_dataset_name
                    )
...
class Catalog:
    ...
    def export_dataset_table(
        ...
        return self.data_storage.export_dataset_table(
            bucket_uri, dataset, version, client_config
        )

    def dataset_table_export_file_names(self, name: str, version: int) -> List[str]:
        dataset = self.get_dataset(name)
        return self.data_storage.dataset_table_export_file_names(dataset, version)
        ...
        *,
        ...
        if signed_urls:
            ...
            try:
                ...
                self.data_storage.remove_shadow_dataset(shadow_dataset)
                raise
