...
class DataSource:
    ...
    def find(self, *, sort=None):
        ...
        if self.node.dir_type == DirType.TAR_ARCHIVE or self.node.vtype != "":
            include_subobjects = True
        else:
            include_subobjects = False
        return self.listing.data_storage.get_subtree_files(
            self.node, sort=sort, include_subobjects=include_subobjects
        )
