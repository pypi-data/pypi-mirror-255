...
def get_partial_indexed_paths(storage):
    p = storage.partials
    return [
        r[0] for r in storage.mdb.execute(select(p.c.path_str).order_by(p.c.path_str))
    ]
...
def test_ls_partial_indexing(cloud_test_catalog, capsys):
    ...
    sleep(0.05)
    captured = capsys.readouterr()
    assert get_partial_indexed_paths(src_storage) == ["cats/", "dogs/", "dogs/others/"]
    assert "Listing" in captured.err
