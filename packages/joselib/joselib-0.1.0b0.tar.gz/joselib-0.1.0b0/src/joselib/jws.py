...
from joselib.backends import Key
from joselib.constants import ALGORITHMS
from joselib.exceptions import JWSError, JWSSignatureError
from joselib.utils import base64url_decode, base64url_encode
...
def sign(payload, key, headers=None, algorithm=ALGORITHMS.HS256):
    ...
    return _sign_header_and_claims(encoded_header, encoded_payload, algorithm, key)
...
def _verify_signature(signing_input, header, signature, key="", algorithms=None):
    alg = header.get("alg")
    if not alg:
        msg = "No algorithm was specified in the JWS header."
        raise JWSError(msg)
    ...
    try:
        ...
        msg = f"Invalid or unsupported algorithm: {alg}"
        raise JWSError(msg) from err
