...
from joselib.backends import get_random_bytes
from joselib.constants import ALGORITHMS, ZIPS
from joselib.exceptions import JWEError, JWEParseError
...
def encrypt(
    ...
    if encryption not in ALGORITHMS.SUPPORTED:
        msg = f"Algorithm {encryption} not supported."
        raise JWEError(msg)
    ...
    return _jwe_compact_serialize(encoded_header, enc_cek, iv, cipher_text, auth_tag)
...
def decrypt(jwe_str, key):
    ...
    try:
        ...
        if alg not in ALGORITHMS.SUPPORTED:
            msg = f"Algorithm {alg} not supported."
            raise JWEError(msg)
        if enc not in ALGORITHMS.SUPPORTED:
            msg = f"Algorithm {enc} not supported."
            raise JWEError(msg)

    except KeyError as err:
        msg = "alg and enc headers are required!"
        raise JWEParseError(msg) from err
    ...
    if alg == ALGORITHMS.DIR:
        ...
        try:
            ...
            msg = f"alg {alg} is not implemented"
    ...
    try:
        ...
        msg = f"enc {enc} is not implemented"
        raise JWEError(msg) from err
...
def get_unverified_header(jwe_str):
    ...
    """
    return _jwe_compact_deserialize(jwe_str)[0]
...
def _get_encryption_key_mac_key_and_key_length_from_cek(cek_bytes, enc):
    ...
    mac_key_bytes = cek_bytes[:derived_key_len]
...
def _jwe_compact_deserialize(jwe_bytes):
    ...
    try:
        ...
        msg = "Invalid cyphertext"
        raise JWEParseError(msg) from err

    try:
        auth_tag = base64url_decode(auth_tag_segment)
    except (TypeError, binascii.Error) as err:
        msg = "Invalid auth tag"
        raise JWEParseError(msg) from err
...
def _get_hmac_key(enc, mac_key_bytes):
    ...
    return jwk.construct(mac_key_bytes, hash_alg)
...
def _compress(zip, plaintext):
    ...
    if zip not in ZIPS.SUPPORTED:
        msg = f"ZIP {zip} is not supported!"
        raise NotImplementedError(msg)
    if zip is None:
...
def _get_direct_key_wrap_cek(key):
    ...
    if jwk_data["kty"] == "oct":
        ...
        msg = f"JWK type {kty} is not supported!"
        raise NotImplementedError(msg)
...
def _get_random_cek_bytes_for_enc(enc):
    ...
    if enc == ALGORITHMS.A128GCM:
        ...
        msg = f"enc {enc} is not supported!"
        raise NotImplementedError(msg)
    return get_random_bytes(num_bits // 8)
