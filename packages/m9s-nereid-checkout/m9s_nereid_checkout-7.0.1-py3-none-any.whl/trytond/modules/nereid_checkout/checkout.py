...
from wtforms import (
    BooleanField, PasswordField, RadioField, SelectField, StringField,
...
from nereid import (
    current_app, current_user, current_website, flash, jsonify, login_required,
    redirect, render_template, request, route, url_for)
...
class CreditCardForm(Form):
    owner = StringField(_('Full Name on Card'), [validators.DataRequired(), ])
    number = StringField(
...
class Checkout(ModelView):
    ...
    def allowed_as_guest(cls, email):
        """
        Check if provided email can checkout as guest or force user to sign in.

        - As current behaviour an existing user should sign in.
        - We check also for inactive (i.e. blocked) users
        """
        pool = Pool()
            ...
            if form.checkout_mode.data == 'guest':
                ...
                if cart.sale.party == current_website.guest_user.party:
                    ...
                    party.add_contact_mechanism_if_not_exists(
                            'email', form.email.data, usage='web')
            ...
            if not current_user.is_anonymous and request.form.get('address'):
                ...
                if not address_form.validate():
                    ...
                    if address_form.phone.data:
                        address.party.add_contact_mechanism_if_not_exists(
                            'phone', address_form.phone.data, 'web')
    ...
    def billing_address(cls):
        ...
        if request.method == 'POST':
            ...
            if not current_user.is_anonymous and request.form.get('address'):
                ...
                # Guest user or registered user creating or updating an address.
                # Only difference is that the party of address depends on guest
                # or not
                if not address_form.validate():
                    address = None
                else:
    ...
    def _process_payment(cls, cart):
        ...
        if not current_user.is_anonymous and \
            ...
            return cls._process_credit_card_payment(cart, credit_card_form)
    ...
    def _process_credit_card_payment(cls, cart, credit_card_form):
        ...
        payment = cart.sale._get_payment_for_gateway(gateway)
        if payment is None:
            cart.sale._add_sale_payment(credit_card_form=credit_card_form)
        return cls.confirm_cart(cart)
    ...
    def confirm_cart(cls, cart, do_redirect=True):
        ...
        flash(_("Your order has been processed"))
        if do_redirect:
    ...
    @classmethod
    @route('/checkout/add-sale-comment', methods=['POST'])
    @not_empty_cart
    def add_sale_comment(cls):
        """
        Add comment to the cart sale.
        ...
        if current_user.is_anonymous:
            ...
            if access_code and access_code == sale.guest_access_code:
                comment_is_allowed = True
        elif (current_user.is_authenticated
                and current_user.party == sale.party):
            comment_is_allowed = True
...
class Address(metaclass=PoolMeta):
    ...
    def create_address(cls):
        ...
        if request.method == 'POST' and form.validate_on_submit():
            ...
            if form.phone.data:
                party.add_contact_mechanism_if_not_exists(
                    'phone', form.phone.data, usage='web')
