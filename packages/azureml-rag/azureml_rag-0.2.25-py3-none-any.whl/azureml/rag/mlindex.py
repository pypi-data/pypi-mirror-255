...
class MLIndex:
        ...
        with track_activity(logger, "MLIndex.as_langchain_vectorstore") as activity_logger:
            ...
            if index_kind == "acs":
                    ...
                    if azureml_faiss_as_langchain_faiss is not None:
                        try:
                            store = azureml_faiss_as_langchain_faiss(
                                FaissAndDocStore.load(self.index_config.get("path", self.base_uri), embeddings.embed_query)
                            )
                        except Exception as e:
                ...
                try:
                    import pinecone
                    from azure.core.credentials import AzureKeyCredential
                    from azureml.rag.tasks.update_pinecone import get_pinecone_environment
                ...
                try:
                    ...
                    mongo_collection = mongo_client[self.index_config.get("database")][self.index_config.get("collection")]
    ...
    def set_embeddings_connection(
        self,
        connection: Optional[Union[str, Connection, WorkspaceConnection]],
        credential: Optional[TokenCredential] = None,
    ) -> "MLIndex":
    ...
    def from_embeddings_container(
        ...
        if index_type == "faiss":
            ...
            if not index_connection:
                ...
                index_config = {
                    ...
                    **{
                        ...
                        "api_version": get_metadata_from_connection(index_connection).get("apiVersion", "2023-07-01-preview"),
                    },
                }
                connection_args = {
                    "connection_type": "workspace_connection",
            ...
            from azureml.rag.tasks.update_milvus import create_index_from_raw_embeddings, try_override_milvus_config_with_connection_metadata, MILVUS_URI_KEY, MILVUS_COLLECTION_NAME_KEY
            if not index_connection:
                ...
                try_override_milvus_config_with_connection_metadata(index_config, MILVUS_COLLECTION_NAME_KEY, connection_metadata)
                connection_args = {
                    "connection_type": "workspace_connection",
                    "connection": {"id": get_id_from_connection(index_connection)},
                }
            ...
            if not index_connection:
                connection_args = {"connection_type": "environment", "connection": {"key": "AZURE_COSMOS_MONGO_VCORE_CONNECTION_STRING"}}
            else:
                if isinstance(index_connection, str):
                    index_connection = get_connection_by_id_v2(index_connection, credential=credential)
                connection_args = {
