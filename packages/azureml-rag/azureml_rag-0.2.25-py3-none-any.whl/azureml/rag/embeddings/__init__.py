...
from azureml.rag.embeddings.florence import FlorenceEmbedder
from azureml.rag.embeddings.openai import OpenAIEmbedder
from azureml.rag.indexes.index_stores import IndexStore, IndexStoreType
...
from azureml.rag.utils.logging import get_logger, track_activity, write_status_log
from azureml.rag.utils.tokens import tiktoken_cache_dir

logger = get_logger(__name__)

MB = 1024 * 1024  # Define a constant for Megabyte
USAGE_THRESHOLD = .8  # Up to 80% of the payload
ACS_DOCUMENT_SIZE_LIMIT = 16 * MB  # ACS recommends batching size up to 16 Mb: https://learn.microsoft.com/en-us/rest/api/searchservice/addupdate-or-delete-documents
...
class EmbeddingsContainer:
    ...
    def get_metadata(self):
        ...
        if "florence" in self.kind:
            if "api_key" in arguments:
                del arguments["api_key"]
            if "is_florence" in arguments:
                del arguments['is_florence']
    ...
    def delete_from_index(self, index_store: IndexStore, batch_size: Optional[int] = None, activity_logger=None, verbosity: int = 1):
        """Delete documents from an index."""
        def batched_docs_to_delete() -> List[str]:
            num_deleted_ids = 0
            deleted_ids = []
            for source_id, source in self._deleted_sources.items():
                logger.info(f"Marking all documents from source {source_id} to be deleted from {index_store.type} index")
        ...
        try:
            ...
            logger.error(f"Failed to delete documents from {index_store.type} index due to: {str(e)}", exc_info=True, extra={'exception': str(e)})
            raise

    def upload_to_index(self, index_store: IndexStore, index_config: dict, batch_size: Optional[int] = None, activity_logger=None, verbosity: int = 1):
        ...
        try:
            ...
            for doc_id, emb_doc in self._document_embeddings.items():
                ...
                if (batch_size and len(batch) % batch_size == 0) or (index_store.type == IndexStoreType.ACS and total_size >= MAX_ACS_REQUEST_SIZE):
                    logger.info(f"Uploading {len(batch)} documents of total size of {total_size} to {index_store.type} index")
            ...
            logger.error(f"Failed to upload documents to {index_store.type} index due to: {str(e)}", exc_info=True, extra={'exception': str(e)})
            raise
    ...
    def mount_and_load(
        ...
        embeddings_connection: str = None,
