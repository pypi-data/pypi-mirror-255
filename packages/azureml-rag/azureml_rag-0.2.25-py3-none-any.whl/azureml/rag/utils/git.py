...
def clone_repo(git_url: str, local_path: Path, branch: Optional[str] = None, authentication: Optional[dict] = None):
    ...
    if authentication is not None:
        import re
        username_match = re.match(r"https:\/\/(.*)@.*", git_repo_branch.git_url)
        if username_match:
            logger.warning(f"git_url '{git_repo_branch.git_url}' already has a username specified, using it.")
            authentication["username"] = username_match.group(1)
        git_repo_branch.git_url = re.sub(r"https:\/\/.*@(.*)", r"https://\1", git_repo_branch.git_url)
