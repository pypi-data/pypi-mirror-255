...
def get_extension_loaders_with_document_intelligence(doc_intel_connection_id: str = None) -> dict:
    ...
    if doc_intel_connection_id is not None:
        from azureml.rag.utils.connections import get_connection_by_id_v2, get_metadata_from_connection, connection_to_credential
        ...
        if endpoint is not None and connection_credential.key is not None:
            from azureml.rag.documents.document_intelligence_loader import DocumentIntelligencePDFLoader
