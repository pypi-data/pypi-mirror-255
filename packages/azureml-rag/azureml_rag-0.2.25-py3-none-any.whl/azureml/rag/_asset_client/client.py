...
def register_new_data_asset_version_workspace(
    ...
    try:
        create_dto = DataVersion(data_type=output_type, data_uri=uri, run_id=run_id, properties=properties, mutable_props=DataVersionMutable(tags=tags))
...
def register_new_data_asset_version(
    ...
    try:
        create_dto = DataVersion(data_type=output_type, data_uri=uri, run_id=run.id, properties=properties, mutable_props=DataVersionMutable(tags=tags))
        data_asset_response = client.data_version.create(
            workspace._subscription_id, workspace._resource_group, workspace._workspace_name, asset_name, create_dto
        )
        data_version = data_asset_response.data_version
