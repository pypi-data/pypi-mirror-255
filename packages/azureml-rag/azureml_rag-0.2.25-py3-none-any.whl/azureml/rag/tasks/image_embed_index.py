...
def read_images_into_documents(
          input_path: Iterator[pathlib.Path]
          ) -> Iterator[Document]:
...
def create_acs_index(
    ...
    index_client = SearchIndexClient(
        ...
        credential=AzureKeyCredential(acs_credentials.key))
    fields = [
        ...
        SearchField(name='image_vector', type=SearchFieldDataType.Collection(SearchFieldDataType.Single), hidden=True, searchable=True, vector_search_configuration='image_vector_config', vector_search_dimensions=1024)
    ]
    index = SearchIndex(
        name=index_name,
        fields=fields)
...
def image_embed_index(args, logger):
    ...
    search_client = create_acs_index(
...
def get_florence_connection_args_from_id(florence_conn_id:str):    
    ...
    florence_connection_args["api_base"] = f"{florence_metadata['endpoint']}?api-version={florence_metadata['api-version']}&model-version={florence_metadata['model-version']}"
    
    return florence_connection_args

def get_ml_index_config(
