    ...
    try:
        if args.git_connection_id is not None and args.git_connection_id != "":
            ...
            if args.git_repository != target and target != "":
                logger.warning(f"Given git repository '{args.git_repository}' does not match the git repository '{target}' specified in the Workspace Connection '{args.git_connection_id}'. Using the arguments git repository.")
                args.git_repository = target

            authentication = {"password": connection_to_credential(connection).token}
            ...
            if "username" in connection_meta:
                ...
                logger.warning(f"Workspace Connection '{args.git_connection_id}' does not have a username specified. Assuming git_url has username.")
        ...
        logger.error(f"Failed to get authentication information from the Workspace Connection '{args.git_connection_id}'.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--git-connection-id", type=str, required=False, default=None, help="The id of the Workspace Connection which contains authentication for the git repository to clone.")
