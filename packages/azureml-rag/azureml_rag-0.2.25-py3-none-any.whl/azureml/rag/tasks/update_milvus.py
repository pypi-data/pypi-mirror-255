...
from azureml.rag.indexes.index_stores import MilvusStore
...
def create_index_from_raw_embeddings(
    ...
    with track_activity(
        ...
        if emb and emb.kind != "none":
            milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["embedding"] = f"contentVector"
        else:
        ...
        if not isinstance(connection_credential, AzureKeyCredential):
            ...
            raise ValueError(f"Expected credential to Milvus index to be an AzureKeyCredential instead got: {type(connection_credential)}")

        client = MilvusClient(
            uri=milvus_config.get(MILVUS_URI_KEY),
            token=connection_credential.key
        )
        collection_name = create_collection_with_index_if_not_exists(milvus_config, client, embeddings=emb)

        batch_size = milvus_config["batch_size"] if "batch_size" in milvus_config else 100
...
def create_collection_with_index_if_not_exists(milvus_config: dict, client: MilvusClient, embeddings: Optional[EmbeddingsContainer] = None):
    """
    ...
    if collection_name not in client.list_collections():
        ...
        for field_type, field_name in milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY].items():
            if field_type in ["content", "url", "filename", "title", "metadata"]:
                fields.append(
                    FieldSchema(name=field_name, dtype=DataType.VARCHAR, max_length=65535)
                )
            elif field_type == "embedding":
                vector_field = FieldSchema(name=field_name, dtype=DataType.FLOAT_VECTOR, dim=embeddings.get_embedding_dimensions())
        ...
        try:
            ...
            logger.error(f"Failed to create collection with name \"{collection_name}\" due to error: {str(e)}")
            raise
...
def main(args, logger, activity_logger):
    try:
        ...
        if args.connection_id is not None:
            ...
            try_override_milvus_config_with_connection_metadata(milvus_config, MILVUS_COLLECTION_NAME_KEY, connection_metadata)
        ...
        with rslex_uri_volume_mount(parent, f"{os.getcwd()}/embeddings_mount", options=mnt_options) as mount_context:
            ...
            if not emb or emb.kind == "none":
                raise RuntimeError(f'Unable to load embeddings from embedding directory')
