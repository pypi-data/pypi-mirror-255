...
from azureml.rag.indexes.index_stores import INDEX_DELETE_FAILURE_MESSAGE_PREFIX, INDEX_UPLOAD_FAILURE_MESSAGE_PREFIX, PineconeStore
from azureml.rag.mlindex import MLIndex
from azureml.rag.utils.connections import get_connection_credential, get_connection_by_id_v2, get_metadata_from_connection
...
def create_index_from_raw_embeddings(
    ...
    with track_activity(logger, "create_index_from_raw_embeddings", custom_dimensions={"num_documents": len(emb._document_embeddings)}) as activity_logger:
        ...
        if has_embeddings:
            ...
            logger.error(f"Documents do not have embeddings and therefore cannot upload to Pinecone index")
            raise RuntimeError(f"Failed to upload to Pinecone index since documents do not have embeddings")
