...
from azureml.rag.tasks.crack_and_chunk import str2bool
...
class MaxTimeExceeded(Exception): pass
...
def add_links_from_html(html, original_url, links, logger, support_http=False):
    ...
    for link in soup.find_all("a"):
        ...
        if url:
            url = url.strip().lower()
            if url.startswith("http://") and support_http and url not in links:
                links.append(url)
            elif url.startswith("https://") and url not in links:
                links.append(url)
                ...
                for link in links[linkIndex:endLinkIndex]:
                    ...
                    try:
                        with time_limit(max_download_time):
                            download(link, output_path, writtenFiles, links, logger, redirectsRemaining, max_file_size, supportHttp=support_http)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_crawl_time", type=int, default=60, help="Maximum download time in seconds for the overall crawl.")
    parser.add_argument("--max_download_time", type=int, default=15, help="Maximum download time in seconds for any single file.")
    parser.add_argument("--max_file_size", type=int, default=5000000, help="Maximum file size for crawler to download.")
    parser.add_argument("--max_crawl_depth", type=int, default=1, help="Maximum depth for crawler to follow links. 0 doesn't crawl any nested links.")
    parser.add_argument("--max_redirects", type=int, default=3, help="Maximum number of redirects for crawler to follow.")
    parser.add_argument("--support_http", type=str2bool, default=False, help="Whether to support http links.")
    parser.add_argument("--output_path", type=str, required=True, help="Output path for the crawler to write the downloaded files.")
    parser.add_argument(
        "--verbosity",
        type=int,
        default=1,
