...
from azureml.rag.indexes.index_stores import INDEX_DELETE_FAILURE_MESSAGE_PREFIX, INDEX_UPLOAD_FAILURE_MESSAGE_PREFIX, AzureCosmosMongoVcoreStore
...
def create_azure_cosmos_mongo_vcore_index_sdk(azure_cosmos_mongo_vcore_config: dict, connection_string: str, embeddings: Optional[EmbeddingsContainer] = None):
    """
    ...
    try:
        ...
        logger.info(f"Ensuring index {index_name} for collection {collection_name} under database {database_name} exists")
        ...
        if (indexes.get(index_name) == None):
            ...
            logger.info(f"Creating {index_name} index of kind {INDEX_KIND} with similarity metric {INDEX_SIMILARITY_METRIC} and dimensions {index_dimensions}")
            indexDefs:List[any] = [
                {
                    "name": index_name,
                    "key": { azure_cosmos_mongo_vcore_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["embedding"]: "cosmosSearch" },
                    "cosmosSearchOptions": {
                        "kind": INDEX_KIND,
        ...
        logger.error(f"Failed to create index. Error: {str(e)}", exc_info=True, extra={'exception': str(e)})
        raise
    ...
    with track_activity(
        ...
        if MLIndex.INDEX_FIELD_MAPPING_KEY not in azure_cosmos_mongo_vcore_config:
            ...
            if emb and emb.kind != "none":
                azure_cosmos_mongo_vcore_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["embedding"] = f"contentVector"

        logger.info(f"Using Index fields: {json.dumps(azure_cosmos_mongo_vcore_config[MLIndex.INDEX_FIELD_MAPPING_KEY], indent=2)}")
        ...
        if not isinstance(connection_credential, AzureKeyCredential):
            raise ValueError(f"Expected credential to Azure Cosmos Mongo vCore index to be an AzureKeyCredential, instead got: {type(connection_credential)}")

        create_azure_cosmos_mongo_vcore_index_sdk(azure_cosmos_mongo_vcore_config, connection_credential.key, embeddings=emb)

        mongo_client = get_mongo_client(connection_credential.key)
        mongo_collection = mongo_client[azure_cosmos_mongo_vcore_config["database_name"]][azure_cosmos_mongo_vcore_config["collection_name"]]

        batch_size = azure_cosmos_mongo_vcore_config["batch_size"] if "batch_size" in azure_cosmos_mongo_vcore_config else 100
        ...
        if has_embeddings:
            ...
            logger.error(f"Documents do not have embeddings and therefore cannot upload to Azure Cosmos Mongo vCore index")
            raise RuntimeError(f"Failed to upload to Azure Cosmos Mongo vCore index since documents do not have embeddings")
...
def main(args, logger, activity_logger):
    try:
        ...
        elif INDEX_DELETE_FAILURE_MESSAGE_PREFIX in exception_str or INDEX_UPLOAD_FAILURE_MESSAGE_PREFIX in exception_str:
...
def main_wrapper(args, logger):
    with track_activity(logger, "update_azure_cosmos_mongo_vcore") as activity_logger, safe_mlflow_start_run(logger=logger):
        try:
