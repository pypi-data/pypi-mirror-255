...
from azureml.rag.documents.chunking import file_extension_splitters, split_documents
from azureml.rag.documents.cracking import BaseDocumentLoader, crack_documents, file_extension_loaders
from azureml.rag.utils.cracking import get_extension_loaders_with_document_intelligence
...
def generate_file_name(output_chunks: str, chunked_document: ChunkedDocument, file_extension: str) -> str:
    file_name = hashlib.sha256(
        chunked_document.source.filename.replace("\\", "_").replace("/", "_").encode("utf-8")
    ).hexdigest()
...
def get_activity_logging_filter(source_glob, activity_logger=None):
    ...
    def filter_and_log_extensions(
        ...
        for source in sources:
            ...
            if allowed_extensions is not None:
                if source.path.suffix is None or source.path.suffix.lower() not in allowed_extensions:
                    skipped_files += 1
...
def main(args, logger, activity_logger):
    ...
    extension_loaders = get_extension_loaders_with_document_intelligence(
    ...
    filter_and_log_extensions = get_activity_logging_filter(args.input_glob, activity_logger)
    ...
    for chunked_document in chunked_documents:
        ...
        if args.output_format == "csv":
            ...
            write_chunks_to_jsonl(
                chunked_document.chunks, generate_file_name(args.output_chunks, chunked_document, "jsonl")
            )
    ...
    activity_logger.activity_info["chunk_count"] = str(chunk_count)
...
def crack_and_chunk_arg_parser():
    ...
    parser.add_argument("--openai_api_version", type=str, default=None)
