...
def get_model_config(llm_config: Dict[str, str], openai_api_type: str, openai_api_version: str, activity_logger: Logger):
    ...
    connection_id = llm_config["llm_connection"] if llm_config["llm_connection"] is not None else os.environ.get("AZUREML_WORKSPACE_CONNECTION_ID_AOAI")
    if connection_id:
        connection_config = {
        ...
        if hasattr(connection, "type"):
            if connection.type == "azure_open_ai":
                model_config["api_base"] = connection.target
                model_config["api_version"] = connection.api_version if hasattr(connection, "api_version") and connection.api_version is not None else "2023-03-15-preview"
                model_config["api_type"] = connection.api_type if hasattr(connection, "api_type") and connection.api_type is not None else "azure"
        elif (
            isinstance(connection, dict)
            and connection.get("properties", {}).get("category", None) == "AzureOpenAI"
        ):
            model_config["api_base"] = connection.get("properties", {}).get("target")
            connection_metadata = connection.get("properties", {}).get("metadata", {})
            model_config["api_version"] = connection_metadata.get(
                "apiVersion", connection_metadata.get("ApiVersion", "2023-03-15-preview")
            )
            model_config["api_type"] = connection_metadata.get(
...
def main(parser_args, run, logger: Logger, activity_logger: Logger):
    ...
    llm_config["llm_connection"] = parser_args.llm_connection
...
if __name__ == '__main__':
    ...
    parser.add_argument("--llm_connection", type=str, required=True)
