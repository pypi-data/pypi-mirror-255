...
from azureml.rag.documents.document import Document, DocumentSource, StaticDocument
from azureml.rag.langchain.vendor.text_splitter import TextSplitter
from azureml.rag.utils import merge_dicts
from azureml.rag.utils.logging import get_logger, safe_mlflow_log_metric
from azureml.rag.utils.tokens import tiktoken_cache_dir, token_length_function
...
def get_langchain_splitter(file_extension: str, arguments: dict) -> TextSplitter:
    ...
    if file_extension == ".py":
        from azureml.rag.langchain.vendor.text_splitter import Language, RecursiveCharacterTextSplitter
    ...
    if use_nltk:
        _init_nltk()
        from azureml.rag.langchain.vendor.text_splitter import NLTKTextSplitter
    ...
    if file_extension == ".txt" or file_extension in formats_to_treat_as_txt_once_loaded:
        from azureml.rag.langchain.vendor.text_splitter import TokenTextSplitter

        with tiktoken_cache_dir():
            return TokenTextSplitter(
                encoding_name="cl100k_base",
                length_function=token_length_function(),
                **{**arguments, "disallowed_special": ()}
            )
        ...
        if use_rcts:
            from azureml.rag.langchain.vendor.text_splitter import MarkdownTextSplitter
...
def split_documents(documents: Iterable[ChunkedDocument], splitter_args: dict, file_extension_splitters=file_extension_splitters, activity_logger=None) -> Iterator[ChunkedDocument]:
    ...
    for i, document in enumerate(documents):
        ...
        if i % log_batch_size == 0:
            safe_mlflow_log_metric("cracked_documents", total_documents, logger=logger, step=int(time.time() * 1000))
        ...
        if i % log_batch_size == 0:
            safe_mlflow_log_metric("chunked_documents", total_splits, logger=logger, step=int(time.time() * 1000))
    ...
    if (activity_logger):
        activity_logger.activity_info["chunk_count"] = str(total_splits)
