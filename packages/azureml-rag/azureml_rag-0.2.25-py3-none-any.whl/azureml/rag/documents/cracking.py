...
from azureml.rag.documents.chunking import ChunkedDocument, DocumentSource
from azureml.rag.documents.document import Document, StaticDocument
from azureml.rag.langchain.vendor.document_loaders.unstructured import UnstructuredFileIOLoader
...
def files_to_document_source(
    ...
    # TODO: Access via fsspec? If it's mounted locally then should be accessed as local filesystem, if it's direct URI then other FS will handle it.
    file_source_path = Path(files_source)
    if file_source_path.is_file():
        relative_path = file_source_path.relative_to(file_source_path.parent)
...
class UnstructuredHTMLFileIOLoader(UnstructuredFileIOLoader, BaseDocumentLoader):
    ...
    def __init__(
        self, file, document_source: DocumentSource, metadata: dict, mode="single", **unstructured_kwargs: Any
    ):
