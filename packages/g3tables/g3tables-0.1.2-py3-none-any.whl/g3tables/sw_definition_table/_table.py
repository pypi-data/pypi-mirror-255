...
class SWDefinitionSheetFormatter:
    ...
    def format_device_varname(cls, varname: str, log: bool = True) -> str:
        """
        Process a string by removing its leading non-alphabetical characters
        and removing all non-alphanumeric characters from the entire string
        (whitespaces and hypens are replaced with underscores).
        ...
        Returns:
            str: The processed string.
        """
        # Remove all non-alphabetical leading characters
        formatted = re.sub(r'^[^a-zA-Z]+', '', varname)
        # Format the name
        formatted = cls.format_device_name(formatted, log=False)
