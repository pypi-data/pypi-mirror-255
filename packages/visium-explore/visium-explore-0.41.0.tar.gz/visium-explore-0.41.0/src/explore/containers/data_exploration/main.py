...
def display_plot_container(file_path: pathlib.Path, plot: Plot, x_axis: str, y_axis) -> None:
    ...
    if plot is not None and y_axis and pd.api.types.is_object_dtype(df[x_axis]):
        groupby_description = df.groupby(x_axis)[y_axis].describe()
