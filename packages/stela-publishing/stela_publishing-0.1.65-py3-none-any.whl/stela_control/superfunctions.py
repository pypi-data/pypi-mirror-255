...
def inventoryData(request):
    if request.method == 'POST':
        ...
        ckeditor = request.POST.get('ckeditor')
        schedule = request.POST.get('schedule')
        print(form_id, type, form_name, formset_name, action, model_parent, model_child, cke, cke_formset, schedule, data, ckeditor)
        
        if form_id:
            
            if form_id == "SimpleFormset":
                form_class = get_form_class_by_name(formset_name)
                class_parent = get_form_class_by_name(model_parent)
                if data:
                    service=class_parent.objects.get(pk=data)
                    ...
                    if form.is_valid():
                        ...
                        if schedule and ckeditor and cke_formset:
                            obj_data = render_to_string('stela_control/load-data/inventory/simple-form.html', { 
                            'form': form,
                            'cke': ckeditor,
                    ...
                    if formset.is_valid():
                        ...
                        if schedule and ckeditor and cke_formset:
                            obj_data = render_to_string('stela_control/load-data/inventory/simple-formset.html', { 
                            'formset': formset,
                            'cke': ckeditor,
                            'cke_formset': cke_formset,
                            'type': type,
        ...
        if action == "SimpleFormset":
            if formset_name:
                formset_class = get_form_class_by_name(formset_name)
                class_parent = get_form_class_by_name(model_parent)
                if data:
                    obj=class_parent.objects.get(pk=data)
                    form=formset_class(instance=obj)
