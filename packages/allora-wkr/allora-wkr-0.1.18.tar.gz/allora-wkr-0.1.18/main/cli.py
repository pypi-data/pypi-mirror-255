...
def run(logs):
    ...
    # compose_dir = os.path.join(os.getcwd(), 'eth-predict') # uncomment for development
...
def deploy():
    ...
    # worker_name = os.path.join(os.getcwd(), 'eth-predict').split('/')[-1] # uncomment for development
    worker_name = os.getcwd().split('/')[-1]
    ...
    # change f002 to actual worker name
    file_configs = [
        {
            "template_name": "values.yaml.j2",
            "file_name": "values.yaml",
            "context": {"worker_image_uri": worker_image_uri, "worker_image_tag": worker_image_tag, "worker_name": 'foo2', "boot_nodes": boot_nodes, "chain_rpc_address": chain_rpc_address, "chain_topic_id": chain_topic_id, "mnemonic": mnemonic}
        }
    ]

    # generate_all_files(file_configs, worker_name) # uncomment for development
