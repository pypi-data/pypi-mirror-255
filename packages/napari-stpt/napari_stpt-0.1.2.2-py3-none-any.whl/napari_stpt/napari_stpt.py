...
from qtpy.QtWidgets import QApplication, QMainWindow, QComboBox, QCheckBox, QListWidget, QListWidgetItem, QVBoxLayout, QWidget
...
#             except Exception:
    ...
    def set_overall_brightness(self):
        ...
        self.overall_brightness = number_of_channels * (1.01 - (float(self.scroll_overall_brightness.value()) / 1000))
    ...
    def Load3D(self, text):
        ...
        self.optical_slices_available, self.value_range = self.data.Load3D(self.viewer, self.image_folder, self.comboBoxPath.currentText(), self.selected_channels, self.default_contrast_limits, self.thresholdN, self.channels_start, self.axio, self.old_method, self.overall_brightness, self.scroll, self.scroll_overall_brightness, self.pixel_size.text(), self.m_slice_spacing.text(), self.start_slice.text(), self.end_slice.text(), self.crop, self.crop_start_x, self.crop_end_x, self.crop_start_y, self.crop_end_y)
        # self.MakeBoundingBox()
