...
class DecoupledLionW8Bit(torch.optim.Optimizer):
    ...
    def step_param(self, p: torch.Tensor, hparams: Dict[str, Any]) -> None:
        ...
        if "exp_avg" not in state:
            ...
            state["exp_avg"] = _MaybeQuantizedTensor(mom, try_quantize=self._quantize)
    ...
    def __setstate__(self, state: Dict[str, Dict[str, Any]]) -> None:
        ...
        for param_id in opt_state:
            ...
            if "errors" in param_state:
                ...
                errs = param_state["errors"].to(dtype=torch.uint8).view(torch.bfloat16)
    ...
    def state_dict(self):
        ...
        for param_id in opt_state:
            ...
            if "errors" in param_state:
                ...
                # as the params
                param_state["errors"] = (
                    param_state["errors"].view(torch.uint8).to(dtype=torch.bfloat16)
                )
...
def lion8b_step_fused(
    ...
    num_groups = (weights.numel() + quantize_group_size - 1) // quantize_group_size
    ...
    for name, tensor, allowed_dtypes in [
        ...
        if not tensor.is_cuda:
            raise ValueError(f"{name} must be on a CUDA device, not {tensor.device}")
        ...
        if tensor.dtype not in allowed_dtypes:
            raise ValueError(
                f"{name} must have dtype {allowed_dtypes}, not " + f"{tensor.dtype}"
            )
    ...
    if grads.dtype in (torch.float16, torch.bfloat16):
        ...
        if weights.dtype not in allowed_dtypes:
            raise ValueError(f"Weights must be f32 or match grad dtype {grads.dtype}")
...
def _lion8b_step(
    ...
    if fused and not momentums.is_quantized():
        raise NotImplementedError("Fused LION step only implemented with quantization.")
