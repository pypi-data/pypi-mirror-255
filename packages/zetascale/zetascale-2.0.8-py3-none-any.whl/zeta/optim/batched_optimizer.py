...
class ScaledAdam(BatchedOptimizer):
    ...
    def _get_clipping_scale(
        ...
        if step % clipping_update_period == 0:
            ...
            logging.info(
                f"Clipping_scale={clipping_scale}, grad-norm quartiles"
                f" {quartiles}, threshold={threshold:.3e},"
                f" percent-clipped={percent_clipped:.1f}"
            )
        ...
        if step < clipping_update_period:
            ...
            try:
                ...
                logging.info(
                    "Warning: model_norm_threshold not in state: possibly you"
                    " changed config when restarting, adding clipping_scale"
                    " option?"
                )
                return 1.0
...
class LRScheduler(object):
    ...
    def print_lr(self, is_verbose, group, lr):
        ...
        if is_verbose:
            logging.info(
                f"Epoch={self.epoch}, batch={self.batch}: adjusting learning"
                f" rate of group {group} to {lr:.4e}."
            )
