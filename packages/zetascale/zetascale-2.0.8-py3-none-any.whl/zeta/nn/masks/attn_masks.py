...
class AttentionBias:
    ...
    When using an :attr:`zeta.nn.AttentionBias`
    ...
    * greater than X + window_size (i.e. "local attention")
    ...
    def __post_init__(self) -> None:
        if self._window_size <= 0:
            raise ValueError(
                "Expected `window_size > 0`, but" f" window_size={self._window_size}"
            )
...
class _PaddedSeqLenInfo(_SeqLenInfo):
    ...
    def from_seqlens(cls, seqlens: Iterable[int]) -> "_SeqLenInfo":
        raise RuntimeError(
            "Use either `_SeqLenInfo.from_seqlens` or"
            " `_PaddedSeqLenInfo.from_seqlens_padded`"
        )
...
class BlockDiagonalMask(AttentionBias):
    ...
    def make_causal_from_bottomright(
        self,
...
class BlockDiagonalCausalMask(BlockDiagonalMask):
    """
    Same as :attr:`zeta.nn.modules.masks.BlockDiagonalMask`, except that each block is causal.
    ...
    def __post_init__(self) -> None:
        for i, ((q_start, q_end), (k_start, k_end)) in enumerate(
            ...
            if num_keys < num_queries:
                raise ValueError(
                    f"Block #{i} has num_keys={num_keys} and"
                    f" num_queries={num_queries}. Expected `num_keys >="
                    " num_queries`"
                )
...
class BlockDiagonalCausalWithOffsetPaddedKeysMask(AttentionBias):
    """
    Same as :attr:`zeta.nn.modules.masks.BlockDiagonalCausalMask`,
    ...
    def __post_init__(self):
        ...
        for q, k in zip(q_seqlen, kv_seqlen):
            if q - self._window_size >= k:
                ...
                raise RuntimeError(
                    f"No keys are attended in q_seqlen {q} k_seqlen {k} with"
                    f" sliding window {self._window_size}"
                )
