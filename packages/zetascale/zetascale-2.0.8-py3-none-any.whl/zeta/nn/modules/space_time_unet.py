...
class FeedForwardV(nn.Module):
    def __init__(self, dim, mult=4):
        super().__init__()

        inner_dim = int(dim * mult * 2 / 3)
        self.proj_in = nn.Sequential(
            nn.Conv3d(dim, inner_dim * 2, 1, bias=False), GEGLU()
        )

        self.proj_out = nn.Sequential(
            RMSNorm(inner_dim), nn.Conv3d(inner_dim, dim, 1, bias=False)
        )
...
class SpatioTemporalAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.spatial_rel_pos_bias = (
            ContinuousPositionBias(dim=dim // 2, heads=heads, num_dims=2)
            if pos_bias
            else None
        )
        ...
        self.temporal_rel_pos_bias = (
            ContinuousPositionBias(dim=dim // 2, heads=heads, num_dims=1)
            if pos_bias
            else None
        )
        ...
        self.ff = FeedForwardV(dim=dim, mult=ff_mult)
    ...
    def forward(self, x, enable_time=True):
        ...
        space_rel_pos_bias = (
            self.spatial_rel_pos_bias(h, w)
            if exists(self.spatial_rel_pos_bias)
            else None
        )
        ...
        if enable_time:
            ...
            time_rel_pos_bias = (
                self.temporal_rel_pos_bias(x.shape[1])
                if exists(self.temporal_rel_pos_bias)
                else None
            )
...
class Block(nn.Module):
    def __init__(
        self, dim, dim_out, kernel_size=3, temporal_kernel_size=None, groups=8
    ):
        super().__init__()
    ...
    def __init__(self, dim, dim_out, *, timestep_cond_dim=None, groups=8):
        ...
        if exists(timestep_cond_dim):
            self.timestep_mlp = nn.Sequential(
                nn.SiLU(), nn.Linear(timestep_cond_dim, dim_out * 2)
            )

        self.block1 = Block(dim, dim_out, groups=groups)
        self.block2 = Block(dim_out, dim_out, groups=groups)
        self.res_conv = (
            PseudoConv3d(dim, dim_out, 1) if dim != dim_out else nn.Identity()
        )
...
class SpaceTimeUnet(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert (
            len(dim_mult)
            == len(self_attns)
            == len(temporal_compression)
            == len(resnet_block_depths)
        )
        ...
        for (
            ...
            self.downs.append(
                mlist(
                    [
                        ResnetBlock(
                            dim_in, dim_out, timestep_cond_dim=timestep_cond_dim
                        ),
                        mlist(
                            [
        ...
        self.conv_in = PseudoConv3d(
            dim=channels, dim_out=dim, kernel_size=7, temporal_kernel_size=3
        )

        self.conv_out = PseudoConv3d(
            dim=dim, dim_out=channels, kernel_size=3, temporal_kernel_size=3
        )
    ...
    def forward(self, x, timestep=None, enable_time=True):
        ...
        t = (
            self.to_timestep_cond(rearrange(timestep, "... -> (...)"))
            if exists(timestep)
            else None
        )
