...
class AssociatedResourceType(str):
...
class MapMatchScope(str):
    ALL = "ALL"
    KEY = "KEY"
    VALUE = "VALUE"


class OversizeHandling(str):
    CONTINUE = "CONTINUE"
    MATCH = "MATCH"
    NO_MATCH = "NO_MATCH"
...
class SensitivityLevel(str):
    LOW = "LOW"
    HIGH = "HIGH"


class SizeInspectionLimit(str):
    KB_16 = "KB_16"
    KB_32 = "KB_32"
    KB_48 = "KB_48"
    KB_64 = "KB_64"
...
class AWSManagedRulesBotControlRuleSet(TypedDict, total=False):
    ...
    EnableMachineLearning: Optional[Boolean]
...
class JA3Fingerprint(TypedDict, total=False):
    ...
    request. This fingerprint serves as a unique identifier for the client's
    TLS configuration. WAF calculates and logs this fingerprint for each
    request that has enough TLS Client Hello information for the
...
class SizeConstraintStatement(TypedDict, total=False):
    ...
    for regional web ACLs, this limit is 8 KB (8,192 bytes) and for
    CloudFront web ACLs, this limit is 16 KB (16,384 bytes). For CloudFront
    web ACLs, you can increase the limit in the web ACL
...
class Wafv2Api:
    ...
    def create_api_key(
        self, context: RequestContext, scope: Scope, token_domains: APIKeyTokenDomains
    ...
    def describe_all_managed_products(
        self, context: RequestContext, scope: Scope
    ...
    def describe_managed_products_by_vendor(
        self, context: RequestContext, vendor_name: VendorName, scope: Scope
    ...
    def get_decrypted_api_key(
        self, context: RequestContext, scope: Scope, api_key: APIKey
