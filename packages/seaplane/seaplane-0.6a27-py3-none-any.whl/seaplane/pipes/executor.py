    ...
    while True:
        ...
        try:
            ...
            for output in gen:
                ...
                wallclock = message_send_time - message_recieved_time
                log.logger.debug(f"{flow.instance_name} sending message after {wallclock} seconds")

                # Notice "recieved_time" doesn't change as we yield new messages.
                recieved_time_key = f"{flow.instance_name}_recieved_time"
                if recieved_time_key not in use_meta:
                    use_meta[recieved_time_key] = str(message_recieved_time)

                send_time_key = f"{flow.instance_name}_sent_time"
                if send_time_key not in use_meta:
                    use_meta[send_time_key] = str(message_send_time)
