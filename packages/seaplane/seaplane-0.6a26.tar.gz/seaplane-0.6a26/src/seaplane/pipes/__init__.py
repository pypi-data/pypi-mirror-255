...
class FlowTopic:
    ...
    def address_tag(self, r: ExecutionResult) -> str:
        ...
        # at publish time.
        subtags = ["default" if r.address_label is None else r.address_label]
...
class Dag:
    ...
    def edge(
        ...
        if isinstance(source, FlowTopic):
            if not isinstance(dest, Flow):
                raise SeaplaneError(f"conditional messages must be sent to Flows, not {dest}")
        ...
        if isinstance(source, Subscription) and isinstance(dest, Flow):
