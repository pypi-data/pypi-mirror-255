...
def test_raman():
    ...
    df = pd.read_csv(f"{base_path}/sample_data/raman.csv", index_col=0)
    ...
    # グラフの設定
    config = AxConfig(
        x=SpineConfig(label=df.columns[0], lim=(400, 1600)),
        y=SpineConfig(label=df.columns[1], visible=False, lim=(-1000, 15000)),
