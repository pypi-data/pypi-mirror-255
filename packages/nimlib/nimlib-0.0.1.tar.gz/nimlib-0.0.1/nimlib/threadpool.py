...
# threads finish.
# Ref: https://github.com/python/cpython/blob/main/Lib/concurrent/futures/thread.py
...
class _WorkItem(object):
    ...
    def run(self):
        ...
        if (not self.future.set_running_or_notify_cancel()) or (
            time.time() - self.start_time > nimlib.__blocktime__
        ):
            return
...
class PriorityThreadPoolExecutor(_base.Executor):
    ...
    def add_args(cls, parser: argparse.ArgumentParser, prefix: str = None):
        ...
        try:
            default_max_workers = (
                os.getenv("NB_PRIORITY_MAX_WORKERS")
                if os.getenv("NB_PRIORITY_MAX_WORKERS") != None
                else 5
            )
            default_maxsize = (
                os.getenv("NB_PRIORITY_MAXSIZE")
                if os.getenv("NB_PRIORITY_MAXSIZE") != None
                else 10
            )
            parser.add_argument(
