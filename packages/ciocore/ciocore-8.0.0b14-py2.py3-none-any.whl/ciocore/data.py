...
def data(force=False):
    ...
    * **software** is a PackageTree object containing either all
    ...
    if __data__ == {}:
        ...
        data_from_cache = data_cache.get_data(account_id)
        
        # PROJECTS
        __data__["projects"] = sorted(data_from_cache["projects"])
        # INST_TYPES
        instance_types = data_from_cache["instance_types"]
        # SOFTWARE
        software = data_from_cache["packages"]
...
def get_account_id():
    ...
    if not creds:
        raise ValueError("No credentials found. Please login with `conductor login`")
    try:
        return api_client.account_id_from_jwt(creds)
    except ValueError:
        raise ValueError("No account_id found. Please login with `conductor login`")
