...
from ciocore.sequence_checker import check_sequence
...
DOWNLOAD_REGEX_HELP = """
A regular expression to filter the files to download. Only files whose names match the regex will be downloaded.
"""
...
@click.option(
    ...
    "--fmt",
    "--format",
    help=SEQ_FORMAT_HELP,
    type=click.Choice(choices=check_sequence.FORMAT_OPTIONS, case_sensitive=False),
    show_choices=True,
    default=check_sequence.FORMAT_OPTIONS[0],
)
@click.option(
    "-lv",
    "--log_level",
...
@click.argument("pattern", nargs=1, type=click.Path(exists=False, resolve_path=True))
def seq(pattern, fmt, log_level):
    """
    Provide information about the integrity of a sequence of files, and facilitate the process of rerendering missing frames.

    Information about the sequence is displayed in a chart, with each bar representing a file in the sequence. You may choose to display the chart as ascii in the terminal, or as an interactive graphical chart.

    You must provide a pattern describing the sequence of files to check. The pattern defines the expected filenames. The pattern consists of the prefix followed by a spec in square brackets, followed by the extension. The spec consists of a frame-spec followed by a padding-spec.

    You can omit the spec entirely, but you must include the brackets. In this case, the expected sequence and padding will be inferred automatically from matching files on disk. In this case, you won't be able to detect missing leading and trailing frames.

    If you do provide a spec, both the frame-spec and the padding-spec must be present. The frame spec can be a single frame number, or a range of frames with optional step frame. The padding spec is a series of hashes representing the number of digits in the padding.

    The pattern can be absolute or relative. If relative, the pattern will be interpreted relative to the current working directory.

    Examples patterns:

    /path/to/my_image[1-100####].jpg
    Chart range will be 1-100, with 4 digits of padding.

    /path/to/my_image[1-100x3#].jpg
    Chart range will be 1-100, no padding. Only every third frame will be displayed. Note you must provide one hash.

    /path/to/my_image[].jpg
    Chart range and padding will be inferred from files on disk. Note you must provide the brackets to let the system know where to expect the number part.

    If you are expecting step frames, you can specify the step with an x, such as /path/to/my_image[1-100x3####].jpg.
    """
    _set_logging(log_level)

    check_sequence.run(pattern=pattern, format=fmt)
