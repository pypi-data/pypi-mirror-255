...
class sensor_version:
    ...
    def parse(self, str):
        ...
        if len(str) > 0:
            ...
            if (str[8] == 'R' and str[10] == 'D'):
                ...
                self.radar_dim = 2
            nodens.logger.info(self.wifi_version, self.radar_version)
        else:
    ...
    def request(self, client, root_topic, sensor_target):
        ...
        nodens.logger.info("trying request")
            ...
            try:
                if isinstance(self.frame[-1], (float, int)) and isinstance(self.frame[-2], (float, int)):
                    if self.frame[-1] > self.frame[-2]:
                        ...
                        nodens.logger.warning(f"Frames not sequential. frame -1: {self.frame[-1]}. frame -2: {self.frame[-2]}")
                else:
                    nodens.logger.warning(f"Frames not a number. frame -1: {self.frame[-1]}. frame -2: {self.frame[-2]}")
            except:
                nodens.logger.warning(f"Frame count error {self.frame[-1]} {self.frame[-2]} {isinstance(self.frame[-1], (float, int))}")


        if max_time_samples < 0:
            nodens.logger.info("WARNING: max_time_samples (= {}) must be greater than 0. Setting to 0.")
...
class sendCMDtoSensor(object):
    ...
    def full_data_off(rcp,cp):
        ...
        ndns_mesh.MESH.multiline_payload(cp.SENSOR_IP,cp.SENSOR_PORT,60, nodens.cp.SENSOR_TOPIC,"", payload_msg)
    ...
    def request_version(rcp,cp,sv):
        ...
        nodens.logger.info("Published sensor version request to {}".format(nodens.cp.SENSOR_TOPIC))
        temp = 0
        while (1):
            if sv.string != []:
