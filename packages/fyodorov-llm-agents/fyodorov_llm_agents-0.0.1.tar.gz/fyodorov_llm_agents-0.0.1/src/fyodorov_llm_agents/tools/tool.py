...
class Tool(BaseModel):
    ...
    @staticmethod
    def from_plugin(url: str) -> Tool:
        """Instantiate Tool from plugin."""
        if not url:
            raise ValueError('Plugin URL is required')
        if not re.match('/\.well-known/.*\.json', url):
            raise ValueError('Plugin URL is not a valid plugin URL')
        try:
            ...
            tool_dict = {
                ...
                'contact_email': EmailStr(json['contact_email']),
                'legal_info_url': HttpUrl(json['legal_info_url']),
            }
