...
class DynamicContentModelMixin(models.Model):
    ...
    def check_parts(self):
        ...
        누락된 placeholders가 있다면 ValidationError를 발생시킵니다.
        """
        if not self.format:
            return  # Format이 설정되지 않은 경우 검증을 건너뜁니다.

        placeholders = self.format.get_placeholders()
        parts_fields = [part.field for part in self.parts.all()]

        missing_placeholders = set(placeholders) - set(parts_fields)
        if missing_placeholders:
            raise ValidationError(f'Missing placeholders in parts: {", ".join(missing_placeholders)}')
