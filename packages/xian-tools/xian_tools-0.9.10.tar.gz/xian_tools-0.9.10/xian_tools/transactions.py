...
from xian_tools.formating import format_dictionary, check_format_of_payload
from xian_tools.encoding import encode
...
def decode_tx(tx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'result' in tx:
        decoded = decode_dict(tx['result']['tx'])
        tx['result']['tx'] = decoded

        if tx['result']['tx_result']['data'] is not None:
            decoded = decode_data(tx['result']['tx_result']['data'])
            tx['result']['tx_result']['data'] = decoded
