...
from spectral.utilities.python23 import IS_PYTHON3, is_string, tobytes
from spectral.utilities.errors import SpyException
...
def gen_params(envi_header):
    ...
    p.psize = h["pixel size"]
    p.desc = h["description"]
    p.offset = int(h["header offset"]) if "header offset" in h else int(0)
    p.byte_order = spy.byte_order #int(h["byte order"])
