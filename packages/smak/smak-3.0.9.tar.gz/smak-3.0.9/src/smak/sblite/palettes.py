...
QUAL_PALETTE_SIZES.update({k: len(v) for k, v in list(SEABORN_PALETTES.items())})
...
def blend_palette(colors, n_colors=6, as_cmap=False, input="rgb"):
    ...
    if not as_cmap:
        ...
        pal = _ColorPalette(list(map(tuple, rgb_array)))
    return pal
...
def _parse_cubehelix_args(argstr):
    ...
    kwargs = {kwarg_map.get(k, k): v for k, v in list(kwargs.items())}
