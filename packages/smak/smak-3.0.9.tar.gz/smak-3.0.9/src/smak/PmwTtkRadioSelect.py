...
if sys.platform!='darwin':
    BStyle.configure("RAISED.TButton",relief='raised',background="#d4d0c8")
    BStyle.configure("SUNKEN.TButton",relief='sunken',background="#d4d0c8")
else:
    BStyle.configure("RAISED.TButton",relief='raised',background="#d4d0c8")
    BStyle.configure("SUNKEN.TButton",relief='sunken',background="#d4d0c8")

class PmwTtkRadioSelect(Pmw.MegaWidget):
    ...
    def add(self, componentName, **kw):
        ...
        if self['buttontype'] == 'radiobutton':
            ...
            if 'anchor' not in kw:
                kw['anchor'] = 'w'
            if 'variable' not in kw:
                kw['variable'] = tkinter.IntVar()

        if self['buttontype'] == 'button': kw['takefocus']='0'
    ...
    def __setSingleValue(self, value):
        ...
        if self['buttontype'] == 'radiobutton':
            ...
            for button in self._buttonList:
                widget = self.component(button)
                if button == value:
                    widget.configure(style="SUNKEN.TButton")#relief='sunken')
                else:
                    widget.configure(style="RAISED.TButton")#relief='raised')
