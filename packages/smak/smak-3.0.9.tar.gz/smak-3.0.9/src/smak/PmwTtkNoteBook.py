...
class PmwTtkNoteBook(Pmw.MegaArchetype):
    ...
    def insert(self, pageName, before = 0, **kw):
        ...
        if self._withTabs:
            ...
            tabOptions.pop('borderwidth')
            if sys.platform!='darwin':
                ttk.Style().configure('NOTEBOOK.TButton',borderwidth='0',background='#d4d0c8')
            else:
                ttk.Style().configure('NOTEBOOK.TButton',borderwidth='0',background='white')                
            tabOptions['style'] = 'NOTEBOOK.TButton'
            tab = self.createcomponent(*(pageName + '-tab',
                    (), 'Tab',
                    ttk.Button, self._hull), **tabOptions)
...
# forwarded.
Pmw.forwardmethods(PmwTtkNoteBook, tkinter.Canvas, '_hull')
