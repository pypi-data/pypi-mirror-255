...
class _DistributionPlotter(VectorPlotter):
    ...
    def _resolve_multiple(
        ...
        if multiple in ("stack", "fill"):
            ...
            baselines = {k: np.zeros_like(v) for k, v in list(curves.items())}
    ...
    def plot_univariate_histogram(
        ...
        data_axis = getattr(ax, str(self.data_variable)+"axis")
        ...
        if element == "bars" and "linewidth" not in plot_kws:
            ...
            hist_metadata = [h.index.to_frame() for _, h in list(histograms.items())]
            binwidth = min([
    ...
    def plot_bivariate_density(
        ...
        if common_norm:
            ...
            draw_levels = {
                ...
                for k, d in list(densities.items())
            }
        ...
        if "hue" in self.variables:
            for param in ["cmap", "colors"]:
                if param in contour_kws:
                    msg = str(param)+" parameter ignored when using hue mapping."
                    warnings.warn(msg, UserWarning)
...
def kdeplot(
    ...
    **kwargs
    ...
    if bw is not None:
        msg = (
            "The `bw` parameter is deprecated in favor of `bw_method` and `bw_adjust`. Using "+str(bw)+" for `bw_method`, but please see the docs for the new parameters and update your code."
        )
        warnings.warn(msg, FutureWarning)
        bw_method = bw
    ...
    if shade_lowest is not None:
        if shade_lowest:
            thresh = 0
        msg = (
            "`shade_lowest` is now deprecated in favor of `thresh`. Setting `thresh="+str(thresh)+"`, but please update your code."
        )
        warnings.warn(msg, UserWarning)
