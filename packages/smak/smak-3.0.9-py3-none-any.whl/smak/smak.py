...
from scipy.interpolate import interp1d as InterpolatingFunction
import sblite
import sivm
import sklearn
...
#local imports 
import absorptionCalc
import ChannelInterpret
import ciexyz
import colormodels
import Deconv
import Display
...
import ScrollTree
import smwPmw
import stitch_hdf_mod
import tkFileDialogDir
import varimax


#local imports GUI Classes
import AdvancedFilteringClass
import BeamDeconvolutionClass
import BatchAnalyze
import BatchDisplay
import BatchMode
import BatchProcess
import ConcentrationStandardClass
import CorrelationPlotClass
import CustomKernelClass
import DataAveragingClass
import DataSummaryClass
import ExportRegistrationClass
import FaderClass
import IR_MathClass
import MakeHistogramClass
import Mask
import MathWindowClass
import MomentAnalysisClass
import MultiFitObj
import MultiMassCalibrationClass
import PCAAnalysisClass
import PCAAnalysisMathClass
import ParticleStatisticsClass
import pyMcaFitWrapper
import pyMcaParamGUI
import QuantThicknessDialog
import RadialProfileClass
import ThresholdingClass
import TriColorWindowClass
import WeightedGroupPCA
...
class Main:
    def __init__(self,master):
        ...
        #MCA alt coord variables
        self.MCAaltcoords=tkinter.IntVar()
        self.MCAaltcoords.set(0)
        self.MCAaltdata=None
        ...
        self.isXSMultiChan=tkinter.IntVar()
        self.isXSMultiChan.set(0)
        self.MCxsList=[]
        ...
        print("script bactch mode window initialized")
        self.groupWtPCAWindow = WeightedGroupPCA.GroupPCA(self.imgwin)
        print("weighted group pca window initialized")        

        #Display
        self.maindisp=Display.Display(self.imgwin,self.viewMCAplottoggle,self.startimgROI,self.showscalebar,self.showscalebarText,self.xyflip,tcrefresh=self.tcrefresh,callback=[None,None,self.displaySpectrumGraph,self.dispAddMarker,self.createXSfromData],rowcb=self.returnPRS,sf=self.dispScaleFactor)
        ...
        menubar.addmenuitem('File','separator')
        menubar.addmenuitem('File','command',label='Convert SSRL Mosaic - Vision',command=self.createSSRLMosaic)
        menubar.addmenuitem('File','command',label='Convert SSRL Mosaic - Static',command=self.createSSRLMosaicB)
        menubar.addmenuitem('File','command',label='Set side crop for Mosaic',command=self.setMosaicCrop)
        ...
        menubar.addcascademenu('File','Multi XS Export')
        menubar.addmenuitem('Multi XS Export','checkbutton',label='Use Multi-Channel Definitions',variable=self.isXSMultiChan,command=self.checkdefineMCexport)        
        menubar.addmenuitem('Multi XS Export','command',label='Define Multi-Channel Cross Sections',command=self.defineMCexport)
...
#        menubar.addmenuitem('File','command',label='Save CT sections',command=self.saveCTsections)
        ...
        menubar.addmenuitem('File','command',label='Add Tag to  Multiple Channels',command=self.asktagmultichannel)
        menubar.addmenuitem('File','separator')
        ...
        menubar.addmenuitem('Process','command',label='Create Masks from Plotmarkers',command=self.makeMaskFromPM)        
        menubar.addmenuitem('Process','separator')      
...
#        menubar.addmenuitem('Process','separator') 
        ...
        menubar.addmenuitem('Analyze','command',label='Group Weighted PCA Analysis',command=self.startPCAwt)
        menubar.addmenuitem('Analyze','separator')
        ...
        menubar.addmenuitem('Analyze','command',label='Ellipsoidal Analysis',command=self.makeElipstart)  
        menubar.addmenuitem('Analyze','separator')        
        ...
        menubar.addmenuitem('Workflow','separator')
        menubar.addmenuitem('Workflow','command',label='Export Workflow',command=self.exportAddinBatch)
        menubar.addmenuitem('Workflow','command',label='Import Workflow',command=self.importAddinBatch)
        menubar.addmenuitem('Workflow','command',label='Remove Workflow',command=self.removeAddinBatch)
        menubar.addmenuitem('Workflow','separator')
        ...
        menubar.addmenuitem('MCA Spectra','separator')
        menubar.addmenuitem('MCA Spectra','checkbutton',label='Define Alternate MCA Coordsystem',command=self.defineMCAalternateCoords,variable=self.MCAaltcoords)
        menubar.addmenuitem('MCA Spectra','separator')        
...
#more?
        ...
        w=15
        #processed data
        b=PmwTtkButtonBox.PmwTtkButtonBox(mbf,label_text="Process File",labelpos='n',hull_background='#d4d0c8',label_background='#d4d0c8',frame_background='#d4d0c8')
        #tempedit
        ...
        #crop
        self.mosaicCropLeft = 0
        self.mosaicCropRight = 0
    ...
    def saveCurrentFileBufferInfo(self):
        ...
        self.dataFileBuffer[self.activeFileBuffer]['FTIRgroup']=self.FTIRgroup
        self.dataFileBuffer[self.activeFileBuffer]['transform']=self.transinfo
        self.dataFileBuffer[self.activeFileBuffer]['MCxs']=[self.isXSMultiChan.get(),self.MCxsList]
    ...
    def loadFromCurrentFileBufferInfo(self,nodata=True):
        ...
        self.FTIRgroup=self.dataFileBuffer[self.activeFileBuffer]['FTIRgroup']
        self.transinfo=self.dataFileBuffer[self.activeFileBuffer]['transform']
        self.isXSMultiChan.set(self.dataFileBuffer[self.activeFileBuffer]['MCxs'][0])
        self.MCxsList=self.dataFileBuffer[self.activeFileBuffer]['MCxs'][1]
    ...
    def clickOnFileTab(self,page):
        ...
        self.FTIRgroup=self.dataFileBuffer[page]['FTIRgroup']
        self.transinfo=self.dataFileBuffer[page]['transform']
        self.isXSMultiChan.set(self.dataFileBuffer[page]['MCxs'][0])
        self.MCxsList=self.dataFileBuffer[page]['MCxs'][1]
...
#        self.contoursOn.set(0)
        ...
        dfinfo['FTIRgroup']=self.FTIRgroup
        dfinfo['transform']=self.transinfo
    ...
    def createSSRLMosaic(self,stype='MV'):
        globalfuncs.setstatus(self.status,"Ready")
        #get file name
        ...
        for impfn in multfn:
            ...
            if stype=='MV': 
                mosaicfn=stitch_hdf_mod.stitch_hdf(impfn,left=self.mosaicCropLeft,right=self.mosaicCropRight)
            elif stype=='man':
                mosaicfn=stitch_hdf_mod.stitch_hdf_man(impfn,left=self.mosaicCropLeft,right=self.mosaicCropRight)
            else:
                print ('invalid mosaic method')
                globalfuncs.setstatus(self.status,"Invalid mosaic method.")
                return                
    ...
    def createSSRLMosaicB(self):
        self.createSSRLMosaic(stype='man')

    def setMosaicCrop(self):
        left=tkinter.simpledialog.askinteger(title='Mosaic Crop Paramters',prompt='Enter pixels to crop on left side',initialvalue=self.mosaicCropLeft)
        if left is None:
            print ('exit, no value')
            return
        else: 
            self.mosaicCropLeft = int(left)
        right=tkinter.simpledialog.askinteger(title='Mosaic Crop Paramters',prompt='Enter pixels to crop on right side',initialvalue=self.mosaicCropRight)
        if right is None:
            print ('exit, no value')
            return
        else: 
            self.mosaicCropRight = int(right)           
    ...
    def checkdefineMCexport(self):
        print (self.isXSMultiChan.get(),self.MCxsList)
        if self.isXSMultiChan.get() and self.MCxsList==[]: 
            self.defineMCexport()

    def defineMCexport(self):
        globalfuncs.setstatus(self.status,"Ready")
        if not self.hasdata:
            print('No Data')
            globalfuncs.setstatus(self.status,'No Data')
            return
        self.multixsexportdialog=Pmw.SelectionDialog(self.imgwin,title="Cross Section Multiple Channels",buttons=('OK','Cancel'),defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Channels',scrolledlist_items=self.mapdata.labels,
                                                   command=self.selectmultixs)
        self.multixsexportdialog.component('scrolledlist').component('listbox').configure(selectmode=tkinter.EXTENDED)
        
    def selectmultixs(self,result):
        listdata=self.multixsexportdialog.getcurselection()
        self.multixsexportdialog.withdraw()
        if result=='Cancel':
            globalfuncs.setstatus(self.status,'No action taken')
            return
        self.MCxsList = listdata
        globalfuncs.setstatus(self.status,'Multi XS set')
...
##        self.domapimage()
    ...
    def importchannel_c(self,result):
        ...
        print ('data',self.mapdata.data.shape[:2])
        print ('imp',self.preimpdata.shape)
        print ('impdelta',self.impsizechange)
    ...
    def importcropdata(self):
        ...
        self.importCropdialog.activate()        
    ...
    def importCdone(self,result):
        ...
        self.impsizechange=array(self.mapdata.data.shape[:2])-array(self.preimpdata.shape)
        self.importCropdialog.deactivate()
...
##            #maybe update plot here?
    ...
    def asktagmultichannel(self):
        ...
        if not self.hasdata:
            ...
            globalfuncs.setstatus(self.status,'No Data')
            return
        self.multitagdialog=Pmw.SelectionDialog(self.imgwin,title="Add Tag to Multiple Channels",buttons=('OK','Cancel'),defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Channels',scrolledlist_items=self.mapdata.labels,
                                                   command=self.tagmultichannel)
        self.multitagdialog.component('scrolledlist').component('listbox').configure(selectmode=tkinter.EXTENDED)  
        
    def tagmultichannel(self,result):
        self.multiTagList=self.multitagdialog.getcurselection()
        self.multitagdialog.withdraw()
        if result=='Cancel':
            globalfuncs.setstatus(self.status,'No action taken')
            return        
        self.multitagpromptdialog = Pmw.PromptDialog(self.imgwin,title="Add Tag to Multiple Channels",buttons=('Start','End','Cancel'),defaultbutton='Cancel',
                                                    command=self.finishtagmultichannel)
       
    def finishtagmultichannel(self,result):
        tag = self.multitagpromptdialog.get()
        if result=='Cancel':
            self.multitagpromptdialog.withdraw()
            globalfuncs.setstatus(self.status,'No action taken')
            return    
        if tag=='' or tag is None:
            ...
            globalfuncs.setstatus(self.status,'Action cancelled')
            return            
        for ch in self.multiTagList:
            if result=='End':
                newch = ch+tag
            else:
                newch = tag+ch
            i=1
            while newch in self.mapdata.labels:
                print ('Channel duplicate...')
                newch = newch+'-'+str(i)
            datind=self.mapdata.labels.index(ch)
            self.mapdata.labels[datind]=newch
            self.updatelists()
        self.changes=1            
        self.multitagpromptdialog.withdraw()            
...
######################## Mapping
    ...
    def domapimagenewwindow(self,event):
        ...
        newdisp=Display.Display(self.imgwin,self.viewMCAplottoggle,self.startimgROI,self.showscalebar,self.showscalebarText,self.xyflip,main=0,callback=[None,None,self.displaySpectrumGraph,self.dispAddMarker,self.createXSfromData],sf=self.dispScaleFactor)
...
########################  Correlation Plot routines
    ...
    def makeMaskFromZoom(self):
         ...
         if self.maindisp.zmxyi[2]!=-1 and self.maindisp.zmxyi[3]!=-1:
             ...
             self.mask.mask=self.mask.mask[::-1,:]
         else:
    ...
    def makeMaskFromPM(self):
        globalfuncs.setstatus(self.status,"Ready")
        if not self.hasdata:
            print('No Data')
            globalfuncs.setstatus(self.status,'No Data')
            return
        if not self.plotmarkerexist:
            print('No plotmarkers')
            globalfuncs.setstatus(self.status,'No plotmarkers')
            return   
        #get radius
        rad=tkinter.simpledialog.askfloat(title='PM Masking',prompt='Enter radius (mm) from marker centers')
        if rad is None:
            print ('exit, no value')
            return
        #get sorttype?
        di = Pmw.ComboBoxDialog(self.imgwin,title='Sort Marker Options',scrolledlist_items=['All','Individual','Color','Shape'])
        resp = di.activate()
        sorttype = di.get()
        print (sorttype)
        
        maskdata = np.zeros(self.mapdata.data.get(0).shape,dtype=np.float32)
        cindex = []
        shapelist = ['sm circle','big circle','sm square','big square','sm emptycircle','big emptycircle','sm emptysquare','big emptysquare','sm triangle','big triangle','text']
        i=0
        mkd={}
        for m in self.plotmarkerlist:
            if m.valid:
                x,y = float(m.xpos.getvalue()),float(m.ypos.getvalue())
                c = m.color
                t = m.marker.getvalue()[0]
            ...
            gridX=self.mapdata.data.get(0)-x
            gridY=self.mapdata.data.get(1)-y         
            R=np.sqrt(gridX**2 + gridY**2)
            isin = np.where(R<=rad,1,0)
            

            if sorttype=='Individual':
                #all particles in individuals
                maskdata+=isin*2**i
            elif sorttype=='Shape':
                #by shape
                tc = shapelist.index(t)
                if tc not in mkd.keys():
                    mkd[tc]=np.zeros(self.mapdata.data.get(0).shape,dtype=np.float32)
                mkd[tc]=np.where(isin==1,isin*2**tc,mkd[tc])
                ##maskdata=np.where(isin==1,isin*2**tc,maskdata)
            elif sorttype=='Color':
                #by color
                cc = cindex.index(c)
                if cc not in mkd.keys():
                    mkd[cc]=np.zeros(self.mapdata.data.get(0).shape,dtype=np.float32)
                mkd[cc]=np.where(isin==1,isin*2**cc,mkd[cc])
                ##maskdata=np.where(isin==1,isin*2**cc,maskdata)
            else: # sorttype=='All':
                #all particles in one
                maskdata=np.where(isin==1,1,maskdata)
        ...
        name=tkinter.simpledialog.askstring(title='Add PM Mask as Channel',prompt="Enter new name for channel.")
        newname=globalfuncs.fixlabelname(name)
        if newname in self.mapdata.labels:
            print('Enter unique channel name')
            globalfuncs.setstatus(self.status,'Enter unique channel name')
            return
        self.addchannel(maskdata,newname)
        globalfuncs.setstatus(self.status,"Done")          
...
#################################  MCA viewing routines
    ...
    def setMCAslopeValue(self):
        mt='Enter number of ev per MCA bin - cancel to accept current values'
    ...
    def updateMCAgraph(self,plot=True):
        ...
        if not self.MCAviewexist:
            ...
            menubar.addmenuitem('MCA','separator')
            menubar.addmenuitem('MCA','checkbutton',label='Define Alternate MCA Coordsystem',command=self.defineMCAalternateCoords,variable=self.MCAaltcoords)
...
##                    self.MCAgraph.element_delete(g)            
        ...
        if self.MCAtracescalevar.get()=='Log':
            ...
            log=None
        for i in range(9):
            if l[i].get()==1 and len(self.MCArawdataBuffer[i])>0:
    ...
    def defineMCAalternateCoords(self):
        if self.MCAaltcoords.get()==0:
            self.MCAaltdata=None
            return
        self.defMCAaltdataDialog=Pmw.SelectionDialog(self.imgwin,title="Select Alt Coordinate Channel",buttons=('OK','Cancel'),defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Channel',scrolledlist_items=self.mapdata.labels,
                                                   command=self.selectdefineMCAalternateCoords)
        #self.defMCAaltdataDialog.component('scrolledlist').component('listbox').configure(selectmode=tkinter.EXTENDED)
        
    def selectdefineMCAalternateCoords(self,result):
        sel = self.defMCAaltdataDialog.getcurselection()
        self.defMCAaltdataDialog.withdraw()
        if result=='Cancel' or len(sel)<1:
            globalfuncs.setstatus(self.status,'No action taken')
            return
        else:
            self.MCAaltdata=sel[0]
    ...
    def testMCAfitdata(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            if sys.platform=='darwin':
                if 'MacOS' in apppath: apppath=apppath.replace('MacOS','Resources')
        #elif __file__:
        #    apppath=os.path.dirname(__file__)+os.sep
        else:
            apppath=os.getcwd()+os.sep
    ...
    def getHDF5MCAfromFile(self,pixno,multi):
        ...
        if "/main/xdata" in fid:
            self.currentMCAXvalues = fid['/main/xdata'][:]
        else:
            self.currentMCAXvalues = np.arange(self.mcamaxno)*self.currentMCAXraySlope

        if not multi:
            if self.MCAaltcoords.get()==1:
                ind = self.mapdata.labels.index(self.MCAaltdata)+2
                pixno=np.ravel(self.mapdata.data.get(ind))[int(pixno)]
            pixno=int(pixno)
            #print 'pix:',pixno
            globalfuncs.setstatus(self.status,'Getting MCA at pixel: '+str(pixno))
            row=(pixno/self.mapdata.nxpts)
            ...
            for p in pixno:
                if self.MCAaltcoords.get()==1:
                    ind = self.mapdata.labels.index(self.MCAaltdata)+2
                    newpixno.append(np.ravel(self.mapdata.data.get(ind))[int(p)])
                else:
                    row=(p/self.mapdata.nxpts)
                    p=p+row*self.mapdata.nxpts*(self.MCAnofiles-1)+(self.MCAoffset-1)*self.mapdata.nxpts+self.MCApixoffs*row-self.MCA1stpixoffs
                    newpixno.append(p)
...
##            self.MCAgraph.bind(sequence="<Motion>",  func=self.MCAmouseDrag)
    ...
    def updateMCASCA(self,*args):
        ...
        if self.MCAXAXIS.get()=='Bins':
            ...
            binmin=float(self.E1.getvalue())
            binmax=float(self.E2.getvalue())            
        width=binmax-binmin
...
#            dataToFit=data[:,binmin:binmax]
        ...
        if self.MCAaltcoords.get()==1:
            print ('before alt',min(matindex),max(matindex))
            #let's create a single row data with the mapind values, eliminating zeros...
            altind = np.ravel(self.mapdata.data.get(self.mapdata.labels.index(self.MCAaltdata)+2))
            oldmatind = matindex.copy()
            #minv=1e100
            #maxv=0
            #for pn in matindex:
            #    if altind[pn]==0: continue
            #    minv=min(minv,altind[pn])
            #    maxv=max(maxv,altind[pn])
            #matindex = np.arange(minv,maxv+1)
            matindex=np.arange(maxlines)
            matindex = matindex.reshape((len(matindex),))
            matindex=np.sort(matindex)
            print("after alt sort",np.min(matindex),np.max(matindex))
            fitShape=matindex.shape
        ##    takezoom=True
            matindex=np.array(matindex,dtype=int)
        ...
        if self.MCAXRFfitType.get()[0:4]=='Fast':
            ...
            for forIter in range(chunk):
                ...
                if self.MCAaltcoords.get()==1:
                    #reshape to "array" lengths
                    fc = globalfuncs.factors(matindex.shape[0])
                    print (fc)
                    if len(fc)>2:
                        matindex=matindex.reshape((fc[-1],fc[-2]))
                        fitShape=matindex.shape
                        fFitShape=fitShape
                        
                print (dataToFit.shape,fFitShape)
...
#                    chsigma=fresult['uncertainties'][ch][::-1,:]
                    ...
                    if chunk==1 and not takezoom:
                        if self.MCAaltcoords.get()==0:
                            ...
                            print ('remapping alt coords:',ch)
                            ind = self.mapdata.labels.index(self.MCAaltdata)+2
                            mindex=self.mapdata.data.get(ind)
                            chdata=np.ravel(chdata[::-1,:])
                            chsigma=np.ravel(chsigma[::-1,:])
                            
                            for xi in range(self.mapdata.nxpts):
                                for yi in range(self.mapdata.nypts):  
                                    if int(mindex[yi,xi])==0: 
                                        zerodata[yi,xi]=-100
                                        continue
                                    zerodata[yi,xi]=chdata[int(mindex[yi,xi])]
                                    zerosigma[yi,xi]=chsigma[int(mindex[yi,xi])]
    ...
    def makeMCAdialog(self,itype="Bins"):
        ...
        self.E1=Pmw.Counter(f,labelpos='n',label_text='Min '+text,entryfield_validate=vt,entry_width=7,increment=5,hull_relief=tkinter.FLAT,entryfield_modifiedcommand=self.updateMCASCA,hull_background='#d4d0c8',label_background='#d4d0c8')
        self.E2=Pmw.Counter(f,labelpos='n',label_text='Max '+text,entryfield_validate=vt,entry_width=7,increment=5,hull_relief=tkinter.FLAT,entryfield_modifiedcommand=self.updateMCASCA,hull_background='#d4d0c8',label_background='#d4d0c8')
    ...
    def MCAdotherebin(self,result):        
        ...
        if self.MCAXAXIS.get()=='Bins':
            ...
            vmin=float(self.E1.getvalue())
            vmax=float(self.E2.getvalue())
    ...
    ##                    sprs=1
        ...
        if self.MCAaltcoords.get()==0:
            if self.mapdata.nypts*self.mapdata.nxpts<len(newdata):
                adddata=np.reshape(newdata[:self.mapdata.nypts*self.mapdata.nxpts-1],(self.mapdata.nypts,self.mapdata.nxpts))
            else:
                adddata=np.reshape(newdata,(self.mapdata.nypts,self.mapdata.nxpts))
            #place new data into main data
        else:
            adddata=np.ravel(np.zeros((self.mapdata.nypts,self.mapdata.nxpts),dtype=np.float32))
            ind = self.mapdata.labels.index(self.MCAaltdata)+2
            inddata=np.ravel(self.mapdata.data.get(ind))
            for mi in range(len(inddata)):
                if inddata[mi]==0: continue
                adddata[mi]=newdata[int(inddata[mi])]
            adddata=np.reshape(adddata,(self.mapdata.nypts,self.mapdata.nxpts))

        self.addchannel(adddata,newname)
        print(time.process_time()-startt)
...
#################################  FTIR Analysis
    ...
    def IRprocessingWindow(self):
        ...
        if not self.IRwinexist:
            ...
            #atmos correction
            gc=Pmw.Group(g2,tag_text="Atmospheric Correction",hull_background='#d4d0c8',tag_background='#d4d0c8',ring_background='#d4d0c8')
            gc.interior().configure(background='#d4d0c8')
            gc.pack(side=tkinter.TOP,padx=5,pady=5,expand='yes',fill='both')       
            self.atmostype=Pmw.ComboBox(gc.interior(),history=0,selectioncommand=self.updateIRgraph,hull_background='#d4d0c8',
                                       labelpos='w',label_text="Method:  ",label_background='#d4d0c8')
            self.atmostype.setlist(self.FTIRgroup.defs.atmos)
            self.atmostype.pack(side=tkinter.TOP,padx=5,pady=5)               
            
            #mie correction
            gc=Pmw.Group(g2,tag_text="Mie Corrections",hull_background='#d4d0c8',tag_background='#d4d0c8',ring_background='#d4d0c8')
            gc.interior().configure(background='#d4d0c8')
            gc.pack(side=tkinter.TOP,padx=5,pady=5,expand='yes',fill='both')            

            self.miereftype=Pmw.ComboBox(gc.interior(),history=0,selectioncommand=self.updateIRgraph,hull_background='#d4d0c8',
                                       labelpos='w',label_text="Method:  ",label_background='#d4d0c8')
            self.miereftype.setlist(self.FTIRgroup.defs.mieref)
            self.miereftype.pack(side=tkinter.TOP,padx=5,pady=5)                
            self.miemaxiter=Pmw.EntryField(gc.interior(),labelpos='w',label_text='Max iterations: ',validate='numeric',entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.mieclusters=Pmw.EntryField(gc.interior(),labelpos='w',label_text='Num Clusters: ',validate='numeric',entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.miemaxiter.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')  
            self.mieclusters.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')   
            
            #denoising
            gc=Pmw.Group(g2,tag_text="Denoising",hull_background='#d4d0c8',tag_background='#d4d0c8',ring_background='#d4d0c8')
            gc.interior().configure(background='#d4d0c8')
            gc.pack(side=tkinter.TOP,padx=5,pady=5,expand='yes',fill='both')            
            self.dnwindow=Pmw.EntryField(gc.interior(),labelpos='w',label_text='Window Size: ',validate='numeric',command=self.updateIRgraph,entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.dnpoly=Pmw.EntryField(gc.interior(),labelpos='w',label_text='Polynomial Order: ',validate='numeric',command=self.updateIRgraph,entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.dnwindow.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')  
            self.dnpoly.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')   
            
            #range seletion
            gc=Pmw.Group(g2,tag_text="Wavenumber Range Selection",hull_background='#d4d0c8',tag_background='#d4d0c8',ring_background='#d4d0c8')
            gc.interior().configure(background='#d4d0c8')
            gc.pack(side=tkinter.TOP,padx=5,pady=5,expand='yes',fill='both')      
            self.rangemin=Pmw.EntryField(gc.interior(),labelpos='w',label_text='Min: ',validate='real',command=self.updateIRgraph,entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.rangemax=Pmw.EntryField(gc.interior(),labelpos='w',label_text='Max: ',validate='real',command=self.updateIRgraph,entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.rangemin.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')  
            self.rangemax.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')   
            
            #baseline
            gc=Pmw.Group(g2,tag_text="Baseline",hull_background='#d4d0c8',tag_background='#d4d0c8',ring_background='#d4d0c8')
            gc.interior().configure(background='#d4d0c8')
            gc.pack(side=tkinter.TOP,padx=5,pady=5,expand='yes',fill='both')            
            self.basetype=Pmw.ComboBox(gc.interior(),history=0,selectioncommand=self.updateIRgraph,hull_background='#d4d0c8',
                                       labelpos='w',label_text="Method:  ",label_background='#d4d0c8')
            self.basetype.setlist(self.FTIRgroup.defs.base)
            self.basetype.pack(side=tkinter.TOP,padx=5,pady=5)                
            
            #normalization
            gc=Pmw.Group(g2,tag_text="Normalization",hull_background='#d4d0c8',tag_background='#d4d0c8',ring_background='#d4d0c8')
            gc.interior().configure(background='#d4d0c8')
            gc.pack(side=tkinter.TOP,padx=5,pady=5,expand='yes',fill='both')
            self.normtype=Pmw.ComboBox(gc.interior(),history=0,selectioncommand=self.updateIRgraph,hull_background='#d4d0c8',
                                       labelpos='w',label_text="Method:  ",label_background='#d4d0c8')
            self.normtype.setlist(self.FTIRgroup.defs.norm)
            self.normtype.pack(side=tkinter.TOP,padx=5,pady=5)                
            self.normval=Pmw.EntryField(gc.interior(),labelpos='w',label_text='WN: ',validate='real',command=self.updateIRgraph,entry_width=20,hull_background='#d4d0c8',label_background='#d4d0c8')
            self.normval.pack(side=tkinter.TOP,padx=5,pady=2,anchor='w')  
            
            Pmw.alignlabels([self.atmostype,self.miereftype,self.basetype,self.normtype,self.miemaxiter, self.mieclusters, self.dnwindow, self.dnpoly, self.rangemin,self.rangemax, self.normval])
            
            self.updateIRwin()
    
            self.IRgraph=MyGraph.MyGraph(ls,whsize=(6.5,3),tool=1,graphpos=[[.15,.1],[.9,.9]],side=tkinter.LEFT)
            
            self.updateIRgraph()

    def updateIRwin(self):        
        self.atmostype.selectitem(self.FTIRgroup.decodeAtmos())
        self.miereftype.selectitem(self.FTIRgroup.mieReference)
        self.miemaxiter.setvalue(self.FTIRgroup.mieMaxIter)
        self.mieclusters.setvalue(self.FTIRgroup.mieClusters)
        self.dnwindow.setvalue(self.FTIRgroup.denoiseWindow)
        self.dnpoly.setvalue(self.FTIRgroup.denoisePolyOrder)
        self.rangemin.setvalue(self.FTIRgroup.rangeSelect[0])
        self.rangemax.setvalue(self.FTIRgroup.rangeSelect[1])
        self.basetype.selectitem(self.FTIRgroup.baseline)
        self.normtype.selectitem(self.FTIRgroup.normalization)
        self.normval.setvalue(self.FTIRgroup.normvalue)
        
    def updateFTIRgroup(self):
        self.FTIRgroup.encodeAtmos(self.atmostype.get())
        self.FTIRgroup.mieReference=self.miereftype.get()
        self.FTIRgroup.mieMaxIter=int(self.miemaxiter.getvalue())
        self.FTIRgroup.mieClusters=int(self.mieclusters.getvalue())
        self.FTIRgroup.denoiseWindow=int(self.dnwindow.getvalue())
        if not self.FTIRgroup.denoiseWindow & 1:
            self.FTIRgroup.denoiseWindow += 1
            self.dnwindow.setvalue(self.FTIRgroup.denoiseWindow)
        self.FTIRgroup.denoisePolyOrder=int(self.dnpoly.getvalue())
        self.FTIRgroup.rangeSelect[0]=float(self.rangemin.getvalue())
        self.FTIRgroup.rangeSelect[1]=float(self.rangemax.getvalue())
        self.FTIRgroup.baseline=self.basetype.get()
        self.FTIRgroup.normalization=self.normtype.get()
        self.FTIRgroup.normvalue=float(self.normval.getvalue())

    def updateIRgraph(self,force=False,*args):
        if not self.IRwinexist: return
        if self.MCAfilename=='': return
        if not self.MCAviewexist: return
        ...
        self.MCAgraph.setTitle('IR processed data')
        ird={}
        irv=[]
        j=0
        for i in range(9):
            if l[i].get()==1 and len(self.MCArawdataBuffer[i])>0:
                irv.append(self.MCArawdataBuffer[i])
                ird[j]=i
                j+=1
        if len(irv)==0: 
            print ('no data yet')
        ...
        irv=np.array(irv)
        #normalize...
        self.FTIRgroup.saveCurrent()
        self.updateFTIRgroup()
        resultx,resulty = IR_MathClass.DoNorm(MCAx,irv,self.FTIRgroup,force=force)

        #plot
        self.IRgraph.cleargraphs()
        for n in ird.keys():
            self.IRgraph.plot(tuple(resultx),tuple(resulty[n,:]),text='IR',color=c[ird[n]])
    ...
    def IRprocessAll(self):
        ...
        if self.MCAXAXIS.get()==('Bins'):
            ...
            if len(self.currentMCAXvalues) != len(self.MCArawdataBuffer[0]):
                ...
                MCAx=self.currentMCAXvalues   #fid['/main/xdata']
        ...
        self.FTIRgroup.saveCurrent()
        self.updateFTIRgroup()        
        resultx,resulty = IR_MathClass.DoNorm(MCAx ,mcadata,self.FTIRgroup)  
        
        newfn=os.path.splitext(self.MCAfilename)[0]+"_ircorr.hdf5"
    ...
    def loadIRsettings(self):
        ...
        infile=globalfuncs.ask_for_file([("JSON IR paramater files","*.json"),("all files","*")],self.filedir.get(),multi=False)
...
#################################  PCA routines
    ...
    def startPCAmini(self):
        ...
        self.pcaminidialog=smwPmw.SelectionBonusDialog(self.imgwin,title="Select PCA Channels",buttons=analysisOptions,defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Channels',scrolledlist_items=self.mapdata.labels,
                                                   command=self.pcamininext,buttonlength=4,radioselect_buttontype='radiobutton')
    ...
    def startPCAVectorAnalysis(self):
        ...
        if not self.maindisp.linegraph4present:
            ...
            bb=PmwTtkButtonBox.PmwTtkButtonBox(g1.interior(),labelpos='n',label_text='Data',orient='vertical',pady=3,frame_background='#d4d0c8',hull_background='#d4d0c8',label_background='#d4d0c8')
            w=15
            bb.add('Export',command=self.PCAVectAnalysisExportButton,style='SBLUE.TButton',width=w)
            bb.pack(side=tkinter.LEFT,fill='both',pady=5)
    ...
    def PCAVectAnalysisExportButton(self):
 
        outtext='Channel\t'+str(self.PCAAnalysisX.getvalue()[0])+'\t'+str(self.PCAAnalysisY.getvalue()[0])+'\n'
        i=0
        for c in self.PCAVectAnalysisPlot.get():
            index = self.PCAlastchans.index(c)
            xindex = int(self.PCAAnalysisX.getvalue()[0].split('Comp')[1])-1
...
#this stays here too... but not for PCA MCA data info -- decorrelated now
    ...
    def startPCAwt(self):
        ...
        if not self.groupWtPCAWindow.exist:
            ps = WeightedGroupPCA.GroupPCAParams(self.maindisp,self.PCAcompMAXFixed,self.PCAcompMAXNO,self.status,self.dataFileBuffer,self.activeFileBuffer,self.addchannel,self.saveEvectFromWtPCA)
            self.groupWtPCAWindow.create(self.mapdata, ps)
        else:
            self.groupWtPCAWindow.win.show()

    def saveEvectFromWtPCA(self,ev,prop,chans):
        print ("saving last PCA result to main...")
        self.PCAlastevect=ev
        self.PCAlastprop=prop
        self.PCAlastchans=chans            
...
################################# Particle Stats?
    ...
    def startPartStats(self,useMask=0,useWater=0):
        ...
        if self.particleStatisticsWindow.exist:
            print("Part Stats EXISTS UPDATING")
            self.particleStatisticsWindow.update(self.mapdata, ps)
        else:
            print("Part Stats BEING CREATED")
...
#################################  Ellipsoidal profiles
    ...
    def makeElipstart(self):
        globalfuncs.setstatus(self.status,"Ready")
        if not self.hasdata:
            print('No Data')
            globalfuncs.setstatus(self.status,'No Data')
            return
        ps = RadialProfileClass.ElipProfileParams(self.maindisp, self.root, self.datachan, self.domapimage, self.status, self.dataFileBuffer, self.activeFileBuffer, self.filedir)
        elipProfileWindow = RadialProfileClass.ElipProfile(self.imgwin, self.mapdata, ps)        
...
#################################  Deconvolution
    ...
    def savedeconvcalculation(self,newd,name):
        ...
        if name=='':
            ...
            globalfuncs.setstatus(self.status,'Enter new channel name')
            return
        #make sure name unique
        newname=globalfuncs.fixlabelname(name)
        if newname in self.mapdata.labels:
...
#################################  Making multi-cross sections

#[self.isXSMultiChan.get(),self.MCxsList]

    def createXSfromData(self,gridxi,gridyi,xsc,ysc,slope,b):
        
        if self.isXSMultiChan.get()==0:
            rawindices = [self.mapdata.labels.index(self.datachan.getvalue()[0])+2]
            nxs=1
        else:
            rawindices = []
            for lab in self.MCxsList:
                rawindices.append(self.mapdata.labels.index(lab)+2)
            ryv=[]
            nxs = len(rawindices)
            
        for rawi in rawindices:
            #reset the bases...
            gridxc=[]
            gridyc=[]
            griddc=[]
            ...
            raw = transpose(self.mapdata.data.get(rawi)[::-1,:])
            for i in gridxi:
                gridxc.append(xsc[i])
                newc=slope*i+b
                ...
                #print f1,f2,t
                if t!=0:
                    gridyc.append((ysc[im]*f1+ysc[ip]*f2)/t)
                    gridz.append((raw[i,im]*f1+raw[i,ip]*f2)/t)
                else:
                    gridyc.append(ysc[int(newc)])
                    gridz.append(raw[i,int(newc)])
            ...
            for i in gridyi:
                ...
                #print f1,f2,t
                if t!=0:
                    gridxc.append((xsc[im]*f1+xsc[ip]*f2)/t)
                    gridz.append((raw[im,i]*f1+raw[ip,i]*f2)/t)
                else:
                    gridxc.append(xsc[int(newc)])
                    gridz.append(raw[int(newc),i])
                griddc.append(math.sqrt((gridxc[len(gridxc)-1]-gridxc[0])**2+(gridyc[len(gridyc)-1]-gridyc[0])**2))
            #order
...
################################# Quantitative Analysis
    ...
    def loadquantstdfile(self):
        ...
        for line in lines:
            ...
            self.quantdict[l[1]]=[l[0],l[2],l[3],l[4],l[5]]
        #correlate defnames to channels
        self.chanquantdict={}
        self.quantloadcorrel(quantvals)
        
    def returntoLoadQuantFile(self):
        #place values
        if self.chanquantdict=={}: return
        for q in self.quantlist:
            cur=self.quantdict[self.chanquantdict[q.name]]
...
#################################  Workflow Ideas
    ...
    def removeAddinBatch(self):
        self.multibatchremovedialog=Pmw.SelectionDialog(self.imgwin,title="Remove Workflow",buttons=('OK','Cancel'),defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Workflow',scrolledlist_items=self.batchData.keys(),
                                                   command=self.selectremovebatchdestination)
        self.multibatchremovedialog.component('scrolledlist').component('listbox').configure(selectmode=tkinter.EXTENDED)
          
    def selectremovebatchdestination(self,result):
        wfs = self.multibatchremovedialog.getcurselection()
        self.multibatchremovedialog.withdraw()
        if result=='Cancel' or len(wfs)==0:
            globalfuncs.setstatus(self.status,'No action taken')
            return
        for scr in wfs:
            del self.batchData[scr]

        self.recreateBatchMenu()
        
        globalfuncs.setstatus(self.status,'Workflows removed...')   

    def exportAddinBatch(self):
        self.multibatchexportdialog=Pmw.SelectionDialog(self.imgwin,title="Export Workflow",buttons=('OK','Cancel'),defaultbutton='Cancel',
                                                   scrolledlist_labelpos='n',label_text='Select Workflow',scrolledlist_items=self.batchData.keys(),
                                                   command=self.selectexportbatchdestination)
        self.multibatchexportdialog.component('scrolledlist').component('listbox').configure(selectmode=tkinter.EXTENDED)

    def selectexportbatchdestination(self,result):
        wfs = self.multibatchexportdialog.getcurselection()
        self.multibatchexportdialog.withdraw()
        if result=='Cancel' or len(wfs)==0:
            globalfuncs.setstatus(self.status,'No action taken')
            return
        ...
        out=''
        for scr in wfs:
            out += json.dumps(self.batchData[scr])+'\n'

        fn=globalfuncs.ask_save_file('batchExport.json',self.filedir.get())
        if fn=='': 
            print('Export cancelled')
            globalfuncs.setstatus(self.status,'Export cancelled')
            return
        ...
        globalfuncs.setstatus(self.status,'Workflows exported...') 
    
    def importAddinBatch(self):

        fn=globalfuncs.ask_for_file([("Batch Commands","*.json"),("all files","*")],self.filedir.get())
        if fn=='': 
            print('Import cancelled')
            globalfuncs.setstatus(self.status,'Import cancelled')
            return
        ...
        self.recreateBatchMenu()
        globalfuncs.setstatus(self.status,'Workflow import complete')

    def recreateBatchMenu(self):
        #clear menu
        for ms in self.addinmenuOpts.keys():
            men = self.mainmenubar.component('Workflow-menu')
            ind = men.index(ms)
...
#################################  View colormaps
    ...
    def savetricolormap(self):
        ...
        if not self.hasdata:
            ...
            globalfuncs.setstatus(self.status,'No Data')
            return
        if not self.triColorWindow.exist:
