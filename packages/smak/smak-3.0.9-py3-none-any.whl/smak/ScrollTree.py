# -*- coding: utf-8 -*-
"""
Created on Mon May 15 12:32:02 2023

@author: samwebb
"""

# -*- coding: utf-8 -*-
'''Treeview and MultiListbox widgets with scrollbars.'''

import tkinter
import tkinter.ttk

ST_Style = tkinter.ttk.Style()
ST_Style.theme_use('default')
ST_Style.configure("Treeview", background='white')
...
class ScrolledTreeViewBox(ScrolledWidget):
    '''MultiListbox widget with one or two static or automatic scrollbars.
    Subwidgets are:
        listbox - ttk Treeview widget
    ...
    def _setScrolledWidget(self):
        ST_Style.configure("Treeview", background='white')  
        ST_Style.map("Treeview", background=[("selected","lightblue")])
        self.listbox = tkinter.ttk.Treeview(self)
        self.labels=[]
        self.selectCB=None
        self.openCB=None
        self.listbox.bind('<<TreeviewSelect>>',self.item_selected)
        self.listbox.bind('<<TreeviewOpen>>',self.item_opened)
        self.colors=None
        return self.listbox

    def identify_column(self,x):
        return self.listbox.identify_column(x)

    def setMode(self,mode):
        self.listbox.configure(selectmode=mode)
        
    def setCols(self,ncols):
        print (ncols)
        #self.ncols=int(ncols)
        #self.listbox["columns"]=range(self.ncols)
        #self.listbox['show']='headings'

    def setColNames(self,colnames):
        self.ncols=len(colnames)
        self.listbox["columns"]=list(range(self.ncols))
        self.listbox['show']='headings'
        for i in range(self.ncols):
            self.listbox.heading(i,text=colnames[i])
        
    def setColWidthAnchor(self,clist):
        for i in range(self.ncols):
            self.listbox.column(i,width=clist[0],anchor=clist[1])

    def setDefaultWA(self):
        for i in range(self.ncols):
            self.listbox.column(i,width=20,anchor='e')
            
    def getNextLabel(self):
        if len(self.labels) == 0:
            return 'L1',0
        else:
            n=int(self.labels[-1].split('L')[1])
            n+=1
            return 'L'+str(n),n-1
            
    def insert(self,values,pos='end',addchild=False):
        text,ind=self.getNextLabel()
        self.labels.append(text)
        if self.colors is None:
            rv = self.listbox.insert("",pos,iid=text,values=values)
        else:
            rv = self.listbox.insert("",pos,iid=text,values=values,tags=str(ind%len(self.colors)))
        if addchild: self.listbox.insert(rv,'end')
        return rv
        
    def setSelect(self,cb):
        self.selectCB=cb
    ...
    def item_selected(self,event):
        if self.selectCB is None:
            return
        self.selectCB(self.curselection())
        
    def item_opened(self,event):
        if self.openCB is None:
            return
        self.openCB(self.curselection())
      
    def setColors(self,colors):
        self.colors=colors
        for i in range(len(self.colors)):
            self.listbox.tag_configure(str(i),background=self.colors[i])
    ...
    def delete(self,i):
        self.listbox.delete(self.labels[i])
        self.labels.pop(i)        
  
    def curselection(self):
        sv = self.listbox.selection()
        rv=[]
        for v in sv:
            rv.append(int(v.split('L')[1])-1)
        return rv
        
    def getitem(self,index):
        return self.labels[index]
        
    def itemEdit(self,item,newval):        
        self.listbox.item(item,values = newval)
        
    def itemEditAtPos(self,item,postion,newval):
        cv = list(self.listbox.item(item,option='values'))
        cv[postion]=newval
        self.listbox.item(item,values = cv)