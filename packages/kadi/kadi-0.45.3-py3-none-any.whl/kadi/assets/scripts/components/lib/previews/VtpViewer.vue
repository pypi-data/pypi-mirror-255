    ...
    <div ref="container" v-else>
      <div class="card toolbar" ref="toolbar">
        <div class="card-body px-1 py-0">
          ...
          <div class="input-group input-group-sm d-md-inline-flex w-auto my-1 mr-0 mr-md-2"
               v-if="colorByOptions.length > 1">
            <div class="input-group-prepend">
              <span class="input-group-text">{{ $t('Field data') }}</span>
            </div>
            <select class="custom-select" v-model="selectedColor">
              <option v-for="option in colorByOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>
          </div>
          <div class="input-group input-group-sm d-md-inline-flex w-auto my-1"
               v-if="componentOptions.length > 0">
            <div class="input-group-prepend">
              <span class="input-group-text">{{ $t('Components') }}</span>
            </div>
            <select class="custom-select" v-model="selectedComponent">
      ...
      <div class="render-container bg-light" ref="renderContainer">
        <div class="text-muted ml-3 my-2" v-if="error">
          <i class="fa-solid fa-triangle-exclamation mr-1"></i> {{ error }}
        </div>
        <div class="text-muted ml-3 my-2" v-else-if="loading">
...
export default {
  ...
  methods: {
    ...
    initializeViewer() {
      ...
      const fontStyle = {fontFamily: 'sans-serif', fontColor: 'black'};
