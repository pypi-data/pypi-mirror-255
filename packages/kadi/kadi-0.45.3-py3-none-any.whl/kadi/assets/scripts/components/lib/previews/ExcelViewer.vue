  ...
  <div>
    ...
    <div class="card" v-else>
      <div class="card-body p-1">
        <div v-if="!loading">
          <div class="table-responsive max-vh-75 mb-3" v-if="currentSheet.length > 0">
          ...
          <div class="mb-2">
            <em class="text-muted" v-if="currentSheet.length === 0">{{ $t('No content.') }}</em>
          </div>
        </div>
        <div class="overflow-auto ws-nowrap">
          <span class="sheet-nav btn btn-sm btn-light"
  ...
  methods: {
    async loadWorkbook() {
      ...
      try {
        ...
        kadi.base.flashDanger($t('Error loading workbook.'), {request: error.request});
      } finally {
        this.loading = false;
      }
    },
