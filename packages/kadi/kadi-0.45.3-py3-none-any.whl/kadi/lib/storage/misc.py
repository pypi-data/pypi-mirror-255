...
def create_misc_storage():
    """Create a local storage that can be used for miscellaneous uploads.

    Will use the local path as specified in ``MISC_UPLOADS_PATH`` in the application's
    configuration as root directory for the storage and only two directories for all
    generated file paths.

    :return: The storage instance.
    """
    return LocalStorage(
        root_directory=current_app.config["MISC_UPLOADS_PATH"], num_dirs=2
    )
...
def preview_thumbnail(identifier, filename):
    ...
    Uses the local storage as returned by :func:`create_misc_storage` to send the
    thumbnails.
