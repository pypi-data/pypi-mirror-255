...
def merge_users(primary_user, secondary_user):
    ...
    the secondary user to the primary user. The primary user will then be able to log in
    ...
    # Migrate all permissions (even though these are currently still unused).
    for permission in secondary_user.permissions:
        if permission not in primary_user.permissions:
            primary_user.permissions.append(permission)

    secondary_user.permissions = []

    # Migrate all roles. In case both users have different roles for the same resource,
    # the primary user could now have multiple of such roles. Since roles are simply
    # groupings of permissions and only the permissions themselves are checked, this
    # does not really matter and would be resolved once a role is changed again. A
    # special case is the creator's role, which cannot be changed, so we handle this
    # case separately.
