...
class RecordPDF(PDF):
    ...
    def print_extras(self):
        ...
        if extras:
            ...
            self._print_placeholder(_("No extra metadata."))
...
class RecordROCrate(ROCrate):
    ...
    def add_record_metadata(self, record, record_export_data):
        ...
        if record_data["tags"]:
            record_entity["keywords"] = ", ".join(record_data["tags"])
        ...
        for file_data in record_data["files"]:
            ...
            if file_data["description"]:
                file_entity["text"] = file_data["description"]
...
def get_record_dict_data(record, export_filter, user):
    ...
    schema = FileSchema(many=True, exclude=["storage", "state", *exclude_attrs])
