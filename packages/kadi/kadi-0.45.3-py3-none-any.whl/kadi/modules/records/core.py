...
def purge_record(record):
    ...
    # Avoids possible race conditions when purging records and collections
    # simultaneously.
    with NestedTransaction(exc=StaleDataError):
        record.collections = []
    ...
    for record_link in record_links:
        ...
        revision_created = create_revision(linked_record)
        db.session.commit()

        if revision_created:
            signal_resource_change(linked_record)
