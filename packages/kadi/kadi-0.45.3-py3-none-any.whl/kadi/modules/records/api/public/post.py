...
from kadi.modules.records.forms import LegacyUploadDataForm
...
def new_upload(id, schema):
    ...
    indicated by the ``upload_type`` property of the returned upload, being either one
    of ``"direct"`` or ``"chunked"``.

    For both upload types, the following :http:put:`endpoint
    </api/records/(int:record_id)/uploads/(uuid:upload_id)>` is used to upload the file
    data, which is also returned as the ``_actions.upload_data`` property of the
    returned upload. For chunked uploads, the required size for uploading chunks (except
    for the last chunk) is returned as the ``_meta.chunk_size`` property.
    """
    record = Record.query.get_active_or_404(id)
    data = schema.load_or_400()
...
def finish_upload(record_id, upload_id):
    ...
    Will finish the chunked upload specified by the given *upload_id* of the record
    specified by the given *record_id*. Only uploads owned by the current user can be
    finished.

    The upload will be finished by starting an upload processing task to finalize it.
    The status of this task can be queried using the following
    :http:get:`endpoint </api/records/(int:record_id)/uploads/(uuid:upload_id)>`, which
    is also returned as the ``_links.self`` property of the returned upload.
    """
...
@permission_required("update", "record", "id")
@scopes_required("record.update")
def upload_file_legacy(id):
    """Legacy endpoint to directly upload a file to a record."""
    record = Record.query.get_active_or_404(id)
    form = LegacyUploadDataForm()

    if not form.validate():
        return json_error_response(400, errors=form.errors)

    if form.size.data > const.UPLOAD_CHUNKED_BOUNDARY:
        return json_error_response(413)

    file = record.active_files.filter(File.name == form.name.data).first()
    ...
    if file is not None:
        ...
        if file.storage_type != storage.storage_type:
            return json_error_response(
                400,
                description=_(
                    "Replacing files of different storage types is not supported."
                ),
            )
    ...
    try:
        file = save_upload_data(upload, form.blob.data, checksum=form.checksum.data)
