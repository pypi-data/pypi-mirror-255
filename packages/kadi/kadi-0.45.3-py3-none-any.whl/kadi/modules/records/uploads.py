...
def save_chunk_data(upload, stream, *, index, size, checksum=None):
    ...
    try:
        ...
        # Always update the upload's timestamp, since it is relevant it's expiration.
...
def merge_chunk_data(upload):
    ...
    Uses :func:`complete_file_upload` to complete the upload process.
...
def complete_file_upload(upload):
    ...
    :return: The newly created or updated file or ``None`` if an error occured while
    ...
    try:
        ...
        # data has most likely already been overwritten, so we delete the file.
    ...
    if revision_created:
        signal_resource_change(file, user=upload.creator, created=upload.file is None)
