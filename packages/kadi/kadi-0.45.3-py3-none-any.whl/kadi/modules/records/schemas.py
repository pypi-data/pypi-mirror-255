...
class RecordSchema(KadiSchema):
    ...
    def _generate_links(self, obj):
        return {
            ...
            "view": url_for("records.view_record", id=obj.id),
        }
    ...
    def _generate_actions(self, obj):
        actions = {
            ...
            "upload_file": url_for("api.upload_file_legacy", id=obj.id),
...
class BaseRecordLinkSchema(KadiSchema):
...
class UploadSchema(KadiSchema):
    ...
    chunks = fields.Nested(
        "ChunkSchema", attribute="active_chunks", many=True, dump_only=True
    )
    ...
    def _generate_meta(self, obj):
        ...
        if obj.upload_type == UploadType.CHUNKED:
            meta["chunk_size"] = const.UPLOAD_CHUNK_SIZE
