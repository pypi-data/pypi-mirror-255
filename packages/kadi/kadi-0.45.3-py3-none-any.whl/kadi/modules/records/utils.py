...
def search_records(
    ...
    :param extras: (optional) A list of dictionaries to specifiy search queries within
        ...
        and one or multiple value restrictions depending on the type. See also
        :attr:`.Record.extras`.
...
def get_user_quota(user=None):
    ...
    Uploads that replace files are only added to the total if their (maximum) size is
    larger than the file they are replacing.
    ...
    # For all active and processing uploads that replace a file, retrieve the maximum
    # upload size per replaced file (taking into account the case of multiple uploads
    # for a single file) and check if its larger than the size of the replaced file. If
    # so, we add the difference to the total.
    for upload in (
        uploads_query.filter(Upload.file_id.is_not(None))
        .with_entities(Upload.file_id, db.func.max(Upload.size).label("max_size"))
        ...
        if upload.max_size > file.size:
            total_size += upload.max_size - file.size
