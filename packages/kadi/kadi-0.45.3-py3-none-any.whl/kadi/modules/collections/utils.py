...
from kadi.modules.records.links import _calculate_link_meta
...
def get_collection_links_graph(collection, records=False, children=False, user=None):
    ...
    data = _get_collection_data(collection)

    if records:
        # Limit the records of the collection to a maximum of 500.
        records_query = (
        ...
        _calculate_link_meta(data["record_links"])
    ...
    if children:
        ...
        for child in child_collections_query:
            data["children"].append(_get_collection_data(child))
