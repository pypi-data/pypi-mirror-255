...
def get_terms(page, per_page, qparams):
    ...
    if not has_capabilities("term_search"):
...
def preview_index_image():
    ...
    if filepath is not None and os.path.isfile(filepath):
        response = send_file(
            filepath, download_name=filename, mimetype=const.MIMETYPE_JPEG
        )
        # There is no more direct way to disable X-Sendfile for this response.
