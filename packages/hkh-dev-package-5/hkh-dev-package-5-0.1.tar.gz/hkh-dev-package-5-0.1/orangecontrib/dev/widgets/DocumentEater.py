...
persist_directory = join(data_dir, "db")
if not os.path.exists(persist_directory):
    os.makedirs(persist_directory)
source_directory = join(data_dir, 'source_documents')
...
def does_vectorstore_exist(persist_directory: str) -> bool:
    ...
    if os.path.exists(os.path.join(persist_directory, 'index.faiss')) and os.path.exists(os.path.join(persist_directory, 'index.pkl')):
        return True
    return False
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        db = FAISS.load_local(persist_directory, embeddings=embeddings)
        # Find in metadata the files already in the index not to add them again
        processed_files = []
        for key, item in db.docstore._dict.items():
            processed_files.append(item.metadata["source"])
        processed_files = list(set(processed_files))
        texts = process_documents(processed_files)
    ...
    db = None
    print(f"{len(texts)} documents added to db...")
