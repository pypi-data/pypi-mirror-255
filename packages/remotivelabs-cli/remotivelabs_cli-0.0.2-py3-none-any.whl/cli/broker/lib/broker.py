...
from .errors import ErrorPrinter as err_printer
...
class Broker:
    ...
    def __init__(self, url: str, api_key):
        ...
        if api_key is None or api_key == "":
            ...
            err_console.print("Option --api-key will is deprecated and will be removed. "
                              "Use access access tokens by logging in with cli.")
            os.environ["ACCESS_TOKEN"] = "false"
    ...
    def seek(self, recording_and_namespace: List, offset: int):

        def to_playback(rec):
            return {
                "namespace": rec['namespace'],
                "path": rec['recording'],
                "mode": br.traffic_api_pb2.Mode.SEEK,
                "offsettime": offset
            }

        playback_list = map(to_playback, recording_and_namespace)

        infos = br.traffic_api_pb2.PlaybackInfos(
    ...
    def play(self, recording_and_namespace: List):
    ...
    def stop_play(self, recording_and_namespace: List):
    ...
    def pause_play(self, recording_and_namespace: List):
    ...
    def stop(self, namespace: str, path: str):
        playback_list = [
            {
                "namespace": namespace,
                "path": path,
    ...
    def pause(self, namespace: str, path: str):
        playback_list = [
            {
                "namespace": namespace,
                "path": path,
            ...
            try:
                self.system_stub.BatchDeleteFiles(
                    br.system_api_pb2.FileDescriptions(
                        fileDescriptions=[br.system_api_pb2.FileDescription(path=file.replace(ntpath.sep,
                                                                                              posixpath.sep))]
                    )
                )
            except grpc.RpcError as rpc_error:
    ...
    def list_signal_names(self, namespace):
        ...
        for networkInfo in configuration.networkInfo:
            ...
            for finfo in res.frame:
                signal_names.append({'signal': finfo.id.name, 'namespace': networkInfo.namespace.name})
    ...
    def list_signal_names(self):
        ...
        for networkInfo in configuration.networkInfo:
            ...
            for finfo in res.frame:
                ...
                signal_names.append({'signal': finfo.signalInfo.id.name, 'namespace': networkInfo.namespace.name})
                for sinfo in finfo.childInfo:
                    signal_names.append({'signal': sinfo.id.name, 'namespace': networkInfo.namespace.name})
        return signal_names
    ...
    def validate_and_get_subscribed_signals(self, subscribed_namespaces: List[str], subscribed_signals: List[str]) \
        ...
        available_signals = list(filter(verify_namespace, self.list_signal_names()))
        ...
        signals_subscribed_to_but_does_not_exist = \
            set(subscribed_signals) - set(map(lambda s: s['signal'], signals_to_subscribe_to))

        if len(signals_subscribed_to_but_does_not_exist) > 0:
            err_printer.print_hint(f"One or more signals you subscribed to does not exist "
                                   f", {signals_subscribed_to_but_does_not_exist}, please try again without those")
            exit(1)

        return list(map(lambda s: SubscribableSignal(s['signal'], s['namespace']), signals_to_subscribe_to))

    def subscribe(self, subscribed_signals: list, subscribed_namespaces: list, on_frame,
