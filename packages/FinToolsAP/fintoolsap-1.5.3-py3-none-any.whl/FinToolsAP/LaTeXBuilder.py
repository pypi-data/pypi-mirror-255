...
2. Create a preamble.tex and endtex.tex file in your configuration directory (~/.config/fintools/LaTeXBuilder/).
...
def df_to_tex_file(df: pandas.DataFrame, 
    ...
    with open(output_path, 'w') as output_file:
        ...
        if(date_col in df_temp.columns):
            df_temp[date_col] = df_temp[date_col].dt.date
        output_file.write(df_temp.to_latex(**kwargs))
        output_file.close()
    ...
    with open(written_file_path, 'a') as output_file:
        ...
        if(is_file):
            ...
            for file in os.listdir(input_path):
                ...
                written = _write_table(input_path, output_file)
...
def _load_preamble(**kwargs) -> tuple[str, str]:
    ...
    **kwargs: Unused keyword arguments.
    ...
    user_config_dir = pathlib.Path(os.path.expanduser('~') + '/.config/fintools/LaTeXBuilder/')
