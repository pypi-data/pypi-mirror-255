...
class LoadThirdModel:

    __EMAIL_PARSER_MODEL_ZIP_URL = 'https://d2olrrfwjazc7n.cloudfront.net/website/assets/entity_extraction_model/output/output_THIRD.zip'
    __BASE_DIR = os.getcwd()
    __local_zip_file_path = os.path.join(__BASE_DIR, 'output_THIRD.zip')
    ...
    def __load_model(cls):
        try:
            logging.info("Checking Third Model File")
            if os.path.exists(cls.__local_zip_file_path) and os.path.exists(os.path.join(cls.__extract_to, 'output_THIRD//model-best')):
                logging.info("Loading Pretrained Third Model")
                nlp = spacy.load(os.path.join(cls.__extract_to, 'output_THIRD//model-best'))
                logging.info("Model Third has been loaded")
                return nlp
                ...
                try:
                    logging.info("Downloading Third Model File")
                    ...
                    if response.status_code == 200:
                        ...
                        logging.error("Failed to download the Third Model file")
                        return None
