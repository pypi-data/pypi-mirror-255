...
class GetArguments:
    ...
    def get_build_year(self):
        if len(self.res['BUILD YEAR']) != 0:
            words_to_remove = ['built','blt','build','b']
            output_list = []
    ...
    def get_cranes(self):
        if len(self.res['CRANES']) != 0:
            words_to_remove = ['cranes', 'crane', 'cr', 'hydraulic','electro', 'deck', 'abt','four','electrohydraulic']
            output_list = []
...
class EntityExtractor(LoadModel, LoadSecondModel, LoadThirdModel):
    ...
    def extract_entities(self):
        ...
        text_to_process = self.__cleaned_text_parts[1]

        # Split text into two chunks with an overlap of up to 20 tokens
        chunk_size = len(text_to_process) // 2
        overlap = 20
        chunks = [text_to_process[i:i+chunk_size+overlap] for i in range(0, len(text_to_process), chunk_size)]

        # Process each chunk using the NLP models
