...
class EntityExtractor(LoadModel, LoadSecondModel, LoadThirdModel):
    ...
    async def process_nlp(self, nlp, entities_dict):
        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(None, nlp, self.__cleaned_text_parts[1])
    ...
    async def extract_entities(self):
        ...
        for nlp_func in [self.__nlp_first, self.__nlp_second, self.__nlp_third]:
            task = asyncio.create_task(self.process_nlp(nlp_func, entities_dict))
            tasks.append(task)
