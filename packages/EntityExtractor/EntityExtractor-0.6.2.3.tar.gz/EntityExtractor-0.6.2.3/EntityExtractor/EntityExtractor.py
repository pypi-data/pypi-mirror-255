...
class GetArguments:
    ...
    def get_cranes(self):
        if len(self.res['CRANES']) != 0:
            words_to_remove = ['cranes', 'crane', 'cr', 'hydraulic','electro', 'deck', 'abt','four']
            output_list = []
...
class EntityExtractor(LoadModel, LoadSecondModel, LoadThirdModel):
    ...
    def extract_entities(self):
        ...
        overlap_size = chunk_size // 2  # You can adjust the overlap size as needed

        # Generate overlapping chunks
        chunks = [self.__cleaned_text_parts[1][i:i + chunk_size] for i in range(0, len(self.__cleaned_text_parts[1]) - chunk_size + 1, overlap_size)]
