...
class ShiftMeasurementsMixin:
    ...
    def shift_measurements_compute_shift_along_axis(
        ...
        shift_array, error, phasediff = skir.phase_cross_correlation(
            ...
            # NB: This is needed to supress a deprecation warning from skir if the error is not requested
            return_error="always",
        )
    ...
    def shift_measurements_compute_gradient(self, strip: np.ndarray, axis: int) -> np.ndarray:
        ...
        gradient = numerator / denominator
        # Trim the ends by the roll amount to remove edge effects resulting from the edge wrap.
        # And, take the median along the opposite axis from which we compute the gradient.
        gradient_median = np.nanmedian(
            gradient[roll_amount:-roll_amount], axis=self.shift_measurements_opposite_axis(axis)
        )
        return gradient_median
