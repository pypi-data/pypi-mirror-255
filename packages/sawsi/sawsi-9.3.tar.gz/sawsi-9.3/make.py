...
@cli.command()
@click.argument('project', 'app')
def init(project_name, app_name):
    # 현재 작업 디렉토리를 기준으로 앱 디렉토리 생성
    project_path = Path(os.getcwd())
    project_path.mkdir(parents=True, exist_ok=True)

    # 기본 파일 구조 생성
    create_base_dir(app_name, project_path, project_name)

    # 앱 구조 생성
    create_app_dir(app_name, project_path)

    click.echo(f"{project_name} > App {app_name} has been created in {project_path}")
