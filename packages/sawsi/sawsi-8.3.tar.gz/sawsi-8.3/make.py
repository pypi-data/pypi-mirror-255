...
def create_base_dir(app_name, project_path):
    target_files = ['api.py', 'errs.py', 'requirements', 'buildspec.yml']
    for target_file in target_files:
        source_file = boilerplate_dir / target_file
        copy_without_overwrite(source_file, project_path / 'requirements.txt')
        click.echo(f"Copied {target_file} to {project_path}")

    # {{app}} 문자열 치환
    replace_app(project_path / 'buildspec.yml', app_name)
