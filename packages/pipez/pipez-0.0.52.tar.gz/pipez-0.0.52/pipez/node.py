...
class Node(ABC):
    ...
    def get(
        ...
        logging.debug(f'{self.name} START get')
        if self._in_queue is None:
            logging.debug(f'{self.name} END get [return None]')
            return None
            ...
            if len(set([len(batch) for batch in results])) > 1:
                logging.debug(f'{self.name} END get [return BatchStatus.ERROR]')
            ...
            logging.debug(f'{self.name} END get [processed queue-LIST, return batch]')
            return batch
        else:
            logging.debug(f'{self.name} END get [processed queue-NOT-LIST, return batch]')
    ...
    def put(
        ...
        logging.debug(f'{self.name} START put')
        if self._out_queue is None:
            logging.debug(f'{self.name} END put [return None]')
            return
        elif isinstance(self._out_queue, list):
            for queue in self._out_queue:
                queue.put(batch)
            logging.debug(f'{self.name} END put [processed queue-LIST]')
        else:
            self._out_queue.put(batch)
            logging.debug(f'{self.name} END put [processed queue-NOT-LIST]')
    ...
    def _step(
        ...
        try:
            st = monotonic()
            logging.debug(f'{self.name} START work_func [input is {input.__class__}]')
            out: Batch = self.work_func() if input is None else self.work_func(input)
            logging.debug(f'{self.name} END work_func [out is {out.__class__}]')
