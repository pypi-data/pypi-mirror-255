...
#path = os.path.split(widgetFile.loaded_file)[0]
path = "C:/Users/U00N7017/Desktop/6 - Classification automatique des Modes de Défaillances des FT"

# Load the embedding model
embeddings_model_name = "all-MiniLM-L6-v2"
embeddings_model_name = "Camembert"
embeddings_model_path = join(path, embeddings_model_name)
embeddings_model_path = "C:/Users/u00b4092/Desktop/Camembert"

model = SentenceTransformer(embeddings_model_path)

corpus_input = in_data
t = time.time()
# description_embeddings = embeddings.embed_documents(corpus_input.metas[:,0])
# cause_sol_embeddings = embeddings.embed_documents(corpus_input.metas[:,3])
A = corpus_input.metas[:,-1]
champs_textuel_embeddings = []
model.max_seq_length=512
for text in A:
    vect = model.encode([text])
    champs_textuel_embeddings.append(vect[0])

# description_embeddings = np.array(description_embeddings)
# cause_sol_embeddings = np.array(cause_sol_embeddings)
champs_textuel_embeddings = np.array(champs_textuel_embeddings)

# print("Length of embeddings description:", len(description_embeddings[0]))
# print("Length of embeddings cause_sol:", len(cause_sol_embeddings[0]))
#doc_embeddings = np.array(description_embeddings, cause_sol_embeddings)

embedding_vars = []
for i in range(len(champs_textuel_embeddings[0])):
    str_name = "embedding_champs_textuel_" + str(i+1)
    embedding_vars.append(ContinuousVariable(str_name))
print(len(embedding_vars))
domain = Domain(attributes=[],
                metas=embedding_vars,  # Using the ContinuousVariable objects
                class_vars=corpus_input.domain.class_vars)

# Create an empty Table with the appropriate number of rows
corpus_output = Table.from_domain(domain, len(corpus_input))

for i in range(len(embedding_vars)):
    corpus_output.metas[:,i] = champs_textuel_embeddings[:,i]

# Set the class variable (if present) from the input Corpus
corpus_output.Y = corpus_input.Y

# Set the output Corpus to the output widget (make sure to name your Corpus widget 'corpus_output')
out_data = corpus_output
