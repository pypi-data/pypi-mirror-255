...
class RecordEditorLock(admin.ModelAdmin):
    table = ""
    session = ""
    timeout = settings.BLOCK_RECORD_TIMEOUT

    def __set_variables(self, path: str, session: str) -> None:
        self.table = settings.SERVICE_NAME + "_" + path
    ...
    def __password_on_change(self, http_referer: str) -> bool:
        ...
        if self.table != settings.SERVICE_NAME + "_" + check_path:
            return False
        self.__set_cache()
        return True
...
def make_site_header(project_module: str) -> SafeString:
    ...
    header = f"Панель администратора: <br> микросервис '{settings.SERVICE_NAME}' (v.{version})"
    return mark_safe(header)


def make_site_title() -> str:
    return f"Микросервис '{settings.SERVICE_NAME.capitalize()}'"
