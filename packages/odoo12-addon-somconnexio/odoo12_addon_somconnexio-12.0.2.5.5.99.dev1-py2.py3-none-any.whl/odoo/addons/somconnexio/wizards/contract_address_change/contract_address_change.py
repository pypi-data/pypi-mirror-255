...
class ContractAddressChangeWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if defaults["contract_id"]:
            contract = self.env["contract.contract"].browse(defaults["contract_id"])
            partner_bank_id = (
                contract.partner_id.bank_ids and contract.partner_id.bank_ids[0]
            )
            defaults["partner_bank_id"] = (
                partner_bank_id.id if partner_bank_id else False
            )
