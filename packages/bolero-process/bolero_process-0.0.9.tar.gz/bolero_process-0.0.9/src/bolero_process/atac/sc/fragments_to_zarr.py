...
def _chrom_sizes_to_chorm_offset_table(chrom_sizes, remove_chr=False):
    ...
    if remove_chr:
        chrom_offset_table.index = chrom_offset_table.index.str.replace("^chr", "", regex=True)
...
def fragments_to_cutsite_zarr(
    ...
    if sort_fragments:
        ...
        sorted_fragment_path = _sort_fragments_by_barcode(fragments_path)
    ...
    chrom_offset_table = _chrom_sizes_to_chorm_offset_table(chrom_sizes, remove_chr=remove_chr)
...
def adata_cutsites_to_zarr(adata_path, output_zarr_path, barcode_prefix=None, chunk_size=20000000):
    ...
    chrom_offset_table = _chrom_sizes_to_chorm_offset_table(chrom_sizes, remove_chr=False)
