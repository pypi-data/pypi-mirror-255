...
from heaobject.root import HEAObject, DesktopObjectTypeVar, DesktopObjectDict, DesktopObject, desktop_object_type_for_name, Permission, PermissionGroup, DefaultPermissionGroup
from heaobject.volume import DEFAULT_FILE_SYSTEM, FileSystem, MongoDBFileSystem
from heaobject.error import DeserializeException
from heaobject.user import NONE_USER
from heaserver.service.oidcclaimhdrs import SUB
from aiohttp import web
import logging
from typing import Union, Optional, Type, cast
from collections.abc import Iterable, Sequence, Mapping, AsyncGenerator
...
def has_permissions(obj: DesktopObject, sub: str | None, permissions: Iterable[Permission] | PermissionGroup) -> bool:
    """
    ...
    :param sub: the user subject. If None, the NONE_USER is assumed.
    :param permissions: an iterable of Permission enum values or an object that adheres to the PermissionGroup protocol
    (required).
    :return: True or False.
    :raises AttributeError: if the permissions argument is neither an Iterable of Permission objects nor an object that
    adheres to the PermissionGroup protocol.
    """
    if isinstance(permissions, Iterable):
        permissions_ = permissions
    else:
        permissions_ = cast(PermissionGroup, permissions).perms
    return obj.has_permissions(sub if sub is not None else NONE_USER, permissions_)
...
def display_name_for_multiple_objects(objs: Sequence[HEAObject]) -> str:
    """
    Returns a user-friendly display name for multiple HEA objects. For one
    object, it will return just the object's display name.

    :param objs: a sequence of HEA objects
    ...
    match objs.length:
        ...
        case _:
            return f'{objs[0].display_name} and {objs.length - 1} other objects'
