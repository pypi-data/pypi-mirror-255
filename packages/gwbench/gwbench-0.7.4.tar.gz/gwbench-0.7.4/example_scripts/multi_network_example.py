...
from gwbench import MultiNetwork, injections_CBC_params_redshift, M_of_Mc_eta, f_isco_Msolar
...
#############################################################################
### MultiNetwork GW Benchmarking
#############################################################################

# initialize MultiNetwork and do general setup
mulnet = MultiNetwork(network_specs, logger_level='INFO', logger_level_network='WARNING')
# pass all the needed and optional variables
mulnet.set_net_vars(wf_model_name=wf_model_name, wf_other_var_dic=wf_other_var_dic, user_waveform=user_waveform,
...
mulnet.calc_errors(only_net=only_net, derivs='num', step=step, method=method, order=order, num_cores=num_cores)
