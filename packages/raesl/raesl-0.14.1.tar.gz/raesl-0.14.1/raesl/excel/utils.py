...
def yield_all_function_specifications(
    ...
    """Yield all function specifications nodes for a given iterable of dependency edges."""
...
def yield_active_functions(
    ...
    """Yield all functions for which the active component matches the given path."""
    for n in function_specifications:
        if get_goal_active(n) == active_path:
            yield n
...
def get_function_flow_variables(function_specification: Node) -> List[str]:
    """Get the variables involved with a function specification."""
    esl_info = function_specification.annotations.get("esl_info", dict())
    return esl_info.get("body", dict()).get("variables", [])
...
def build_active_goal_data(graph: Graph, goal: Node) -> Generator[Dict[str, Any], None, None]:
    ...
    target = get_goal_passive(goal)
    flows = get_function_flow_variables(goal)
    types = [get_variable_type(graph[f]) for f in flows if graph.node_dict.get(f, False)]
    types = ", ".join(types)
