...
def get_driver_connection(connection: sqlalchemy.engine.base.Connection):
    try:
        # SQLAlchemy 2.0
...
class ShardMapper:
    ...
    def execute_chooser(self, context: sqlalchemy.orm.session.ORMExecuteState):
        ...
        """
        shards = set()
        if context.is_relationship_load and isinstance(
            context.lazy_loaded_from.object, EntityMixin
        ):
        ...
        if not shards and "clause" in context.bind_arguments:
            ...
            for key, val in clause_compiled.params.items():
...
def _get_select_comparisons(statement) -> list[tuple[Any, Any, Any]]:
    ...
    """
    binds: dict[BinaryExpression, Any] = {}
