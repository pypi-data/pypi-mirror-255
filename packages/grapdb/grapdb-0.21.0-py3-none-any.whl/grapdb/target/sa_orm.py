...
def generate(graph: Graph, path: Path, version: int) -> None:
    assert version in {13, 14}
    mod = generate_user_types_module(graph, version == 14)
...
def generate_user_types_module(graph: Graph, v14: bool) -> str:
    cw = CodeWriter()
    cw.emit_raw(preamble_v14 if v14 else preamble)
    for enum in graph.enums.values():
        ...
        for value in sorted_values:
            cw.emit(f"{enum.name}_{fmt_underscores(value)}: Final = '{value}'")
        cw.emit_list(
    ...
    for vertex in graph.vertices.values():
        ...
        with cw.indent():
            ...
            for col in vertex.extra_cols:
                if col.drop:
                    continue
                dt = graph_type_to_sa_type(col.data_type, v14)
...
def graph_type_to_sa_type(data_type: DataType, v14: bool) -> str:
    if isinstance(data_type, (ColoRef, Vertex)):
        ...
        if v14:
            return "ARRAY({}, dimensions=1)".format(graph_type_to_sa_type(data_type.element_type, v14))
        elif isinstance(data_type.element_type, (ColoRef, Vertex, CompositeType)):
            return "CompositeArray({})".format(
                graph_type_to_sa_type(data_type.element_type, v14)
            )
        else:
            return "ARRAY({})".format(graph_type_to_sa_type(data_type.element_type, v14))
    else:
    ...
    if isinstance(data_type, (ColoRef, Vertex)):
        ...
        if depth == 0:
            ...
            return 'NamedTuple("{}", [{}])'.format(
                data_type.name, ", ".join(named_tuple_fields)
            )
        else:
        ...
        return "list[{}]".format(
            graph_type_to_mypy_type(data_type.element_type),
        )
    else:
