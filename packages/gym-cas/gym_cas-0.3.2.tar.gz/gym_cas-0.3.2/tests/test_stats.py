...
def test_degroup():
    ...
    x2 = degroup([[1, 2, 3, 8], [1, 2, 3, 10]], [[[0, 1],[1,2],[2,3],[3,10]],[[-5, 5],[5,10],[10,20],[20,100]]])
    assert x1==x2
    assert x1[0][0] == 0.5
    assert x1[0][-1] == 10 - (10-3)/9
    assert x1[1][0] == 0
...
def frekvenstabel():
    table = frekvenstabel([8, 3, 2, 1, 1, 2, 3, 4])
    ...
    table1 = frekvenstabel([8, 3, 2, 1], [[1, 2], [2, 3], [3, 4], [4, 5]])
    table2 = frekvenstabel([8, 3, 2, 1], [1, 2, 3, 4, 5])
...
def test_percentiles():
    assert (percentile([1,1,1,3,7,8],[20,50,80],method="inverted_cdf") == percentile([1,1,1,3,7,8],[20,50,80])).all()
    assert (percentile([1,1,1,3,7,8],[20,50,80]) != percentile([1,1,1,3,7,8],[20,50,80],method="linear")).any()
...
def test_groups():
    ...
    assert group_std([1, 2, 3, 1], [1, 2, 3, 4, 5],ddof=1) > group_std([1, 2, 3, 1], [1, 2, 3, 4, 5])
