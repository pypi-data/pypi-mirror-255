import os
import subprocess
from datetime import datetime
import ipywidgets as widgets
from IPython.display import clear_output, display
import re
import json
import string
import threading
import time

def get_video_id(url):
    youtube_regex = (
        r'(https?://)?(www\.)?'
        '(youtube|youtu|youtube-nocookie)\.(com|be)/'
        '(watch\?v=|embed/|v/|live/|.*&v=)?([^&=%\?]{11})')
    youtube_regex_match = re.match(youtube_regex, url)
    if youtube_regex_match:
        return youtube_regex_match.group(6)
    return None

def get_upload_date_and_time(link):
    cmd = ["yt-dlp", "-j", link]
    result = subprocess.run(cmd, stdout=subprocess.PIPE, universal_newlines=True)
    video_info = json.loads(result.stdout)
    return video_info.get("upload_date"), video_info.get("timestamp")

def get_video_title(link):
    cmd = ["yt-dlp", "--get-title", link]
    result = subprocess.run(cmd, stdout=subprocess.PIPE, universal_newlines=True)
    return result.stdout.strip()

def get_video_info(link):
    cmd = ["yt-dlp", "-F", link]
    result = subprocess.run(cmd, stdout=subprocess.PIPE, universal_newlines=True)
    lines = result.stdout.split('\n')
    max_resolution = 0
    for line in lines:
        if "mp4" in line and "video" in line:
            match = re.search(r'(\d{3,4})x(\d{3,4})', line)
            if match:
                width, height = map(int, match.groups())
                max_resolution = max(max_resolution, height)
    return max_resolution

def sanitize_filename(title):
    valid_chars = "-_.() %s%s" % (string.ascii_letters, string.digits)
    sanitized_filename = ''.join(c for c in title if c in valid_chars)
    return sanitized_filename

def monitor_download_progress(output_video, output_audio, video_info):
    progress_pattern = re.compile(r'\[download\]\s+(\d+\.?\d*%)\s+of\s+(\d+\.?\d*\w+)')
    video_size, audio_size = 0, 0
    while True:
        if os.path.exists(output_video):
            video_size = os.path.getsize(output_video) / (1024 ** 2)  # Converte para MB
        if os.path.exists(output_audio):
            audio_size = os.path.getsize(output_audio) / (1024 ** 2)  # Converte para MB
        
        clear_output(wait=True)
        print(f"Nome do vídeo: {video_info['title']}")
        print(f"Resolução: {video_info['resolution']}")
        print(f"Data de upload: {video_info['upload_date']}")
        print(f"Vídeo: {video_size:.2f} MB")
        print(f"Áudio: {audio_size:.2f} MB")
        
        if not threading.active_count() > 2:
            break
        time.sleep(1)

def start_download(link, extension, video_info):
    date_folder = datetime.now().strftime("%Y-%m-%d")
    folder_id = "DOWNLOADER"
    folder_path = f"/content/{folder_id}/{date_folder}"
    os.makedirs(folder_path, exist_ok=True)

    output_video = os.path.join(folder_path, "video.mp4")
    output_audio = os.path.join(folder_path, "audio.mp4")

    cmd_video = ["yt-dlp", "-f", "bestvideo[vcodec^=avc][ext=mp4]", "-o", output_video, link]
    cmd_audio = ["yt-dlp", "-f", "bestaudio", "-o", output_audio, link]

    threading.Thread(target=lambda: subprocess.run(cmd_video)).start()
    threading.Thread(target=lambda: subprocess.run(cmd_audio)).start()

    threading.Thread(target=monitor_download_progress, args=(output_video, output_audio, video_info)).start()

def play_all():
    link_input = widgets.Text(description="Link da aula:")
    mp4_button = widgets.Button(description=".MP4")

    def on_button_click(button, extension):
        url = link_input.value
        if url:
            video_info = {
                'title': get_video_title(url),
                'resolution': get_video_info(url),
                'upload_date': get_upload_date_and_time(url)[0]
            }
            start_download(url, extension, video_info)
    
    mp4_button.on_click(lambda b: on_button_click(b, '.MP4'))
    display(link_input, mp4_button)