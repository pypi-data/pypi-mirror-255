...
def monitor_download_progress(folder_path, title, resolution):
  while True:
...
def start_download(link, folder_path):
    ...
    output_video = os.path.join(folder_path, f"{sanitized_title}_video{extension}")
    output_audio = os.path.join(folder_path, f"{sanitized_title}_audio{extension}")
    ...
    monitor_thread = threading.Thread(target=monitor_download_progress, args=(folder_path, title, resolution), daemon=True)
    monitor_thread.start()

    video_thread.join()
    audio_thread.join()

    final_output = os.path.join(folder_path, f"{sanitized_title}{extension}")
    ...
    if os.path.exists(final_output):
        clear_output()
        print(f"Iniciando download do arquivo final: {final_output}")
        try:
            files.download(final_output)
        except Exception as e:
            print(f"Erro no download do arquivo final: {e}")
    else:
        print("Error: Arquivo não encontrado após download.")
