...
def monitor_download_progress(output_video, output_audio, video_info, folder_path):
    # Monitora o progresso do download e exibe as informações em tempo real
    while True:
        ...
        pirnt(f"Áudio: {audio_size:.2f} MB")

        # Verifica se os downloads terminaram verificando o tamanho dos arquivos ao longo do tempo
        time.sleep(1)  # Atualiza a cada segundo

        if not any(thread.is_alive() for thread in threading.enumerate() if thread.name.startswith("Download")):
            # Mescla áudio e vídeo após os downloads
...
def start_download(link, extension, video_info):
    ...
    # Iniciar threads para download de vídeo e áudio
    ...
    # Iniciar thread para monitorar o progresso e exibir informações
