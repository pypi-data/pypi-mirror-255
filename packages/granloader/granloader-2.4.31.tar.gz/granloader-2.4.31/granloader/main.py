...
def monitor_download_progress(folder_path, video_info):
    # Verifica se os arquivos de download estão crescendo
    previous_video_size = previous_audio_size = 0
    while True:
        time.sleep(1)  # Atualiza a cada segundo
        video_file = find_latest_file("video", folder_path)
        audio_file = find_latest_file("audio", folder_path)
        current_video_size = os.path.getsize(video_file) / (1024 ** 2) if video_file else 0
        current_audio_size = os.path.getsize(audio_file) / (1024 ** 2) if audio_file else 0
        
        # Atualiza a tela apenas se houver mudança no tamanho dos arquivos
        if current_video_size != previous_video_size or current_audio_size != previous_audio_size:
            clear_output(wait=True)
            print(f"Nome do vídeo: {video_info['title']}\nResolução: {video_info['resolution']}\nVídeo: {current_video_size:.2f} MB\nÁudio: {current_audio_size:.2f} MB")
            previous_video_size = current_video_size
            previous_audio_size = current_audio_size

        # Encerra a thread se os downloads estiverem completos (ou seja, se os tamanhos pararem de mudar)
