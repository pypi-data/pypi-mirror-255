...
def find_latest_file(startswith, folder_path):
    relevant_files = [f for f in os.listdir(folder_path) if f.startswith(startswith)]
...
def start_download(link, folder_path, video_info):
    ...
    final_output = os.path.join(folder_path, sanitize_filename(video_info['title']))
    ...
    def on_button_click(button):
        ...
        if url:
            video_info = {
                ...
                # Inclua outras informações necessárias como 'upload_date', etc.
            }
            # Aqui passamos o caminho do diretório e as informações do vídeo para start_download
