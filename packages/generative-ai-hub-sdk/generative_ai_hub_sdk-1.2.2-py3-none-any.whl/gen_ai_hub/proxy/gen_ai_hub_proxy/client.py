...
from gen_ai_hub.proxy.core.utils import PredictionURLs, lru_cache_extended, OMIT
...
# 'OMIT' is a special value that will lead to deployment.prediction_url being None.
# This is useful to indicate to use the .url attribute of the deployment instead and let the llm client handle the suffix
...
class GenAIHubProxyClient(BaseProxyClient):
    ...
    def get_request_header(self):
        ...
        header.update({'Authorization': self.get_ai_core_token()})
        return header
    ...
    def update_deployments(self):
        deplomyent_set = {} # use dict to remove duplicates based on deployment id
        ...
        for scenario_info in self.foundational_model_scenarios:
            ...
            for deployment in query.resources:
                if any(fnmatch(deployment.configuration_name, n) for n in scenario_info.config_names):
                    deplomyent_set[deployment.id] = Deployment(
                        ...
                        **config_parameters(scenario_info.model_name_parameter, self.ai_core_client,
                                            deployment))
        self._deployments = [*deplomyent_set.values()]
