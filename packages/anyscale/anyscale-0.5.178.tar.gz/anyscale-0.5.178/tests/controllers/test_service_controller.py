...
def test_generate_config_from_file(
    ...
    with patch(
        ...
        config = service_controller.generate_config_from_file(
            "test_ser8vice_config_file", auto_complete_rollout=True
        )
...
def test_config_validation(add_extra_field, setup_service_controller):
    """
...
def test_generate_config_from_file_inside_workspace_no_working_dir_override(
    ...
    with patch.dict(
        ...
        config = service_controller.generate_config_from_file(
            "test_service_config_file", auto_complete_rollout=True
        )
...
@pytest.mark.parametrize(
    ("override_canary_percent"), [None, 0, 0, 40, 40],
)
...
def test_override_runtime_env_config_and_infer_project_id_v2(
    ...
    with patch.multiple(
        ...
        service_controller.rollout(service_config, auto_complete_rollout=True)
