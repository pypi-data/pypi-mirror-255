...
def migration_update_imported_drug():
    init_db(db, 'database_migration')
    migrator = MySQLMigrator(db)

    try:
        # delete order_data_id column and add batch_drug_data_id column
        if ImportedDrug.table_exists():
            migrate(
                migrator.drop_column(ImportedDrug._meta.db_table,
                                     ImportedDrug.order_data_id.db_column),
                migrator.add_column(ImportedDrug._meta.db_table,
                                    ImportedDrug.batch_drug_data_id.db_column,
                                    ImportedDrug.batch_drug_data_id))
        print("Deleted column order_data_id & Added column order_data_id in ImportedDrug")
    except Exception as e:
        settings.logger.error("Error while adding columns in ImportedDrug: ", str(e))


if __name__ == '__main__':
    migration_update_imported_drug()
