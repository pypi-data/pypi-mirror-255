...
class IntermediateDisabledCanisterLocation(BaseModel):
    ...
    comment = CharField()
    location_id = ForeignKeyField(LocationMaster, null=True, related_name='location_id_id')
    created_by = IntegerField()
    created_date = DateTimeField(default=get_current_date_time)
...
class OldCanisterDrawers(BaseModel):
    id = PrimaryKeyField()
    robot_id = ForeignKeyField(RobotMaster, null=True)
    ...
    if model_class.table_exists():
        migrate(
            migrator.add_column(
                ...
                model_class.robot_id
            ),
            migrator.add_column(
                model_class._meta.db_table,
                model_class.canister_number.db_column,
                model_class.canister_number
            )
        )
        print("column added: " + str(model_class))

        query = model_class.select(LocationMaster.device_id.alias('device_id'), LocationMaster.location_number.alias('location_number'),
                                   fn.GROUP_CONCAT(model_class.id).coerce(False).alias('table_id')).dicts() \
            .join(LocationMaster,
                  on=(model_class.location_id == LocationMaster.id))\
            .group_by(LocationMaster.id)

        update_dict = dict()
        for record in query:
            update_dict[int(record['device_id']), int(record['location_number'])] = list(map(int, record['table_id'].split(',')))


        for key, value in update_dict.items():
            model_class.update(robot_id=key[0], canister_number=key[1]).where(model_class.id << value).execute()

        print("Column values updated " + str(model_class))

        if remove_null:
            try:
                migrate(migrator.add_not_null(model_class._meta.db_table, model_class.robot_id.db_column))
                migrate(migrator.add_not_null(model_class._meta.db_table, model_class.canister_number.db_column))
...
def migrate_device_id(migrator, model_class, remove_null=True):
    ...
    if model_class.table_exists():
        ...
        query = model_class.select(model_class.device_id.alias('device_id'),
        ...
        for key, value in update_dict.items():
            model_class.update(robot_id=key).where(model_class.id << value).execute()
...
def migrate_location_id(migrator, model_class, remove_null=True):
    ...
    if model_class.table_exists():
        ...
        update_dict = defaultdict(list)
        for record in query:
            update_dict[(int(record['device_id']), record['location_number'])].append(record['table_id'])
        ...
        migrate(
            migrator.drop_column(
                model_class._meta.db_table,
                model_class.location_id.db_column, migrator,
            )
        )
        print("Column dropped " + str(model_class))


def migrate_robot_data():
        ...
        with db.transaction():
            ...
            migrate(
                migrator.rename_column(
                    ...
                    IntermediatePackAnalysisDetails.location_number.db_column,
                    'canister_number'
                )
            )
