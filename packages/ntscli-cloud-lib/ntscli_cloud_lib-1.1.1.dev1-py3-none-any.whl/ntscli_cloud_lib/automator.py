...
class TestCase(betterproto.Message):
    ...
    # The full URL to the test including host.
...
class GetTestPlanRequestOptions(betterproto.Message):
    ...
    # UUID4 string. If there is a known filter ID to get instead of the standard
    # dynamic test plan from NTS, provide it here
    dynamic_filter_id: Optional[str] = betterproto.message_field(
        3, wraps=betterproto.TYPE_STRING
    )
    # Optional parameter when filterId is passed. This is used to prevent
    # filtering by the device test plan. Instead, we filter by the namespace (ie
    # 5.3, 2023.1, ...) passed in this param.
    single_namespace_filter: Optional[str] = betterproto.message_field(
        4, wraps=betterproto.TYPE_STRING
    )
...
class InitialSavedData(betterproto.Message):
    ...
    nts_cli_client_version: Optional[str] = betterproto.message_field(
        2, wraps=betterproto.TYPE_STRING
    )
