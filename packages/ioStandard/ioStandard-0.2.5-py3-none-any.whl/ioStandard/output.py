...
class text:
    ...
    def alignToText(self, otherText:'text', type="centre"):
        """Accepts the following alignment types:
        - 'centre'
        - 'left'
        - 'right'
        ...
        otherTextSurfaceRect = otherText.textSurface.get_rect()
        textSurfaceRect = self.textSurface.get_rect()

        x = otherText.position[0]
        y = otherText.position[1]
    ...
    def updateTextSurface(self):
        ...
        maxWidth = pygame.display.get_surface().get_size()[0] - 50
        lines = []
        currentLine = ""
        ...
        renderedLines = [self.font.render(line, self.aa, self.colour) for line in lines]
        self.textSurface = pygame.Surface((maxWidth, len(lines)*fontHeight), pygame.SRCALPHA)

        yOffset = 0
        for line in renderedLines:
            lineRect = line.get_rect(center=(maxWidth//2, yOffset + fontHeight //2))
            self.textSurface.blit(line, lineRect)
            yOffset += fontHeight
