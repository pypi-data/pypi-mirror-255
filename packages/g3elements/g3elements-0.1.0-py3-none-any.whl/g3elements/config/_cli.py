...
from g3tables.utils.gdrive import list_tables_from_gdrive

from ._system_config_processor import SystemConfigProcessor


logger = logging.getLogger('g3project_builder.system_data')
...
def get_input_args() -> InputArgs:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'for bulk data collection. Project G3 tables in the folder '
            'are identified automatically. Any parsed table data source '
            'is overridden by a corresponding explicitly provided argumement. '
            'Format as follows: '
            '"<gdrive_folder>" or "<gdrive_folder>@<system_name>"'
            )
        )
    parser.add_argument(
        '-sw',
        ...
        default=(None, None),
        help=(
            'SW Definition Table path. Format as follows: '
            '"<table_path>@<system_name>".'
            )
        )
    parser.add_argument(
        '-io',
        ...
        default=(None, []),
        help=(
            'PLC Composition IO table path. Format as follows: \n'
            '1) "<table_path>" (no sheet names provided);\n'
            '2) "<table_path>@<cabinet_name>" (one sheet name provided);\n'
            '3) "<table_path>@<cabinet_name1>+<cabinet_name2>+<cabinet_name3>"'
            ' (two or more sheet names provided).'
            )
        )
    parser.add_argument(
        '-visu',
        ...
        required=False,
        default=None,
        help='Visualization table path. Format as follows: "<table_path>".'
        )
    parser.add_argument(
        '-core',
        '--g3core-path',
        ...
        help=(
            'G3 Core library files path. Should be formatted as follows: \n'
            '1) "<local_path>";\n'
            '2) "remote" (default brach "master" is used);\n'
            '3) "remote@<branch>" (the branch may be referred by '
            'its name or by its hash).'
            )
        )
    parser.add_argument(
        '-u',
...
class InputArgsProcessor:
    ...
    def gdrive_tables(self) -> list[str]:
        ...
        if not gdrive_folder:
            return []
        return list_tables_from_gdrive(project=gdrive_folder, with_meta=False)
