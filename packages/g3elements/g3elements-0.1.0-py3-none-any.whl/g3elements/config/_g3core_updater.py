...
logger = logging.getLogger('g3project_builder.system_data.g3core_validator')
...
class SWSystemDictG3CoreUpdater:
            ...
            case _:
                if var.name not in connectors:
                    logger.warning(
                        ...
                        f'"{device_data["general"]["name"]}": '
                        f'"{var.name}" connector is added automatically '
                        f'without a refecence to any device.'
                        )
                    var.value = None
                    return
    ...
    def update_device_connectors_control(
        ...
        for conn_name, conn_value in connectors.items():
            if any(conn_name == var.name for var in fb.var_in_out):
                ...
                logger.warning(
                    f'{device_type.capitalize()} '
                    f'"{device_data["general"]["name"]}": "{fb.name}" '
                    f'function block does not have a "{conn_name}" '
                    f'IN_OUT variable. The connector will be removed. '
                    )
    ...
    def update_device_connectors_test(
        ...
        for var in fb.var_in_out:
            match var.name:
                case 'dev':
                    var.value = f"Test{device_data['general']['varname']}"
                case 'comm':
                    control_commfunc = (
                        device_data['control']['connector'].get('comm')
                        )
                    if control_commfunc:
                        var.value = f"Test{control_commfunc}"
                case 'system':
                ...
                case _:
                    if var.name not in device_data['test']['connector']:
                        logger.warning(
                            f'test/connector/{var.name}: '
                            f'Reference to connected device is empty.'
                            )
                        continue
                    ...
                    if not control_device_data:
                        raise ValueError  # do warning here
                    connected_device_varname = (
                        f"Test{control_device_data['general']['varname']}"
                        )
                    var.value = connected_device_varname
            ...
            match var.name:
                ...
                case 'parentBroker':
                    ...
                    if not server_addr or not isinstance(server_addr, str):
                        logger.warning(
                            f'"parentBroker/connection/serverAddress": cannot '
                            f'format value "{server_addr}" to data type '
                            f'"string". Default value '
                            f'"nirvana.elektroline.cz" will be used instead.'
                            )
                    ...
                    try:
                        ...
                        logger.warning(
                            f'"parentBroker/connection/serverPort": cannot '
                            f'format value "{server_port}" to data type '
                            f'"uint" ({err}). Default value '
                            f'"3756" will be used instead.'
                            )
                        server_port = 3756
    ...
    def update_device_config_test(
        ...
        for var in config_struct.members:
            if var.name in device_data['test']['config']:
                ...
                logger.warning(
                    f'test/config/{var.name}: config parameter is empty.'
                    )
    ...
    def prefill_device_iomap_test(
        ...
        for var in fb.var_input:
            device_data['test']['iomap'][var.name] = ''
        for var in fb.var_out:
            device_data['test']['iomap'][var.name] = ''
    ...
    def update_device(
        ...
        if not vars_dev:
            logger.info(
                f'"dev" member was not found in {device_domain} function '
                f'block "{fb.name}". Device config data will be discarded.'
                )
        ...
        try:
            ...
            logger.info(
                f'"config" member was not found in {device_domain} function '
                f'block "{fb.name}". Device config data will be discarded.'
                )
            device_data[device_domain]['config'] = {}
            return
