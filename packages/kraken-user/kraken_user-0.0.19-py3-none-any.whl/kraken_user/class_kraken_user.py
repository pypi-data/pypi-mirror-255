...
class User:
    ...
    def create(self):
        """Create a new user
        """
        record = m.create_user(self.record)
        if record is False:
            return False
        self._record = record
        return True
    ...
    def api_get_all(self):
        """Retrieve all user records from api
        """
        records = m.get_users(self.program) 
        users = []
        if records:
            for i in records:
                user = User()
    ...
    HTML templates
    """
    def get_login_form(self, referrer=None, action_url='/login'):
        """
        """
        return m.get_login_form(self.membershipNumber, referrer, action_url)

    def get_register_form(self, referrer=None, action_url='/register'):
        """
        """
        return m.get_register_form(self.membershipNumber, referrer, action_url)