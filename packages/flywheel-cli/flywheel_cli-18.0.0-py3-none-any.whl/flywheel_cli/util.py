...
except (pytz.exceptions.UnknownTimeZoneError, zoneinfo.ZoneInfoNotFoundError):
...
def load_auth_config():
    """Load authentication config from system"""
    try:
        api_key = keyring.get_password("Flywheel Credentials", "fw_api_key")
        config = {"key": api_key} if api_key is not None else {}
    except keyring.errors.NoKeyringError as e:
        log.debug(f"No Keyring Backend: {e} ")
        config = {}
    return config


def save_api_key(api_key, root=False):
    """Securely stores the data within the following per OS:
    - Windows: data will be stored within Windows credential manager
    - MacOS data will be stored securely in the MacOS Keychain
    - Linux: data will be stored with GNOME keyring

    Args:
        api_key (dict): user api key
    """
    if not api_key:
        return
    if platform.system() in ("Linux", "Darwin", "Windows"):
        keyring.set_password("Flywheel Credentials", "fw_api_key", api_key)
...
def get_http_config() -> HttpConfig:
    """Get config for HTTP transfer protocols and configuring fw-http-client"""
    config = HttpConfig(
        client_name="CLI (Legacy)",
...
def get_sdk_client(api_key: str) -> flywheel.Client:
    ...
    log.debug(f"Core-API Version: {client.fw.get_version().get('release')}")
...
class SDKClient:
    ...
    def multipart_signed_url_upload(
        ...
        for part_num, part_url in enumerate(signed_urls):
            ...
            # Init fw_http_client
            client = HttpClient(get_http_config())
            r = client.put(part_url, data=reader, headers=headers, stream=True)
            if not r.ok:
                log.debug(
...
def cli_args(argv: List):
    """
    Remove sensitive FW data from cli sys.argv list
    """
    # Api key RE Pattern
    if "login" in argv:
        pattern = r"apikey|:.{57}"
        filter_argv = [arg for arg in argv if not re.search(pattern, arg)]
        return filter_argv
    return
