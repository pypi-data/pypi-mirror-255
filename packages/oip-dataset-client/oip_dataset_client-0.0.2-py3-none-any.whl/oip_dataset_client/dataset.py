...
class DatasetClient(object):
    ...
    def _update_status(self, new_status: str) -> None:
        ...
        if new_status not in STATUS_FLOW[self._status]:
            self._save(
                "update", {"id": self._id, "status": DatasetStatusEnum.failed.value}
            )
    ...
    def create(
        ...
        for p in parent_datasets_:
            if not is_first_version and p.name != name:
                raise ValueError("can't inherit from a parent with a different name")
        ...
        if not dataset._version:
            ...
            if highest_version is not None:
                dataset._version = str(Version(highest_version).get_next_version())
            else:
    ...
    def get_local_copy(
        ...
        target_base_folder: str = self._create_ds_target_base_folder(parent_folder)
    ...
    def _create_ds_target_base_folder(self, parent_folder: Optional[str] = None) -> str:
        ...
        if parent_folder:
            target_base_folder = os.path.join(parent_folder, target_base_folder)
    ...
    def _get_artifacts_to_download(self) -> Dict[str, List[Dict]]:
        ...
        for file in self.file_entries:
            if (
                ...
                already_added[file.parent_dataset_id].append(file.artifact_name)
    ...
    def upload(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            total_chunks: int = self.total_size // (chunk_size * 1024 * 1024) + 1
            if verbose:
    ...
    def generate_object_name(
        ...
        """
        workspace_name: str = os.environ["OIP_WORKSPACE_NAME"]
    ...
    def generate_state_object_name(cls, dataset_name: str, dataset_id: str) -> str:
        ...
        """
        workspace_name = os.environ["OIP_WORKSPACE_NAME"]
