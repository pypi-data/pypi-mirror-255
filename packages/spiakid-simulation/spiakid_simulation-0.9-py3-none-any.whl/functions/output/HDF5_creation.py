...
def save_dict_to_hdf5(dic, filename,sim,size):
    ...
    for i in range(size):
        for j in range(size):
            h5file['wavelength/'+str(i)+str(j)] = sim.wavelength[i,j]
            h5file['time/'+str(i)+str(j)] = sim.time[i,j]
            h5file['phase/'+str(i)+str(j)] = sim.phase[i,j]
...
def save_photon_list(filename,dic,filtered_signal,noise):
    ...
    for i in range(size):
        for j in range(size):
            ...
            h5file['Photon/'+str(i)+str(j)]  = [np.float32(filtered_signal[i,j][0][peaks]), np.float32(filtered_signal[i,j][1][peaks])]
