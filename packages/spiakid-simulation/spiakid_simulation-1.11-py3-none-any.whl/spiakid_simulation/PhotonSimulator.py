    ...
    def __init__(self, yaml_path):
        ...
        if Path(path).exists():
            ...
            if sky['fov_method'] == 'fix':
                ...
                sky_fov = pix_size * exposure_time   # A revoir !
            else:
                print('sky FOV is not defined')

                
            #   Creation of stars with their spectrum
            self.star_pos,self.star_spec = IG.image_sim(Image_size=int(pix_nbr/pix_size*sky_fov), object_number=nb_object, distance=distance, Path_file=path,Wavelength=wavelength_array,save = False)
