...
class GherkinInMarkdownTokenMatcher:
    ...
    def match_FeatureLine(self, token: Token) -> bool:  # noqa: N802
    ...
    def match_RuleLine(self, token):  # noqa: N802
        return self._match_title_line(KEYWORD_PREFIX_HEADER, self.dialect.rule_keywords, ":", token, "RuleLine")

    def match_ScenarioLine(self, token):  # noqa: N802
        return self._match_title_line(
            KEYWORD_PREFIX_HEADER, self.dialect.scenario_keywords, ":", token, "ScenarioLine"
        ) or self._match_title_line(
            KEYWORD_PREFIX_HEADER, self.dialect.scenario_outline_keywords, ":", token, "ScenarioLine"
        )

    def match_BackgroundLine(self, token):  # noqa: N802
        return self._match_title_line(
            KEYWORD_PREFIX_HEADER, self.dialect.background_keywords, ":", token, "BackgroundLine"
        )

    def match_ExamplesLine(self, token):  # noqa: N802
        return self._match_title_line(KEYWORD_PREFIX_HEADER, self.dialect.examples_keywords, ":", token, "ExamplesLine")

    def match_TableRow(self, token: Token) -> bool:  # noqa: N802
    ...
    def match_StepLine(self, token):  # noqa: N802
        non_star_step_keywords = (
    ...
    def match_Comment(self, token):  # noqa: N802
        result = False
    ...
    def match_Empty(self, token):  # noqa: N802
        result = False
    ...
    def match_Language(self, token):  # noqa: N802
        if not token:
            raise ValueError("no token")
        return False

    def match_TagLine(self, token):  # noqa: N802
        tags = []
        matching_tags = re.finditer("`(@[^`]+)`", token.line.get_line_text())
        idx = 0
        for match in matching_tags:
            tags.append({"column": token.line.indent + match.start(idx) + 2, "text": match.group(1)})
    ...
    def match_DocStringSeparator(self, token):  # noqa: N802
    ...
    def _match_DocStringSeparator(self, token, separator, is_open):  # noqa: N802
    ...
    def match_Other(self, token):  # noqa: N802
    ...
    def match_EOF(self, token):  # noqa: N802
        if not token.eof():
            return False
    ...
    def _match_title_line(self, prefix: Any, keywords: Any, keyword_suffix: Any, token: Token, token_type: Any) -> bool:
        keywords_or_list = "|".join(map(lambda x: re.escape(x), keywords))
        match = re.search("{}({}){}(.*)".format(prefix, keywords_or_list, keyword_suffix), token.line.get_line_text())
        ...
        if match:
            ...
            if matched_keyword in self.keyword_types:
                matched_keyword_type = self.keyword_types[matched_keyword][0]
            ...
            self._set_token_matched(
                ...
                match.group(3).strip(),
                matched_keyword,
                keyword_type=matched_keyword_type,
                indent=indent,
            )
            return True
        return False

    def _set_token_matched2(
        self,
        token,
        matched,
        indent=None,
    ...
    def _change_dialect(self, dialect_name: str, location: Location = None) -> None:
