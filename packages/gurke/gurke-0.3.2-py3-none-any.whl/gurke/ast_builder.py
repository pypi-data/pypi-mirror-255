...
class AstBuilder:
    ...
    def build(self, token: Token) -> None:
        if token.matched_type == "Comment":
            self.comments.append({"location": self.get_location(token), "text": token.matched_text})
        else:
    ...
    def get_location(self, token: Token, column: Optional[int] = None) -> Location:
        return token.location if not column else Location(token.location.line, column)
    ...
    def ensure_cell_count(self, rows):
        ...
        for row in rows:
            if len(row["cells"]) != cell_count:
                raise AstBuilderException("inconsistent cell count within the table", row["location"])

    def get_cells(self, table_row_token):
        return [
            self.reject_nones(
                {"location": self.get_location(table_row_token, cell_item["column"]), "value": cell_item["text"]}
            )
    ...
    def transform_node(self, node: AstNode) -> Any:
        ...
        if node.rule_type == "DocString":
            ...
            media_type = separator_token.matched_text if len(separator_token.matched_text) > 0 else None
        ...
        if node.rule_type == "Description":
            ...
            last_non_empty = next(i for i, j in reversed(list(enumerate(line_tokens))) if j.matched_text)
