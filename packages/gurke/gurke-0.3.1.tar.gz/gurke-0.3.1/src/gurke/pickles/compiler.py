            ...
            for values in examples["tableBody"]:
                ...
                if scenario["steps"]:
                    for step in background_steps:
                        last_keyword_type = (
                            last_keyword_type if step["keywordType"] == "Conjunction" else step["keywordType"]
                        )
                        steps.append(self._pickle_step(step, last_keyword_type))
                ...
                if scenario["steps"]:
                    for outline_step in scenario["steps"]:
                        ...
                        step_text = self._interpolate(outline_step["text"], variable_cells, value_cells)
                        argument = self._create_pickle_arguments(outline_step, variable_cells, value_cells)
                        _pickle_step = {
                ...
                pickle = {
                    ...
                    "id": self.id_generator.get_next_id(),
                    "name": self._interpolate(scenario["name"], variable_cells, value_cells),
                    "language": language,
                    "steps": steps,
                    "tags": self._pickle_tags(tags),
                    "uri": uri,
    ...
    def _create_pickle_arguments(self, step, variables, values):
        if "dataTable" in step:
            ...
            for row in step["dataTable"]["rows"]:
                cells = [{"value": self._interpolate(cell["value"], variables, values)} for cell in row["cells"]]
                table["rows"].append({"cells": cells})
            return {"dataTable": table}

        if "docString" in step:
            argument = step["docString"]
            docstring = {"content": self._interpolate(argument["content"], variables, values)}
