...
class TokenMatcher:
    ...
    _active_doc_string_separator: Optional[str]
    ...
    def match_RuleLine(self, token: Token) -> bool:  # noqa: N802
        return self._match_title_line(token, "RuleLine", self.dialect.rule_keywords)

    def match_ScenarioLine(self, token: Token) -> bool:  # noqa: N802
        return self._match_title_line(token, "ScenarioLine", self.dialect.scenario_keywords) or self._match_title_line(
            token, "ScenarioLine", self.dialect.scenario_outline_keywords
        )

    def match_BackgroundLine(self, token: Token) -> bool:  # noqa: N802
        return self._match_title_line(token, "BackgroundLine", self.dialect.background_keywords)

    def match_ExamplesLine(self, token: Token) -> bool:  # noqa: N802
    ...
    def match_StepLine(self, token: Token) -> bool:  # noqa: N802
        keywords = (
    ...
    def match_Comment(self, token: Token) -> bool:  # noqa: N802
        if not token.line.startswith("#"):
            return False
    ...
    def match_Empty(self, token: Token) -> bool:  # noqa: N802
    ...
    def match_Language(self, token: Token) -> bool:  # noqa: N802
    ...
    def match_TagLine(self, token: Token) -> bool:  # noqa: N802
        if not token.line.startswith("@"):
            return False
    ...
    def match_DocStringSeparator(self, token: Token) -> bool:  # noqa: N802
        if not self._active_doc_string_separator:
            # open
            return self._match_DocStringSeparator(token, '"""', True) or self._match_DocStringSeparator(
                token, "```", True
            )
    ...
    def _match_DocStringSeparator(self, token: Token, separator: str, is_open: bool) -> bool:  # noqa: N802
    ...
    def match_Other(self, token: Token) -> bool:  # noqa: N802
    ...
    def match_EOF(self, token: Token) -> bool:  # noqa: N802
        if not token.eof():
            return False
    ...
    def _match_title_line(self, token: Token, token_type: str, keywords: List[str]) -> bool:
    ...
    def _change_dialect(self, dialect_name: str, location: Optional[Location] = None) -> None:
        dialect = Dialect.for_name(dialect_name)
        if not dialect:
            raise NoSuchLanguageException(dialect_name, location or Location(0, 0))
