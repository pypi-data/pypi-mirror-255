...
from tests.interpolation import LinearInterpolation
...
class ImageSourceBilinear:
    ...
    def brightness_point(self, xycoords: np.array) -> np.array:
        model = LinearInterpolation(
            self.source_space,
            'source',
            xycoords.reshape(2, -1).shape,
            'lens')
        return model(
            ift.makeField(
                model.domain,
                {'source': self.source, 'lens': xycoords.reshape(2, -1)})
        ).val.reshape(xycoords.shape[1:])
...
def get_lens_light(lens_space_model: SpaceModel, mock_config: dict, seed: int):
    ...
    if mock_config['lens_light'] is not None and mock_config['lens_light'].lower() in ['self_consistent']:
...
def get_source_light_and_projection(
    ...
    if mock_config['source_light'].lower() in ['self_consistent']:
        ...
        source_evaluater = LinearInterpolation(
            source_space_model.space, 'source',
            y.reshape(2, -1).shape, 'points')
...
def create_mock_data(
    ...
    if mock_config.get('SNR', None) is not None:
        noise_scale = noise_scale_for_target_snr(
            lensed_light, mock_config['SNR'])
    else:
        noise_scale = mock_config['noise_scale']
...
def func_to_optimize(noise_scale: float, data: np.ndarray, target_snr: float) -> float:
    noise = np.random.normal(scale=noise_scale, size=data.shape)
    noisy_data = data + noise
    estimated_snr = estimate_snr(noisy_data, noise_scale)
...
def noise_scale_for_target_snr(
        data: np.ndarray,
        target_snr: float,
        fun: Callable = func_to_optimize
) -> np.ndarray:
    # initial guess for the bounds of noise_scale
    lower_bound = 0.001
    upper_bound = 10.0
    noise_scale = brentq(
        fun, lower_bound, upper_bound, args=(data, target_snr))
    return noise_scale
