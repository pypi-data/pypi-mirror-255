...
class BlobAccessor(object):
    ...
    def get_file_dataflow(self, **kwargs) -> dprep.Dataflow:
        from azureml.dataprep.api.readers import from_file_path
        self._check_dataprep()
        dflow = from_file_path(self.get_urls(**kwargs))
    ...
    def get_tabular_dataflow(
        ...
        if self._file_extension == "parquet":
            ...
            dflow = dflow.parse_delimited(
                ...
                quoting=False,
                include_path=True)
            column_types_builder = dflow.builders.set_column_types()
            column_types_builder.learn()
            if len(column_types_builder.ambiguous_date_columns) > 0:
                column_types_builder.ambiguous_date_conversions_keep_day_month()
            dflow = column_types_builder.to_dataflow()
            ...
            if DateTimePartitionName.YEAR.value in partitions:
                ...
                if end_date:
                    ...
                    if limit and limit > 0:
                        end_date = min(end_date, start_date + self._get_time_step(limit))
                    dflow = dflow.filter((dflow['PathDate'] >= start_date) & (dflow['PathDate'] <= end_date))
                else:
    ...
    def _convert_frontdoor_dataflow(
        ...
        dflow = dflow._add_transformation(
            'transform_columns',
            {'language': 'Native',
             'transformations': [{
                 'columns': 'FilePath',
                 'function_source': '{"r":["Function",[[],{"r":[]},{"r":["Invoke",[{"r":["Identifier","StrReplace"]}'
                 ',["blob.core.windows","azurefd",false]]]}]]}'}]})
