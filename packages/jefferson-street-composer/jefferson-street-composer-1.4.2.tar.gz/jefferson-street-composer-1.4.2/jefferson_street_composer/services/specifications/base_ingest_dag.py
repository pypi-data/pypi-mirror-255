...
class JeffersonStreetBaseIngestDag():
    ...
    def run(self):
        ...
        with DAG(self.name, default_args=dag_default_args.to_dict(), schedule_interval=self.schedule_interval, catchup=False) as dag:
            volume_mount = k8s.V1VolumeMount(name='persist-disk',
                           mount_path='/files',
                           sub_path=None,
                           read_only=False)
            volume = k8s.V1Volume(name='persist-disk', persistent_volume_claim=k8s.V1PersistentVolumeClaimVolumeSource(claim_name="persist-pods-disk-claim"))
