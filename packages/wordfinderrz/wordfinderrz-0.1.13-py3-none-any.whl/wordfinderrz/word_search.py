...
# How about a little bit of... A/B testing.
# Compare linear regression results with one using sum(word_len) instead of bank len
...
from typing import Annotated, Final, Literal, ParamSpec, TypeVar, cast, overload
...
class WordSearch:
    ...
    def _infer_dimensions(self) -> tuple[int, int]:
        ...
        return (int(0.2019348819251314 * len(self.bank) + 16.616929879377363),) * 2
    ...
    def _step(
        ...
        if WordSearch._grid_index_is_in_param_bounds(self.width, self.height, result):
            return result
