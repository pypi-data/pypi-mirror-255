...
class Beam(Oasys.gRPC.OasysItem, metaclass=BeamType):
    _props = {'data', 'include', 'index', 'integrationPoints', 'label', 'material', 'model', 'part', 'type'}
...
# Static methods
    ...
    def GetAll(model):
        ...
        -------
        array
            Array of :py:class:`Beam<Beam>` objects
        """
    ...
    def GetMultipleData(component, items, options=Oasys.gRPC.defaultArg):
        ...
        -------
        $object
            Dict containing the data. A property is created in the object for each beam with the label. The value of the property is a number if a scalar component or an array if a vector or tensor component (or None if the value cannot be calculated)
        """
