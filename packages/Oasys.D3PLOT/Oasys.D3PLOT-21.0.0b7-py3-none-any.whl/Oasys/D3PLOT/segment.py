...
class Segment(Oasys.gRPC.OasysItem, metaclass=SegmentType):
    _props = {'data', 'include', 'index', 'label', 'material', 'model', 'part', 'type'}
...
# Static methods
    ...
    def GetAll(model):
        ...
        -------
        array
            Array of :py:class:`Segment<Segment>` objects
        """
    ...
    def GetMultipleData(component, items, options=Oasys.gRPC.defaultArg):
        ...
        -------
        $object
            Dict containing the data. A property is created in the object for each segment with the label. The value of the property is a number if a scalar component or an array if a vector or tensor component (or None if the value cannot be calculated)
        """
