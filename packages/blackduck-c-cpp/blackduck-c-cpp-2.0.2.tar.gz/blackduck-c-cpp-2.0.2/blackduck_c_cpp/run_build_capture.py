...
class RunBuildCapture(object):
    ...
    def __init__(self, my_args, log_path, blackduck_output_dir):
        ...
        self.cov_prj = util.get_absolute_path(
            self.my_args.args.cov_output_dir) if self.my_args.args.cov_output_dir else os.path.join(self.home,
                                                                                                    ".synopsys",
                                                                                                    "blackduck-c-cpp",
                                                                                                    "output",
    ...
    def run_build(self, cov_emit_link_path, cov_bld_or_cpt_path, cov_build_run):
        ...
        if cov_build_run:
            ...
            # cov-cli
            if os.path.exists(cov_emit_link_path):
                self.bld_cmd = '"{}" capture -o analyze.location=connect -o capture.languages.include=c-family -o capture.build.cov-build-args=--emit-link-units{} --dir "{}"{}'.format(
                    cov_bld_or_cpt_path,
                    conf_file,
                    self.cov_dir, cov_add_cmd)
            else:
                self.bld_cmd = '"{}" capture -o analyze.location=connect -o capture.languages.include=c-family{} --dir "{}"{}'.format(
                    cov_bld_or_cpt_path, conf_file,
                    self.cov_dir, cov_add_cmd)
