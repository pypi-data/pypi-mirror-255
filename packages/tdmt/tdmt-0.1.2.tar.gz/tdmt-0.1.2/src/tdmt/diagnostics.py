...
def run_executable_and_capture_terminal_output_as_list(
    ...
    terminal_output_as_list = subprocess.check_output(
        f"python3 {executable_name}.py --workflow={workflow_map_name}",
        shell=True,
...
def check_for_invalid_option_argument(
    executable_name: str, workflow_map_name: str, invalid_argument: str
) -> str:
    """
    Checks for an invalid option argument by running the executable with it and capturing the penultimate terminal output.

    Parameters:
    - executable_name: Name of the executable file.
    - workflow_map_name: Name of the workflow map to be passed as an argument.
    - invalid_argument: The invalid argument to be passed to the executable.

    Returns:
    - The penultimate line of terminal output as a string.
    """
    terminal_output_as_list = subprocess.check_output(
        f"python3 {executable_name}.py {invalid_argument} {workflow_map_name}",
        shell=True,
        universal_newlines=True,
    ).split("\n")
