...
class PIH:
    ...
    class SERVICE(ServiceTool):
            ...
            return DataTool.filter(
                lambda item: item.name.startswith(
                    j(
                        (
                            SUPPORT_NAME_PREFIX,
                            ServiceRoleTool.service_description(service_role_or_description).name,
                        )
                    )
                ),
        ...
        class ADMIN(ServiceAdminTool):
            ...
            def check_on_startable(
                ...
                service_description: ServiceDescription = if_else(
                    isinstance(service_role_or_description, ServiceRoles),
                    lambda: ServiceRoleTool.service_description(service_role_or_description),
                    service_role_or_description,
                )
            ...
            def change_host_on_local(
                ...
                ServiceRoleTool.service_description(service_role_or_description).host = (
                    PIH.OS.host()
                )
            ...
            def kill_all(
                ...
                for server_role in ServiceRoles:
                    host: str | None = ServiceRoleTool.service_description(server_role).host
                    if ne(host):
                        hosts.add(host)
            ...
            def as_developer(
                ...
                description: ServiceDescription = ServiceRoleTool.service_description(service_role)
            ...
            def start_certainly(
                ...
                return PIH.SERVICE.ADMIN.start(
                    service_role=service_role_or_description,
                    host=host,
            ...
            def start(
                ...
                assert isinstance(service_role, ServiceRoles), "type service_role must be ServiceRoles"
                if n(host_is_linux):
                    host_is_linux = PIH.OS.is_linux(host)
                if PIH.OS.python_exists(host, host_is_linux=host_is_linux):
                    service_description: ServiceDescription = ServiceRoleTool.service_description(
                        service_role, True
                    )
                    can_use_standalone: bool = nn(
                        service_description.standalone_name
                    ) and n(service_description.use_standalone)
                    as_standalone = (
                        (as_standalone and can_use_standalone)
                        or nn(service_description.standalone_name)
                        and service_description.use_standalone
                    )
                    service_description.pih_version = pih_version or PIH.VERSION.value
                    ...
                    if as_standalone:
                        PIH.UPDATE.install_module(
                            ...
                            version=service_description.version,
                            host=host,
                            host_is_linux=host_is_linux,
                            show_output=show_output,
                        )
                    ...
                    try:
                        ...
                        if host_is_linux or (
                            ...
                            return print(PIH.RESULT.SSH.execute_python_file(
                                js(command_list),
                                host,
                                in_background=True,
                                as_standalone=as_standalone,
            ...
            def serve(
                ...
                #service_description.pih_version = service_description.pih_version or PIH.VERSION.value
            ...
            def stop(
                ...
                description: ServiceDescription | None = ServiceRoleTool.service_description(
                    service_role_or_description
                )
                    ...
                    DataTool.map(
                        ...
                        DataTool.rpc_unrepresent(
                            PIH.SERVICE.call_command(
                                ...
                                (
                                    (
                                        RPC.service_information or STUB
                                    ).name
                                ),
                            )
                        )
        ...
        def get_host(
            ...
            information: ServiceDescriptionBase | None = ServiceRoleTool.service_description(
                service_role_or_description
            )
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool, ServiceRoleTool):
        class JOURNAL:
            @staticmethod
            def type_by_any(
    ...
    class CARD_REGISTRY:
        ...
        def is_folder_registered(name: str) -> bool:
            return ne(
                PIH.RESULT.EVENTS.get(*PIH.EVENT.BUILDER.card_registry_folder_was_registered(name))
            )
        ...
        def get_statistics() -> list[CardRegistryFolderStatistics]:
            ...
            return PIH.RESULT.map(map_function, PIH.RESULT.DATA_STORAGE.execute(query)).data
        ...
        def get_suitable_folder_name_for_person(
            ...
            return DataTool.get_first_item(
                PIH.RESULT.map(map_function, PIH.RESULT.DATA_STORAGE.execute(query)).data
            ).name
