...
class Output(OutputBase, OutputStub):
    ...
    def service_header(self, information: ServiceInformation) -> None:
        ...
        with self.make_indent(1):
            ...
            self.value(
                "Version/PIH version:",
                self.bold(j((information.version, information.pih_version), "/")),
            )
...
class PIH:
    ...
    class UPDATE:
        ...
        def modules(
            ...
            if ne(value) and update:
                installed_module_list: list[str] = {
                    package.key.lower() for package in pkg_resources.working_set
                }
        ...
        def localy(version: str | None = None, show_output: bool = False) -> bool:
            return PIH.UPDATE.install_module(
        ...
        class ADMIN:
            ...
            def start(
                ...
                if PIH.OS.python_exists(host, host_is_linux=host_is_linux):
                    ...
                    as_standalone = (
                        as_standalone
                        and nn(description.standalone_name)
                        and n(description.standalone)
                    )
                    if nn(description.standalone_name) and n(description.standalone):
                        description.standalone = as_standalone
                    description.pih_version = pih_version or PIH.VERSION.value
                    ...
                    if as_standalone:
                        PIH.UPDATE.install_module(
                            module_name=j((A.NAME, description.standalone_name), "-"),
                    ...
                    try:
                        PIH.SERVICE.ADMIN.check_on_startable(
                            service_role_or_information
                        )
                        if check_if_started:
                        ...
                        if host_is_linux or (
                            n(host_is_linux)
                            and n(
                                PIH.EXECUTOR.extract_returncode(complete_process, True)
                            )
                        ):
            ...
            def serve(
                ...
                service_description.standalone = not isolate and (
                    as_standalone and nn(service_description.standalone_name)
