...
class PIH:
    ...
    class UPDATE:
        ...
        def command_for_module(
            ...
            if virtual_environment:
                executor_path = PIH.PATH.join(PIH.PATH.FACADE.WINDOWS_VIRTUAL_ENVIRONMENT_PATH(), CONST.PYTHON.EXECUTOR)
            else:
                executor_path = (
        ...
        def install_module(
            ...
            virtual_environment: bool = PIH.PATH.path(sys.executable).find(PIH.PATH.FACADE.WINDOWS_VIRTUAL_ENVIRONMENT_FOLDER) != -1
            if virtual_environment:
            ...
            if name not in CACHE:
                ...
                command_list: list[str] = PIH.UPDATE.command_for_module(
                    module_name, version, is_local, virtual_environment
                )
                if host_is_linux:
    ...
    class SERVICE(ServiceTool):
        ...
        class ADMIN(ServiceAdminTool):
            ...
            def check_on_startable(
                ...
                service_description: ServiceDescription = if_else(
                    ...
                    lambda: ServiceRoleTool.service_description(
                        service_role_or_description
                    ),
                    service_role_or_description,
                )
            ...
            def kill_all(
                ...
                for server_role in ServiceRoles:
                    host: str | None = ServiceRoleTool.service_description(
                        server_role
                    ).host
                    if ne(host):
                        hosts.add(host)
            ...
            def start(
                ...
                assert isinstance(
                    service_role, ServiceRoles
                ), "type service_role must be ServiceRoles"
                if n(host_is_linux):
                    host_is_linux = PIH.OS.is_linux(host)
                if PIH.OS.python_exists(host, host_is_linux=host_is_linux):
                    service_description: ServiceDescription = (
                        ServiceRoleTool.service_description(service_role, True)
                    )
                    ...
                    try:
                        ...
                        if host_is_linux or (
                            ...
                            return print(
                                PIH.RESULT.SSH.execute_python_file(
                                    js(command_list),
                                    host,
                                    in_background=True,
                                    as_standalone=as_standalone,
        ...
        def get_host(
            ...
            information: ServiceDescriptionBase | None = (
                ServiceRoleTool.service_description(service_role_or_description)
            )
            if isinstance(information, ServiceDescription):
                if ne(information.host):
