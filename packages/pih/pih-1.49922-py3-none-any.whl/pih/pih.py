        ...
        def command_for_module(
            ...
            if virtual_environment:
                executor_path = PIH.PATH.join(
                    PIH.PATH.FACADE.WINDOWS_VIRTUAL_ENVIRONMENT_PATH(),
        ...
        class ADMIN(ServiceAdminTool):
            ...
            def start(
                ...
                if PIH.OS.python_exists(host, host_is_linux=host_is_linux):
                    ...
                    if PIH.OS.is_virtual_environment() and as_standalone:
                        as_standalone = False
                        ipih_install = False
                        A.O.error(
                            "Sorry, you can not start service in standalone mode for virtual environemnt. Service will be started in normal mode"
                        )
        ...
        def is_virtual_environment() -> bool:
            return (
                PIH.PATH.path(sys.executable).find(
                    PIH.PATH.FACADE.WINDOWS_VIRTUAL_ENVIRONMENT_FOLDER
                )
                != -1
            )
