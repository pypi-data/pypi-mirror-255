...
struct bundle sliceFiber( struct bundle fibras, int sliceNum)
{
    ...
    for(k=0;k<fibras.nfibers;k++)
    {
        ...
        *(sB.points[k]+2) = *(fibras.points[k]+2);
        ...
        float fiberlength = 0;
        float * acc_length = (float *) malloc(sizeof(float) * fibras.npoints[k]);
        acc_length[0] = 0;
...
float** matriz = (float**) malloc (f1.nfibers*sizeof(float*));
    ...
    //}
    float * distancias, * distancias1, maximo, maximo1, distancia;
    distancias = (float *)malloc(sizeof(float) * f1.npoints[0]);
    distancias1 = (float *)malloc(sizeof(float) * f1.npoints[0]);
