...
void main() {
	...
	ivec2 fValidFetcher = ivec2(intV2F%texture1DMax, intV2F/texture1DMax);

	bool valid = bool(texelFetch(fiberValidator, fValidFetcher, 0).r);

	ivec2 bundleColorFetcher = ivec2(int(vsColor[0])%bundleTex1DMax, int(vsColor[0])/bundleTex1DMax);
