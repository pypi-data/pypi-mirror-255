...
async def _check_admin(auth0_sub: str, pool: Pool):
...
class VerifyToken:
    ...
    def __init__(self, pool: Pool, auth0_config: Optional[Auth0Config] = None):
    ...
    async def verify(
        ...
        if "admin" in security_scopes.scopes:
            if not _check_admin(payload["sub"], self.pool):
                raise UnauthorizedException(detail="User is not an admin")
