...
""")
    ...
    with open("views/users_view.py", 'w') as file:
        file.write("""from kongfu_api import route
from kongfu_api import jwt_encode
...
""")
    ...
    with open("app.py", 'w') as file:
        file.write("""from kongfu_api import route, run, application
...
from peewee import *
from kongfu_api.db import BaseModel
...
MODEL_MAPPING = {
    # 生成全套接口['list', 'get', 'post', 'put', 'delete', 'drop']，如：'table1': Table1
    # 如需要定制接口，请写成以下形式：'table1': (Table1, ['list', 'get'])
    "users": (Users, []),
}
""")
        print("创建models.py")
