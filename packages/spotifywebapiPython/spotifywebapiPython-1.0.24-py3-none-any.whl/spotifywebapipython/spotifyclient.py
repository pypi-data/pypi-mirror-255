...
class SpotifyClient:
    ...
    def MakeRequest(self, method:str, msg:SpotifyApiMessage) -> int:
        ...
        try:
            ...
            # reset urlopen keyword arguments, so nothing is miinterpreted in the request!
            urlopen_kw:dict = {}

            # in the logic below, ensure that ALL arguments are passed to the urllib3.request method!
            # if not, it causes problems in Home Assistant with kwargs references!
            ...
            if msg.HasUrlParameters:
                ...
                response = self._Manager.request_encode_url(method, url, fields=None, headers=msg.RequestHeaders, **urlopen_kw)
                ...
                if msg.IsRequestDataEncoded:
                    ...
                    response = self._Manager.request(method, url, body=msg.RequestData, fields=None, headers=msg.RequestHeaders, json=None, **urlopen_kw)
                   
                else:

                    _logsi.LogDictionary(SILevel.Verbose, "SpotifyClient http request: '%s' (with body)" % (url), msg.RequestData, prettyPrint=True)
                    response = self._Manager.request_encode_body(method, url, fields=msg.RequestData, headers=msg.RequestHeaders, encode_multipart=False, multipart_boundary=None, **urlopen_kw)
                ...
                response = self._Manager.request(method, url, body=None, fields=None, headers=msg.RequestHeaders, json=msg.RequestJson, **urlopen_kw)
                
            else:

                _logsi.LogDictionary(SILevel.Verbose, "SpotifyClient http request: '%s' (no body)" % (url), msg.RequestData, prettyPrint=True)
                response = self._Manager.request(method, url, body=None, fields=None, headers=msg.RequestHeaders, json=None, **urlopen_kw)
