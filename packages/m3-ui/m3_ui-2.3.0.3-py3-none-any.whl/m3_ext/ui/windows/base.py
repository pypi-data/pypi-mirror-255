...
class BaseExtWindow(ExtUIComponent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # По умолчанию отступ внутри границ 5 px
        ...
        # В пререндере, в контексте окна, m3_window_id подменяется на client_id, чтобы дочерние окна имели в родителях
        # породившее их окно. Такой подход не подходит для случая, когда есть одно основное окно и запускаются
        # действия, порождающие диалоговые окна для общения с пользователем. В этом случае, все они крутятся вокруг
        # основного окна и по логике вещей, как раз основное окно для них и является родительским.
        self.safe_m3_window_id = False
        ...
        # Признак, указывающий на необходимость запуска метода show окна
    ...
    def render_params(self):
        ...
        if self.help_topic:
            self._put_params_value(
                'helpTopic',
                self._get_help_topic_prefix() + self._help_topic_full_path(),
            )
    ...
    def pre_render(self):
        ...
        if hasattr(self.action_context, 'm3_window_id'):
            ...
            # Осуществляется проверка необходимости указания окна, в качестве родительского для всех действий и
            # потомков, порождаемых из него.
            if not self.safe_m3_window_id:
    ...
    def _get_help_topic_prefix(self) -> str:
        """Возвращает путь до директории с файлами справочника."""
        return settings.HELP_PREFIX

    def _help_topic_full_path(self) -> str:
        """Возвращает квалифицирующее имя топика помощи."""

        if self.help_topic:
            assert isinstance(self.help_topic, tuple)
            assert len(self.help_topic) > 0

            result = self.help_topic[0] + '.html' + ('#' + self.help_topic[1] if len(self.help_topic) > 1 else '')
        else:
            result = ''
