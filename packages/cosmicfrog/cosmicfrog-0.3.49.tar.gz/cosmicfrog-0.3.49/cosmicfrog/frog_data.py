...
class FrogModel:
    ...
    def __init__(
        ...
        if model_name and not (connection_string or engine):
            ...
            if app_key:
                ...
                job_app_key = FrogModel.class_app_key
            else:
    ...
    def geocode_table(self, table_name: str, geoprovider: str = "MapBox", geoapikey: str = None, ignore_low_confidence: bool = True):
        """
        Geocode a geocodable entity table (Customers, Facilities, Suppliers) in model
        """

        return FrogUtils.geocode_table(self.model_name, table_name, self._app_key, geoprovider=geoprovider, geoapikey=geoapikey, ignore_low_confidence=ignore_low_confidence)
    ...
    def upsert_csv(self, table_name: str, filename: str, _correlation_id: str = "") -> (int, int):
        """
    ...
    def __generate_index_sql(self, index_name, table_name, key_column_list) -> str:
        ...
        return f"CREATE INDEX IF NOT EXISTS {index_name} ON {table_name}({coalesced_columns});"
    ...
    def _create_upsert_index_for_table(
        ...
        upsert_index_name = "cf_upsert_index_" + table_name
        ...
        with self.begin() as connection:
            ...
            with connection.connection.cursor() as cursor:
                ...
                if combined_key_columns:
                    ...
                    # Creating an index, outside of transaction for concurrency - minimise potential deadlock issues
