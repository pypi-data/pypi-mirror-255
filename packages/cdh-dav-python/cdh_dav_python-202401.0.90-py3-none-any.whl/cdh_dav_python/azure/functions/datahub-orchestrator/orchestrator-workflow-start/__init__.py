...
from shared_code import datalake_helper, config_file_helper, app_config, app_utils, email_helper, storage_queue_helper
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        config_file_helper.add_error_message_and_save(config_file_name=file_name, error_text=str(ex))
...
def process_message(msg_body: str):
    ...
    storage_queue_helper.send_message_to_queue(queue_name=app_config.QUEUE_OBSERVABILITY_LOGGING_NAME,
                                               message=json.dumps(msg_json))

    try:
        email_helper.send_workflow_start_email(data_source=data_source, workflow_id=workflow_id,
                                               delivery_date=delivery_date,
                                               vendor_delivery_location=vendor_delivery_location,
                                               triggerred_by=triggerred_by)

        start_workflow_handler.process(data_source=data_source, delivery_date=delivery_date, workflow_id=workflow_id,
                                       file_name=file_name, vendor_delivery_location=vendor_delivery_location,triggerred_by=triggerred_by)


    except Exception as ex:
        logging.error(f"Error processing message {msg_body}. \n \n error: " + str(ex))
        config_file_helper.add_error_message_and_save(config_file_name=file_name, error_text=str(ex))
        email_helper.send_unhandled_error_email(error=str(ex), data_source=data_source, workflow_id=workflow_id)
