...
dbutils.widgets.dropdown("partitioning_scheme", "none", ["none","file_based", "column_based"])
...
dbutils.widgets.text('partitioning_scheme_file_based_datetime_extract_index_after_regex', '0')
...
import  azure.databricks.etl.shared.cdh_helper as cdh_helper
...
exclude_tables_from_parquet_conversion = dbutils.widgets.get('exclude_tables_from_parquet_conversion')
...
partitioning_scheme_file_based_datetime_extract_regex = dbutils.widgets.get('partitioning_scheme_file_based_datetime_extract_regex')
partitioning_scheme_file_based_datetime_extract_index_after_regex = dbutils.widgets.get('partitioning_scheme_file_based_datetime_extract_index_after_regex')
partitioning_scheme_column_based_table_column_name = dbutils.widgets.get('partitioning_scheme_column_based_table_column_name')

table_name_regex_replace_array = [] if table_name_regex_replace is None or len(table_name_regex_replace) == 0 else table_name_regex_replace.split(',')

exclude_tables_from_parquet_conversion_array = [] if exclude_tables_from_parquet_conversion is None or len(exclude_tables_from_parquet_conversion) == 0 else exclude_tables_from_parquet_conversion.split(',')
exclude_tables_from_parquet_conversion_array = [item.strip().lower() for item in exclude_tables_from_parquet_conversion_array]

partitioned_tables_include_array = [] if partitioned_tables_include is None or len(partitioned_tables_include) == 0 else partitioned_tables_include.split(',')
partitioned_tables_include_array = [item.strip().lower() for item in partitioned_tables_include_array]

partitioned_tables_exclude_array = [] if partitioned_tables_exclude is None or len(partitioned_tables_exclude) == 0 else partitioned_tables_exclude.split(',')
partitioned_tables_exclude_array = [item.strip().lower() for item in partitioned_tables_exclude_array]

partition_by_array = [] if partition_by is None or len(partition_by) == 0 else partition_by.split(',')
partition_by_array = [item.strip().lower() for item in partition_by_array]

table_renaming_map_array = [] if table_renaming_map is None or len(table_renaming_map) == 0 else table_renaming_map.split('|')
...
print(f"exclude_tables_from_parquet_conversion_array : {exclude_tables_from_parquet_conversion_array}")
...
file_based_partition_dates_df = spark.createDataFrame(emptyRDD,file_based_partition_dates_df_schema)
...
output_file_directory = os.path.join(baseLocation, f"work/datahub/completed/{data_source}/{workflow_id}/misc/")
...
def should_table_be_partitioned(table_name): 
    ...
    if len(partitioned_tables_include_array) ==0 and len(partitioned_tables_exclude_array) == 0:
        return True
    
    if table_name.casefold() in partitioned_tables_include_array:
        return True
    
    if table_name.casefold() in partitioned_tables_exclude_array or 'all' in partitioned_tables_exclude_array:
        return False
    ...
    for source_file_path in source_files_paths:
            ...
            if file_name_with_extension != '': 
                ...
                if file_extension.casefold() == source_files_extension.casefold():                    
                    ...
                    if table_name.casefold() not in exclude_tables_from_parquet_conversion_array:
...
def save_metadata_to_db():
    ...
    spark.sql(f"DELETE from {ingest_table} where workflow_id= '{workflow_id}' and dataset_name='{data_source}'") 
    ...
    for row in audit_log_output_array:      
        ...
        sql_array.append(f" ('{workflow_id}', '{data_source}',\
                            '{table_name}',\
                            '{source_path}',\
                            '{source_file_count}',\
                            '{source_file_format}',\
...
def generate_file_name_to_year_quarter_month_partition_dataframe(source_files_paths):
    ...
    for source_files_path in source_files_paths:
        ...
        date_time_re = re.findall(partitioning_scheme_file_based_datetime_extract_regex,file_name)
        date_time = (date_time_re[int(partitioning_scheme_file_based_datetime_extract_index_after_regex)]).strip()
        if year_start_index is not None and len(year_start_index) > 0:
            year = date_time[int(year_start_index):int(year_length)] if int(year_length) > 0 else date_time[int(year_start_index)]
        else:
            year = 0
        if quarter_start_index is not None and len(quarter_start_index) > 0:    
            quarter = date_time[int(quarter_start_index):int(quarter_length)] if int(quarter_length) > 0 else date_time[int(quarter_start_index)]
        else:
            quarter = 0
        if month_start_index is not None and len(month_start_index) > 0:
            month = date_time[int(month_start_index):int(month_length)] if int(month_length) > 0 else date_time[int(month_start_index)]
        else:
            month = 0
        ...
        source_file_partition_array.append([source_files_path,file_name,date_time,int(year),int(quarter),int(month)])

    df_dates = spark.createDataFrame(data=source_file_partition_array, schema =  file_based_partition_dates_df_schema) 
    #display(df_dates)
    return df_dates
...
def csv_to_parquet_transform(source_to_dest):
    ...
    try:        
        ...
        df = spark.read.options(delimiter=source_files_delimiter, 
        ...
        partition_and_save_staging_table(df=df,table_name=table_name,source_files=source_files,destination_table_folder=destination_table_folder)
                
        end_time = datetime.now(tz=timezone.utc)        
        src_file_count, src_file_size_mb = get_file_count_and_size_in_mb(files=source_files)
        dest_file_count, dest_file_size_mb = get_file_count_and_size_in_mb(directory=destination_table_folder)
        audit_entry = {
...
def parquet_to_parquet_transform(source_to_dest):
    ...
    start_time = datetime.now(tz=timezone.utc)
    clean_staging_table_and_directory(table_name)
    df = spark.read.parquet(source_path).withColumn("full_input_file", input_file_name())
    destination_table_folder = os.path.join(baseLocation,destination_folder,table_name)
    partition_and_save_staging_table(df=df,table_name=table_name,source_files=None,destination_table_folder=destination_table_folder)
    
    end_time = datetime.now(tz=timezone.utc)        
    src_file_count, src_file_size_mb = get_file_count_and_size_in_mb(directory=source_path)
    dest_file_count, dest_file_size_mb = get_file_count_and_size_in_mb(directory=destination_table_folder)
    audit_entry = {
        'table_name': table_name, 
...
def clean_staging_table_and_directory(table_name:str):
    ...
    print(f'Finished dropping database and deleting directory: {destination_table_folder}')
    
    
def partition_and_save_staging_table(df:any,table_name:str, source_files: any,destination_table_folder:str):
    ...
    if do_partition_table:
        ...
        if partitioning_scheme.casefold() == "file_based":
            ...
            if "year" in listColumns:
                print(f'{table_name}: year column exists already. Using the existing column.')
                df2 = df
                df2 = df2.drop("full_input_file").drop("file_path")
            else:
                file_based_partition_dates_df=generate_file_name_to_year_quarter_month_partition_dataframe(source_files)
                df2=df.join(file_based_partition_dates_df,df.full_input_file ==  file_based_partition_dates_df.file_path,"leftouter")
                df2 = df2.drop("full_input_file").drop("file_path").drop("file_name").drop("date_time")
                df2 = df2.drop("quarter") if "quarter" not in partition_by_array else df2
                df2 = df2.drop("month") if "month" not in partition_by_array else df2
            
            
            df2.write.format("parquet").partitionBy(partition_by_array)\
                    .mode("overwrite")\
                    .option("path",destination_table_folder)\
                    .saveAsTable(f"{stage_database_name}.{table_name}")

        if partitioning_scheme.casefold() == "column_based":
            if year_start_index is not None and len(year_start_index) > 0:                
                df = df.withColumn("year",substring(partitioning_scheme_column_based_table_column_name,int(year_start_index),int(year_length)).cast('int'))            
                
            if quarter_start_index is not None and len(quarter_start_index) > 0:    
                df = df.withColumn("quarter",substring(partitioning_scheme_column_based_table_column_name,int(quarter_start_index),int(quarter_length)).cast('int'))
                
            if month_start_index is not None and len(month_start_index) > 0:
                df = df.withColumn("month",substring(partitioning_scheme_column_based_table_column_name,int(month_start_index),int(month_length)).cast('int'))
            ...
            df.write.format("parquet").partitionBy(partition_by_array)\
                      .mode("overwrite")\
                      .option("path",destination_table_folder)\
                      .saveAsTable(f"{stage_database_name}.{table_name}")
...
def process():  
    ...
    if source_files_format.casefold() == "csv":
        source_files_paths = cdh_helper.get_all_files_in_directory(source_folder_location)
...
try:
    process()
    cdh_helper.publish_event_status(status_success=True, data_source=data_source, workflow_id=workflow_id, request_id=request_id, outputs=[])
except Exception as ex:
    print(ex)
    ex_formatted = cdh_helper.exception_to_string(ex)
    print(ex_formatted)
    cdh_helper.publish_event_status(status_success=False, data_source=data_source, workflow_id=workflow_id, request_id=request_id, outputs=[{"error":str(ex)}])
