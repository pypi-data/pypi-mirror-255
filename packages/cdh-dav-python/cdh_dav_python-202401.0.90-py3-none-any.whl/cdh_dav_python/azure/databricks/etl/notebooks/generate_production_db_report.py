...
def get_database_metrics():
    for database in databases:
        ...
        for table in tables:
            try:
                ...
                row_count = spark.sql("select count(*) as count from  " + full_table_name).collect()[0]["count"]
                if df.count() > 1:
                    previous_version = df.collect()[1]['version']
                    previous_row_count = spark.sql(f"select count(*) as count from  {full_table_name}@v{previous_version}").collect()[0]["count"]
                else:
    ...
    full_output_path = os.path.join(base_path, output_file_directory ,"production_validation.json")
    if workflow_id is not None and workflow_id != "" and workflow_id.casefold() != "ISOLATED".casefold() and storage_account_name is not None:
    ...
    for database in databases:
        ...
        for db_result in results_array:
            ...
            for table in db_result:
                ...
                row_count_delta_formatted = "{:,}".format(row_count - previous_row_count)
            ...
            email_body = email_body + html_template.replace("#BODY#", html_body).replace("#DATABASE_NAME#", db) + "<br/><br/>"      
    
    full_email_body = email_body        
    url = email_service_url
    email_subject = f"CDH Database Report - {len(databases)} databases."
    if len(databases) == 1 and workflow_id is not None and workflow_id != "" and workflow_id.casefold() != "ISOLATED".casefold():
        email_subject = f"[{app_environment}] ETL completed for {data_source} - {delivery_date} ."
        full_email_body = EMAIL_TEMPLATE_HEAD.replace("#WORKFLOW_ID#", workflow_id).replace("#DELIVERY_DATE#", delivery_date).replace("#DATA_SOURCE#", data_source).replace("#LOAD_TYPE#", delivery_type) + email_body
