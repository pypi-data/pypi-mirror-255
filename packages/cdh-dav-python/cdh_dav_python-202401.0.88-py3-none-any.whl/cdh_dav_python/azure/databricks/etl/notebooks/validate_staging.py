...
dbutils.widgets.dropdown('validation_mode', 'vendor_specific', ['vendor_specific','vendor_common','skip_validation'])
dbutils.widgets.dropdown("send_email_mode", "on_needs_review", ["always","on_needs_review"])
...
import  azure.databricks.etl.shared.data_load_helper as data_load_helper
from  azure.databricks.etl.shared import constants
...
canonical_validation_file_name = f"{data_source}_{delivery_date}_validation_metadata.txt"
manifest_file_folder_full_path = os.path.join(base_path,manifest_file_folder)
vendor_canonical_level_1_path = os.path.join(base_path,manifest_file_folder,manifest_file)
vendor_canonical_level_2_path = os.path.join(base_path,manifest_file_folder,manifest_file_level_2)
output_file_directory = os.path.join(base_path,f"work/datahub/completed/{data_source}/{workflow_id}/misc")
canonical_validation_output_directory = os.path.join(output_file_directory,canonical_validation_file_name)
ge_nb_params_json = {
...
def process_vendor_specific():
    ...
    canonical_df.coalesce(1).write.mode('overwrite').option("header",True).csv(dest_path_temp)
...
def process_premier():
    file_paths = cdh_helper.get_specific_files_in_directory(manifest_file_folder_full_path,manifest_file)
    ...
    for key in result_array.keys():    
            ...
            newRow = spark.createDataFrame([(data_source,table_name,"","total_count",metric_value)], canonical_df_schema)
...
def process_pointclickcare():
    file_paths = cdh_helper.get_specific_files_in_directory(manifest_file_folder_full_path,manifest_file,exact_match=False)
    df = spark.read.format("csv") \
                    .option("inferSchema", True) \
                    .option("header", True) \
                    .option("sep","|") \
    ...
    for row in df.collect():
        ...
        canonical_metric = "max_value" if metric.casefold()== "maximumvalue"\
                        else "min_value" if metric.casefold()== "minimumvalue"\
                        else "mean_value" if metric.casefold()== "averagevalue"\
                        else "total_count" if metric.casefold()== "totalcount"\
                        else "unique_value_count" if metric.casefold()== "uniquecount"\
                        else ""
        temp_key = f"{table_name}*{column_name}*{canonical_metric}"
        # TODO : need to figure out what defaultvaluecount from PCC means
        if metric.casefold() in ["uniquecount","totalcount","maximumvalue","minimumvalue","averagevalue"]\
                ...
                if temp_key in result_array.keys():
                    ...
                    if metric.casefold() in ["uniquecount","totalcount"] :
                        ...
                        if avg_tracker_key in average_metric_file_count_tracker.keys():
                            average_metric_file_count_tracker[avg_tracker_key] = int(average_metric_file_count_tracker.get(avg_tracker_key)) + 1
                        else:
    ...
    for key in result_array.keys():    
        ...
        metric_value = result_array.get(key) if metric.casefold() != "mean_value" else float(result_array.get(key))/int(average_metric_file_count_tracker.get(f"{table_name}*{column_name}"))

        newRow = spark.createDataFrame([(data_source,table_name,column_name,metric,metric_value)], canonical_df_schema)
        canonical_df = canonical_df.union(newRow)
...
def process_vendor_common():
    ...
    file_paths = cdh_helper.get_specific_files_in_directory(manifest_file_folder_full_path,manifest_file)
    df_table_level = spark.read.format("csv") \
                    .option("inferSchema", True) \
                    .option("header", True) \
                    .option("sep",",") \
                    .load(file_paths)
    ...
    file_paths = cdh_helper.get_specific_files_in_directory(manifest_file_folder_full_path,manifest_file_level_2)
    df_col_level = spark.read.format("csv") \
                    .option("inferSchema", True) \
                    .option("header", True) \
                    .option("sep",",") \
                    .load(file_paths)
    ...
    for row in df_table_level.collect():
            newRow = spark.createDataFrame([(data_source,row["table"],"","total_count",row["table_row_counts"])], canonical_df_schema)
            canonical_df = canonical_df.union(newRow)
    ...
    for row in df_col_level.collect():
        if row["max_value"] is not None and row["max_value"] != "":
            newRow = spark.createDataFrame([(data_source,row["table"],row["column"],"max_value",row["max_value"])], canonical_df_schema)
            canonical_df = canonical_df.union(newRow)

        if row["min_value"] is not None and row["min_value"] != "":
            newRow = spark.createDataFrame([(data_source,row["table"],row["column"],"min_value",row["min_value"])], canonical_df_schema)
            canonical_df = canonical_df.union(newRow)

        if row["unique_value_count"] is not None and row["unique_value_count"] != "":
            newRow = spark.createDataFrame([(data_source,row["table"],row["column"],"unique_value_count",row["unique_value_count"])], canonical_df_schema)
            canonical_df = canonical_df.union(newRow)

        if row["mean_value"] is not None and row["mean_value"] != "":
            newRow = spark.createDataFrame([(data_source,row["table"],row["column"],"mean_value",row["mean_value"])], canonical_df_schema)
            canonical_df = canonical_df.union(newRow) 
...
def process_skip_validation():
    ...
    for table in tables:
        ...
        row_count = df.count()
        newRow = spark.createDataFrame([(data_source,table,"","total_count",row_count)], canonical_df_schema)
        canonical_df = canonical_df.union(newRow)
...
def save_metadata_to_db(canonical_df):
    ...
    for row in canonical_df.collect():
        ...
        sql_array.append(f" ('{workflow_id}', '{data_source}', '{table_name}','{column_name}','{metric}','{metric_value}', '{time_now}')")
...
def send_validation_email(ge_nb_output_body):
    ...
    for element in ge_nb_output_body:
        ...
        if table_name != "_summary":
            ...
            for detail in details:
                ...
                detail_html = detail_html + f""" <tr  style="text-align: left;">
            ...
            if stripe_row:
                css_style = "border-bottom: 1px solid #dddddd;background-color: #f2f2f2;"
                stripe_row = False
            else:
    ...
    validation_mode_str = validation_mode if adjust_metrics_range == False else f"{validation_mode} <br/> **<i>Metrics Adjusted by 1 for range Calculations</>"
    ...
    if (send_email_mode.casefold() == "on_needs_review" and validation_needs_review) or send_email_mode.casefold() == "always":
        ...
        print(f"NOT sending email since send_email_mode is {send_email_mode} and validation_needs_review is {validation_needs_review}")
...
def process():    
    ...
    ge_nb_output = dbutils.notebook.run(
            "validate_staging_ge", 36000, ge_nb_params_json
        )
