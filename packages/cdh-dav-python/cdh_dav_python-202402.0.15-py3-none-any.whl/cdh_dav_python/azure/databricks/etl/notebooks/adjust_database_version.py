        ...
        for item in results:
            ...
            if maxVersion != item['version']:
                ...
                for n in range(i, maxVersion + 1):
                    ...
                    print('''Alter table {DB}.{TB} SET TBLPROPERTIES ('cdh_comment'='{adjust_reason}')'''.format(DB=item['database'], TB=item['table'], adjust_reason=adjust_reason))
                    sql('''Alter table {DB}.{TB} SET TBLPROPERTIES ('cdh_comment'='{adjust_reason}')'''.format(DB=item['database'], TB=item['table'], adjust_reason=adjust_reason))
                sql_array.append(f"""('{workflow_id}', '{data_source}', '{item['database']}' , '{item['table']}', '{item['version']}', '{maxVersion}', '{time_now}' )""")
