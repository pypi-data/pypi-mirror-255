...
exclude_tables_from_parquet_conversion_array = [item.strip() for item in exclude_tables_from_parquet_conversion_array]
...
partitioned_tables_include_array = [item.strip() for item in partitioned_tables_include_array]
...
partitioned_tables_exclude_array = [item.strip() for item in partitioned_tables_exclude_array]
...
print(f"partitioning_scheme_file_based_datetime_extract_index_after_regex : {partitioning_scheme_file_based_datetime_extract_index_after_regex}")
...
output_file_directory = os.path.join(baseLocation, f"work/datahub/inprogress/{workflow_id}/parquet_conversion/")
...
def should_table_be_partitioned(table_name): 
    ...
    if table_name in partitioned_tables_exclude_array or 'all' in partitioned_tables_exclude_array:
        return False
...
def csv_to_parquet_transform(source_to_dest):
    ...
    try:        
        ...
        df = spark.read.options(delimiter=source_files_delimiter, header=source_files_has_header, multiline=source_files_is_multiline,escape=source_files_escape_character,badRecordsPath=badRecordsPath)\
                        .csv(source_files)\
        ...
        partition_and_save_staging_table(df=df,table_name=table_name,source_files=source_files)
    except:
