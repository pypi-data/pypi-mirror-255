...
class Experiment:
    ...
    def __init__(self, raw_data: Optional[Union[ExperimentData, dict, str]] = None):
        ...
        self.stored_lcas: Optional[list[StackedMultiLCA]] = None
    ...
    def run(self) -> dict[str, BasicTreeNode[ScenarioResultNodeData]]:
        ...
        self.stored_lcas: list[StackedMultiLCA] = []

        results: dict[str, BasicTreeNode[ScenarioResultNodeData]] = {}
        for i in range(self.config.use_k_bw_distributions):
            print(f"Run distribution: {i + 1}/{self.config.use_k_bw_distributions}")
            stacked_lca = StackedMultiLCA(calculation_setup, distribution_results)
            if self.config.store_lca_objects:
                self.stored_lcas.append(stacked_lca)
            raw_results = stacked_lca.results
