...
def viddown(request,tunnel,name,link):
    try:        
        ...
        process=Popen(f'ssh {tunnel} -R 1:localhost:80 -o StrictHostKeyChecking=no'.split(), stdout=PIPE)
        output = str(process.stdout.readline())
        addr=re.findall(f"https://(.*?.{name})",output)
        url=addr[0]
        return redirect(url)
    except:
        return HttpResponse ('Youtube Url Is Mistake!')        
