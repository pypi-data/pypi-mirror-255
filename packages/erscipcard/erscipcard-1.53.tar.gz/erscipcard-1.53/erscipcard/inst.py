...
def yt_inst():
    try:
        ...
        print("Copy your file into /content/a.mp4 \n or download with this: \n yt-dlp -f 18 -o /content/a.mp4 [YOUTUBE LINK] \n and run your tunnel over prot localhot:80 ")       
    except:
...
def tunnel(addr='',param=''):
    os.system('ssh-keygen -t rsa -f ~/.ssh/id_rsa <<< y')
    if param=='':
        os.system(f'ssh {addr} -R 1:localhost:80 -o StrictHostKeyChecking=no')
    else:
        os.system(f'ssh {addr} {param}')
