...
class CVT:
    ...
    used by PyIAC itself and not for other purposes

    Usage:
    
    ```python
    >>> from PyIAC.tags import CVT
    >>> _cvt = CVT()
    ```
    ...
    def get_value(self, name, unit:str=None):
        ...
        _new_object = copy.copy(_tag.get_value())
...
class CVTEngine(Singleton):
    ...
    so each sub-thread within the PyIAC application can access tags
