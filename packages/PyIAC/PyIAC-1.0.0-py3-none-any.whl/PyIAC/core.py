...
This module implements the core app class and methods for PyIAC
"""
import time
import sys
import logging
...
class PyIAC(Singleton):
    r"""
    PyIAC is a [singleton](https://en.wikipedia.org/wiki/Singleton_pattern) class to develop multi threads web application
    ...
    def _append_machine(self, machine, interval:float=1, mode:str="sync"):
        ...
        * **machine** (`PyIACStateMachine`): a state machine object.
    ...
    def get_machine(self, name:str):
        """
        Returns a PyIAC State Machine defined by its name.

        **Parameters:**

        * **name** (str): a PyIAC state machine name.
    ...
    def get_machines(self)->list:
        """
        Returns all defined PyIAC state machines.
    ...
    def define_machine(self, name="", interval=1, mode="sync", **kwargs):
        ...
        **Returns** Class (cls)
        ...
        ```python
        >>> from PyIAC import PyIAC, PyIACStateMachine
        >>> app = PyIAC()
        >>> @app.define_machine(name='state_machine_name', interval=1, mode='async')
            class StateMachine(PyIACStateMachine):
                ...
        ```
        """
        def decorator(cls):
    ...
    def thread(self, function=None, **kwargs):
        ...
        This method will register into the PyIAC application
