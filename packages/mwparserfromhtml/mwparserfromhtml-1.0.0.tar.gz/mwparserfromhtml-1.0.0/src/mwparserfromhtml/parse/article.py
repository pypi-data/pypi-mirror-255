...
class WikiStew(BeautifulSoup):
    ...
    def get_images(self) -> typing.List[Media]:
        ...
        for img in self.find_all("img"):
            try:
                caption = img.parent.parent.find("figcaption").text
                # TODO: handle galleries
            except AttributeError:
                caption = ""
            images.append(Media(html_tag=img, is_image=True, caption=caption))
        return images
    ...
    def get_audio(self) -> typing.List[Media]:
        ...
        """
        audio = []
        for aud in self.find_all("audio"):
            if aud.parent.parent.find("figcaption"):
                caption = aud.parent.parent.find("figcaption").text
            else:
                caption = ""
            audio.append(Media(html_tag=aud, is_image=False, caption=caption))
        return audio
    ...
    def get_video(self) -> typing.List[Media]:
        ...
        """
        videos = []
        for vid in self.find_all("video"):
            if vid.parent.parent.find("figcaption"):
                caption = vid.parent.parent.find("figcaption").text
            else:
                caption = ""
            videos.append(Media(html_tag=vid, is_image=False, caption=caption))
        return videos
