...
from micpy.matplotlib import _register_colormaps
...
class File:
    ...
    def index(
        ...
        Args:
            ...
            resume (bool, optional): Resume building the index. Defaults to False.
        ...
        restart = not self._index or chunk_size != self._chunk_size or force
        ...
        if restart:
            ...
            self._index.extend(
                Index.from_file(**index_params, position=self._index[-1])
            )

        return self._index

    def refresh(self, force: bool = False, show_progress: bool = True):
        """Refresh the index.
        ...
        Args:
            ...
            show_progress (bool, optional): Show progress bar. Defaults to True.
        """
        old_fields = len(self._index)

        self.index(force=force, resume=True, show_progress=show_progress)

        new_fields = len(self._index) - old_fields

        if new_fields:
            print(f"Index refreshed: {new_fields} field(s) added")
            return True
