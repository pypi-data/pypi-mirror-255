...
async def get_user(
    m: typing.Union[Pyrofork.types.Message, Pyrofork.types.CallbackQuery]
) -> Pyrofork.types.User or bool:
    """
    ### `get_user`
    - Gets a User from Message/RepliedMessage/CallbackQuery
    - Parameters:
      - m (`~Pyrofork.types.Message` || `~Pyrofork.types.CallbackQuery`)
    - Returns:
      - `Pyrofork.types.User` on Success
      - `False` on Error
    ...
    """
    if isinstance(m, Pyrofork.types.Message):
        message = m
        client = m._client
    if isinstance(m, Pyrofork.types.CallbackQuery):
        message = m.message
    ...
    if command and (command.startswith("@") or command.isdigit()):
        with contextlib.suppress(
            Pyrofork.errors.exceptions.bad_request_400.UsernameNotOccupied,
            Pyrofork.errors.exceptions.bad_request_400.UsernameInvalid,
            Pyrofork.errors.exceptions.bad_request_400.PeerIdInvalid,
            IndexError,
        ):
