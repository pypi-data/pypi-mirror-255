...
class CurveNotFoundException(Exception):
    pass
...
class Session(object):
    """Establish a connection to AmpData API

    Creates an object that holds the state which is needed when talking to the
    Ampiato data center. To establish a session, you have to provide
    ...
    client_secret:
        Your client secret.
    auth_urlbase: url
        Location of AmpData authentication service
    timeout: float
        Timeout for REST calls, in seconds

    Returns
    -------
    session: :class:`ampdata.session.Session` object
    ...
    def read_config_file(self, config_file):
        ...
        if hasattr(config_file, "read"):
            ...
            if not files_read:
                raise ConfigException(
                    "Configuration file with name {} " "was not found.".format(
                        config_file
                    )
                )
        ...
        if auth_type == "OAuth":
            ...
            token = config.get(auth_type, "token")
            self.auth = auth.Token(self, token)
        else:
            print("No auth provided. All requests will fail.")
    ...
    def get_curve(self, id=None, name=None):
        ...
        curve object
            ...
            :class:`~ampdata.curves.TimeSeriesCurve`,
            :class:`~ampdata.curves.TaggedCurve`,
            :class:`~ampdata.curves.InstanceCurve`,
            :class:`~ampdata.curves.TaggedInstanceCurve`.
        """
        if id is not None:
            warnings.warn(
                "Looking up a curve by ID will be removed in the future.",
                FutureWarning,
                stacklevel=2,
            )
    ...
    def search(
        ...
        :class:`~ampdata.curves.TimeSeriesCurve`,
        :class:`~ampdata.curves.TaggedCurve`,
        :class:`~ampdata.curves.InstanceCurve` or a
        :class:`~ampdata.curves.TaggedInstanceCurve` object.
        ...
        if id is not None:
            warnings.warn(
                "Searching for curves by ID will be removed in the future.",
                FutureWarning,
                stacklevel=2,
            )
        args = []
    ...
    def events(self, curve_list, start_time=None, timeout=None):
        ...
        return events.EventListener(
            self, curve_list, start_time=start_time, timeout=timeout
        )
    ...
    def get_attribute(self, attribute):
        ...
        raise MetadataException(
            "Failed loading {}: {}".format(attribute, response.content.decode())
        )
    ...
    def _build_curve(self, metadata):
        for key in self._meta_keys:
            if key not in metadata:
                raise MetadataException(
                    "Mandatory key {} not found in metadata".format(key)
                )
    ...
    def send_data_request(
        ...
        if (
            timeout is not None
            or (500 <= res.status_code < 600)
            or res.status_code == 408
        ) and retries > 0:
            if RETRY_DELAY > 0:
                time.sleep(RETRY_DELAY)
            return self.send_data_request(
                req_type,
                urlbase,
                url,
                data,
    ...
    def data_request(
        ...
        res = self.send_data_request(
            req_type, urlbase, url, data, rawdata, headers, authval, stream, retries
        )
        return res

    def handle_single_curve_response(self, response):
        if response.status_code == 404:
            raise CurveNotFoundException("Curve was not found.")
        if not response.ok:
            raise MetadataException(
                "Failed to load curve: {}".format(response.content.decode())
            )
