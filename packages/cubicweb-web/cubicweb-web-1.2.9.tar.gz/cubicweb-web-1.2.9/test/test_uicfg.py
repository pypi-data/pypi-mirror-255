...
class DefinitionOrderCWTC(WebCWTC):
    ...
    def setUp(self):
        ...
        new_def = (
            (("*", "login", "*"), {"formtype": "main", "section": "hidden"}),
            (("*", "login", "*"), {"formtype": "muledit", "section": "hidden"}),
            (("CWUser", "login", "*"), {"formtype": "main", "section": "attributes"}),
            (
                ("CWUser", "login", "*"),
                {"formtype": "muledit", "section": "attributes"},
            ),
            (("CWUser", "login", "String"), {"formtype": "main", "section": "inlined"}),
            (
                ("CWUser", "login", "String"),
    ...
    def test_definition_order_hidden(self):
        ...
        expected = {"main_inlined", "muledit_attributes", "inlined_attributes"}
    ...
    def test_uicfg_primaryview_set_fields_order(self):
        ...
        self.assertEqual(
            pvdc.get("CWUser", "firstname", "String", "subject"), {"order": 1}
        )
        self.assertEqual(
            pvdc.get("CWUser", "surname", "String", "subject"), {"order": 2}
        )
...
class AutoformSectionTC(BaseTestCase):
    def test_derivation(self):
        ...
        self.assertEqual(
            afs2.etype_get("Tag", "tags", "subject", "Personne"),
            {"main_relations", "muledit_hidden", "inlined_relations"},
        )
        self.assertEqual(
            afs2.etype_get("Tag", "tags", "subject", "Societe"),
            {"main_attributes", "muledit_hidden", "inlined_attributes"},
        )
        self.assertEqual(
            afs2.etype_get("Personne", "travaille", "subject", "Societe"),
            {"main_attributes", "muledit_hidden", "inlined_attributes"},
        )
        self.assertEqual(
            afs2.etype_get("Societe", "travaille", "object", "Personne"),
            {"main_attributes", "muledit_hidden", "inlined_attributes"},
        )
