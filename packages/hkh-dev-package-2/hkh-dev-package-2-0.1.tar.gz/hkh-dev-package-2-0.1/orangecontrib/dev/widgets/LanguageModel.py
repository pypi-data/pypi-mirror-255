...
# This widget calls a LLM on chunks to extract the main ideas, in form of a list.
...
from Orange.widgets.data.owpythonscript5 import OWPythonScript
...
def get_widgets():
    ...
    for obj in gc.get_objects():
        if isinstance(obj, OWFile) and obj.captionTitle == "Point to pointer.pkl":
            widgets["File"] = obj
        if isinstance(obj, OWPythonScript) and obj.captionTitle == "Main ideas extraction":
            widgets["Python Script"] = obj
    return widgets

def get_ideas(chunk, model):
    preprompt = "USER: Tu es un écrivain hors pair, auteur du texte suivant :\\n\\n\\n'"
    postprompt = "'\\n\\n\\nQuelles sont les informations essentielles à retenir ? ASSISTANT: Voici les informations essentielles :\\n"
    output = model(preprompt+chunk+postprompt,
                         max_tokens=4096,
                         stop=[],
                         echo=False)
    ideas_chunk = output["choices"][0]["text"]
    return ideas_chunk
...
class Thread(QThread):
    ...
    def run(self):
        print("Waiting for pre-processing thread so stop")
        time.sleep(10)
        ...
        ideas_chunks = []
        i = 0
        for chunk in tqdm(self.parameters["chunks"]):
            ideas_chunk = get_ideas(chunk[0], model)
            ideas_chunks.append([ideas_chunk])
            i += 1
            value = int(i/len(self.parameters["chunks"])*100)
            update_pbar(signal=self.signal_pbar,
                        value=value,
                        widgets=widgets)

        ideas_chunks_var = StringVariable(name="Main ideas")
        ideas_chunks_domain = Domain([], metas=[ideas_chunks_var])
        out_data = Corpus.from_list(domain=ideas_chunks_domain,
                                    rows=ideas_chunks)
        titles = ["Main ideas : chunk {}".format(i+1) for i in range(len(ideas_chunks))]
        out_data._titles = np.array(titles)
        self.signal_output.emit(out_data, widgets["Python Script"])

def send_table(data, widgetPythonScript):
    getattr(widgetPythonScript.Outputs, "data").send(data)
