...
class Compare:
    ...
    def __init__ (self, tested, expected, key=None, verbose=False, low_memory=False, strict_schema=False, case_sensitive_column_names=False, numeric_tolerance=4, test_name=None, tested_frame_name=None, expected_frame_name=None):
        self.metadata = {"Test Name": "Test Metadata", "Header": test_name, "Tested Frame": tested_frame_name, "Expected Frame": expected_frame_name, "Executed On": datetime.now().strftime("%Y-%m-%d %H:%M"), "Compared on": key, "Numeric Tolerance": f"{numeric_tolerance} decimal places"}              
