...
from sqlalchemy import ColumnClause, ColumnElement, TextClause, literal_column
from sqlalchemy import types as sqltypes
from sqlalchemy.dialects.postgresql import DOUBLE_PRECISION
from sqlalchemy.sql import operators
from sqlalchemy.sql.expression import text
from sqlalchemy.sql.functions import _FunctionGenerator  # noqa: PLC2701
from sqlalchemy.sql.operators import custom_op
from sqlalchemy.types import UserDefinedType

INTERSECTS: custom_op = operators.custom_op("&&")
INTERSECTS_ND: custom_op = operators.custom_op("&&&")
OVERLAPS_OR_TO_LEFT: custom_op = operators.custom_op("&<")
OVERLAPS_OR_TO_RIGHT: custom_op = operators.custom_op("&>")
OVERLAPS_OR_BELOW: custom_op = operators.custom_op("&<|")
TO_LEFT: custom_op = operators.custom_op("<<")
BELOW: custom_op = operators.custom_op("<<|")
TO_RIGHT: custom_op = operators.custom_op(">>")
CONTAINED: custom_op = operators.custom_op("@")
OVERLAPS_OR_ABOVE: custom_op = operators.custom_op("|&>")
ABOVE: custom_op = operators.custom_op("|>>")
CONTAINS: custom_op = operators.custom_op("~")
SAME: custom_op = operators.custom_op("~=")
DISTANCE_CENTROID: custom_op = operators.custom_op("<->")
DISTANCE_BOX: custom_op = operators.custom_op("<#>")


class BaseComparator(UserDefinedType.Comparator):
    """A custom comparator base class.

    It adds the ability to call spatial
    ...
    This comparator is used by the :class:`geoalchemy2_mssql.types.Raster`.
    """
    ...
    def intersects(self, other: object) -> ColumnElement:
        """The ``&&`` operator. A's BBOX intersects B's."""
        return self.operate(INTERSECTS, other, result_type=sqltypes.Boolean)

    def overlaps_or_to_left(self, other: object) -> ColumnElement:
    ...
    def overlaps_or_to_right(self, other: object) -> ColumnElement:
...
class Comparator(BaseComparator):
    ...
    def contained(self, other: object) -> ColumnElement:
        """The ``@`` operator. A's BBOX is contained by B's."""
        return self.operate(CONTAINED, other, result_type=sqltypes.Boolean)

    def overlaps_or_above(self, other: object) -> ColumnElement:
        """The ``|&>`` operator. A's BBOX overlaps or is above B's."""
        return self.operate(OVERLAPS_OR_ABOVE, other, result_type=sqltypes.Boolean)

    def above(self, other: object) -> ColumnElement:
        """The ``|>>`` operator. A's BBOX is strictly above B's."""
        return self.operate(ABOVE, other, result_type=sqltypes.Boolean)

    def contains(self, other: object, **_: dict) -> ColumnElement:
        """The ``~`` operator. A's BBOX contains B's."""
        result: ColumnClause = literal_column(
            f"{self.expr}.STContains(geography::STGeomFromText('{other}', 4326)) = 1",
        )

        return result

    def distance(self, other: object, **_: dict) -> str:
        """The ``~`` operator. A's BBOX contains B's."""
        return f"{self.expr}.STDistance(geography::STGeomFromText('{other}', 4326))"

    def as_text(self) -> TextClause:
        """AsText."""
        return text(f"{self.expr}.STAsText()")

    def same(self, other: object) -> ColumnElement:
        """The ``~=`` operator. A's BBOX is the same as B's."""
        return self.operate(SAME, other, result_type=sqltypes.Boolean)

    def distance_centroid(self, other: object) -> ColumnElement:
        """The ``<->`` operator. The distance between two points."""
        return self.operate(DISTANCE_CENTROID, other, result_type=DOUBLE_PRECISION)

    def distance_box(self, other: object) -> ColumnElement:
        """The ``<#>`` operator.
    ...
    def intersects_nd(self, other: object) -> ColumnElement:
        """The ``&&&`` operator.

        Returns TRUE if the n-D bounding box of geometry A
