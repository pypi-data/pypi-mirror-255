...
from alembic.autogenerate.render import _add_column, _drop_column, renderers  # noqa: PLC2701
...
from . import Geography, Geometry, Raster, _check_spatial_type


def render_item(obj_type: Any, obj: Any, autogen_context: Any) -> str | bool:
    """Apply custom rendering for selected items."""
    if obj_type == "type" and isinstance(obj, Geometry | Geography | Raster):
        import_name = obj.__class__.__name__
        autogen_context.imports.add(f"from geoalchemy2_mssql import {import_name}")
        return "%r" % obj
...
def include_object(obj: Any, name: str, obj_type: str, *_: Any, **__: Any) -> bool:
    """Do not include spatial indexes if they are automatically created by GeoAlchemy2."""
    if obj_type == "index" and len(obj.expressions) == 1:
        try:
...
class AddGeospatialColumn(ops.AddColumnOp):
    ...
    This methodology originates from:
    https://alembic.sqlalchemy.org/en/latest/api/operations.html#operation-plugins
    """

    @classmethod
    def add_geospatial_column(
        cls, operations: Any, table_name: str, column: Any, schema: Any = None
    ) -> AddColumnOp:
        """Handle the different situations arising from adding geospatial column to a DB."""
        operation = cls(table_name, column, schema=schema)
        invoke: AddColumnOp = operations.invoke(operation)
        return invoke
...
class DropGeospatialColumn(ops.DropColumnOp):
    ...
    def drop_geospatial_column(
        cls, operations: Any, table_name: str, column_name: str, schema: Any = None, **_: dict
    ) -> DropColumnOp:
        """Handle the different situations arising from dropping geospatial column from a DB."""
        operation = cls(table_name, column_name, schema=schema)
        drop_op: DropColumnOp = operations.invoke(operation)
        return drop_op
...
def add_geospatial_column(operations: Any, operation: Any) -> None:
    ...
    if isinstance(operation.column, TypeDecorator):
        # Will be either geoalchemy2_mssql.types.Geography or geoalchemy2_mssql.types.Geometry,
...
def drop_geospatial_column(operations: Any, operation: Any) -> None:
    ...
    if "sqlite" in dialect.name:
        ...
        # the column itself. The next call actually removes the geospatial column,
        # IF the underlying SQLite package version >= 3.35
...
def render_add_geo_column(autogen_context: Any, operation: Any) -> str:
    """Render the add_geospatial_column operation in migration script."""
    col_render = _add_column(autogen_context, operation)
    replace: str = col_render.replace(".add_column(", ".add_geospatial_column(")
    return replace


@renderers.dispatch_for(DropGeospatialColumn)
def render_drop_geo_column(autogen_context: Any, operation: Any) -> str:
    """Render the drop_geospatial_column operation in migration script."""
    col_render = _drop_column(autogen_context, operation)
    replace: str = col_render.replace(".drop_column(", ".drop_geospatial_column(")
    return replace
