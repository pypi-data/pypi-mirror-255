"""Defines all functions for geo types."""

from .geo_types import Geography, Geometry, Raster
...
_FUNCTIONS = [
    ...
    (
        ...
        None,
        """Ensures geometry columns are defined type modifiers or spatial constraints.""",
    ),
    (
        "UpdateGeometrySRID",
        None,
    ...
    ("ST_MakePoint", Geometry, """Creates a 2D, 3DZ or 4D Point."""),
    ("ST_MakePointM", Geometry, """Creates a Point from X, Y and M values."""),
    (
        "ST_MakePolygon",
        Geometry,
    ...
    (
        ...
        OR
        [raster] Returns a multipolygon geometry formed by the union of pixels that have a pixel
        value that is not no data value. If no band number is specified, band num defaults to 1""",
    ),
    (
        "ST_TileEnvelope",
        Geometry,
    ...
    ("ST_Boundary", Geometry, """Returns the boundary of a geometry."""),
    ...
    (
        "ST_IsPolygonCCW",
        None,
        """Tests if Polygons have exterior rings oriented CCW and interior rings oriented CW.""",
    ),
    (
        "ST_IsPolygonCW",
        None,
        """Tests if Polygons have exterior rings oriented CW and interior rings oriented CCW.""",
    ),
    (
        "ST_IsClosed",
        None,
        """Tests if a LineStrings's start and end points are coincident.
        For a PolyhedralSurface tests if it is closed (volumetric).""",
    ),
    ...
    (
        ...
        OR
        [raster] Returns true if the raster is empty (width = 0 and height = 0).
    ...
    (
        "ST_NumInteriorRing",
        None,
        """Returns the number of (holes) of a Polygon. Aias for ST_NumInteriorRings""",
    ),
    (
        "ST_NumPatches",
        None,
        """Return the number of faces on a Polyhedral Surface.
        Will return null for non-polyhedral geometries.""",
    ),
    (
        "ST_NumPoints",
        None,
    ...
    ("ST_StartPoint", Geometry, """Returns the first point of a LineString."""),
    (
        "ST_Summary",
        None,
    ...
    ("ST_AddPoint", Geometry, """Add a point to a LineString."""),
    (
        "ST_CollectionExtract",
        Geometry,
        """Given a (multi)geometry, return a (multi)geometry consisting only of elements of the
         specified type.""",
    ),
    (
    ...
    ("ST_Force2D", Geometry, """Force the geometries into a \"2-dimensional mode\"."""),
    (
        "ST_Force3D",
        Geometry,
        (
    ...
    (
        "ST_ForcePolygonCCW",
        Geometry,
        """Orients all exterior rings CCW and all interior rings CW.""",
    ),
    (
        "ST_ForceCollection",
        Geometry,
    ...
    (
        "ST_ForcePolygonCW",
        Geometry,
        """Orients all exterior rings CW and all interior rings CCW.""",
    ),
    (
        "ST_ForceSFS",
        Geometry,
    ...
    ("ST_Multi", Geometry, """Return the geometry as a MULTI* geometry."""),
    ("ST_Normalize", Geometry, """Return the geometry in its canonical form."""),
    (
        "ST_QuantizeCoordinates",
        Geometry,
        """Sets least significant bits of coordinates to zero""",
    ),
    ("ST_RemovePoint", Geometry, """Remove point from a linestring."""),
    ("ST_Reverse", Geometry, """Return the geometry with vertex order reversed."""),
    (
        "ST_Segmentize",
        Geometry,
        """Return a modified geometry/geography with no segment longer than the given distance.""",
    ),
    ("ST_SetPoint", Geometry, """Replace point of a linestring with a given point."""),
    (
        "ST_SnapToGrid",
        Geometry,
        """[geometry] Snap all points of the input geometry to a regular grid.
        OR
        [raster] Resample a raster by snapping it to a grid. New pixel values are computed
         using the NearestNeighbor (english or american spelling), Bilinear, Cubic, CubicSpline
    ...
    (
        "ST_IsValidDetail",
        None,
        """Returns a valid_detail row stating if a geometry is valid, and if not a reason
         why and a location.""",
    ),
    (
        "ST_IsValidReason",
    ...
    (
        ...
        nOR
        [raster] Sets the SRID of a raster to a particular integer srid defined in the
         spatial_ref_sys table.""",
    ),
    (
        "ST_SRID",
        None,
        """[geometry] Returns the spatial reference identifier for the ST_Geometry as defined
         in spatial_ref_sys table.
        OR
        [raster] Returns the spatial reference identifier of the raster as defined in spatial_ref
        _sys table.""",
    ),
    (
        "ST_Transform",
        Geometry,
        """[geometry] Return a new geometry with its coordinates transformed to a different
         spatial reference system.
        OR
        [raster] Reprojects a raster in a known spatial reference system to another known spatial
         reference system using specified resampling algorithm. Options are NearestNeighbor,
    ...
    (
        "ST_BdMPolyFromText",
        Geometry,
        """Construct a MultiPolygon given an arbitrary collection of closed linestrings as a
         MultiLineString text representation Well-Known text representation.""",
    ),
    (
        "ST_GeogFromText",
        Geography,
        """Return a specified geography value from Well-Known Text representation or extended
         (WKT).""",
    ),
    (
    ...
    (
        "ST_LineFromText",
        Geometry,
        """Makes a Geometry from WKT representation with the given SRID.
         If SRID is not given, it defaults to 0.""",
    ),
    (
        "ST_MLineFromText",
        Geometry,
    ...
    (
        "ST_MPointFromText",
        Geometry,
        """Makes a Geometry from WKT with the given SRID. If SRID is not given,
         it defaults to 0.""",
    ),
    (
        "ST_MPolyFromText",
        Geometry,
        """Makes a MultiPolygon Geometry from WKT with the given SRID. If SRID is not given,
         it defaults to 0.""",
    ),
    (
        "ST_PointFromText",
    ...
    (
        "ST_GeogFromWKB",
        Geography,
        """Creates a geography instance from a Well-Known Binary geometry representation
         (WKB) or extended Well Known Binary (EWKB).""",
    ),
    (
        "ST_GeomFromEWKB",
        Geometry,
        """Return a specified ST_Geometry value from Extended Well-Known Binary representation
         (EWKB).""",
    ),
    (
        "ST_GeomFromWKB",
        Geometry,
        """Creates a geometry instance from a Well-Known Binary geometry representation (WKB)
         and optional SRID.""",
    ),
    ("ST_LineFromWKB", Geometry, """Makes a LINESTRING from WKB with the given SRID"""),
    (
        "ST_LinestringFromWKB",
        Geometry,
        """Makes a geometry from WKB with the given SRID.""",
    ),
    ("ST_PointFromWKB", Geometry, """Makes a geometry from WKB with the given SRID"""),
    (
        "ST_WKBToSQL",
        Geometry,
    ...
    ("ST_GeomFromGeoHash", Geometry, """Return a geometry from a GeoHash string."""),
    (
        "ST_GeomFromGML",
        Geometry,
    ...
    (
        "ST_GeomFromGeoJSON",
        Geometry,
        """Takes as input a geojson representation of a geometry and outputs a PostGIS geometry
         object""",
    ),
    (
        "ST_GeomFromKML",
    ...
    (
        "ST_GMLToSQL",
        Geometry,
        """Return a specified ST_Geometry value from GML representation. This is an alias name
         for ST_GeomFromGML""",
    ),
    (
    ...
    ("ST_PointFromGeoHash", Geometry, """Return a point from a GeoHash string."""),
    (
        "ST_AsEWKT",
        None,
    ...
    (
        "ST_AsText",
        None,
        """Return the Well-Known Text (WKT) representation without SRID metadata.""",
    ),
    (
        "ST_AsBinary",
        None,
        """[gometry] Return the Well-Known Binary (WKB) representation of the geometry/geography
    ...
    (
        "ST_AsEWKB",
        None,
        """Return the WKB representation of the geometry with SRID meta data.""",
    ),
    (
        "ST_AsHEXEWKB",
        None,
    ...
    (
        "ST_AsX3D",
        None,
        """Returns a Geometry in: ISO-IEC-19776-1.2-X3DEncodings-XML""",
    ),
    ("ST_GeoHash", None, """Return a GeoHash representation of the geometry."""),
    (
        "ST_3DIntersects",
        None,
        """Returns TRUE if the Geometries \"spatially intersect\" in 3D - only for points,
         linestrings, polygons, polyhedral surface (area).""",
    ),
    (
        "STContains",
        None,
        """[geometry] Returns true if and only if no points of B lie in the exterior of A,
         and at least one point of the interior of B lies in the interior of A.
         OR
         [raster] Return true if no points of raster rastB lie in the exterior of raster rastA
          and at least one point of the interior of rastB lies in the interior of rastA.""",
    ),
    (
        "ST_ContainsProperly",
        None,
        """[geometry] Returns true if B intersects the interior of A but not the boundary
         (or exterior). A does not contain properly itself, but does contain itself.
         OR
         [raster] Return true if rastB intersects the interior of rastA but not the boundary
    ...
    (
        "ST_CoveredBy",
        None,
        """[geometry] Returns 1 (TRUE) if no point in Geometry/Geography A is outside
         Geometry/Geography B\nOR\n[raster] Return true if no points of raster rastA lie outside
          raster rastB.""",
    ),
    (
        "ST_Crosses",
        None,
        """Returns TRUE if the supplied geometries have some, but not all, interior points in
         common.""",
    ),
    (
        "ST_LineCrossingDirection",
        None,
        """Given 2 linestrings, returns a number between -3 and 3 denoting what kind of crossing
         behavior. 0 is no crossing.""",
    ),
    (
        "ST_Disjoint",
        None,
        """[geometry] Returns TRUE if the Geometries do not \"spatially intersect\" - if they
         do not share any space together.\nOR\n[raster] Return true if raster rastA does not
          spatially intersect rastB.""",
    ),
    (
        "ST_Equals",
        None,
    ...
    (
        "ST_Intersects",
        None,
        """[geometry] Returns TRUE if the Geometries/Geography \"spatially intersect in 2D\"
         - (share any portion of space) and FALSE if they don't (they are Disjoint).
          For geography tolerance is 0.00001 meters (so any points that close are considered
           to intersect)\nOR\n[raster] Return true if raster rastA spatially intersects
            raster rastB.""",
    ),
    (
        "ST_OrderingEquals",
        None,
        """Returns true if the given geometries represent the same geometry and points are in
         the same directional order.""",
    ),
    (
        "ST_Overlaps",
        None,
        """[geometry] Returns TRUE if the Geometries share space, are of the same dimension,
         but are not completely contained by each other.\nOR\n[raster] Return true if raster
          rastA and rastB intersect but one does not completely contain the other.""",
    ),
    (
    ...
    (
        "ST_Relate",
        None,
        """Returns true if this Geometry is spatially related to anotherGeometry, by testing
         for intersections between the Interior, Boundary and Exterior of the two geometries
          as specified by the values in the intersectionMatrixPattern. If no
    ...
    (
        "ST_Touches",
        None,
        """[geometry] Returns TRUE if the geometries have at least one point in common, but
         their interiors do not intersect.\nOR\n[raster] Return true if raster rastA and rastB
    ...
    (
        ...
        OR
        [raster] Return true if no points of raster rastA lie in the exterior of raster rastB
         and at least one point of the interior of rastA lies in the interior of rastB.""",
    ),
    (
        "ST_3DDWithin",
        None,
        """For 3d (z) geometry type Returns true if two geometries 3d distance is within number
         of units.""",
    ),
    (
        "ST_3DDFullyWithin",
        None,
        """Returns true if all of the 3D geometries are within the specified distance of one
         another.""",
    ),
    (
        "ST_DFullyWithin",
        None,
        """[geometry] Returns true if all of the geometries are within the specified distance
         of one another
         OR
         [raster] Return true if rasters rastA and rastB are fully within the specified distance
          of each other.""",
    ),
    (
        "ST_DWithin",
        None,
        """[geometry] Returns true if the geometries are within the specified distance of one
         another. For geometry units are in those of spatial reference and for geography units
          are in meters and measurement is defaulted to use_spheroid=true),
           for faster check, use_spheroid=false to measure along sphere.\nOR\n[raster] Return true
            if rasters rastA and rastB are within the specified distance of each other.""",
    ),
    ("ST_Area", None, """Returns the area of a polygonal geometry."""),
    (
        "ST_Azimuth",
        None,
        """Returns the north-based azimuth as the angle in radians measured CW from the vertical
         on pointA to pointB.""",
    ),
    (
        "ST_Angle",
    ...
    (
        "ST_ClosestPoint",
        Geometry,
        """Returns the 2D point on g1 that is closest to g2. This is the first point of the
         shortest line.""",
    ),
    (
        "ST_3DClosestPoint",
        Geometry,
        """Returns the 3D point on g1 that is closest to g2. This is the first point of the 3D
         shortest line.""",
    ),
    (
        "ST_Distance",
    ...
    (
        "ST_3DDistance",
        None,
        """Returns the 3D cartesian minimum distance (based on spatial ref) between two geometries
         in projected units.""",
    ),
    (
        "ST_DistanceSphere",
        None,
        """Returns minimum distance in meters between two lon/lat geometries using a spherical
         earth model.""",
    ),
    (
        "ST_DistanceSpheroid",
        None,
        """Returns the minimum distance between two lon/lat geometries using a spheroidal earth
         model.""",
    ),
    (
        "ST_FrechetDistance",
    ...
    (
        "ST_3DMaxDistance",
        None,
        """Returns the 3D cartesian maximum distance (based on spatial ref) between two geometries
         in projected units.""",
    ),
    (
        "ST_MinimumClearance",
    ...
    (
        "ST_Buffer",
        Geometry,
        """(T) Returns a geometry covering all points within a given distance from the input
         geometry.""",
    ),
    (
        "ST_BuildArea",
    ...
    (
        "ST_ConcaveHull",
        Geometry,
        """The concave hull of a geometry represents a possibly concave geometry that encloses
         all geometries within the set. You can think of it as shrink wrapping.""",
    ),
    (
        "ST_ConvexHull",
        Geometry,
        """[geometry] Computes the convex hull of a geometry.\nOR\n[raster] Return the
        convex hull geometry of the raster including pixel values equal to BandNoDataValue.
        For regular shaped and non-skewed rasters, this gives the same result as ST_Envelope
    ...
    (
        "ST_Difference",
        Geometry,
        """Returns a geometry that represents that part of geometry A that does not intersect
         with geometry B.""",
    ),
    (
        "ST_FlipCoordinates",
        Geometry,
        """Returns a version of the given geometry with X and Y axis flipped. Useful for people
    ...
    (
        ...
        OR
        [raster] Returns a raster or a set of geometry-pixelvalue pairs representing the shared
        portion of two rasters or the geometrical intersection of a vectorization of the
    ...
    ("ST_Node", Geometry, """Node a set of linestrings."""),
    (
        "ST_OffsetCurve",
        Geometry,
    ...
    ("ST_WrapX", Geometry, """Wrap a geometry around an X value."""),
    (
        "ST_Simplify",
        Geometry,
        """Returns a simplified version of the given geometry using the Douglas-Peucker
        algorithm.""",
    ),
    (
    ...
    ("ST_FilterByM", Geometry, """Filters vertex points based on their m-value"""),
    (
        "ST_SetEffectiveArea",
        Geometry,
    ...
    (
        ...
        OR
        [raster] Returns the union of a set of raster tiles into a single raster
        composed of 1 or more bands.""",
    ),
    (
        "ST_UnaryUnion",
        Geometry,
    ...
    (
        "ST_VoronoiPolygons",
        Geometry,
        """Returns cells of the Voronoi diagram constructed from the vertices of a geometry.""",
    ),
    ("ST_Affine", Geometry, """Apply a 3D affine transformation to a geometry."""),
    ("ST_Rotate", Geometry, """Rotates a geometry about an origin point."""),
    ("ST_RotateX", Geometry, """Rotates a geometry about the X axis."""),
    ("ST_RotateY", Geometry, """Rotates a geometry about the Y axis."""),
    ("ST_RotateZ", Geometry, """Rotates a geometry about the Z axis."""),
    ("ST_Scale", Geometry, """Scales a geometry by given factors."""),
    ("ST_Translate", Geometry, """Translates a geometry by given offsets."""),
    (
        "ST_TransScale",
        Geometry,
    ...
    (
        ...
        Geometry,
        """Returns a point interpolated along a line. Second argument is a float8 between
        0 and 1 representing fraction of total length of linestring the point has to be
        located.""",
    ),
    (
        ...
        between 0 and 1 representing fraction of total length of linestring the point has
        to be located.""",
    ),
    (
    ...
    (
        ...
        the given fractions of total 2d length. Second and third arguments are float8
        values between 0 and 1.""",
    ),
    (
        "ST_LocateAlong",
        Geometry,
        """Return a derived geometry collection value with elements that match the specified
    ...
    (
        ...
        Geometry,
        """Return a derived geometry (collection) value with elements that intersect the
        specified range of elevations inclusively.""",
    ),
    (
        "ST_InterpolatePoint",
        None,
        """Return the value of the measure dimension of a geometry at the point closed to
        the provided point.""",
    ),
    (
        "ST_AddMeasure",
        Geometry,
        """Return a derived geometry with measure elements linearly interpolated between
    ...
    (
        "ST_CPAWithin",
        None,
        """Returns true if the closest point of approach of two trajectories is within the
        specified distance.""",
    ),
    ("postgis_sfcgal_version", None, """Returns the version of SFCGAL in use"""),
    ("ST_Extrude", Geometry, """Extrude a surface to a related volume"""),
    (
        "ST_StraightSkeleton",
        Geometry,
    ...
    ("ST_ForceLHR", Geometry, """Force LHR orientation"""),
    ("ST_MinkowskiSum", Geometry, """Performs Minkowski sum"""),
    (
        "ST_ConstrainedDelaunayTriangles",
        Geometry,
        """Return a constrained Delaunay triangulation around the given input geometry.""",
    ),
    ("ST_3DIntersection", Geometry, """Perform 3D intersection"""),
    ("ST_3DDifference", Geometry, """Perform 3D difference"""),
    ("ST_3DUnion", Geometry, """Perform 3D union"""),
    (
        "ST_3DArea",
        None,
    ...
    (
        "ST_Tesselate",
        Geometry,
        """Perform surface Tessellation of a polygon or polyhedralsurface and returns as a
        TIN or collection of TINS""",
    ),
    (
        "ST_Volume",
        None,
        """Computes the volume of a 3D solid. If applied to surface (even closed) geometries
        will return 0.""",
    ),
    (
        "ST_MakeSolid",
        Geometry,
        """Cast the geometry into a solid. No check is performed. To obtain a valid solid,
        the input geometry must be a closed Polyhedral Surface or a closed TIN.""",
    ),
    (
        "ST_IsSolid",
        None,
    ...
    (
        ...
        None,
        """Packages and upgrades postgis extensions (e.g. postgis_raster, postgis_topology,
         postgis_sfcgal) to latest available version.""",
    ),
    (
    ...
    ("PostGIS_AddBBox", Geometry, """Add bounding box to the geometry."""),
    (
        "PostGIS_DropBBox",
        Geometry,
    ...
    (
        ...
        Raster,
        (
            """Returns an empty raster (having no bands) of given dimensions (width & height),
            upperleft X and Y, pixel size and rotation (scalex, scaley, skewx & skewy) and
            reference system (srid). If a raster is passed in, returns a new raster with the
            same size, alignment and SRID. If srid is left out, the spatial ref is set to
            unknown (0).""",
    ...
    (
        ...
        (
            ...
            (longitude and latitude) given a column and row. Column and row starts at 1.""",
    ...
    (
        ...
        None,
        (
            """Returns the geometric Y coordinate upper left corner of a raster, column and row.
            Numbering of columns and rows starts at 1.""",
    ...
    (
        ...
        (
            ...
            (longitude and latitude) or a point geometry expressed in the spatial reference
    ...
    (
        ...
        None,
        (
            """Returns the column in the raster of the point geometry (pt) or a
            X and Y world coordinate (xw, yw) represented in world spatial reference
            system of raster.""",
    ...
    (
        ...
        (
            ...
            a X and Y world coordinate (xw, yw) represented in world spatial reference
            system of raster.""",
    ...
    (
        ...
        None,
        (
            """Returns basic meta data for a specific raster band.
            band num 1 is assumed if none-specified.""",
            "RT_ST_BandMetaData",
        ),
    ),
    (
    ...
    (
        ...
        None,
        (
            """Returns system file path to a band stored in file system.
            If no bandnum specified, 1 is assumed.""",
            "RT_ST_BandPath",
        ),
    ),
    (
    ...
    (
        ...
        None,
        (
            """Returns the file timestamp of a band stored in file system.
            If no bandnum specified, 1 is assumed.""",
    ...
    (
        ...
        (
            ...
            If no band number is specified, then band number 1 is assumed.""",
            "RT_ST_HasNoBand",
        ),
    ),
    (
        ...
        Geometry,
        (
            """Returns the polygon geometry that bounds the pixel for a particular row and
            column.""",
    ...
    (
        ...
        None,
        (
            """Returns the polygon geometry that bounds every pixel of a raster band along with
    ...
    (
        ...
        None,
        (
            """Returns a point geometry for each pixel of a raster band along with the value,
            the X and the Y raster coordinates of each pixel. The coordinates of the point
            geometry are of the pixel's upper-left corner.""",
            "RT_ST_PixelAsPoints",
        ),
    ),
    (
    ...
    (
        ...
        None,
        (
            """Returns the centroid (point geometry) for each pixel of a raster band along
    ...
    (
        ...
        (
            ...
            particular geometric point. Band numbers start at 1 and assumed to be 1 if not
             specified. If exclude_nodata_value is set to false, then all pixels include
             nodata pixels are considered to intersect and return value.
             If exclude_nodata_value is not passed in then reads it from metadata of raster.""",
            "RT_ST_Value",
        ),
    ),
    (
        ...
        None,
        (
            """Returns the nearest non-NODATA value of a given band's pixel specified by a
            columnx and rowy or a geometric point expressed in the same spatial reference
    ...
    (
        ...
        (
            ...
            band's pixel specified by either a columnX and rowY or a geometric point expressed
    ...
    (
        ...
        Raster,
        (
            """Returns modified raster resulting from setting the value of a given band in a
            given columnx, rowy pixel or the pixels that intersect a particular geometry.
            Band numbers start at 1 and assumed to be 1 if not specified.""",
            "RT_ST_SetValue",
        ),
    ),
    (
    ...
    (
        ...
        Raster,
        (
            """Sets the georeference X, Y skew (or rotation parameter). If only one is passed in,
             sets X and Y to the same value.""",
            "RT_ST_SetSkew",
        ),
    ),
    (
        ...
        Raster,
        (
            """Sets the value of the upper left corner of the pixel of the raster to projected
            X and Y coordinates.""",
            "RT_ST_SetUpperLeft",
        ),
    ),
    (
        ...
        (
            ...
            an arbitrary grid corner and a set of raster georeferencing attributes defined or
    ...
    (
        ...
        Raster,
        (
            """Resample a raster by adjusting only its scale (or pixel size).
            New pixel values are computed using the NearestNeighbor (english or american spelling),
    ...
    (
        ...
        (
            ...
            Band 1 is assumed if no band is specified. To mark a band as having no nodata value,
    ...
    (
        ...
        (
            """Returns the number of pixels in a given band of a raster or raster coverage.
             If no band is specified defaults to band 1. If exclude_nodata_value is set to true,
             will only count pixels that are not equal to the nodata value.""",
            "RT_ST_Count",
        ),
    ),
    (
    ...
    (
        ...
        None,
        (
            """Returns a set of record summarizing a raster or raster coverage data distribution
            separate bin ranges. Number of bins are autocomputed if not specified.""",
            "RT_ST_Histogram",
        ),
    ),
    (
        ...
        None,
        (
            """Compute quantiles for a raster or raster table coverage in the context of the
            sample or population. Thus, a value could be examined to be at the raster's
            25%, 50%, 75% percentile.""",
            "RT_ST_Quantile",
        ),
    ),
    ...
    (
        ...
        None,
        (
            """Returns a set of records containing a pixel band value and count of the number of
            pixels in a given band of a raster (or a raster coverage) that have a given set of
            values. If no band is specified defaults to band 1. By default nodata value pixels
            are not counted. and all other values in the pixel are output and pixel band values
    ...
    (
        ...
        (
            ...
            are one of those supported by your compiled library. Use ST_GDALDrivers()
    ...
    (
        ...
        None,
        (
            """Return the raster tile selected bands as a single Joint Photographic Exports Group
            (JPEG) image (byte array). If no band is specified and 1 or more than 3 bands,
            then only the first band is used. If only 3 bands then all 3 bands are used
            and mapped to RGB.""",
            "RT_ST_AsJPEG",
        ),
    ),
    (
        ...
        None,
        (
            """Return the raster tile selected bands as a single portable network graphics (PNG)
            image (byte array). If 1, 3, or 4 bands in raster and no bands are specified,
            then all bands are used. If more 2 or more than 4 bands and no bands specified,
    ...
    (
        ...
        None,
        (
            """Return the raster selected bands as a single TIFF image (byte array).
            If no band is specified or any of specified bands does not exist in the raster,
    ...
    (
        ...
        Raster,
        (
            """Returns the raster clipped by the input geometry. If band number not is specified,
             all bands are processed.
    ...
    (
        ...
        Raster,
        (
            """Creates a new raster of up to four 8BUI bands (grayscale, RGB, RGBA) from the
            source raster and a specified band. Band 1 is assumed if not specified.""",
            "RT_ST_ColorMap",
        ),
    ),
    (
        ...
        Raster,
        (
            """Creates a new one-8BUI band raster from the source raster and specified bands
    ...
    (
        ...
        None,
        (
            """[raster] Callback function version - Returns a one-band raster given one or more
            input rasters, band indexes and one user-specified callback function.
            OR
            [raster] Expression version - Returns a one-band raster given one or two input rasters,
            band indexes and one or more user-specified SQL expressions.""",
            "RT_ST_MapAlgebra",
        ),
    ),
    (
        ...
        Raster,
        (
            """[raster] 1 raster band version: Creates a new one band raster formed by applying a
             valid PostgreSQL algebraic operation on the input raster band and of pixeltype
             provided. Band 1 is assumed if no band is specified.
             OR
             [raster] 2 raster band version: Creates a new one band raster formed by applying a
             valid PostgreSQL algebraic operation on the two input raster bands and of pixeltype
             provided. band 1 of each raster is assumed if no band numbers are specified.
             The resulting raster will be aligned (scale, skew and pixel corners) on the grid
             defined by the first raster and have its extent defined by the \"extenttype\"
             parameter. Values for \"extenttype\" can be: INTERSECTION, UNION, FIRST, SECOND.""",
    ...
    (
        ...
        (
            ...
            PostgreSQL function on the input raster band and of pixeltype prodived.
            ...
            Band 1 is assumed if no band is specified.
            Extent type defaults to INTERSECTION if not specified.""",
            "RT_ST_MapAlgebraFct",
        ),
    ),
    (
        ...
        Raster,
        (
            """1-band version: Map Algebra Nearest Neighbor using user-defined PostgreSQL function.
            Return a raster which values are the result of a PLPGSQL user function involving a
    ...
    (
        ...
        (
            ...
            The nband is the band to be changed. If nband is not specified assumed to be 1.
            All other bands are returned unchanged.
            Use case: convert a 16BUI band to a 8BUI and so forth for simpler rendering as
            viewable formats.""",
            "RT_ST_Reclass",
        ),
    ),
    ...
    (
        ...
        None,
        (
            """Raster processing function that interpolates a pixel's value from the pixel's
            neighborhood.""",
    ...
    (
        ...
        None,
        (
            """Raster processing function that calculates the minimum pixel value
            in a neighborhood.""",
            "RT_ST_Min4ma",
        ),
    ),
    (
        ...
        None,
        (
            """Raster processing function that returns the minimum distance(in number of pixels)
    ...
    (
        ...
        None,
        (
            """Raster processing function that calculates the range of pixel
            values in a neighborhood.""",
            "RT_ST_Range4ma",
        ),
    ),
    (
        ...
        None,
        (
            """Raster processing function that calculates the standard deviation of pixel values in
            a neighborhood.""",
            "RT_ST_StdDev4ma",
        ),
    ),
    ...
    (
        ...
        Raster,
        (
            """Returns the hypothetical illumination of an elevation raster band using provided
            azimuth, altitude, brightness and scale inputs.""",
            "RT_ST_HillShade",
        ),
    ),
    (
    ...
    (
        ...
        None,
        (
            """Returns a set of geomval (geom,val) rows, from a given raster band.
    ...
    (
        ...
        (
            """Returns true if rasters have same skew, scale, spatial ref, and offset.
             pixels can be put on same grid without cutting into pixels.
             and false if they don't with notice detailing issue.""",
            "RT_ST_SameAlignment",
        ),
    ),
    (
    ...
    (
        "ST_Distance_Sphere",
        None,
        """Returns minimum distance in meters between two lon/lat geometries.
        Uses a spherical earth and radius of 6370986 meters.
        Faster than ``ST_Distance_Spheroid``, but less accurate.
