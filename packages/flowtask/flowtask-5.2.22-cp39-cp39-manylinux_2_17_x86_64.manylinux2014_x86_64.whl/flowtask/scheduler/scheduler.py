...
class NavScheduler:
    ...
    def setup(self, app: web.Application):
        self.db = PostgresPool(
            dsn=default_dsn, name="FlowTask.Scheduler", startup=self.startup
        )
    ...
    async def event_reader(self, channel: aioredis.client.PubSub):
        try:
            ...
            self.logger.warning(f"Error reading from Task Event Subscription {err}")
        finally:
    ...
    async def startup(self, app: web.Application, conn: Callable):
        ...
        try:
            ...
            raise RuntimeError(f"{err!s}") from err
        ...
        # adding listeners
        self.add_listeners()
        self.logger.info(f"Scheduled Started at {datetime.now()}")
        try:
            # starting scheduler
    ...
    async def create_jobs(self):
        ...
        for job in jobs:
            ...
            if job_struct:
                ...
                self.logger.error("Scheduler: Missing Scheduled Job Structure")
