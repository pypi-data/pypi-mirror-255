...
class SpeechHelper():
    ...
    def build_speech(self):
        ...
        result = qs.query(self.topic,
                          ...
                          summarizer="vectara-summary-ext-v1.3.0", summary_result_count=10)
        ...
        search_results = []
        for index, response in enumerate(result.response):
            search_result = {
                'num': str(index+1),
                'doc_id': documents[response.documentIndex].id,
                'text': response.text,
