...
def deploy_results_to_conviso_beta(
        conviso_api, results_filepaths, asset_id, company_id, commit_ref=None, deploy_id=None
):
    results_context = click.progressbar(
        results_filepaths, label="Sending SASTBox reports to Conviso Platform"
    )
...
def run(
    ...
    if not from_ast:
        ...
        params_to_copy = [
            ...
            'experimental', 'asset_name', 'vulnerability_auto_fix'
        ]
    ...
    try:
        ...
        if send_to_flow:
            ...
            vuln_closer.VulnCloser.verify_issues_to_close(
                conviso_api=conviso_beta_api, company_id=company_id, asset_id=asset_id, scan_type='sast'
            )
