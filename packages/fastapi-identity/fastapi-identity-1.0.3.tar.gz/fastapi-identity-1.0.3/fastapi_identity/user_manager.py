...
import random
import uuid
from typing import Final, Optional, Generic, cast, overload, Iterable

from fastapi_identity.resources import Resources
from fastapi_identity.exc import (
...
from fastapi_identity.lookup_normalizer import ILookupNormalizer
from fastapi_identity.options import IdentityOptions
from fastapi_identity.password_hasher import (
    IPasswordHasher,
    PasswordHasher,
...
from fastapi_identity.stores import (
    IUserStore, IUserAuthenticationTokenStore, IUserAuthenticatorKeyStore,
    IUserTwoFactorRecoveryCodeStore, IUserTwoFactorStore, IUserPasswordStore, IUserSecurityStampStore,
    IUserRoleStore, IUserLoginStore, IUserEmailStore, IUserPhoneNumberStore, IUserClaimStore,
    IUserLockoutStore
)
from fastapi_identity.token_provider import IUserTwoFactorTokenProvider
...
class UserManager(Generic[TUser]):
    ...
    def __init__(
        ...
        :param user_validators: A collection of IUserValidator[TUser] to validate users against.
        :param key_normalizer: The ILookupNormalizer to use when generating index keys for users.
        ...
        from fastapi_identity import PasswordValidator, UserValidator, UpperLookupNormalizer
    ...
    async def get_users_in_role(self, role: str) -> list[TUser]:
        """
        Returns a list of users from the user stores who are members of the specified role_name.

        :param role: The name of the role whose users should be returned.
        :return:
        """
        if not role:
            raise ArgumentNoneException("role")
    ...
    async def is_phone_number_confirmed(self, user: TUser) -> bool:
        """
        Gets a flag indicating whether the specified users telephone number has been confirmed.
    ...
    def generate_new_authenticator_key(self) -> str:
        ...
        :return:
        """
        return self._new_security_stamp()
    ...
    def _get_random_recovery_code_char(self) -> str:  # noqa
        """"""
        return "".join(random.choices("23456789BCDFGHJKMNPQRTVWXY", k=5))
