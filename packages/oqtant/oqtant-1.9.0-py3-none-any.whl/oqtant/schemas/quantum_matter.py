...
class QuantumMatter(BaseModel):
    ...
    def run(self) -> int:
        ...
        Returns:
            int: The current run number of the job
        """
        if self.result:
    ...
    def input(self) -> job_schema.InputValues:
        ...
        Returns:
            bert_schemas.job.InputVales: The job input values of the QuantumMatter object's current result
        """
    ...
    def output_values_to_oqtant_output(
        ...
        Args:
            output_values (PlotOutput|NonPlotOutput):  The output values to convert to OqtantPlotOutput
    ...
    def get_sim_result(self) -> None:
        """Method to get the results of a simulator job. Alerts the user if simulation results are invalid due
    ...
    def corrected_rf_powers(
        self, frequencies: list[float], powers: list[float]
    ) -> list[float]:
    ...
    def get_ideal_optical_potential(
        self, time: float, positions: list[float]
    ) -> list[float]:
...
class QuantumMatterFactory:
    ...
    def get_batch_result(self, matter: QuantumMatter, run: int) -> QuantumMatter:
        """Method to get the results of a batch job

        Args:
            matter (QuantumMatter): The QuantumMatter object associated to the batch job
            run (int, optional): The specific run to get
    ...
    def create_rf_shield(
        power: float = 0, frequency: float = 0, lifetime: float = 1.0
    ) -> RfShield:
