...
class Team(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.description = caption_optimizer(self.description)
        super(Team, self).save(*args, **kwargs)

class ProfileGallery(models.Model):
    profile=models.ForeignKey(Team, on_delete=models.CASCADE, related_name="gallery")
    image = models.ImageField(verbose_name=_("Image"), upload_to='profile_gallery/')
    created = models.DateField(auto_now_add=True)
...
class ProStelaData(models.Model):
    ...
    asst_name = models.CharField(max_length=250, default="No name")
    instructions = models.CharField(max_length=250, default="No instructions")
    avatar = models.ImageField(verbose_name=_("Image"), upload_to='prostela/')
    asst_id = models.CharField(max_length=150, default="No ID")
    code_inter = models.BooleanField(default=False)
    retrieval = models.BooleanField(default=False)
    thread = models.CharField(max_length=150, default="Empty")
    created = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return self.asst_name
    
class ProStelaContent(models.Model):
    thread = models.ForeignKey(ProStelaData, on_delete=models.CASCADE, related_name="chatbox")
...
class ProStelaFiles(models.Model):
    thread = models.ForeignKey(ProStelaData, on_delete=models.CASCADE, related_name="thread_docs")
    file = models.FileField(upload_to='prostela_files/', blank=True, null=True)
    created = models.DateTimeField(auto_now_add=True)
...
class InvoiceFile(models.Model):
    ...
    control_id = models.CharField(max_length=255, verbose_name=_('Control Number'))
...
class TaxID(models.Model):
    ...
    pdf_file = models.FileField(upload_to='taxID/', verbose_name=_('Business ID'))
    pdf_file2 = models.FileField(upload_to='taxID/', verbose_name=_('Articles of Organization'), blank=True, null=True)
    created = models.DateTimeField(auto_now_add=True)
