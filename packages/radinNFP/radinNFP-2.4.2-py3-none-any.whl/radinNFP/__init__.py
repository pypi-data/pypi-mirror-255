...
def finalNFP(polygonA,polygonB):
    ...
    if(len(polygonA[0]) > 2 and len(polygonB[0]) > 2 ):
        outerNfp = NFP(a_RefPoint,b_highPoint,polygonA[0],polygonB[0],edgesA)
        result.append(outerNfp[0])
    elif(len(polygonA[0]) > 2 ):
        outerNfp = NFP(a_RefPoint,b_highPoint,polygonA[0],polygonB,edgesA)
        result.append(outerNfp[0])
    elif(len(polygonB[0]) > 2 ):
        outerNfp = NFP(a_RefPoint,b_highPoint,polygonA,polygonB[0],edgesA)
        result.append(outerNfp[0])
    else:
        outerNfp = NFP(a_RefPoint,b_highPoint,polygonA,polygonB,edgesA)
        result.append(outerNfp[0])
        ...
        for innerP in polygonB[1:]:
            ...
            for nfp in holeNfp:
                if(len(nfp) == 0):
                    ...
                    result.append([[x[0] + difference[0],x[1] + difference[1]] for x in nfp])
    return result