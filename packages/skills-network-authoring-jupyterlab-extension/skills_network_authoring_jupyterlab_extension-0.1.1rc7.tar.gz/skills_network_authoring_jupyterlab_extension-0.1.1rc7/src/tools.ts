...
export const getCellContents = (cell: Cell<ICellModel>): ICellData => {
  console.log(`Getting contents for cell with id: ${cell.model.id}`);
...
export const getCellContentsOmitID = (cell: Cell<ICellModel>): ICellData => {
  console.log(
    `Getting contents for cell with id: ${cell.model.id}, omitting ID`
  );
  const source = cell.model.toJSON().source;
  const sourceArray: string[] = typeof source === 'string' ? [source] : source;
  const cellData: ICellData = {
...
export const getFileContents = (
  ...
  console.log(`Extracting file contents from notebook: ${context.path}`);
  const allCells: any[] = [];
  ...
  console.log(`Retrived notebook metadata: ${config_meta}`);
  ...
  console.log(
    `File contents extracted: ${JSON.stringify(rawFile).substring(0, 50)}...`
  );
...
export async function updateLabCommitID(
  ...
  console.log(`Updating commit ID for notebook: ${context.path}`);
  ...
  console.log(
    `Set commit ID in notebook metadata: ${Globals.PREV_PUB_HASH}: ${commitID}`
  );
...
export function getLabCommitID(labContent: ILabContent): string {
  console.log('Retrieving lab commit ID...');
...
function isBlankLab(labContent: ILabContent): boolean {
  console.log('Checking if lab content is blank...');
  if (typeof labContent === 'string') {
    try {
      labContent = JSON.parse(labContent);
    } catch (e) {
...
export const getFileCellContentsOmitID = (
  ...
  console.log(
    '[getFileCellContentsOmitID] Extracting cell contents, omitting IDs.'
  );
  const allCells: any[] = [];
  panel.content.widgets.forEach((cell: Cell<ICellModel>) => {
  ...
  console.log('[getFileCellContentsOmitID] Cell contents extracted.');
...
export const openIndependentLab = async (
  ...
  console.log('[openIndependentLab] Opening independent lab with AWB token.');
  ...
  if (!prevFile) {
    console.log(
      '[openIndependentLab] No previous file found, creating new notebook.'
    );
    nbPanel = docManager.createNew(labFilename, 'notebook', {
    ...
    await loadLabContents(
      nbPanel,
      instructions_content as unknown as nbformat.INotebookContent
    );
  } else {
    console.log('[openIndependentLab] Opening existing notebook.');
    nbPanel = docManager.openOrReveal(labFilename) as NotebookPanel;
  }

  console.log('[openIndependentLab] Independent lab opened successfully.');
  return nbPanel;
};
...
export const openLab = async (
  ...
  console.log('[openLab] Opening lab with token.');
  const { instructions_file_path, body: instructions_content } =
    await getLabModel(axiosHandler(token));
  ...
  let nbPanel: NotebookPanel | undefined;
  if (!prevFile) {
    console.log('[openLab] No previous file found, creating new notebook.');
    nbPanel = docManager.createNew(labFilename, 'notebook', {
      name: Globals.PY_KERNEL_NAME
    }) as NotebookPanel;
    if (nbPanel === undefined) {
    ...
    await loadLabContents(
      nbPanel,
      JSON.parse(instructions_content) as unknown as nbformat.INotebookContent
    );
  } else {
    console.log('[openLab] Opening existing notebook.');
    nbPanel = docManager.openOrReveal(labFilename) as NotebookPanel;
  }

  console.log('[openLab] Lab opened successfully.');
  return nbPanel;
};
...
async function checkAndBackupSaveFile(
  ...
  console.log(
    '[checkAndBackupSaveFile] Checking for and backing up save file.'
  );
  ...
  if (prevFile) {
    ...
    if (currCommitID !== prevCommitID) {
      console.log(
        '[checkAndBackupSaveFile] Commit ID mismatch, converting file to backup.'
      );
      try {
        ...
        console.log('[checkAndBackupSaveFile] Lab was updated successfully');
      } catch (e) {
...
async function deleteIfEmptyFile(
  ...
  console.log('[deleteIfEmptyFile] Checking if file is empty to delete.');
  const filePath = `./${labFilename}`;
  const file: Contents.IModel | null = await getFile(filePath, contentsManager);
  if (file) {
    console.log("[deleteIfEmptyFile] File found, checking if it's empty.");
    await closeWithoutSaving(filePath, docManager);
    if (isBlankLab(file)) {
      console.log('[deleteIfEmptyFile] File is empty, deleting.');
...
export const loadLabContents = async (
  ...
  console.log('[loadLabContents] Loading lab contents.');
  // Wait for widget to initalize correctly before making changes
  await widget.context.ready;

  if (author_env !== 'local') {
    console.log(
      '[loadLabContents] Authoring environment is not local, loading content.'
    );
    // Load content
    ...
    try {
      ...
      console.log('[loadLabContents] Lab contents loaded and saved.');
    } catch (error) {
...
export const getLabFileName = (lab_filepath: any): string => {
  console.log('[getLabFileName] Getting lab file name.');
...
async function getFile(
  ...
  console.log(`[getFile] Attempting to get file: ${path}`);
  try {
    const file = await contentsManager.get(path);
    console.log(`[getFile] Successfully retrieved file: ${path}`);
    return file;
  } catch (error) {
    console.log(`[getFile] Error retrieving file: ${path}`, error);
    if (error instanceof Error) {
      ...
      if (axiosError.response && axiosError.response.status !== 404) {
        ...
        console.log('[getFile] File not found, returning null:', path);
        return null;
      }
    } else {
  ...
  if (widget) {
    try {
      if (widget.context.model.dirty) {
        console.log(
          `[saveAndCloseOpenFiles] Saving dirty widget for path: ${path}`
        );
        await widget.context.save();
      }
      console.log(`[saveAndCloseOpenFiles] Closing widget for path: ${path}`);
      widget.close();
    } catch (error) {
      console.error(
    ...
    console.log(
      `[saveAndCloseOpenFiles] No open widget found for path: ${path}`
    );
  }
}
...
async function closeWithoutSaving(
  ...
  console.log(
    `[closeWithoutSaving] Attempting to close without saving for path: ${path}`
  );
  const widget = docManager.findWidget(path);
  if (widget) {
    try {
      ...
      if (context && context.model) {
        ...
        console.log(
          `[closeWithoutSaving] Marked context as not dirty for path: ${path}`
        );
      }
      widget.close();
      console.log(`[closeWithoutSaving] Widget closed for path: ${path}`);
    } catch (error) {
      console.error('[closeWithoutSaving] Error closing widget:', error);
    }
  } else {
    console.log(`[closeWithoutSaving] No open widget found for path: ${path}`);
  }
}
...
async function convertFileToBackup(
  ...
  console.log(
    `[convertFileToBackup] Attempting to convert file to backup for path: ${file.path}`
  );
  ...
  console.log(`[convertFileToBackup] New path for backup: ${newPath}`);
  const prevFile = await getFile(newPath, contentsManager);

  if (prevFile) {
    console.log(
      `[convertFileToBackup] Previous backup file exists, deleting: ${newPath}`
    );
    try {
  ...
  try {
    ...
    console.log(
      `[convertFileToBackup] File successfully renamed to backup: ${newPath}`
    );
  } catch (error) {
    console.error(
...
export async function generateHash(data: string): Promise<string> {
  console.log('[generateHash] Generating hash for data');
  ...
  const hash = Array.from(new Uint8Array(hashBuffer))
    ...
    .join('');
  console.log('[generateHash] Hash generated successfully');
  return hash;
}
