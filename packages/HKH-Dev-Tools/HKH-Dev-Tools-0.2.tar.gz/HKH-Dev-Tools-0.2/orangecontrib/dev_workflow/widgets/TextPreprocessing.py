...
# This widget clears the text to reduce the amount of sentences that are hard to understand. The following operations are applied :
#    - Extra \\n are removed
#    - Sentences without conjugated verb are removed
#    - Sentences longer than 1500 words are removed
# Then sentences are gathered into chunks to be processed by a LLM.
...
from Orange.widgets.data.owpythonscript6 import OWPythonScript
from PyQt5.QtCore import QThread, pyqtSignal

def get_widgets():
    widgets = dict()
    for obj in gc.get_objects():
        if isinstance(obj, OWFile) and obj.captionTitle == "Point to pointer.pkl":
            widgets["File"] = obj
        if isinstance(obj, OWPythonScript) and obj.captionTitle == "Text preprocessing":
            widgets["Python Script"] = obj
    return widgets

def clear_spaces(text):
    separator_patterns = r'(\. |\.\\n)'
    #Split to try and separate sentences
    text_split = re.split(separator_patterns, text)
    sentences = [text_split[i] + text_split[i+1] for i in range(0, len(text_split)-1, 2)]
    # Remove \\n preceded or followed by lowercase
    sentences = [re.sub(r'(?<=[a-z(),])\\n(?=[a-z(),])', " ", sentence) for sentence in sentences]
    return sentences

def is_verb_conjug(sentence):
    nlp = spacy.load("fr_dep_news_trf")
    doc = nlp(sentence)
    type_list = [token for token in doc if token.pos_=="VERB" or token.pos_=="AUX"]
    for verb in type_list:
        detail = verb.morph
        form = detail.get("VerbForm")
        if form == ["Inf"]:
            pass
        else:
            return True
...
def split_into_chunks(sentences, model, chunk_size=1500, widgets=None, signal=None):
    chunks = []
    ignored_sentences = []
    current_chunk = ""
    i = 0
    for sentence in tqdm(sentences):
        if is_verb_conjug(sentence):
            concat = current_chunk + sentence
            tokens = model.tokenize(concat.encode())
            if len(tokens) <= chunk_size:
                current_chunk += sentence
            elif len(model.tokenize(sentence.encode())) <= chunk_size:
                chunks.append(current_chunk)
                current_chunk = sentence
            else:
                ignored_sentences.append(sentence)
        else:
            ignored_sentences.append(sentence)
        i += 1
        update_pbar(signal, int(i/len(sentences)*100), widgets)
    chunks.append(current_chunk)
    return chunks, ignored_sentences


class Thread(QThread):
    # Send responses as Orange Table
    signal_output = pyqtSignal(Table, Table, dict)
    signal_pbar = pyqtSignal(int, dict)
    ...
    def run(self):
        print(self.parameters["model_path"])
        model = Llama(model_path=self.parameters["model_path"],
                      n_ctx=4096,
                      n_gpu_layers=43,
                      verbose=True)

        text = self.parameters["text"]
        sentences = clear_spaces(text)
        chunks, ignored_sentences = split_into_chunks(sentences,
                                                      model,
                                                      chunk_size=1500,
                                                      widgets=widgets,
                                                      signal=self.signal_pbar)
        chunks = [[chunk] for chunk in chunks]
        ignored_sentences = [["\\n".join(ignored_sentences)]]

        chunks_var = StringVariable(name="Chunks")
        ignored_var = StringVariable(name="Ignored sentences")
        chunks_domain = Domain([], metas=[chunks_var])
        ignored_domain = Domain([], metas=[ignored_var])
        out_data = Corpus.from_list(domain=chunks_domain,
                                    rows=chunks)
        titles = ["Chunk {}".format(i+1) for i in range(len(chunks))]
        out_data._titles = np.array(titles)

        out_object = Corpus.from_list(domain=ignored_domain,
                                      rows=ignored_sentences)
        titles = ["Ignored {}".format(i+1) for i in range(len(ignored_sentences))]
        out_object._titles = np.array(titles)

        self.signal_output.emit(out_data, out_object, widgets)

def send_table(data, obj, widgets):
    getattr(widgets["Python Script"].Outputs, "data").send(data)
    getattr(widgets["Python Script"].Outputs, "object").send(obj)
    widgets["Python Script"].progressBarFinished()


def update_pbar(signal, value, widgets):
    signal.emit(value, widgets)
def _update_pbar(value, widgets):
    widgets["Python Script"].progressBarSet(value)


out_data = None
if in_object is not None:
    widgets = get_widgets()
    widgets["Python Script"].progressBarInit()
    parameters = dict()
    
    working_dir = os.path.dirname(widgets["File"].loaded_file)
    dir_model = os.path.join(working_dir, "Models")
    model_path = os.path.join(dir_model, "wizardlm-13b-v1.2.ggmlv3.q6_K.bin")
    parameters["model_path"] = model_path
    
    text = in_object.to_string().split("content:")[-1].split("str_index")[0].strip()[1:-1]
    parameters["text"] = text
    
    thread = Thread(parameters, widgets)
    thread.signal_output.connect(send_table)
    thread.signal_pbar.connect(_update_pbar)
    thread.start()
...
class OWPythonScript(OWWidget):
    ...
    category = "HKH pre-release"
    icon = "icons/TextPreprocessing.svg"
    priority = 3150
