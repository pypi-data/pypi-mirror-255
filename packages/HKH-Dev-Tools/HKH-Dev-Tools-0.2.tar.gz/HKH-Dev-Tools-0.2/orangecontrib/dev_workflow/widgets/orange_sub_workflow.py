...
from Orange.widgets.hkh import HKH_Utils_Box
from Orange.widgets.dev_workflow import shared_variables, shared_functions, MetManagement
...
class OrangeSubWorflow(widget.OWWidget):
    ...
    def __init__(self):
        ...
        uic.loadUi('/widget_designer/orange_sub_workflow.ui', self)
        ...
        self.push_button.setIcon(QIcon('/icons/Logo_HKH.svg'))
        ...
        if self.run_in_Orange:
            ...
            for i in range(len(shared_variables.vect_doc_ptr)):
                if shared_variables.vect_doc_ptr[i][0] == self.run_ows:
                    ...
                    break
        if alert_manager:
            if HKH_Utils_Box.create_message_box_question_yes_no("WARNING: The file " + self.run_ows + " is not open, which will cause a crash if we execute the module without opening it. Do you want to open it?"):
    ...
    def chose_ows(self):
        self.run_ows=HKH_Utils_Box.get_file_windows(self,"*.ows")
        self.validate_OWS_input_output()
        self.reload_label()
        self.Outputs.object.send(self.captionTitle)
