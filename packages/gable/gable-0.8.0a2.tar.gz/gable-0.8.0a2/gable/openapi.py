...
class CheckContractViolationResponse(BaseModel):
    message: str
...
class Input(BaseModel):
    ...
    gzipPayload: Optional[str] = Field(
        None,
        description="Base64 encoded gzipped tar archive (.tar.gz) containing a project",
    )
    uploadKey: Optional[str] = Field(
        None,
        description="Key returned by the GET /data-asset/ingest endpoint for the file that was uploaded",
    )
    decryptionKey: Optional[str] = Field(
        None,
        description='One-time use AES 256 key and Initialization Vector (16 bytes) to decrypt the payload that was uploaded to S3. String should be in the format of "key_hex:iv_hex". This key will not be persisted in Gable\'s backend.\n',
        example="1671f6eb56A1f5193111eefe19df4735c07e3577bdf74ffbae5b44e5c35ec593:8e33ec06437989ba0ac17b4d77014438",
    )
    emitterFunction: Optional[str] = Field(
        None, description="Name of the emitter function"
    )
    emitterFunctionPayloadParameter: Optional[str] = Field(
        None, description="Name of the payload parameter of the emitter function"
    )
    eventNameKey: Optional[str] = Field(
        None,
        description="Name of the key in the event payload that identifies the event",
    )
    emitterFilePath: Optional[str] = Field(
        None,
        description="Relative path from the root of the project to the file that contains the emitter function",
    )
