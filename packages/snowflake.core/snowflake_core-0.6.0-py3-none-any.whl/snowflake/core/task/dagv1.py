...
This set of higher-level classes provides a more convenient way to create, deploy, and manage DAGs of Tasks
than the lower-level Task APIs in `snowflake.core.task`.

Example 1: Create a DAG that has two Tasks.
...
Example 2: Create a DAG that uses Cron, Branch, and function return value as Task return value
...
class DAG:
    ...
    Snowflake doesn't have a first-class DAG entity, so this is a client-side object representation which
    manages Task relationship. A root :class:`Task` and its successors logically form a DAG (Directed Acyclic
    Graph).  Refer to `DAG of tasks <https://docs.snowflake.com/en/user-guide/tasks-intro#dag-of-tasks>`_.

    When a DAG is deployed, all child tasks are created in Snowflake. A dummy Task is created as the root.
    A task's predecessor is the dummy task if it's added to the DAG with no other predecessors.
    ...
    def __init__(
        ...
        *,
        ...
        #: Schedule of the DAG. Refer to :attr:`snowflake.core.task.Task.schedule`.
        ...
        #: The default stage location where this DAG's tasks code will be stored
    ...
    def add_task(self, task: "DAGTask") -> None:
        ...
        if task.dag is not self:
            raise ValueError("Cannot move a task to a different DAG")
...
class DAGRun:
    ...
    but tweaked a little bit to fit into DAG context.
    """

    dag_name: str  #: Name of the root task and the DAG.
    database_name: str  #: Name of the database that contains the graph.
    schema_name: str  #: Name of the schema that contains the graph.
    state: str  #: state of the DAG run. One of ["SCHEDULED", "EXECUTING", "SUCCEEDED", "FAILED", "CANCELLED"].
    ...
    # Time when the root task is next scheduled to start running, assuming the current run of the DAG started
    ...
    graph_version: int  #: Integer identifying the version of the DAG that was run, or is scheduled to be run.
...
class DAGOperation:
    ...
    def iter_dags(self, *, like: str) -> List[str]:
        ...
        Args:
            like: The pattern of the DAG name. Use ``%`` represent any number of characters and ``?`` for a single
                character.
    ...
    def deploy(self, dag: DAG, mode: CreateMode = CreateMode.error_if_exists) -> None:
        ...
        def topological_sort() -> Iterator[DAGTask]:
            ...
            if len(sorted_nodes) != len(dag.tasks):
                raise ValueError("There is a cycle in the DAG.")
