...
class ServicesResource(ResourceBase):
    ...
    def desc_service(self) -> Tuple[str, Dict[str, Any]]:
        ...
        response_data_e = response_data[0] if len(response_data) > 0 else dict()
        result_spec = {"spect_content": response_data_e["spec"], "spec_type": "from_inline", "specification": response_data_e["spec"]}
        response_data_e["spec"] = result_spec
        return sql_str, response_data_e

    def show_status(self) -> Tuple[str, Optional[str]]:
        ...
        response_data = self.snow_exec.execute(sql_str)
        response_data_e = (
            next(iter(response_data[0].values())) if len(response_data) > 0 else None
        )
        return sql_str, response_data_e

    def show_logs(self) -> Tuple[str, Any]:
        # TODO Validate that all information is present in the call.
        sql_str = "CALL SYSTEM$GET_SERVICE_LOGS('{}.{}.{}','{}', '{}')".format(
            self._database,
            self._schema,
            self._service_name,
            self._query_params["instance_id"],
            self._query_params["container_name"],
        )
        response_data = self.snow_exec.execute(sql_str)
        response_data_e = (
            next(iter(response_data[0].values())) if len(response_data) > 0 else None
        )
        return sql_str, response_data
    ...
    def create_service(self) -> Tuple[str, Dict[str, Any]]:
        ...
        if "spec" in self._prop:
            if self._prop["spec"]["spec_type"] == "from_file":
                sql_str = sql_str + " FROM {stage} SPEC = '{file}'".format(stage=self._prop["spec"]["stage_location"], file=self._prop["spec"]["spec_file"])
            elif self._prop["spec"]["spec_type"] == "from_inline":
                sql_str = sql_str + " FROM SPECIFICATION '{}'".format(self._prop["spec"]["specification"])
