...
def test_task_context():
    ...
    session.sql.side_effect = side_effect_sql
    ...
    session.call.side_effect = side_effect_call
    ...
    def side_effect_call_none(sp_name, para=None):
        if sp_name == "SYSTEM$GET_TASK_GRAPH_CONFIG":
            return ''
    session.call.side_effect = side_effect_call_none
    ...
    session.sql.side_effect = side_effect_raise_null_result
    ...
    session.sql.side_effect = side_effect_raise_other_error
...
def test_get_current_task_short_name():
    session = create_autospec(Session)
    task_context = TaskContext(session)
