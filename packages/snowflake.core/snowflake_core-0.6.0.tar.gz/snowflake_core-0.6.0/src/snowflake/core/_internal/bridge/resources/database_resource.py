...
from ...utils import try_single_quote_value, normalize_name, unquote_name
...
class DatabaseResource(ResourceBase):
    ...
    def create_db(self) -> tuple[str, Dict[str, Any]]:
        ...
        # TODO: this type ignore shouldn't be here, but either _query_params
        # should never be a Dict[str, Any], or a lot more type handling will
        # be necessary
        clone: Dict[str, Any] | None = self._query_params.get("clone")  # type: ignore[assignment]
        if clone is not None:
            sql_str += f"CLONE {normalize_name(clone['source'])} "
            pot = clone.get("point_of_time")
            if pot is not None:
                sql_str += f"{pot['reference'].upper()} {pot['point_of_time_type'].upper()} => {pot['when']} "
