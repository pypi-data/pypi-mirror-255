...
class UpdateHandler:
    ...
    """Updates tool instances according to events sent to the tool session."""
    ...
    def _naive_update_element(
        self,
        tool_or_container: Any,  # noqa: ANN401
        element_id: str,
        element_name: str,
        ...
        if element_id in element_ids_to_names:
            ...
            tool_or_container.__dict__[element_name] = value
            return True

        for child_name in element_ids_to_names.values():
            child = getattr(tool_or_container, child_name)
