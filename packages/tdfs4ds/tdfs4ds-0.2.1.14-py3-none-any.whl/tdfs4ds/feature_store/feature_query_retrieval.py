...
def get_feature_store_content(entity_id, data_domain=None, convert_to_date=False, time_manager = None):
    ...
    - time_manager (object, optional): An object that specifies the schema and table name of a time management table to filter records based on business date. The object should have `schema_name` and `table_name` attributes.

    Returns:
    - DataFrame: A DataFrame containing the retrieved content, with the 'ValidStart' column and the entity ID(s) from each table in the feature store that matches the criteria, optionally filtered by business date.
    ...
    - The function assumes that the `time_manager` object, if provided, correctly specifies the schema and table for retrieving the current business date to apply as a filter for the query.
    """
    if data_domain is None:
        data_domain = tdfs4ds.DATA_DOMAIN
    if type(entity_id) == str:
    ...
    if time_manager is None:
        ...
        if convert_to_date:
            query = '\nUNION ALL\n'.join(
                [f"SELECT DISTINCT CAST(ValidStart AS DATE) AS ValidStart, {','.join([k for k in entity_id])} FROM {x} WHERE ValidStart < (SEL BUSINESS_DATE FROM {time_manager.schema_name}.{time_manager.table_name})" for x in table_list])
        else:
            query = '\nUNION ALL\n'.join(
                [f"SELECT DISTINCT ValidStart, {','.join([k for k in entity_id])} FROM {x} WHERE ValidStart < (SEL BUSINESS_DATE FROM {time_manager.schema_name}.{time_manager.table_name})" for x in table_list])
