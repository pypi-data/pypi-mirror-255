...
def list_features():
    query = f"CURRENT VALIDTIME SEL * FROM {tdfs4ds.SCHEMA}.{tdfs4ds.FEATURE_CATALOG_NAME}"
...
def get_available_features(entity_id, display_details=False):
    if tdfs4ds.FEATURE_STORE_TIME == None:
        ...
        validtime_statement = f"VALIDTIME AS OF '{tdfs4ds.FEATURE_STORE_TIME}'"
...
def get_list_entity(domain=None):
    ...
    query = f"CURRENT VALIDTIME SEL DISTINCT ENTITY_NAME FROM {tdfs4ds.SCHEMA}.{tdfs4ds.FEATURE_CATALOG_NAME} where DATA_DOMAIN = '{domain}'"
...
def get_list_features(entity_name, domain=None):
    ...
    query = f"CURRENT VALIDTIME SEL FEATURE_NAME FROM {tdfs4ds.SCHEMA}.{tdfs4ds.FEATURE_CATALOG_NAME} where entity_name = '{entity_name}' AND DATA_DOMAIN = '{domain}'"
