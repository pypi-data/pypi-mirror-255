...
def PlayMacro(filename, options=Oasys.gRPC.defaultArg):
    ...
    options : Object
        ...
        view [logical] If views encoded in the macro file for picks/drags should be replayed. If omitted the current value from macro window will be used.
        delay [integer] Delay in ms between commands when replaying. If omitted the current value from macro window will be used.
        variables [object] Object containing names and values for variables in the macro. If None or omitted no variables are used.
