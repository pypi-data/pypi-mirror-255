    ...
    def _plot_clusters_dbscan(self, X, labels):
        ...
        for k, col in zip(unique_labels, colors):
            if k == -1:
                ...
                # Ajustamos la transparencia para los puntos que no son ruido.
                col = list(col)  # Convertimos a lista para poder modificar
                col[3] = 0.5  # Ajustamos el valor alfa para transparencia

            class_member_mask = (labels == k)

            xy = X_pca[class_member_mask]
            plt.plot(xy[:, 0], xy[:, 1], 'o', markerfacecolor=tuple(col), markeredgecolor='k', markersize=5 if k == -1 else 10)

        plt.title('Improved Clusters Visualization by DBSCAN')
        plt.show()
