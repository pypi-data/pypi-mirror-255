...
def create_regscale_incidents(
    incidents: list[dict], regscale_id: int, regscale_module: str
) -> None:
    """
    ...
    for incident in incidents:
        ...
        if not incidents_exist(title, existing_incidents):
            regscale_incident = create_regscale_incident(
                incident, regscale_id, regscale_module, user_id
            )
...
def create_regscale_incident(
    ...
    return Incident(
        ...
        description=format_data_to_html(
            incident
        ),  # Assuming this function formats the data to HTML
        ...
        dateResolved=(
            incident["end"]
            if "end" in incident and map_status_to_phase(incident["status"]) == "Closed"
            else None
        ),
        createdById=user_id,
    )
...
def create_and_associate_incident_properties(
    incident: dict, incident_id: int, user_id: Optional[str] = None
) -> None:
    """
    ...
    for host in incident.get("hosts", []):
        create_asset(
            data=host, parent_id=incident_id, parent_module="incidents", user_id=user_id
        )
...
def sync_compliance(ssp_id: int, catalog_id: int, framework: str) -> None:
    ...
    create_control_implementations(
        ...
        existing_implementation_dict=ControlImplementation.get_existing_control_implementations(
            parent_id=ssp_id
        ),
...
@click.option(
    "--ssp_id", "-s", type=click.INT, required=True, help="The ID of the SSP record."
)
@click.option(
    "--catalog_id",
    "-c",
...
def create_control_implementations(
    ...
    post_batch_if_needed(
        app, to_create, ControlImplementation.post_batch_implementation
    )
...
def process_controls(
    ...
    for control in controls:
        ...
        status = check_implementation(
            full_controls, partial_controls, failing_controls, lower_case_control_id
        )

        if control["controlId"] not in existing_implementation_dict:
            cim = create_new_control_implementation(
                control, parent_id, parent_module, status, user_id
            )
            to_create.append(cim)
        else:
            update_existing_control_implementation(
...
def create_new_control_implementation(
    ...
    :param int parent_id: Identifier for the parent control
    :param str parent_module: Name of the parent module
    :param status: Status of the control implementation
    :param Optional[str] user_id: ID of the user performing the operation, defaults to None
...
def update_existing_control_implementation(
    ...
    remove_keys(
        existing_imp, ["createdBy", "systemRole", "controlOwner", "lastUpdatedBy"]
    )
...
def post_batch_if_needed(
    ...
    :param post_function: The function to call for posting the batch
    :return: None
    """
    if to_create:
...
def put_batch_if_needed(
    app: Application, to_update: list, put_function: Callable[[Application, list], None]
) -> None:
    """
    ...
    :param put_function: The function to call for putting the batch
    :return: None
    """
    if to_update:
