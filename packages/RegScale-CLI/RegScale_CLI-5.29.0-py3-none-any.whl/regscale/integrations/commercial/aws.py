...
# noqa: E402
def sync_findings(regscale_ssp_id: int, create_issue: bool = False) -> None:
...
def get_failed_checklist_controls(existing_checklists) -> list[dict]:
    """Extracts failed checklist controls from a list of checklists

    :param existing_checklists: List of existing checklists
    :return failed_checklist_controls: List of failed checklist controls
    :rtype: list[dict]
    """
...
def update_control_implementation(
    api: Api, app: Application, control_implementation
) -> None:
    """Updates a control implementation using the RegScale API

    :param Api api: Api instance
    :param Application app: Application instance
    :param control_implementation: RegScale control implementation
    :return: None
    """
    res = api.put(
...
def update_implementations(app: Application, regscale_ssp_id: int) -> None:
    ...
    for control in failed_checklist_controls:
        ...
        if control_implementation_data:
            ...
            control_implementation["plannedImplementationDate"] = (
                convert_datetime_to_regscale_string(
                    planned_datetime,
                    "%Y-%m-%d",
                )
...
def get_earliest_date_performed(checklists) -> str:
...
def get_asset_link(checklist: Checklist, app: Application) -> str:
    ...
    :rtype: str
    """
    return f"""<a href="{app.config['domain']}/assets/form/{checklist.assetId}" \
...
def create_or_update_regscale_issue(
    ...
    if issue is not None:
        if issue_id is None:
            # post
            post_res = api.post(
                url=app.config["domain"] + "/api/issues", json=issue.dict()
            )
            post_fail = not post_res.ok
            if post_res.ok:
                logger.info("Successfully created issue %s", issue.title)
                return post_res.json()["id"]
        else:
            # update
            put_res = api.put(
                url=app.config["domain"] + f"/api/issues/{str(issue_id)}",
                json=issue.dict(),
            )
            put_fail = not put_res.ok
            if put_res.ok:
                logger.info("Successfully updated issue %s", issue.title)
                return put_res.json()["id"]
        # check if either call was successful
        if post_fail:
            logger.error(
                f"Unable to create {issue.title}.\n"
                f"Create error: {post_res.status_code}: {post_res.reason}-{post_res.text}\n"
            )
        if put_fail:
            logger.error(
                f"Unable to update issue #{issue.id} {issue.title}.\n"
                f"Update error: {put_res.status_code}: {put_res.reason}-{put_res.text}"
            )
...
def create_or_update_regscale_checklists(*args) -> Checklist:
    ...
    :return: A new security checklist object
    :rtype: Checklist
    """
...
def fetch_aws_findings_and_sync_regscale(
    ...
    :param int regscale_ssp_id: RegScale System Security Plan ID
    :param bool create_issue: Create Issue in RegScale from vulnerabilities in AWS Security Hub, defaults to False
