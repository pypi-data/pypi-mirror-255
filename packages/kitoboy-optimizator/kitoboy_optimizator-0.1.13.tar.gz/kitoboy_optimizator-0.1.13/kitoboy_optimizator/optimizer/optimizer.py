...
class Optimizer:
    ...
    def process_results(self):
        ...
        best_sample_backtest_results = self.backtester.execute_backtest(
            strategy=self.strategy,
        ...
        html = self.html_builder.generate_html(best_sample_backtest_results, self.backtest_options.get("initial_capital"))
        timestamp_now = int(1000 * dt.datetime.now().timestamp())
        html_file_path = os.path.join(f"{self.results_dir}/{self.strategy_name}/{self.exchange_name}", f"{self.symbol_params.get('symbol')}_{timestamp_now}.html")
