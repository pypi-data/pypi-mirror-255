...
class HTMLBuilder():
    ...
    def generate_html(self, log: np.ndarray, initial_capital: float) -> str|None:
        ...
        if preprocessed_log.shape[0] > 0:
            ...
            return self.create_html(preprocessed_log, performance_metrics)
        else:
            return None
    ...
    def create_html(self, strategy_name: str, exchange_name: str, symbol: str, interval: str, log: np.ndarray):
...
"""
    ...
    def generate_html_body(self, strategy_name: str, exchange_name: str, symbol: str, interval: str, log: np.ndarray) -> str:
        title = f"{strategy_name} {symbol} {interval} - {exchange_name}"
        list_of_trades = self.generate_list_of_trades_html_table(log)
        perfomance_summary = self.generate_performance_summary_html_table(log)
        body = f"""
<body>
...
"""
    ...
    def generate_list_of_trades_html_table(self, log: np.ndarray) -> str:
        table_body = self.generate_list_of_trades_table_body(log)
...
"""
    ...
    <td class="text-end" rowspan="2">{log.iat[i, 7]} {self.symbol[:symbol.rfind('USDT')]}</td>
...
"""
    ...
    def generate_performance_summary_html_table(self, log: np.ndarray) -> str:
        table_body = self.generate_list_of_trades_table_body(log)
