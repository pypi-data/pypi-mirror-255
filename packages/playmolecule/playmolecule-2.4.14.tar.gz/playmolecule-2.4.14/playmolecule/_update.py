...
def update_hashes(target):
    ...
    for tgt in target:
        ...
        md5sum = hashlib.md5(open(tgt, "rb").read()).hexdigest()
...
class GCloudStorage:
    ...
    def list_files(self, source):
        import subprocess
        from playmolecule import envvar
        ...
        pm_home = os.environ[envvar]
        with open(os.path.join(pm_home, "license_path.txt"), "r") as f:
        ...
        if self.license_checker:
            ...
            licensed_files = []
            for ff in files:
                pieces = ff.replace("pmws-apps/", "").replace("/", "").split("_v")
                retcode = subprocess.call(
                    [self.license_checker, "-name", pieces[0], "-version", pieces[1]],
...
def update_apps(mode="all", automatic=False):
    ...
    if envvar not in os.environ:
        raise RuntimeError(f"Could not find {envvar} environment variable")

    pm_home = os.environ[envvar]
    appdir = os.path.join(pm_home, "apps")

    service_acc_json = os.path.join(pm_home, "slpm-service-account.json")
    ...
    for app in tqdm(answer, desc="Checking hashes and updating"):
        ...
        with open(os.path.join(outdir, "run.sh"), "w") as f:
            f.write(
                f'#!/bin/bash\njob_dir=`dirname "$(realpath $0)"`\n{pm_home}/apptainer_run.sh {name.lower()}/{version}/{appname} $job_dir'
            )

        if app == "acellera-protocols/":
            _update_protocols(datadl, app, pm_home)
