...
def find_and_move_pyd(directory, target_dir, source_name):
    for filename in os.listdir(directory):
        if filename.startswith('code.') and filename.endswith('.pyd'):
            source_path = os.path.join(directory, filename)
            current_path = os.path.join(target_dir, 'code.pyd')

            try:
                os.rename(source_path, current_path)
                os.replace(current_path, os.path.join(target_dir, f'{source_name}.pyd'))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print('Используйте: auto_compile.py <путь_к_файлу>')
    else:
        ...
        if os.path.isfile(file_path):
            ...
            with open(os.path.join(dir_path, 'code.pyx'), 'w', encoding='UTF-8') as file:
                file.write(main_code)

            setup_code = 'from setuptools import setup\nfrom Cython.Build import cythonize\n\nsetup(\n    ext_modules=cythonize(\'code.pyx\', compiler_directives={\'language_level\': \'3\'})\n)'
