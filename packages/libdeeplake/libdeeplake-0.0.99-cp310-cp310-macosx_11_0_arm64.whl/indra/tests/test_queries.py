...
def test_dataset_query_results(ds_name, queries):
    ds = dataset_to_libdeeplake(
        agree(IMAGENET_DS_NAME, token=os.getenv(ENV_ACTIVELOOP_PLATFORM_TOKEN))
        if ds_name == IMAGENET_DS_NAME
        else deeplake.load(ds_name, token=os.getenv(ENV_ACTIVELOOP_PLATFORM_TOKEN))
    )
...
def test_query_selection_expressions(tmp_datasets_dir):
    ...
    with ds:
        ds.create_tensor(
            "image", dtype=np.uint8, htype="image", sample_compression="png"
        )
    ...
    assert np.all(
        (view.score[0].numpy() - np.linalg.norm(ds.image[0].numpy().reshape(120000), 2))
        / view.score[0].numpy()
        < 0.0001
    )
...
def test_order_by_asc_desc_query(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "order_by_query_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "label",
            dtype=np.int32,
...
def test_embeddings_positive_and_negative(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "embeddings_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "embeddings",
            dtype=np.int32,
...
def test_row_number(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "row_number_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "dummy",
            dtype=np.int32,
