...
import uuid
from rova_client.utils import UniqueIDGenerator, SessionTracker, TraceTracker, DateTracker, simplify_dict, reverse_simplify_dict, order_by
...
class Rova:
    ...
    def __init__(self, auth, timeout=1440):
        ...
        self.new_sessions = dict()
        generator = UniqueIDGenerator()
        
        # init clients
        self.session_client = SessionTracker(timeout, generator)
        self.trace_client = TraceTracker(generator)
    ...
    def format_to_rova(self, obj):
        ...
        args['session_id'], self.new_sessions[args['user_id']] = self.session_client.get_session_id(args['user_id'], args['timestamp'])
        args['distinct_id'] = uuid.uuid4()

        if(args['event_type']=='llm'):
            args['trace_id'] = self.trace_client.get_trace_id(args['user_id'], self.new_sessions[args['user_id']], 'llm')
