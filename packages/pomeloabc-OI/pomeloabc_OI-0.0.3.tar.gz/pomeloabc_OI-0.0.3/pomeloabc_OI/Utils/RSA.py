...
class RSA():
    ...
    def __generate__(self):
        if not os.path.exists("./RSA_system"):
            os.mkdir("./RSA_system")
        if not os.path.exists("./RSA_system/pub_key.pem"):
            with open("./RSA_system/pub_key.pem", "wb") as file:
                file.write(self.__pub_key)
        if not os.path.exists("./RSA_system/pri_key.pem"):
            with open("./RSA_system/pri_key.pem", "wb") as file:
                file.write(self.__pri_key)

    def __get_pub__(self):
        with open("./RSA_system/pub_key.pem", "rb") as file:
            return rsa.import_key(file.read())
    
    def __get_pri__(self):
        with open("./RSA_system/pri_key.pem", "rb") as file:
            return rsa.import_key(file.read())
    ...
    def text_encode(self, data, charset = 'utf-8'):
        ...
        byte_data = b''.jpomeloabc_OIn(res)
    ...
    def text_decode(self, data, sentinel = b'Decrypt Error'):
        ...
        return str(b''.jpomeloabc_OIn(res), encoding = "utf-8")
