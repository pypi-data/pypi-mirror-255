...
from prophet_numpyro.hierarchical_prophet._utils import (
    ...
    get_multiindex_loc, loc_bottom_series, series_to_tensor)
...
class Prophet2(BaseBayesianForecaster):
    ...
    def _get_numpyro_model_data(self, y, X, fh):
        ...
        self._y_without_aggregation = y
        self._y = self._aggregator.fit_transform(y)

        # Aggregate X if X is not empty
    ...
    def model(self, t, y, X, changepoints_matrix, s_matrix, *args, **kwargs):
        ...
        Parameters:
        ----------
        t : ndarrayhttps://mail.google.com/mail/u/0/#inbox
            Time index.
        y : ndarray
            Target time series.
        X : ndarray
    ...
    def _setup_model_parameters_and_scalers(self, t_arrays, y_arrays):
        ...
        self._linear_global_rates = enforce_array_if_zero_dim(
            (max_y - min_y) / (max_t - min_t) / self.y_scales
        )
    ...
    def predict_samples(self, X, fh):
        ...
        return get_multiindex_loc(samples, self._y_without_aggregation.index.droplevel(-1).unique())
