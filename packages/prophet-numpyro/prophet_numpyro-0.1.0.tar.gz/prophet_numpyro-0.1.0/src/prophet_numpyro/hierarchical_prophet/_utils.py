...
def get_multiindex_loc(df, idx_values):
    ...
    mask = df.index.droplevel(
        list(range(-1, -1 - (nlevels - nlevels_to_loc), -1))
    ).isin(idx_values)
    return df.loc[mask]
...
def convert_index_to_days_since_epoch(idx: pd.Index):
    """
...
def convert_dataframe_to_tensors(df: pd.DataFrame):
    """
