...
# @profile
def model_selection_for_precomputed_kernel_new(
		datafile,
		estimator,
	...
	str_fw = '###################### log time: ' + datetime.datetime.now().strftime(
		"%Y-%m-%d %H:%M:%S"
	) + '. ######################\n\n'
	...
	if read_gm_from_file == False:
		...
		for idx, params_out in enumerate(param_list_precomputed):
			...
			#			plt.show()
			graph_kernel = estimator(
				node_labels=['label_0'], #['atom'],  #todo: check if the node label is correct here. #
				edge_labels=['label_0'], #['bond_type'],  # dataset.edge_labels,
				ds_infos={'directed': False},#dataset.get_dataset_infos(
					#keys=['directed']
				#),
				height=params_out['height'],
			)
			...
			if params_out == {}:
				...
				if verbose:
					print(
						'the gram matrix with parameters', params_out,
						'is: \n\n'
					)
			...
			if len(Kmatrix) < 2:
				...
				if np.isnan(Kmatrix).any(
					...
					if nb_g_ignore > 0:
						if verbose:
							print(
								', where %d graphs are ignored as their graph kernels with themselves are zeros.' % nb_g_ignore
								)
		...
		if len(gram_matrices) == 0:
			...
			trial_do_partial = partial(
				parallel_trial_do, param_list_pre_revised, param_list, y,
				model_type
			)
			train_pref = []
			val_pref = []
			...
			if verbose:
				iterator = tqdm(
					...
					), desc='cross validation', file=sys.stdout
				)
			else:
				iterator = pool.imap_unordered(
					trial_do_partial, range(NUM_TRIALS), chunksize
				)
			...
			std_gram_matrix_time = np.std(gram_matrix_time, ddof=1) if len(
				gram_matrix_time
			) > 1 else 0
			...
			std_bgmt = np.std(best_gram_matrix_time, ddof=1) if len(
				best_gram_matrix_time
			) > 1 else 0
			if verbose:
				print(
				...
				print(
					'time to calculate best gram matrix: {:.2f}±{:.2f}s'.format(
						ave_bgmt, std_bgmt
					)
				)
				print(
	...
	# read gram matrices from file.
		...
		pool = Pool(
			processes=n_jobs, initializer=init_worker, initargs=(gram_matrices,)
		)
		trial_do_partial = partial(
			parallel_trial_do, param_list_pre_revised, param_list, y, model_type
		)
		train_pref = []
		val_pref = []
		test_pref = []
	...
	str_fw += printResultsInTable(
		param_list, param_list_pre_revised, average_val_scores,
...
def trial_do(
		param_list_pre_revised, param_list, gram_matrices, y, model_type, trial
):  # Test set level
	...
	rdm_seed_out_l = rdm_out.uniform(
		high=len(param_list_pre_revised) * 10,
	...
	for index_out, params_out in enumerate(param_list_pre_revised):
		...
		rdm_seed_out = (rdm_seed_out + int(rdm_seed_out_l[index_out])) % (
					2 ** 32 - 1)
		...
		rdm_seed_in_l = rdm_out.uniform(
			high=len(param_list) * 10,
			size=len(param_list)
		)
		...
		for index_in, params_in in enumerate(param_list):
			...
			rdm_seed_in = (rdm_seed_in + int(rdm_seed_in_l[index_in])) % (
						2 ** 32 - 1)
			#			print("rdm_seed_in2: ", trial, index_in, rdm_seed_in)
			inner_cv = KFold(
				n_splits=5, # todo: 10
				shuffle=True, random_state=rdm_seed_in
			)
			current_train_perf = []
			current_valid_perf = []
			current_test_perf = []
			...
			# For clcassification use SVM
				...
				svc = SVC(
					kernel='precomputed', cache_size=200,
...
def parallel_trial_do(param_list_pre_revised, param_list, y, model_type, trial):
	train_pref, val_pref, test_pref = trial_do(
		param_list_pre_revised,
		param_list, G_gms, y,
		model_type, trial
	)
...
def compute_gram_matrices(
		dataset, y, estimator, param_list_precomputed,
		output_dir, ds_name,
...
def printResultsInTable(
	...
	table_dict['params'] = [{**param_out, **param_in}
	                        for param_in in param_list for param_out in
	                        param_list_pre_revised]
	...
	table_dict['valid_perf'] = [
		'{:.2f}±{:.2f}'.format(
			average_val_scores[index_out][index_in],
	...
	table_dict['test_perf'] = [
		'{:.2f}±{:.2f}'.format(
			average_perf_scores[index_out][index_in],
	...
	table_dict['train_perf'] = [
		'{:.2f}±{:.2f}'.format(
			average_train_scores[index_out][index_in],
