...
class Evaluation:
    ...
    def plot_heat_maps(self, annot=False) -> plt:
        ...
        _, axes = plt.subplots(1, 2, figsize=(18, 10), gridspec_kw={'left': 0.1, 'wspace': 0.4})  # Adjust left parameter here
        sns.heatmap(df1.corr(), annot=annot, cmap='viridis', ax=axes[0], cbar=False)  # Disable color scale
        axes[0].set_title('Original', fontsize=5)  # Adjust font size here
        axes[0].set_xticklabels(axes[0].get_xticklabels(), fontsize=5)  # Adjust font size of x-axis labels
        axes[0].set_yticklabels(axes[0].get_yticklabels(), fontsize=5)  # Adjust font size of y-axis labels

        sns.heatmap(df2.corr(), annot=annot, cmap='viridis', ax=axes[1])
        axes[1].set_title('Synthetic', fontsize=5)  # Adjust font size here
        axes[1].set_xticklabels(axes[1].get_xticklabels(), fontsize=5)  # Adjust font size of x-axis labels
        axes[1].set_yticklabels(axes[1].get_yticklabels(), fontsize=5)  # Adjust font size of y-axis labels
        cbar = axes[1].collections[0].colorbar  # Access colorbar
        cbar.ax.tick_params(labelsize=5)  # Adjust font size of color scale labels
