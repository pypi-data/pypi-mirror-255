...
class TrustwiseCallbackHandler(BaseCallbackHandler):
    ...
    def set_experiment_id(self):
        """
        Function to set or generate unique experiment id
        :return: unique_id
        """
        user_input = input("Enter Experiment ID or leave empty for automatic generation: ")

        if user_input.strip():  # If user entered something
            self.experiment_id = user_input
            logger.info(f"Experiment ID manually set: {self.experiment_id}")
        else:  # If user left it empty
            ...
            print("Automatically generated Experiment ID:", self.experiment_id)
            logger.info(f"Experiment ID automatically generated: {self.experiment_id}")
    ...
    def set_api_key(self, api_key: str):
        ...
        if self._authenticated:
            ...
            # If authentication is not required, set user_id as '00000000'
            self._user_id = 00000000
            print("Authentication is disabled. User ID set as '00000000'.")
    ...
    def log_to_mongodb(self, event: CBEvent, parent_id: str = "") -> None:
        ...
        try:
            ...
            response = requests.post(url="http://35.245.106.138/safety/v2/log_events", json=payload_dict)
            response.raise_for_status()  # Raise HTTPError for bad responses

            logger.info("Event logged to MongoDB Successfully")
