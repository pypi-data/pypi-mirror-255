...
from chatllm.utils import to_openai_completion_params
...
@router.post("/v1/chat/completions")  # 多个v1
async def create_chat_completions_with_ad(
    request: ChatCompletionRequest,
    auth: Optional[HTTPAuthorizationCredentials] = Depends(get_bearer_token),
):
    api_key = auth and auth.credentials or None
    ...
    response = OpenAI(api_key=api_key, base_url='https://api.chatllm.vip/v1').chat.completions.create(**data)
    if request.stream:
        _ = itertools.chain((chat_completion_chunk_slogan,), response)
        generator = (chunk.model_dump_json() for chunk in _)
