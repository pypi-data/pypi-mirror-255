...
def yield_instances(rg):
    """
    yields instances from a call from describe_instances
    """
    for x in rg:
        if 'Instances' in x:
            yield from x['Instances']
        else:
            yield x
...
def reservation_pricing(ctx, quiet=False, session=None, **kwargs):
    ...
    notice('instances')
    instances = yielder(ec2, 'describe_instances')
    instances = list(yield_instances(instances))
    notice_end()
    for instance in instances:
        sig = instance_sig(instance)
        costs[sig[0]][sig[1]][sig[2]] = Decimal(0)
