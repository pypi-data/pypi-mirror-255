...
class Block(nn.Module):
    def __init__(self, h, w, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., add_spect=True, init_values=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 window_size=None, attn_head_dim=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.add_spect = add_spect
        self.attn = Attention(
...
class RelativePositionBias(nn.Module):
    def __init__(self, window_size, num_heads):
        ...
        relative_coords[:, :, 1] += window_size[1] - 1
        relative_coords[:, :, 0] = 2 * window_size[1] - 1
        relative_position_index = relative_coords.sum(-1) # WhWw, Wh*Ww
...
class ViTSpectralRoPE(nn.Module):
