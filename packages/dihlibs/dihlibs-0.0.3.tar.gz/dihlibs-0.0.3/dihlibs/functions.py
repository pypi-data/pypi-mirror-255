...
def get_config(config_file="/dih/common/configs/${proj}.json"):
    with open(config_file) as file:
        x = json.load(file) if ".json" in config_file else yaml.safe_load(file)
    c = x["cronies"]
    c["country"] = x["country"]
...
def get_commandline_args():
    parser = argparse.ArgumentParser(
        description="Script for pushing data from data warehouse to dhis2"
    )
    parser.add_argument("-m", "--month", type=str, help="Date in format YYYY-MM")
    parser.add_argument(
        "-n", "--only-new", action="store_true", help="Flag for new elements only"
    )
