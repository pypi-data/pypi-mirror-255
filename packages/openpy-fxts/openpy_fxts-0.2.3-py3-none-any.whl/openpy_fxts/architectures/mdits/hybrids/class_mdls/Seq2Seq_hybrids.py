...
# Encoders Models
from .Seq2Seq.encoder_mdl import encoder_lstm
from .Seq2Seq.encoder_mdl import encoder_lstm2
from .Seq2Seq.encoder_mdl import encoder_batch2_lstm2_drop2
from .Seq2Seq.encoder_mdl import encoder_bilstm_state
from .Seq2Seq.encoder_mdl import encoder_bilstm2
from .Seq2Seq.encoder_mdl import encoder_conv1d
# Decoders Models
from .Seq2Seq.decoder_mdl import decoder_lstm
from .Seq2Seq.decoder_mdl import decoder_lstm2
from .Seq2Seq.decoder_mdl import decoder_lstm2_bach_drop
from .Seq2Seq.decoder_mdl import decoder_bilstm
from .Seq2Seq.decoder_mdl import decoder_bilstm2
from .Seq2Seq.decoder_mdl import decoder_bilstm_time_dist
...
class seq2seq_lstm(tkm.Model):
    def __init__(self, n_future=None, n_out_ft=None, config=None):
        ...
        self.encoder = encoder_lstm(n_future)
        self.decoder = decoder_lstm(n_out_ft)

    def call(self, inputs):
        seq2seq = self.mask(inputs)
        seq2seq = self.encoder(seq2seq)
...
class seq2seq_lstm2(tkm.Model):
    def __init__(self, n_future=None, n_out_ft=None, config=None):
        ...
        self.mask = tkl.Masking(mask_value=value_miss)
        self.encoder = encoder_lstm2(n_future)
        self.decoder = decoder_lstm2(n_out_ft)
...
class seq2seq_lstm_batch_drop(tkm.Model):
    ...
    def __init__(self, n_future=None, n_out_ft=None, config=None):
        ...
        self.mask = tkl.Masking(mask_value=value_miss)
        self.encoder = encoder_batch2_lstm2_drop2(n_future)
        self.decoder = decoder_lstm2_bach_drop(n_out_ft)
...
class seq2seq_bilstm(tkm.Model):
    def __init__(self, n_future=None, n_out_ft=None, config=None):
        ...
        self.mask = tkl.Masking(mask_value=value_miss)
        self.encoder = encoder_bilstm_state(n_future)
        self.decoder = decoder_bilstm(n_out_ft)
...
class seq2seq_bilstm2(tkm.Model):
    def __init__(self, n_future=None, n_out_ft=None, config=None):
        ...
        self.mask = tkl.Masking(mask_value=value_miss)
        self.encoder = encoder_bilstm2(n_future)
        self.decoder = decoder_bilstm2(n_out_ft)
...
class seq2seq_conv1d_bilstm(tkm.Model):
    def __init__(self, n_future=None, n_out_ft=None, config=None):
        ...
        self.mask = tkl.Masking(mask_value=value_miss)
        self.encoder = encoder_conv1d(n_future)
        self.decoder = decoder_bilstm_time_dist(n_out_ft)
...
def seq2seq_multihead_conv1d_bilstm(n_past, n_inp_ft, n_future, n_out_ft, units=128, dropout=0.2):
    ...
    for i in range(0, n_inp_ft):
        mask = tkl.Masking(mask_value=value_miss)(input_layer)
...
def seq2seq_bilstm_with_attention(n_past, n_inp_ft, n_future, n_out_ft, units=128, dropout=0.2):
    ...
    mask = tkl.Masking(mask_value=value_miss)(enc_inputs)
    # ENCODER
...
def seq2seq_lstm_with_loung_attention(n_past, n_inp_ft, n_future, n_out_ft, units=128, dropout=0.2):
    ...
    mask = tkl.Masking(mask_value=value_miss)(input_train)
