...
class birnn_timedist_dense_class(tkm.Model):
    ...
    def __init__(
        ...
        if config is None:
            config = _get_dict_fxts('BiRNN_TimeDist_Dense').config_birnn()
...
class birnn_conv1d_dense(tkm.Model):
    ...
    def __init__(
        ...
        if config is None:
            config = _get_dict_fxts('BiRNN_Conv1D').config_birnn()
        self.n_future = n_future,
        self.n_out_ft = n_out_ft,
        self.config = config
...
class birnn_bahdanau_attention_conv1d_dense(tkm.Model):
    ...
    def __init__(
        ...
        if config is None:
            config = _get_dict_fxts('BiRNN_Bahdanau_Attention_Conv1D').config_birnn()
        self.config = config
...
class birnn_luong_attention_conv1d_dense(tkm.Model):
    ...
    def __init__(
        ...
        if config is None:
            config = _get_dict_fxts('BiRNN_Luong_Attention_Conv1D').config_birnn()
        self.config = config
        self.multi_hidden_layers = multi_rnn_attention_conv1d(
            mdl=type_rnn(config['rnn_layers']['type']),
            bidirectional=config['rnn_layers']['bidirectional'],
...
class birnn_multihead_attention_conv1d_dense(tkm.Model):
    ...
    def __init__(
        ...
        if config is None:
            config = _get_dict_fxts('BiRNN_MultiHeadAttention_Conv1D').config_birnn()
        self.config = config
