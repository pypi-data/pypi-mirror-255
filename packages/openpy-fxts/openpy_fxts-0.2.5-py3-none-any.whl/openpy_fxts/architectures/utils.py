...
def callbacks(
    ...
    callbacks_list = []
    if model_checkpoint:
        filepath = str(filepath) + f'/{label}.hdf5'
...
class process_values_preliminary:
    ...
    def get_values(
        ...
        X_test = self.reshape_X_test()
        if self.app == 'mdits':
            list_case = [
                'BiRNN_Bahdanau_Attention_Conv1D',
                'BiRNN_Luong_Attention_Conv1D',
                'BiRNN_MultiHeadAttention_Conv1D'
            ]
            if self.name_mdl in list_case:
                X_test = tf.where(tf.math.is_nan(X_test), value_miss, X_test)

        # read file contains weights
        list_files = [x for x in os.listdir(filepath) if 'weights' in x]
        print(f'Best Model => {list_files[-1]}')
        self.model.load_weights(str(filepath) + f'/{list_files[-1]}')
        ##

        yhat = self.model.predict(X_test)
        yhat = self.reshape_yhat(yhat)
        yhat_inverse, y_test_inverse = _inverse_transform(
            yhat,
            self.dict_test['test'],
            self.config_data['y_colname']
        )
        if self.app == 'mdits':
            yhat_inverse = _reemplace_NaN(
                yhat_inverse,
                self.config_data,
            )
...
def _reemplace_NaN(
    ...
    df_orig = config['dataset_orig']
    df_test = df_orig.filter(items=list(df_orig.index)[config['n_past']:], axis=0)
    #df_test.loc[df_test[df_test.duplicated()].index.tolist(), ] = np.NaN
    df_yhat = pd.DataFrame(
        yhat_inverse.reshape(yhat_inverse.shape[0], yhat_inverse.shape[2]),
        index=list(df_orig.index)[config['n_past']:],
        columns=df_orig.columns
    )
    for ii in range(len(df_test.columns)):
        df_test.loc[df_test[df_test.duplicated()].index.tolist(), df_test.columns[ii]] = np.NaN
        index_nan = df_test[df_test.columns[ii]][df_test[df_test.columns[ii]].isnull()].index
        df_test.loc[index_nan, df_test.columns[ii]] = df_yhat.loc[index_nan, df_test.columns[ii]]
    #df_test.filter(items=df_orig[df_orig.columns].isnull().index, axis=0)

    return df_test.to_numpy().reshape(df_test.shape[0],1,df_test.shape[1])
