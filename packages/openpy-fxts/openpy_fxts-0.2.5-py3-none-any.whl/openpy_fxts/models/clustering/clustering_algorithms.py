...
class clustering_ts:
    ...
    def get_df_aux(self, user):
        ...
        if self.is_hours:
            ...
            data[data.columns[0]].apply(lambda x: x.strftime('%m-%Y'))
        else:
            data[data.columns[0]] = data[data.columns[0]].apply(lambda x: x.strftime('%H:%M'))
        ...
        if self.is_hours:
            ...
            df_aux = pd.DataFrame(data.T.loc[user]).T
            df_aux.columns = [str(i) for i in df_aux.columns.values]
            return df_aux
        else:
            df_aux = pd.pivot(
                data,
    ...
    def get_centroids_for_clustering(
        ...
        if plt_bar_summary:
            fx_clu_plots.all_barycenter(True, False, plot_n_cluster, gamma, dict_bar)
        if plt_bar_cluster:
            fx_clu_plots.all_barycenter(False, True, plot_n_cluster, gamma, dict_bar)
        if view_clusters:
            self._view_cluster(n_clusters, dict_bar, gamma)
        if pct_clusters:
            fx_clu_plots.pct_clusters(labels_clu)
    ...
    def train_clu_model(
        ...
        if self.reduced is False and self.clu_lbl is None:
            ...
            if self.type_dr is not None:
                ...
                data = ts_scaler
            return mdl_clu.check_model(data, ts_scaler, dict_scaler)
        if self.reduced and self.clu_lbl is None:
            dict_sce = self.reshape_to_clu_atypical_mean()
            dict_scaler = self._scaler_data(dict_sce)
            ts_scaler = dict_scaler['dataset'].values
        ...
        if self.reduced and self.clu_lbl is not None:
            ...
            for i in self.clu_lbl.unique():
                ...
                if self.type_dr is not None:
                    ...
                    data = ts_scaler
                models_with_labels[i] = mdl_clu.check_model(data, ts_scaler, dict_scaler)
    ...
    def cluster_ts_extraction(
        ...
        if self.reduced and self.clu_lbl is not None:
            ...
            dict_bar_labels = {}
            for i in self.clu_lbl.unique():
                dict_bar_labels[i] = self._dict_barycenter(
    ...
    def _dict_barycenter(self, train_model, bool_bar, bool_plots, n_clusters, max_iter, tol, verbose, gamma):
        ...
        if bool_plots['summary_barycenters']:
            fx_clu_plots.all_barycenter(True, False, bool_plots['plot_n_cluster'], gamma, dict_bar)
        if bool_plots['plt_all_barycenters']:
            fx_clu_plots.all_barycenter(False, True, bool_plots['plot_n_cluster'], gamma, dict_bar)
        if bool_plots['view_clusters']:
            self._view_cluster(n_clusters, dict_bar, gamma)
        if bool_plots['pct_clusters']:
            fx_clu_plots.pct_clusters(train_model['cluster_labels'])
        dict_bar['dataset']['cluster'] = train_model['cluster_labels']
        dict_bar['dataset']['user'] = train_model['dict_scaler']['user']
        fx_clu_plots._clu_barplot(dict_bar)
        return dict_bar
    ...
    def optimal_number_of_clusters(
        ...
        if plot:
            fx_clu_plots.optimal_numbers_clustering(metrics, self.model)
        return metrics
    ...
    def performance_to_clu(
        ...
        if self.reduced and self.clu_lbl is not None:
            ...
            dict_dates = self.get_summation_MEAS_Barycenter(dict_results, aux)
            if preview['PDC_AMI']:
                fx_clu_plots.summation_MEAS_Barycenters(dict_dates, preview['n_plots'], self.seed)
            if metrics:
    ...
    def calculate_metrics(self, dict_dates, dict_metrics):
        ...
        if dict_metrics['plots']['boxplot_global']:
            fx_clu_plots.plt_global(metrics_aux, True, False)
        if dict_metrics['plots']['violinplot_global']:
            fx_clu_plots.plt_global(metrics_aux, False, True)
        if dict_metrics['plots']['boxplot_nMEAS']:
            fx_clu_plots.plt_partial(metrics_aux, True, False)
        return metrics_aux
...
class fx_clu_plots:
    ...
    def optimal_numbers_clustering(
        ...
        if metrics is None:
            ...
            p3, = twin2.plot(metrics['clusters_range'], metrics['calinski_harabasz'], "yx-", label="Calinski Harabasz")
    ...
    def check_model(self, data, ts_scaler, dict_scaler):
        ...
        if self.model == 'SOM':
            ...
            if self.points_2d:
                ...
                plt.legend()
            if self.view_clu:
                fx_clu_plots.clu_model_view(train_model)
            return train_model
        ...
        #train_model['dataset']['cluster'] = train_model['cluster_labels']
        #train_model['dataset']['user'] = train_model['dict_scaler']['user']
        # train_model['clusters'] = [x.T for x in train_model['ts_clustered']]
        '''
        if self.points_2d:
