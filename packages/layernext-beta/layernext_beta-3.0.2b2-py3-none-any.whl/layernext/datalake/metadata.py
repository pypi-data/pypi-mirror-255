                ...
                for validation_obj in validation_list:
                    ...
                    for i in range(len(metaData_json_array)):
                        metaData_json_array[i]["metadata"]['Tags'] = [
                            oldTag if item == newTag else item for item in metaData_json_array[i]["metadata"]['Tags']]

            # validate metadata fields using datalake api
            if len(unique_field_names) > 0:
                meta_field_validation_list = self._client.datalake_interface.validate_meta_fields(
                    unique_field_names)
                ...
                for validation_obj in meta_field_validation_list:
                    ...
                    for i in range(len(metaData_json_array)):
                        metaData_json_array[i]["metadata"][oldField] = metaData_json_array[i]["metadata"][newField]
