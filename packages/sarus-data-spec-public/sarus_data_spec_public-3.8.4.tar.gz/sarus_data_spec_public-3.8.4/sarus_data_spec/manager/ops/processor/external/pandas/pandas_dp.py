...
class pd_sum_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        max_mul, sum_tasks = retrieve_or_compute_max_mult(
            max_mult, tasks, parent_ds, random_generator, epsilon
        )

        sum_dp_dict = {}
        schema_fields = parent_schema.data_type().children()
        for task, (column_name, column_type) in zip(
            sum_tasks, (schema_fields.items())
        ):
            subtasks = [unwrap(task) for task in task.subtasks]
...
class pd_mean_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        random_generator = np.random.default_rng(abs(seed_value))
        max_mul, mean_tasks = retrieve_or_compute_max_mult(
            max_mult, tasks, parent_ds, random_generator, epsilon
        )
        mean_dp_dict = {}
        schema_fields = parent_schema.data_type().children()
        for task, (column_name, column_type) in zip(
            mean_tasks, (schema_fields.items())
        ):
            subtasks = [unwrap(task) for task in task.subtasks]
...
class pd_median_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        random_generator = np.random.default_rng(abs(seed_value))
        max_mul, median_tasks = retrieve_or_compute_max_mult(
            max_mult, tasks, parent_ds, random_generator, epsilon
        )
        ...
        schema_fields = parent_schema.data_type().children()
        for task, (column_name, column_type) in zip(
            median_tasks, (schema_fields.items())
        ):
            subtasks = [unwrap(task) for task in task.subtasks]
...
class pd_std_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        random_generator = np.random.default_rng(abs(seed_value))
        max_mul, std_tasks = retrieve_or_compute_max_mult(
            max_mult, tasks, parent_ds, random_generator, epsilon
        )
...
class pd_count_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        random_generator = np.random.default_rng(abs(seed_value))
        max_mul, count_tasks = retrieve_or_compute_max_mult(
            max_mult, tasks, parent_ds, random_generator, epsilon
        )
        count_dp_dict = {}
        ...
        for count_parameter, (column_name, column_type) in zip(
            count_tasks, (schema_fields.items())
        ):
...
class pd_value_counts_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        for count_parameters, (column_name, column_type) in zip(
            shape_task, (schema_fields.items())
        ):
            histogram = t.cast(
                t.Dict[str, float],
                HistogramOp(
...
class pd_corr_dp(ExternalOpImplementation):
    ...
    async def call_dp(self, signature: SarusBoundSignature) -> t.Any:
        ...
        max_mul, corr_task = retrieve_or_compute_max_mult(
            max_mult, tasks, parent_ds, random_generator, epsilon
        )
        assert len(corr_task) == 1
        corr_dp = CorrOp(
            parent_ds,
            corr_task[0].parameters['epsilon'],
            corr_task[0].parameters['dims'],
...
def retrieve_or_compute_max_mult(
    ...
    if max_mult == '':
        ...
        other_tasks = [tasks]  # for type compatibility mainly
    return ceil(max_mul), other_tasks
