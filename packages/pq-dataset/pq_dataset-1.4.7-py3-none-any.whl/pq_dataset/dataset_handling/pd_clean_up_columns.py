...
def pd_clean_up_columns(df) -> pd.DataFrame:
    ...
    sum_zero = df_temp.sum(skipna=True, numeric_only=True)
    zero_sum_colunns = sum_zero[sum_zero==0].index
    static_cols = nb_unique[nb_unique == 1].index  # Cols containing only one value - though NANs as well
    static_cols_without_zero_sum = static_cols[~static_cols.isin(zero_sum_colunns)]
    
    columns_with_nan = df_temp.columns[df_temp.isna().any()].tolist()
    possible_cols = [col for col in static_cols_without_zero_sum if col not in columns_with_nan]
