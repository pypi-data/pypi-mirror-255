    ...
    def create(
        ...
        *,
        ...
        if isinstance(deployment, str) and deployment == "auto":
            ...
            compatible_serverless_llms = [llm for llm in llms if
                                          self._adapter_compatible(llm.model_name, adapter.base_model_name) and
                                          llm.is_shared]
            compatible_dedicated_llms = [llm for llm in llms if
                                         self._adapter_compatible(llm.model_name, adapter.base_model_name) and
                                         not llm.is_shared]
