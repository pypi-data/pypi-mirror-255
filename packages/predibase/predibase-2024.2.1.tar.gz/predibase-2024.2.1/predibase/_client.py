...
class Predibase:
    ...
    def __init__(
        ...
        * Hard-coded defaults
        ...
        if not api_token:
            raise PredibaseError("An api_token must either be explicitly provided or set via the PREDIBASE_API_TOKEN "
        ...
        if not api_gateway.startswith("https://"):
            logger.info(f"No HTTP scheme provided for PREDIBASE_GATEWAY (got: {api_gateway}), defaulting to `https://`.")
    ...
    def _post(self, endpoint: str, data: Optional[Any] = None, json: Optional[Any] = None, **kwargs):
        return self._http.post(urljoin(self.api_gateway, endpoint), data=data, json=json,
    ...
    def _get_serving(self, endpoint: str):
        return self._http.get(
            urljoin(self.serving_http_endpoint, endpoint), headers=self.default_headers)
