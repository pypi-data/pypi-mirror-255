...
def _match_list_el(source: list, dest: list, key: str):
    src_values = non_empty_list([_get_dict_key(x, key) for x in source])
    dest_values = non_empty_list([_get_dict_key(x, key) for x in dest])
...
def _match_el(source: dict, dest: dict, fields: list):
    def match(key: str):
        ...
        is_list = len(keys) >= 2 and (
            isinstance(_get_dict_key(source, keys[0]), list) or
            isinstance(_get_dict_key(dest, keys[0]), list)
        )
        return _match_list_el(
            _get_dict_key(source, keys[0]) or [],
            _get_dict_key(dest, keys[0]) or [],
            '.'.join(keys[1:])
        ) if is_list else _get_dict_key(source, key) == _get_dict_key(dest, key)
