...
from few_shot_priming.utils import get_max_topic_count
...
def sample_similar_examples(test_index, similarities, df_training, few_shot_size, experiment="ibmsc"):
    ...
    if len(df_pro_instances)< class_few_shot_size:
        df_pro_instances = df_pro_instances.sample(class_few_shot_size, replace=True)
    if len(df_con_instances) < class_few_shot_size:
        df_con_instances = df_con_instances.sample(class_few_shot_size, replace=True)
    if experiment == "vast" and len(df_neutral_instances) < class_few_shot_size:
        df_neutral_instances = df_neutral_instances.sample(class_few_shot_size, replace=True)
    instances = []
    ...
    for index in items_indices:
        ...
        if len(pro_instance) and pro_instances_count<class_few_shot_size:
            instances.append(pro_instance)
            pro_instances_count = pro_instances_count + 1
        elif len(con_instance) and con_instances_count < class_few_shot_size:
            instances.append(con_instance)
            con_instances_count = con_instances_count +1
        elif experiment == "vast" and neutral_instances_count < class_few_shot_size:
            instances.append(neutral_instances)
        if experiment == "ibmsc" and pro_instances_count + con_instances_count == few_shot_size:
            break
        if experiment == "vast" and pro_instances_count + con_instances_count + neutral_instances == few_shot_size:
            break
...
def save_similar_examples(few_shot_size, save_text=False):

    for experiment in ["ibmsc", "vast"]:
        for experiment_type in ["validation", "test"]:
            is_validate = experiment_type == "validation"
            topic_counts = get_topic_count_range(experiment, is_validate)
            similarities = load_similarities(experiment, experiment_type)
            all_topic_count_instances = []
            for topic_count in topic_counts:
                dataset = load_splits(experiment, is_validate, topic_count= topic_count)
                for argument in dataset[experiment_type]:
                    test_id = argument["id"]
                    similar_examples = sample_similar_examples(test_id, similarities, dataset["training"], topic_count = topic_count, few_shot_size=few_shot_size, experiment=experiment)
            ...
            path_examples_topic_counts = get_similar_examples_path(experiment, experiment_type, topic_count=True)
            ...
            dataset = load_splits(experiment, is_validate)
            ...
            path_similar_examples_ks = get_similar_examples_path(experiment, experiment_type, topic_count=False)
...
def load_similar_examples(experiment, experiment_type, topic_count=None, few_shot_size=16):
    if topic_count:
        path_examples = get_similar_examples_path(experiment, experiment_type, topic_count=True)
    else:
        path_examples = get_similar_examples_path(experiment, experiment_type, topic_count=False)
...
def get_similar_examples(test_index, df_examples):
    return df_examples[df_examples["test-index"]==test_index]