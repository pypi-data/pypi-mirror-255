...
def save_splits_topic_counts():
    ...
    for experiment in ["ibmsc", "vast"]:
        ...
        df_training.to_csv(f"/home/yamen/tmp/{experiment}-training.csv", sep=",", encoding="utf-8", index=False)
        df_validation.to_csv(f"/home/yamen/tmp/{experiment}-validation.csv", sep=",", encoding="utf-8", index=False)
...
def load_splits(experiment, oversample=True, validate=True, best_split=False, adapt=True, topic_count=None, debug=False):
    ...
    if debug:
        #df_training = df_training.sample(100)
        df_validation = df_validation.sample(200)
