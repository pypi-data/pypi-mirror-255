...
def get_prompt_fine_tuning_best_params(experiment):
    """
    Load the prompting approach best params
    :return:
    """
    prompting_config = get_prompt_fine_tuning()
    return prompting_config["best-params"][experiment]
...
def get_similar_examples_path(experiment, experiment_type, topic_count):
    config = load_config()
    token = f"{experiment}-{experiment_type}"
    if topic_count:
