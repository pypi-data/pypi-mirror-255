...
class MCPipeJob(CTAJob):
    ...
    def __init__(self) -> None:
        super().__init__(we_type="mcsimulation", cpu_time=259200)
    ...
    def set_div_ang(self, div_ang=None) -> None:
        ...
        if div_ang is not None:
            if str(div_ang).replace(", ", ",").split(sep=",") != div_default:
                div_ang_str: str = ", ".join(str(x) for x in div_default)
                DIRAC.gLogger.error(
                    f"Unknown div_ang option: {div_ang}. Option for simulation step is: {div_ang_str}"
                )
                DIRAC.exit(-1)
            else:
    ...
    def build_file_metadata(self, combination) -> tuple[dict[str, str], Any, str, str]:
    ...
    def run_corsika_sim_telarray(self, i_step, debug=False) -> None:
        prod_exe = "./dirac_prod_run"
        if self.div_ang:
            prod_args: str = (
                f"--start_run {self.start_run_number} --run {self.run_number} "
                f"--sequential --divergent {self.moon} {self.sct} {self.magic} "
                f"{self.site} {self.particle} {self.pointing_dir} {self.zenith_angle}"
            )
        else:
            #prod_args = f"--start_run {self.start_run_number} --run {self.run_number} "
            #f"{self.moon} {self.sct} {self.magic} {self.site} {self.particle} {self.pointing_dir} {self.zenith_angle}"
            prod_args = f"--start_run {self.start_run_number} --run-number {self.run_number} --rn-format %06d --hyper-layout {self.moon} {self.sct} {self.magic} {self.site} {self.particle} {self.pointing_dir} {self.zenith_angle}"
            print(prod_args)

        cs_step = self.setExecutable(
            prod_exe, arguments=prod_args, logFile="CorsikaSimtel_Log.txt"
        )
        cs_step["Value"]["name"] = f"Step{i_step}_CorsikaSimtel"
        cs_step["Value"]["descr_short"] = "Run Corsika piped into simtel"

    def init_debug_step(self, i_step) -> None:
        super().init_debug_step(i_step)
        env_step = self.setExecutable("/bin/env", logFile="Env_Log.txt")
        env_step["Value"]["name"] = f"Step{i_step}_Env"
        env_step["Value"]["descr_short"] = "Dump environment"

    def verify_number_of_simtel_events(
        self, data_output_pattern, log_str, i_step, i_substep
    ) -> None:
    ...
    def set_metadata_and_register_data(self, i_step) -> None:
        ...
        for element in itertools.product(*self.output_file_metadata.values()):
            ...
            self.verify_number_of_simtel_events(
                data_output_pattern, log_str, i_step, i_substep
            )
            i_substep += 1
    ...
    def run_dedicated_software(self, i_step) -> None:
        self.run_corsika_sim_telarray(i_step)
...
class MCPipeMuonJob(MCPipeJob):
    ...
    def set_metadata_and_register_data(self, i_step) -> None:
        ...
        for degraded in self.degraded_values:
            ...
            self.verify_number_of_simtel_events(
                data_output_pattern, "", i_step, i_substep
            )
            i_substep += 1
