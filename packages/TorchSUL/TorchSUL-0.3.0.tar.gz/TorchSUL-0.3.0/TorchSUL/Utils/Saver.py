...
class Saver():
    ...
    def restore(self, path: str, strict: bool=True):
        ...
        if path[-4:] == '.pth':
            ...
            elif isinstance(self.model, (nn.DataParallel, nn.parallel.DistributedDataParallel)):  # type: ignore
            ...
            if path:
                if isinstance(self.model, (nn.DataParallel, nn.parallel.DistributedDataParallel)):  # type: ignore
                    state_dict = torch.load(path, map_location=device)
                    self.model.module.load_state_dict(state_dict, strict=strict)
                else:
                    state_dict = torch.load(path, map_location=device)
