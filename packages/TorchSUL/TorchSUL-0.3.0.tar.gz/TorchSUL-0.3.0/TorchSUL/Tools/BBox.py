...
class BBox():
    ...
    def __init__(self, bbox: Tensor, box_format: Literal['xyxy', 'x1y1wh', 'xcycwh'], conf: Optional[Tensor]=None): 
        ...
        assert box_format in ['xyxy', 'x1y1wh', 'xcycwh'], "box_format must be one of ['xyxy', 'x1y1wh', 'xcycwh']"
    ...
    def convert(self, target_format: Literal['xyxy', 'x1y1wh', 'xcycwh']) -> Self:
        assert target_format in ['xyxy', 'x1y1wh', 'xcycwh'], "target_format must be one of ['xyxy', 'x1y1wh', 'xcycwh']"
        ...
        elif self.box_format==target_format:
            new_obj = self.copy()
        else:
            raise Exception(f'Failed to convert from {self.box_format} to {target_format}')  # this line should never be reached 
        return new_obj
    ...
    def cat(self, other: Self, dim: int = 0) -> Self: 
        assert self.box_format==other.box_format, 'box format should be the same for concatenation'
        new_box = torch.cat([self.bbox, other.bbox], dim=dim)
        if (self.conf is not None) and (other.conf is not None):
            new_conf = torch.cat([self.conf, other.conf], dim=dim)
        else:
            new_conf = None 
        return type(self)(new_box, self.box_format, new_conf)
    ...
    def __call__(self):
        if self.conf is not None:
            return torch.cat([self.bbox, self.conf.unsqueeze(-1)], dim=-1)
        else:
            return self.bbox
