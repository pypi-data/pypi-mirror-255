...
class fcLayer(Model):
    ...
    def build(self, *inputs):
        ...
        if self._quant:
            bit_type: QBitTypes = self.get_flag('QActBit')    # type: ignore
            if bit_type is None:
                bit_type = 'int8'
            obs_type: QObserverTypes = self.get_flag('QActObserver')    # type: ignore
            if obs_type is None:
                obs_type = 'minmax'
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.norm:
            with torch.no_grad():
                norm = x.norm(p=2, dim=1, keepdim=True)    # type: ignore   # p can be int, but type hinting will raise error
                wnorm = self.weight.norm(p=2,dim=1, keepdim=True)    # type: ignore   # p can be int, but type hinting will raise error
            x = x / norm
