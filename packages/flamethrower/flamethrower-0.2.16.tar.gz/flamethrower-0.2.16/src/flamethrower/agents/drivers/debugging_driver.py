...
Here are some points to take note:
  - After writing code, try to test it in the same way that the user did it that caused the error.
    - Usually you live in the same directory as the user, so you can run the same commands.
  - If the solution is not obvious, **strongly consider entering a debugging mindset, and write a set of effective print statements** to identify the root cause.
