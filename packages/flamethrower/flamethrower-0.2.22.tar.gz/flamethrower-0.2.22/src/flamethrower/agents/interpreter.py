...
You **should choose multiple actions to perform**. For example:
  ...
  - However, **you must always return a `completed` action on its own**, never with other actions.
    - You can only be completed when the previous response was a positive response.
