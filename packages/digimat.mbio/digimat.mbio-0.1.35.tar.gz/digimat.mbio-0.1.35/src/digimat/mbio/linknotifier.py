...
class MBIOLink(object):
                ...
                try:
                    ...
                    while count>0:
                        ...
                        for data in self._autoDeclareItems.items():
                            (item, value)=data
                            self.logger.info(">DECLAREVALUE %s @%d" % (value, item))
                            lp=self.packetManager.lp()
                            up=lp.up(kMBIO_DECLAREVALUE)
                            up.writeWord(item)  # Hint only
                            up.writeByte(value.isWritable())
                            up.writeStrField(value.key)
                            # This can help for similar values lookup
    ...
    def RAZCONFIG(self):
        if self._connected:
            self.logger.warning(">RAZCONFIG (%s)" % self._passkey)
            lp=self.packetManager.lp()
            up=lp.up(kMBIO_RAZCONFIG)
            if self._passkey:
                up.writeStr(self._passkey)
            else:
                up.writeStr('NONE')
            up.store()
            if lp.send():
...
class MBIOTaskLinkNotifier(MBIOTask):
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        for declare in xml.children('declare'):
            ...
            for value in declare.children('values'):
                # FIXME: not allowed to change order of "devices" (if already declared on remote)
                # We should get the first source-key item in cpu before declaring items
                values=mbio.values(value.get('key'), sort=True)
                part=None
                for v in values:
                    if v.keyrootpart()!=part:
                        if part is not None:
                            item+=skip
                        part=v.keyrootpart()
    ...
    def RAZCpuConfig(self):
        try:
            return self._link.RAZCONFIG()
        except:
            pass
