...
LOCATION_DESCRIPTORS = ["latitude", "longitude", "height_of_station"]

TIME_DESCRIPTORS = ["year", "month", "day", "hour", "minute"]

ID_DESCRIPTORS = ["icao_location_indicator"]
...
class BUFRParser:
    ...
    def get_time(self) -> str:
        ...
        if None in [year, month, day, hour, minute]:
            msg = f"Invalid date ({year}-{month}-{day} {hour}:{minute}) in BUFR data"  # noqa
            LOGGER.error(msg)
        ...
        if hour == 24:
            ...
            offset = 0
        time_ = f"{year:04d}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:00"  # noqa
    ...
    def get_icao(self) -> str:
        """
        Function returns ICAO Location ID as string

        :returns: ICAO Location ID.
        """

        return self.get_identification()["icao"]

    def get_identification(self) -> dict:
        """
        Function extracts identification information from qualifiers.

        :returns: dictionary containing any class 01 qualifiers as dict.  # noqa
        """
        ...
        while codes_bufr_keys_iterator_next(key_iterator):
            ...
            if xx < 9:
                ...
                if value is not None or description is not None:
                    ...
                    icao = self.get_icao()
                    feature_id = f"ICAO_{icao}_{characteristic_date}T{characteristic_time}"  # noqa
