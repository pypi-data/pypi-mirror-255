...
# https://git.astron.nl/RD/rapthor/-/tree/master/rapthor/lib/oservation.py
...
from ska_sdp_wflow_low_selfcal.pipeline.support import miscellaneous as misc
from ska_sdp_wflow_low_selfcal.pipeline.support.cluster import (
    get_fast_solve_intervals,
    get_slow_solve_intervals,
)
...
class Observation(object):
    ...
    starttime : float, optional
        The start time of the observation (in MJD seconds). If None, the start
        time is the start of the MS file
    ...
    def scan_ms(self):
        ...
        # DPPP takes ceil(startTimeParset - startTimeMS), so ensure that our
        # start time is slightly less than the true one (if it's slightly
        # larger, DPPP sets the first time to the next time, skipping the first
        # time slot)
        self.starttime -= 0.1
        ...
        self.ra = misc.normalize_ra(
            np.degrees(float(obs.col("REFERENCE_DIR")[0][0][0]))
        )
        self.dec = misc.normalize_dec(
        ...
        el_values = pt.taql(
            "SELECT mscal.azel1()[1] AS el from "
            + self.ms_filename
            + " limit ::10000"
        ).getcol("el")
    ...
    def set_calibration_parameters(self, parset, ndir, nobs):
        ...
        target_fast_timestep = parset["calibration_specific"][
            "fast_timestep_sec"
        ]
        target_fast_freqstep = parset["calibration_specific"][
            "fast_freqstep_hz"
        ]
        target_slow_timestep_joint = parset["calibration_specific"][
            "slow_timestep_joint_sec"
        ]
        target_slow_timestep_separate = parset["calibration_specific"][
            "slow_timestep_separate_sec"
        ]
        target_slow_freqstep = parset["calibration_specific"][
            "slow_freqstep_hz"
        ]
        ...
        solint_fast_timestep = max(
            1, int(round(target_fast_timestep / timepersample))
        )
        solint_fast_freqstep = max(
            1, self.get_nearest_frequstep(target_fast_freqstep / channelwidth)
        )

        # Adjust the solution interval if needed to fit the fast solve into the
        # available memory and determine how many calibration chunks to make
        # (to allow parallel jobs)
        samplesperchunk, solint_fast_timestep = get_fast_solve_intervals(
            parset["cluster_specific"],
            self.numsamples,
            nobs,
        ...
        if (myendtime - mystarttime) > chunksize:
            ...
            nchunks = int(
                np.ceil(float(self.numsamples) * timepersample / chunksize)
            )
        else:
            nchunks = 1
        starttimes = [mystarttime + (chunksize * i) for i in range(nchunks)]
        if starttimes[-1] >= myendtime:
            # Make sure the last start time does not equal or exceed the end
            # time
            starttimes.pop(-1)
            nchunks -= 1
        ...
        self.parameters["starttime"] = [
            self.convert_mjd(t) for t in starttimes
        ]
        ...
        target_slow_timestep_joint = max(
            1, int(round(target_slow_timestep_joint / timepersample))
        )
        target_slow_timestep_separate = max(
            1, int(round(target_slow_timestep_separate / timepersample))
        )
        solint_slow_freqstep = max(
            1, self.get_nearest_frequstep(target_slow_freqstep / channelwidth)
        )

        # Adjust the solution intervals if needed to fit the slow solves into
        # the available memory and determine how many calibration chunks to
        # make (to allow parallel jobs)
        samplesperchunk, solint_slow_timestep = get_slow_solve_intervals(
            parset["cluster_specific"],
            self.numchannels,
            nobs,
        ...
        if (myendfreq - mystartfreq) > chunksize:
            ...
            nchunks = int(
                np.ceil(float(self.numchannels) * channelwidth / chunksize)
            )
        else:
            nchunks = 1
        ...
        if self.nfreqchunks_joint > 1:
            ...
            infix = ""
        self.log.debug(
            "Using {0} frequency chunk{1} for the joint slow-gain "
            "calibration".format(self.nfreqchunks_joint, infix)
        )
        self.parameters["freqchunk_filename_joint"] = [
            self.ms_filename
        ] * self.nfreqchunks_joint
        self.parameters["startchan_joint"] = [
            samplesperchunk * i for i in range(nchunks)
        ]
        self.parameters["nchan_joint"] = [samplesperchunk] * nchunks
        self.parameters["nchan_joint"][
            -1
        ] = 0  # set last entry to extend until end
        self.parameters["slow_starttime_joint"] = [
            self.convert_mjd(self.starttime)
        ] * nchunks
        self.parameters["slow_ntimes_joint"] = [self.numsamples] * nchunks
        self.parameters["solint_slow_timestep_joint"] = [
            solint_slow_timestep
        ] * self.nfreqchunks_joint
        self.parameters["solint_slow_freqstep_joint"] = [
            solint_slow_freqstep
        ] * self.nfreqchunks_joint
        ...
        if self.nfreqchunks_separate > 1:
            ...
            infix = ""
        self.log.debug(
            "Using {0} frequency chunk{1} for the separate slow-gain "
            "calibration".format(self.nfreqchunks_separate, infix)
        )
        self.parameters["freqchunk_filename_separate"] = [
            self.ms_filename
        ] * self.nfreqchunks_separate
        self.parameters["startchan_separate"] = [
            samplesperchunk * i for i in range(nchunks)
        ]
        self.parameters["nchan_separate"] = [samplesperchunk] * nchunks
        self.parameters["nchan_separate"][
            -1
        ] = 0  # set last entry to extend until end
        self.parameters["slow_starttime_separate"] = [
            self.convert_mjd(self.starttime)
        ] * nchunks
        self.parameters["slow_ntimes_separate"] = [self.numsamples] * nchunks
        self.parameters["solint_slow_timestep_separate"] = [
            solint_slow_timestep
        ] * self.nfreqchunks_separate
        self.parameters["solint_slow_freqstep_separate"] = [
            solint_slow_freqstep
        ] * self.nfreqchunks_separate

        # Set the number of segments to split the h5parm files into for screen
        # fitting. Try to split so that each file gets at least two solutions
        self.parameters["nsplit_fast"] = [
            max(1, int(self.numsamples / solint_fast_timestep / 2))
        ]
        self.parameters["nsplit_slow"] = [
            max(1, int(self.numsamples / solint_slow_timestep / 2))
        ]

        # Set the smoothnessreffrequency for the fast solves, if not set by
        # the user
        fast_smoothnessreffrequency = parset["calibration_specific"][
            "fast_smoothnessreffrequency"
        ]
        if fast_smoothnessreffrequency is None:
            # Select a frequency at the midpoint of the frequency coverage of
            # this observation
    ...
    def set_prediction_parameters(self, sector_name, patch_names, scratch_dir):
        ...
        # The filename of the sector's data with all non-sector sources peeled
        # off and/or with the weights adjusted (i.e., the data used as input
        ...
        # The start time and number of times (since an observation can be a
        # part of its associated MS file)
    ...
    def set_imaging_parameters(
        ...
        if self.numsamples == 1:
            raise RuntimeError(
                ...
                "processed with the selfcal_data_fraction parameter or supply "
                "a measurement set with more time slots."
            )
        ...
        if use_screens:
            # Ensure we don't average more than the solve time step, as we
            # want to preserve the time resolution that matches that of the
            # screens
            target_timewidth_sec = min(
                target_timewidth_sec, solve_fast_timestep
            )
        ...
        target_bandwidth_mhz = min(
            ...
            self.get_target_bandwidth(
                ...
                delta_theta_deg,
                resolution_deg,
                peak_smearing_rapthor,
            ),
        )
        target_bandwidth_mhz = min(
            target_bandwidth_mhz, solve_slow_freqstep / 1e6
        )
        self.log.debug(
            "Target timewidth for imaging is {} s".format(target_timewidth_sec)
        )
        self.log.debug(
            "Target bandwidth for imaging is {} MHz".format(
        ...
        image_freqstep = max(
            1,
            min(int(round(target_bandwidth_mhz * 1e6 / chan_width_hz)), nchan),
        )
        self.parameters["image_freqstep"] = self.get_nearest_frequstep(
            image_freqstep
        )
        ...
        self.log.debug(
            "Using averaging steps of {0} channels and {1} time slots "
            "for imaging".format(
    ...
    def get_bandwidth_smearing_factor(
        self, freq, delta_freq, delta_theta, resolution
    ):
        """
        ...
        reduction_factor = ((np.pi**0.5) / (gamma * beta)) * (
            erf(beta * gamma / 2.0)
        )
    ...
    def get_target_bandwidth(
        self, freq, delta_theta, resolution, reduction_factor
    ):
        """
