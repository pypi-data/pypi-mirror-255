...
# https://git.astron.nl/RD/rapthor/-/tree/master/rapthor/lib/cluster.py
"""
...
def get_fast_solve_intervals(
    cluster_parset, numsamples, numobs, target_timestep, antenna, ndir
):
    """
    Returns the optimal solution interval and chunk size (both in number of
    time slots) for an observation so that the fast-phase solves in DPPP will
    fit in memory
    ...
    def get_mem_per_solution(target_timestep, ndir):
        ...
        #
        # Note: the numbers below were determined empirically from typical
        # (Dutch-only) datasets
        if antenna == "HBA":
    ...
    # Determine whether we need to adjust the solution interval to fit the
    # solve in memory
    if (
        "mem_per_node_gb" in cluster_parset
        and cluster_parset["mem_per_node_gb"] != 0
    ):
...
def get_slow_solve_intervals(
    ...
    solint : int
        Solution interval in number of frequency channels that will ensure the
        solve fits in the available memory
    """
    ...
    def get_gb_per_solution(target_freqstep, target_timestep, ndir):
        ...
        #
        # Note: the numbers below were determined empirically from typical
        # (Dutch-only) datasets by fitting a 2-D quadratic curve of the form:
        #    Z = C[4]*X**2. + C[5]*Y**2. + C[3]*X*Y + C[1]*X + C[2]*Y + C[0]
        # where X = ndir, Y = target_freqstep, and Z is the memory usage for 25
        # time slots (the usage as a function of number of time slots was
        # observed to be a simple linear scaling so was not included for
        # simplicity)
        if antenna == "HBA":
        ...
        gb_per_25_timeslots = (
            coef[4] * ndir**2
            + coef[5] * target_freqstep**2
            + coef[3] * ndir * target_freqstep
            + coef[1] * ndir
            + coef[2] * target_freqstep
            + coef[0]
        )

        # Return total usage, scaled to the requested time interval, plus a
        # 20% safety buffer
    ...
    # the solve in memory. We adjust the time interval rather than the
    # frequency one, as it is less critical and usually has a finer sampling
    if (
        "mem_per_node_gb" in cluster_parset
        and cluster_parset["mem_per_node_gb"] != 0
    ):
    ...
    # Determine the size of the frequency chunks into which to split the
    # calibration.
    #
    # Try to make at least as many total chunks (over all observations) as
    # there are nodes to maximize the parallelization.
    #
    # Also, unlike for the fast-phase solves, the memory usage scales with the
    # chunk size (since the solve is parallelized over the channels of a
    # chunk, so larger chunks require more memory). Therefore, we also need
    # to ensure that the chunk size works with the available memory.
    target_numchunks = np.ceil(cluster_parset["max_nodes"] / numobs)
    samples_per_chunk = int(np.ceil(numsamples / target_numchunks))
