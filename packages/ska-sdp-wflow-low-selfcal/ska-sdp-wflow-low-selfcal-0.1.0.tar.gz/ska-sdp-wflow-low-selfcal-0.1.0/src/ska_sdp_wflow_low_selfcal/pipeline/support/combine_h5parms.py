...
def combine_phase1_amp1_amp2(ss1, ss2, sso):
    ...
    Note: solset ss1 is assumed to have the coarser time grid. The frequency
    grid of the two solsets is assumed to be the same.
    ...
    # Note: the output axes and their values are the same as for the amplitude
    # solutions
...
def combine_h5parms(
    ...
    mode : str
        ...
        'p1a1a2' - phases and amplitudes from 1 and amplitudes from 2
        (amplitudes 1 and 2
        are multiplied to create combined amplitudes)
        'p1p2a2' - phases from 1 and phases and amplitudes from 2 (phases 2
        are averaged
        over XX and YY, then interpolated to time grid of 1 and summed)
        'p1p2a2_diagonal' - phases from 1 and phases and amplitudes from 2,
        XX and YY for
        both
    ...
    cal_names = string2list(cal_names)
    cal_fluxes = string2list(cal_fluxes)
    ...
    # Reweight
    if reweight:
        # Use the scatter on the solutions for weighting, with an additional
        # scaling by the calibrator flux densities in each direction
        ho = h5parm(outh5parm, readonly=False)
        sso = ho.getSolset(solset="sol000")

        # Reweight the phases. Reweighting doesn't work when there are too few
        # samples so check there are at least 10
        soltab_ph = sso.getSoltab("phase000")
        if len(soltab_ph.time) > 10:
            # Set window size for std. dev. calculation. We try to get one of
            # around 30 minutes, as that is roughly the timescale on which the
            # global properties of the ionosphere are expected to change
            ...
            losoto.operations.reweight.run(
                soltab_ph, mode="window", nmedian=3, nstddev=nstddev
            )
        ...
        if len(soltab_amp.time) > 10:
            # Set window size for std. dev. calculation. We try to get one of
            # around 90 minutes, as that is roughly the timescale on which the
            # global properties of the beam errors are expected to change
        ...
        # Use the input calibrator flux densities to adjust the weighting done
        # above to ensure that the average weights are proportional to the
        # square of the calibrator flux densities
        sso = ho.getSolset(solset="sol000")
        soltab_ph = sso.getSoltab("phase000")
        soltab_amp = sso.getSoltab("amplitude000")
        ...
        # changing the overall normalization, which should be the inverse
        # square of the uncertainty (scatter) in the solutions).
        ...
        for d in range(len(dir_names)):
            ...
            if mode == "p1p2a2_scalar":
                norm_factor = cal_weights[d] / np.nanmedian(
                    weights_amp[:, :, :, d]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reweight", help="Reweight solutions", type=str, default="False"
    )
    parser.add_argument(
        "--cal_names", help="Names of calibrators", type=str, default=""
    )
    parser.add_argument(
        "--cal_fluxes",
        help="Flux densities of calibrators",
        type=str,
        default="",
    )
