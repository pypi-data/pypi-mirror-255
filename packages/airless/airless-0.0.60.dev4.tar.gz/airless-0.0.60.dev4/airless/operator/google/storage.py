            ...
            if b.time_deleted is None:
                ...
                if (len(tables[key]['files']) > threshold['file_quantity']):
                    if tables[key]['size'] < threshold['size_small']:
                        self.send_to_process(bucket=bucket, directory=directory, files=tables[key]['files'], size=ProcessTopic.SMALL, data=data)
                    elif tables[key]['size'] < threshold['size_medium']:
                        self.send_to_process(bucket=bucket, directory=directory, files=tables[key]['files'], size=ProcessTopic.MEDIUM, data=data)
                    else:
                        self.send_to_process(bucket=bucket, directory=directory, files=tables[key]['files'], size=ProcessTopic.LARGE, data=data)
