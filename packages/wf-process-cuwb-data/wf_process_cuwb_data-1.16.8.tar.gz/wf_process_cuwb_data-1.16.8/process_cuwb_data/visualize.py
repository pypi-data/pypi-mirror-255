...
def plot_positions_multiple_devices(df, room_corners, **kwargs):
    for (device_id, device_serial_number, entity_type, person_short_name, material_name), group_df in df.fillna(
        "NA"
    ).groupby(["device_id", "device_serial_number", "entity_type", "person_short_name", "material_name"]):
...
def plot_positions_topdown_multiple_devices(df, room_corners, **kwargs):
    for (device_id, device_serial_number, entity_type, person_short_name, material_name), group_df in df.fillna(
        "NA"
    ).groupby(["device_id", "device_serial_number", "entity_type", "person_short_name", "material_name"]):
        entity_name = material_name
...
def plot_accelerations_multiple_devices(df, **kwargs):
    for (device_id, device_serial_number, entity_type, person_short_name, material_name), group_df in df.fillna(
        "NA"
    ).groupby(["device_id", "device_serial_number", "entity_type", "person_short_name", "material_name"]):
        entity_name = material_name
        if entity_type == "Person":
            entity_name = person_short_name
        plot_accelerations(df=group_df, entity_name=entity_name, device_serial_number=device_serial_number, **kwargs)


def plot_positions_and_accelerations_multiple_devices(df_position, df_acceleration, room_corners, **kwargs):
    for (
        device_id,
        ...
        person_short_name,
        material_name,
    ), df_position_group_df in df_position.fillna("NA").groupby(
        ["device_id", "device_serial_number", "entity_type", "person_short_name", "material_name"]
    ):
        entity_name = material_name
        if entity_type == "Person":
            entity_name = person_short_name
...
def plot_accelerations(
    ...
    for axis_index in range(3):
        plots[axis_index] = axes[axis_index].scatter(
            df.index.values, df[acceleration_column_names[axis_index]], marker=marker, alpha=alpha
        )
...
def plot_tray_motion_features(
    ...
    for axis_index, axis_name in enumerate(["x", "y"]):
        ...
        if velocity_limits is not None:
            axes[2 + axis_index].set_ylim(velocity_limits[0], velocity_limits[1])
