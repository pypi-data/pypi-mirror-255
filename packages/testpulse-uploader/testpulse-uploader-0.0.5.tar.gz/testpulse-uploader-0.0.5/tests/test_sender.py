...
from uploader.sender import generate_data_and_headers, send_test_results
...
def test_simple_false():
    ...
    resp2 = send_test_results(zip=Path('tests/fixtures/test_results.zip'),
                              localhost=False)
...
def test_generate_data_and_headers_mocked():
    ...
    assert data['user'] == "testpulse/myrepo"
