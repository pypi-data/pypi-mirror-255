...
def update_version(name: str, version: str) -> None:
    ...
    current_fpm_pool_version_without_dot = "".join(
        current_fpm_pool.version.split(".")
    )
...
def update_max_requests(
    name: str,
    max_requests: Optional[int] = typer.Argument(
...
@app.command()
@catch_api_exception
def update_process_idle_timeout(
    name: str,
    process_idle_timeout: Optional[int] = typer.Argument(
        default=None, help=EMTPY_TO_CLEAR_MESSAGE
    ),
) -> None:
...
def update_cpu_limit(
    name: str,
    limit: Optional[int] = typer.Argument(
        default=None, help=EMTPY_TO_CLEAR_MESSAGE
    ),
) -> None:
    """Update CPU limit."""
