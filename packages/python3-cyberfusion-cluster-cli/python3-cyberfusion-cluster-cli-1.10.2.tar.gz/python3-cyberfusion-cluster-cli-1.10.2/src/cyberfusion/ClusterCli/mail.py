...
def get_account(email_address: str) -> None:
    ...
    console.print(
        get_support().get_table(
            objs=[
                get_object(
                    get_support().mail_accounts, email_address=email_address
                )
            ]
        )
    )
...
def get_alias(email_address: str) -> None:
    ...
    console.print(
        get_support().get_table(
            objs=[
                get_object(
                    get_support().mail_aliases, email_address=email_address
                )
            ]
        )
    )
...
def create_account(
    ...
    mail_account.create(
        ...
        password=password,
        quota=quota,
        mail_domain_id=get_object(
            get_support().mail_domains, domain=domain
        ).id,
    )
...
def remove_alias_forward_addresses(
    ...
    # Exit code will be 64 if at least 1 given address could not be removed
    # Success will only be False (and exit code 1) if no addresses were removed
