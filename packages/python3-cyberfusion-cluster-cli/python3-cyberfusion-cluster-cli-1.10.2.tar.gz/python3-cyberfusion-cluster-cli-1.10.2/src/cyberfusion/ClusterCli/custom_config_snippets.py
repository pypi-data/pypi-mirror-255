...
def get(
    ...
    console.print(
        get_support().get_table(
            objs=[get_object(get_support().custom_config_snippets, name=name)],
            detailed=detailed,
        )
    )
...
@app.command()
@catch_api_exception
def get_contents(name: str) -> None:
    """Show custom config snippet contents."""
    custom_config_snippet = get_object(
        get_support().custom_config_snippets, name=name
    )
