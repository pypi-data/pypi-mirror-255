...
def get(
    ...
    console.print(
        get_support().get_table(
            objs=[get_object(get_support().virtual_hosts, domain=domain)],
            detailed=detailed,
        )
    )
...
def get_document_root(domain: str) -> None:
    ...
    document_root = get_object(
        get_support().virtual_hosts, domain=domain
...
def get_custom_config(domain: str) -> None:
    """Show custom config."""
    virtual_host = get_object(get_support().virtual_hosts, domain=domain)
...
def create(
    ...
    create_virtual_host(
        ...
        server_software_name=(
            server_software_name.value
            if server_software_name is not None
            else None
        ),
        add_www=add_www,
...
def create_virtual_host(
    ...
    allow_override_directives: Optional[List[None]]
    allow_override_option_directives: Optional[List[None]]
...
def add_allow_override_directive(
    domain: str, allow_override_directive: str
) -> None:
...
def add_allow_override_option_directive(
    domain: str, allow_override_option_directive: str
) -> None:
...
def update_document_root(
    ...
    virtual_host = get_object(get_support().virtual_hosts, domain=domain)
    unix_user = get_object(
        get_support().unix_users, id_=virtual_host.unix_user_id
    )
...
def update_fpm_pool(
    ...
    if fpm_pool_name:
        fpm_pool_id = get_object(
            get_support().fpm_pools, name=fpm_pool_name
        ).id
...
def sync_domain_root(
    ...
    left_virtual_host = get_object(
        get_support().virtual_hosts, domain=left_virtual_host_domain
    )
    right_virtual_host = get_object(
        get_support().virtual_hosts, domain=right_virtual_host_domain
    )
