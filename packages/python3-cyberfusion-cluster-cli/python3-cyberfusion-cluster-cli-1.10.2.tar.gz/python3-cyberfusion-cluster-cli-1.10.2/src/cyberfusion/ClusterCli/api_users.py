...
def create(
    ...
    customer_team_code: Optional[str] = None,
    is_active: bool = typer.Option(True, "--active/--inactive"),
    is_superuser: bool = typer.Option(False, "--superuser"),
    trusted_ip_networks: Optional[List[str]] = typer.Option(default=None),
) -> None:
    """Create API user."""
...
@app.command()
@catch_api_exception
def add_trusted_ip_networks(
    username: str, trusted_ip_networks: List[str]
) -> None:
...
@catch_api_exception
@exit_with_status
def remove_trusted_ip_networks(
    username: str, trusted_ip_networks: List[str]
) -> int:
    ...
    for ip_network in trusted_ip_networks:
        try:
            ...
            print_warning(
                f"Trusted IP network '{ip_network}' not found, skipping."
            )
            exit_code = 64
...
def list_grants(
    ...
    if cluster_name is not None:
        args["cluster_id"] = get_object(
            get_support().clusters, name=cluster_name
        ).id
