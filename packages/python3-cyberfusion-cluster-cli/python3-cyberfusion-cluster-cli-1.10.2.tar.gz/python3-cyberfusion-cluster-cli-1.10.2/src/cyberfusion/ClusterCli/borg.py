...
def list_archives(borg_repository_name: str) -> None:
    ...
    console.print(
        get_support().get_table(
            objs=get_support().get_borg_archives(
...
def list_archive_contents(name: str, path: Optional[str] = None) -> None:
    ...
    console.print(
        "Fetching archive contents. This may take up to several minutes."
    )
    console.print(
        get_support().get_table(
...
def create_unix_user_repository(
    ...
    borg_server_cluster = get_borg_server_cluster(
        name=borg_server_cluster_name
    )
...
def create_database_repository(
    ...
    if add_public_key:
        borg_server_ssh_key_name = f"{borg_server_unix_user.username}-borg-client-{borg_client_cluster.id}"
...
def get_borg_server_node(
    hostname: Optional[str], borg_server_cluster_id: int
) -> Node:
    """Get Borg server node."""
...
def get_or_create_borg_server_unix_user(
    *, username: str, borg_server_cluster: Cluster
    ...
    if unix_users:
        console.print(
            f"Using existing Borg server UNIX user: {unix_users[0].username}"
        )
...
def get_or_create_borg_server_ssh_public_key(
    *,
    ...
    ssh_keys = get_support().get_ssh_keys(
        name=name, cluster_id=borg_server_cluster_id
    )
...
def create_repository(
    *,
    ...
    remote_path = os.path.join(
        borg_server_unix_user.borg_repositories_directory, name
    )
...
def update_repository_borg_client_ssh_key(
    ...
    if borg_repository.unix_user_id is None:
        handle_manual_error(
            "SSH key can not be updated on database repository"
        )
...
def download_archive(
    name: str, path: str = typer.Argument(default=None)
) -> None:
