...
def create_nodejs(
    ...
    environment_variables_arg: Optional[List[str]] = typer.Option(
        None, "--env", help="Can be used multiple times. Use key=value pairs"
    ),
    max_pool_size: int = 5,
...
def update_environment_variables(
    name: str, environment_variables: List[str]
) -> None:
...
def delete_environment_variables(
    name: str,
    environment_variables: List[str] = typer.Argument(default=None),
    ...
    if delete_all:
        ...
        for var in environment_variables:
            try:
                ...
                print_warning(
                    f"Environment variable '{var}' does not exist, skipping."
                )
                exit_code = 64
