...
def print_notification_msg() -> None:
    mutation_logger.info(
        ...
        You can follow the test's progress at {Constants.MUTATION_DASHBOARD}"""
    )
...
def load_mutation_conf(mutation_conf: Optional[str]) -> Any:
    if mutation_conf is None:
        return None
...
class MutateApp(SimpleNamespace):
    def __init__(self, *args: str, **kwargs: int) -> None:
        ...
        self.manual_mutants: List[Mutant] = list()
    ...
    def validate_args(self) -> None:
        ...
        if not self.mutation_conf:
            if not self.collect_file:  # A hidden flag
                raise Util.CertoraUserInputError("Must use --mutation_conf")
        # TODO - in app verification
        if not self.prover_conf and not self.orig_run and not self.collect_file:
    ...
    def submit(self) -> None:
        ...
        if Constants.MANUAL_MUTANTS in mutation_conf:
            self.manual_mutants = self.parse_manual_mutations()
        ...
        if self.prover_conf is not None:
            ...
            if certora_run_result:
                ...
                if local:
                    ...
                    if not self.optimistic_original_run:
    ...
    def run_mutant(self, mutant: Mutant, src_dir: Path, trg_dir: Path, orig_conf: Path,
                   mutation_test_id: str) -> MutantJob:
        # first copy src_dir
    ...
    def parse_manual_mutations(self) -> List[Mutant]:
        ...
        ret_mutants = []
        with mutation_conf.open() as conf:
            gambit_cfg = json.load(conf)
            for filename_to_mutate, manual_mutant_dir_rel in gambit_cfg.get(Constants.MANUAL_MUTANTS, {}).items():
                file_to_mutate = os.path.normpath(mutation_conf.parent / filename_to_mutate)
                ...
                if not path_to_orig.exists():
                    ...
                    continue
                manual_mutant_dir = mutation_conf.resolve().parent.joinpath(manual_mutant_dir_rel).resolve()

                if not manual_mutant_dir.is_dir():
                    raise Util.CertoraUserInputError(
                        f"Manual mutations directory '{manual_mutant_dir_rel}' was not found at {manual_mutant_dir}")

                manual_mutants = [mm for mm in sorted(manual_mutant_dir.glob("*.sol")) if mm.is_file()]
                for manual_mutant in manual_mutants:
                    manual_mutant = generate_manual_mutant_object(manual_mutant, file_to_mutate)
                    ret_mutants.append(manual_mutant)
        return ret_mutants
    ...
    def run_certora_prover(self, conf_file: Path,
        ...
        if self.optimistic_original_run:
            certora_args.append("--send_only")
        else:
    ...
    def generate_csv(self, verification_results: List[MutantJobWithResults]) -> None:
        ...
        try:
            with open(self.dump_csv, 'w', newline='') as ui_out_csv:
                ...
                row1 = [str(mutant.mutant_job) for mutant in verification_results]
                wr.writerow(row1)
    ...
    def write_report_file(self, original_result: MutantJobWithResults,
        ...
        valid_rule_names = set([result.name for result in original_result.rule_results
                                if result.status == MutationTestRuleStatus.SUCCESS])
