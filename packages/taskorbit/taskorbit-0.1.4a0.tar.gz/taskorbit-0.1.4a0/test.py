...
async def main():
    ...
    await broker.pub({"uuid": uuid.uuid4().hex, "type_event": "TEST_FUNCTION2", "data": {"some_data": 123}})
    ...
    logger.debug('Dispatcher', handler=dp.handlers)
    for child in dp.child_routers:
        logger.debug(f"Handlers:Router:{child}", handlers=child.handlers)
...
@router2.include_handler(F.metadata.type_event == "TEST_FUNCTION2")
async def handle_function2(metadata: Message, name: str) -> None:
    """
    Easy function, we can still set timeouts and callbacks for them. This is done in the decorator.
    This is a simple variant of the handler.
