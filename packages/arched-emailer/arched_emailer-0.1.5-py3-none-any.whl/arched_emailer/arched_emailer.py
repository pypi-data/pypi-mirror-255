...
BASE_URL = "http://0.0.0.0:5000"  # "https://arched.dev"
...
class ArchedEmailer:
    ...
    def _get_email_path(self, typ="error"):
        """
        ...
        resp_text = self._make_request(f"{BASE_URL}/email/{typ}")
        if resp_text[2] == 200:
    ...
    def _send_to_db(self, typ="error", customer_id: Optional[int] = None, **kwargs: dict):
        """
        ...
        Returns:
            None
        """
        if os.environ.get("ARCHED_DB_HOST"):
            requests.post(
                f"{os.environ.get('ARCHED_DB_HOST')}/email/update/{typ}",
                json=kwargs,
            )
    ...
    def send_success_email(self, recipients: Union[str, list], dump_time_taken: Optional[bool] = True,
                           ...
                           sender_name: Optional[str] = None, app: Optional[str] = None, **kwargs):
        """
        ...
        try:
            ...
            email_path = self._get_email_path(typ="success")
    ...
    def send_error_email(self, recipients: Union[str, list],
        ...
        try:
            ...
            # gets and creates the email template
            email_path = self._get_email_path(typ="error")
            ...
            issue = str(exception) or str(traceback.format_exc())
            if issue:
                if self._allowed_to_send(issue, allowed_minutes=allowed_minutes):
                    ...
                    self.mailer.send_email(recipients, subject=f"Error: {app_name} - ID: {error_id}",
                                           ...
                                           exception=exception, **template_data)

        except:
            pass

    def try_log_function(self, error_recipients: Union[str, list], send_success: Optional[bool] = False,
                         success_recipients: Optional[Union[str, list]] = None, allowed_minutes=60, *args, **kwargs):
        """
            ...
            def wrapper(*func_args, **func_kwargs):
                ...
                try:
                    ...
                    if send_success and success_recipients:
                        self.send_success_email(success_recipients, **kwargs)
                except Exception as e:
                    self.send_error_email(error_recipients, exception=e, allowed_minutes=allowed_minutes, **kwargs)
                finally:
                    self._log_timer(True)
                    return result
