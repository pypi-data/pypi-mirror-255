...
def preprocess_credit_score_data(df):
    ...
    columns_to_convert = ['Age', 'Annual_Income', 'Monthly_Inhand_Salary', 
    ...
    loan_types = ['Credit-Builder Loan', 'Personal Loan', 'Debt Consolidation Loan', 
    ...
    df[columns_to_convert] = df[columns_to_convert].replace(to_replace=r'[^\d.]+', value='', regex=True).astype(float)
    ...
    for loan_type in loan_types:
        df[loan_type] = df['Type_of_Loan'].apply(lambda x: 1 if loan_type in str(x) else 0)
    #Droping 'Type_of_Loan' column
    df = df.drop(columns=['Type_of_Loan']) 

    #Extracting year from 'Credit_History_Age'
    df['Credit_History_in_Year'] = df['Credit_History_Age'].str.extract(r'(\d+) Years').astype(float)
