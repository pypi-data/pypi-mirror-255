...
def check_options(options, id_):
    assert all([isinstance(o[1], str) for o in
                options]), f"Title of setting {id_} must by of type 'str'"
    assert len([o[0] for o in options]) == len(
        list(set([o[0] for o in options]))), f"{id_} must unique options."
...
class Decimal(Input):
    def __init__(self, id_: str, title: str, default: Number,
                 _min: Number = -1e9, _max: Number = 1e9):
...
class Integer(Input):
    def __init__(self, id_: str, title: str, default: Number, _min: Number = 0,
                 _max: Number = 1e9):
...
class SingleChoice(Input):
    def __init__(self, id_: str, options: Options_type, default: object,
                 title: str):
...
class MultipleChoice(Input):
    def __init__(self, id_: str, options: Options_type, default: List[object],
                 title: str):
        ...
        if isinstance(context, LocalAnalysisContext):
            for k, v in context._LocalAnalysisContext__settings.items():
                if isinstance(v, dict) and "files" in v:
                    context._LocalAnalysisContext__settings[k] = context._LocalAnalysisContext__settings[k]["files"]

        filter_names = list()
        for filter_name in self.file_filter.split(
                "["):  # captures the end of file filter name
            filter_names.append(filter_name.split(" ")[-1].replace("(",
                                                                   ""))  # gets the last string (file filter name)

        for file_filter_condition_name in filter_names[:-1]:  # the last element is not the file filter name.
            logger.info(f"Getting files from : {file_filter_condition_name}")
            fhandlers_ = context.get_files(self.id,
                                           file_filter_condition_name=file_filter_condition_name)
            for fh in fhandlers_:
                ...
                fpath = os.path.join(analysis_dir, "input_folder", self.id,
                                     folder_to_save_file)
