...
def build_local_dockerfile() -> None:
    ...
    if not os.path.exists(local_dockerfile):
        with open(local_dockerfile, "w") as w1:
            with open(os.path.join(repo_dir, "templates_tool_maker", "Dockerfile_schema")) as r1:
                w1.write(r1.read())
...
def build_script(code: str) -> None:
    ...
    """
    with open(os.path.join(repo_dir, "templates_tool_maker", "tool_schema")) as r1:
        with open(SCRIPT, "w") as w1:
            w1.write(r1.read().replace("TOOL_CLASS", convert_to_camel_case(code)))
...
def build_description() -> None:
    ...
    """
    with open(os.path.join(repo_dir, "templates_tool_maker", "description_schema.html")) as r1:
...
def build_test(code: str, tool_folder: str, version: str) -> None:
    ...
    os.chdir("test")
    with open(os.path.join(repo_dir, "templates_tool_maker", "test_tool_schema")) as r1:
