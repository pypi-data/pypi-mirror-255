...
class ToolScreen:
    def __init__(self, tool_code: ToolWidget, title: str = "", config: dict = {}):
        self.type = "tool"
...
class PapayaViewer:
    ...
    def add_file(self, file: str, coloring: Coloring):
        self.config["images"].append({"file": file, "coloring": coloring.value})
    ...
    def __init__(
        ...
        if orientation == OrientationLayout.horizontal.value:
            for element in children:
                ...
                assert element.height != "", f"'height' of {element} must be defined in {element.__str__()}."
            ...
            for element in children:
                ...
                assert element.width != "", f"'width' of {element} must be defined in {element.__str__()}."
        else:
...
class Tab:
    def __init__(
        ...
        for element in children:
            assert element.button_label != "", f"'button_label' of {element} must be defined."
...
class ResultsConfiguration:
    ...
    def add_visualization(self, new_element: Union[PapayaViewer, HtmlElement, HtmlInject, ActionBlock]):
    ...
    def generate_results_configuration_file(
        self, build_screen: Union[List, Tab, Split], tool_path: str, testing_configuration: bool = False
    ):
        ...
        if testing_configuration:
            return output
        with open(os.path.join(tool_path, "results_configuration.json"), "w") as f:
            f.write(encoded)
