...
class TestFileInput(LocalFile):
    ...
    def __init__(
        ...
        super().__init__(os.path.split(path)[0], os.path.split(path)[1], modality, tags, file_info)
        if file_info is None:
            file_info = {}
        if tags is None:
            tags = list()
        self.file_name_path = os.path.basename(path) if not file_name_path else file_name_path
        self.path = path
        self.name = path
