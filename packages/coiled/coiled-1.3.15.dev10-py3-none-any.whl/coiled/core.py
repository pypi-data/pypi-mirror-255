...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _tail_software_build_logs(
        ...
        account = account or self.default_account
        build = await self._fetch_software_build(senv_build_id, account=account)
        page_token = None
        logs = await self._fetch_software_build_logs(senv_build_id, account=account, page_token=page_token)
        printed_logs = set()
    ...
    async def _fetch_software_build(self, senv_build_id: int, account: Optional[str] = None):
        account = account or self.default_account
        resp = await self._do_request(
            "GET",
            self.server + f"/api/v2/software-environment/account/{account}/build/{senv_build_id}",
        )
    ...
    async def _fetch_software_build_logs(
        ...
        resp = await self._do_request(
            "GET",
            self.server + f"/api/v2/software-environment/account/{account}/build/logs/{senv_build_id}",
            params=params,
        )
        return await resp.json()
...
def performance_report(
    ...
    try:
        ...
        raise Exception("Exception in coiled.performance_report() context").with_traceback(ex.__traceback__) from ex
    finally:
...
def set_backend_options(
    ...
    **kwargs,
    ...
    with Cloud(account=account) as cloud:
        ...
        text.append("\n\n")
        text.append(
            f"You can view your account backend options here: {account_options_url}",
