...
class ExtractTracket(SalureConnect):
    ...
    def get_worklogs(self, date_start: str = None, date_end: str = None, created_since: str = None, created_up_to: str = None, updated_since: str = None, updated_up_to: str = None):
        ...
        if created_since:
            endpoint = f'{endpoint}createdDateStartAt={created_since}&'
        if created_up_to:
            endpoint = f'{endpoint}createdDateEndAt={created_up_to}&'
        if updated_since:
            endpoint = f'{endpoint}updatedDateStartAt={updated_since}&'
        if updated_up_to:
            endpoint = f'{endpoint}updatedDateEndAt={updated_up_to}&'
        continue_loop = True
        df = pd.DataFrame()
        full_url = endpoint
        while continue_loop:
            ...
            if next_cursor:
                full_url = f'{endpoint}next={next_cursor}'
            else:
                continue_loop = False
        return df
