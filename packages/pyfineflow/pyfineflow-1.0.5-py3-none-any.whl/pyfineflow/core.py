...
from .utils import parse_func_info, NodeValue, type_parse, read_text_if_path_exit, recursive_update, get_md5_hash
...
class Fine:
    def __init__(self, mod: str, key: str = None, github_url=None, des=None, use_ws=False):
        if not key:
            ...
            self.key = get_md5_hash(github_url)
        else:
            self.key = key
    ...
    def save_nodes(self, name="nodes.json"):
        json.dump(self.nodes, open(name, 'w', encoding="utf-8"), ensure_ascii=False)
    ...
    def node(self, category: str = '', name: str = None, config: Node = {},
        ...
        def warp_func(func):
            ...
            node_conf['githubUrl'] = self.github_url
            node_conf['mod'] = self.mod
            node_conf["serverKey"] = self.key
    ...
    def init_router(self):
        router = APIRouter(prefix=f"/{self.key}")
    ...
    def init_server(self, host: str = "127.0.0.1", port: int = 8080):
        app = self.init_app()
        app.include_router(self.init_router())
...
class FinePlugins:
    ...
    def set_nodes_func(self, node_conf_map, node_func_map):
        self.node_conf_map = node_conf_map
        self.nodes = list(node_conf_map.values())
        self.node_func_map = node_func_map

    def init_router(self, prefix=""):
