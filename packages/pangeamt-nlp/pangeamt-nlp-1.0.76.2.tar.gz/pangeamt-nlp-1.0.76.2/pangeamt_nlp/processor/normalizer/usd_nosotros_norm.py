...
class UsdNosotrosNorm(NormalizerBase):
    ...
    def process_tgt_decoding(self, seg: Seg) -> None:
        logger.debug(f"SRC value -> {seg.src}")
        logger.debug(f"SRC_Raw value -> {seg.src_raw}")
        matches = _re.findall(self._regex, seg.src_raw)
        logger.debug(f"SRC matches -> {matches}")
        if len(matches)>0:
            logger.debug(f"Normalizing -> {seg.tgt}")
            seg.tgt = self.normalize(seg.tgt)
            logger.debug(f"Normalized -> {seg.tgt}")
