...
class PostRulesManager:
    ...
    def create_post_rules(self, name, action, description, from_zone, source_group,
                          to_zone, destination_group, service, profile_type, profile):
        params = {'name': name, 'location': 'shared', 'device-group': 'TestGroup'}
        payload = self._get_post_rules_payload(name, action, description, from_zone, source_group,
                                               to_zone, destination_group, service, profile_type, profile)
        response = requests.post(url=self.url, headers=self.headers, params=params, data=json.dumps(payload),
                                 verify=self.verify)
        return json.loads(response.content.decode('utf-8'))

    def _get_post_rules_payload(self, name, action, description, from_zone, source_group,
                          to_zone, destination_group, service, profile_type, profile):
