...
def start(as_standalone: bool = False) -> None:
    ...
    def load_job_config_file() -> None:
        job_config_file_path: str | None = A.SE.named_arg("admin")
        ...
        if not A.PTH.exists(job_config_file_path) and A.PTH.exists(job_config_file_default_path):
            job_config_file_path = job_config_file_default_path
            A.O.error("Config job file not found")
            A.O.blue(f"Used default config job file: {job_config_file_default_path}")
    ...
    A.SE.add_arg("admin", nargs="?", const="True", type=str, default="False")
    # A.SE.add_arg("job_config", nargs="?", type=str, help="Robocopy job config file")
    ...
    def service_call_handler(sc: SC, pl: ParameterList, context) -> dict:
        if sc == SC.heart_beat:
