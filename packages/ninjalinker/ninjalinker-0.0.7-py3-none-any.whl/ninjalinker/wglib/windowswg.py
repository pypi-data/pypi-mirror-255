...
class windowswireguard:
    ...
    def addip(self):
        ...
        try:
            ...
            # ]
            # ippanel = Panel("\n".join(ipmodify), title='SELFMADE-NINJA-ACADEMY-CLI-VPN', style='bold orchid')
            # console = Console()
    ...
    def add_device(self):
        try:
            ...
            print("[bold red]:Prohibited: Keyboard Interrupted [/bold red]")
    ...
    def configfilecreate(self):
        try:
            ...
            session = ob.getsess()  # Ensure ob is properly defined
            headers = {"Cookie": f"PHPSESSID={session}"}
            configresponse = requests.get(deviceiplabconf + encodepublickey, headers=headers)
            privatekey_replace = configresponse.text.replace("{private_key}", self.getwinprivatekey())
            finalconfig = privatekey_replace.replace("vpn.selfmade.ninja:44556", "106.51.76.75:44556")
            with open("C:\\Program Files\\WireGuard\\Win.conf", "w") as w:
                w.write(finalconfig)
        except Exception as e:
            print(f"An error occurred: {e}. Please configure and try again.")
    ...
    def wgconnect(self):
        if ob.getkey() is not None and is_admin():
            try:
                ...
                panel = Panel("\n".join(messages),title="SELFMADE-NINJA-ACADEMY-CLI-VPN",style="bold orchid")
                console.print(panel)
            ...
            uabletoconnect = Panel("\n".join(unabletolog),title='SELFMADE-NINJA-ACADEMY-CLI-VPN',style='bold orchid')
    ...
    def wgdisconnect(self):
        if ob.getkey() is not None and is_admin():
            try:
                ...
                disconnet_pannel = Panel("\n".join(disconnect_message),title='SELFMADE-NINJA-ACADEMY-CLI-VPN',style='bold orchid')
    ...
    def logout(self):
        ...
        if session is not None:
            ...
            gitlout = Panel("\n".join(gitloboutmessages),title="SELFMADE-NINJA-ACADEMY-CLI-VPN",style="bold orchid")
            console.print(gitlout)
        else:
            notinlabs = [
                "[bold green1]:small_blue_diamond: You are not [red1]Logged[/red1] in gitlabs :Prohibited:[/bold green1]",
            ]
            notlogedin = Panel("\n".join(notinlabs),title="SELFMADE-NINJA-ACADEMY-CLI-VPN",style="bold orchid")
