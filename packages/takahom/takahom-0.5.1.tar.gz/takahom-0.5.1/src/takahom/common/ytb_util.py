...
class YtbUtil:
    ...
    @classmethod
    def parse_ytbid_vcode_from_downloaded_filename(cls, dkf: DkFile | str) -> YtbId | None:
        basename = dkf.basename if isinstance(dkf, DkFile) else dkf
        if not basename.endswith('.mp4'):
            return None
        bname = basename[:-4]  # 去掉 .mp4 字符
...
102S202 宗教哲學[PLCX-BLZ1hDpCrvRshzjwJllCfqvW9k10n].description
        ...
        df = [df for df in DkFile2.oswalk_simple_a(down_dir, only_file=True)
        ...
        name_part = f_trim_by_accumulate(name_part, lmt_bytes, acc_len=lambda c: len(c.encode('utf-8')))
        max_screen_width = 25 * 2
        name_part = f_trim_by_accumulate(name_part, max_screen_width, acc_len=lambda c: 1 if c.isascii() else 2)
        r = name_part
