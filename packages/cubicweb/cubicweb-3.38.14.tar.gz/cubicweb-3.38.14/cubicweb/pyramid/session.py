...
class JSONSerializerWithPickleFallback:
    def __init__(self):
    ...
    class CWSession(SignedCookieSession):
        ...
        def _set_accessed(self, value):
            ...
            with unsafe_cnx_context_manager(self.request) as cnx:
                value_rset = cnx.execute(
                    "Any D WHERE X eid %(x)s, X cwsessiondata D", {"x": self.sessioneid}
                )
        ...
        def _set_cookie(self, response):
            ...
            with self.request.registry["cubicweb.repository"].internal_cnx() as cnx:
                if not sessioneid:
                    session = cnx.create_entity("CWSession", cwsessiondata=data)
            ...
            if self.new or (
                self._reissue_time is not None
                and self.accessed - self.renewed > self._reissue_time
            ):
                dict.clear(self)
                dict.__setitem__(self, "sessioneid", sessioneid)
                return super()._set_cookie(response)
