...
def _isUsingModule(moduleNames: List[str], pyProps: RuntimePythonProps) -> bool:
  nsModules = [n.split(".")[0] for n in (pyProps.namespaceModules or [])]
  for moduleName in moduleNames:
    if moduleName in nsModules:
      return True
  return False
...
def sourceForLoadingSecrets(pyProps: RuntimePythonProps) -> Optional[str]:
  ...
  if _isUsingModule(["openai", "llama_index", "langchain_openai"], pyProps):
    lines.append(_loadEnvVarSecretLine("OPENAI_API_KEY"))
  if _isUsingModule(["neptune"], pyProps):
    lines.append(_loadEnvVarSecretLine("NEPTUNE_API_TOKEN"))
  if _isUsingModule(["arize"], pyProps):
