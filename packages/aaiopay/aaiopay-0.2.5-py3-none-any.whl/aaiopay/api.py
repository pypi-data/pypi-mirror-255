...
class AaioPay(RequestsClient):
    ...
    async def generatepaymenturl(
        ...
        if not self.__secretkey or not self._shop:
            raise Exception('Не указан secret_key или shop_id')
    ...
    async def getOrderInfo(
        ...
        if not self._shop:
            raise Exception('Не указан shop_id.')
