...
from ._data import PosDecoderCtx, PosDecoderCarryData, PosDecoderData
from ._position import decode_positions, cpr_coordidnates

VecIndex: tp.TypeAlias = vmt.UInt32
...
def nuc_p(data: vmt.Message, tc_data: vmt.TypeCode) -> vmt.MaUInt32:
    ...
    return np.ma.column_stack([hpl, rcu, rcv])  # type: ignore
...
def position(
    ...
    crr_data = vstack(pd_prev.data, pos_msg)
    time_data = hstack(pd_prev.time, time_data[idx_pos])
    icao_data = hstack(pd_prev.icao, icao_data[idx_pos])
    tc_data = hstack(pd_prev.typecode, tc_data[idx_pos])
    ...
    pd = PosDecoderData(time_data, icao_data, idx_surface, cpr_fmt, cpr_coord)
    idx_valid, pos, new_last_pos, carry_over = decode_positions(ctx, pd)
    ...
    prev_positions: vmt.Position = positions[:pd_prev.size]  # type: ignore
    # positions for new message data
    new_positions: vmt.Position = positions[pd_prev.size:]  # type: ignore

    carry = PosDecoderCarryData(
        crr_data[carry_over],
    ...
    return vmt.PositionData(new_ctx, new_positions, prev_positions)
