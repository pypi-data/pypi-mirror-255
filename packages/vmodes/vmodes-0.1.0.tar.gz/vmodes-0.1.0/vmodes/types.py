...
# MArray: tp.TypeAlias = MaskedArray[tp.Any, np.dtype[T]]
class MArray(MaskedArray[tp.Any, np.dtype[T]]):
    def compressed(self) -> npt.NDArray[T]:
        return super().compressed()  # type: ignore[no-any-return, no-untyped-call]
...
LastPosition: tp.TypeAlias = dict[int, tp.Any]
...
class PositionData:
    ...
    :var prev_position: Aircraft position coordinates for carried over data.
    """
    ctx: PosDecoderCtx
    position: Position
