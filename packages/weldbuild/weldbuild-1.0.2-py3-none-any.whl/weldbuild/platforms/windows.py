...
class WindowsBuild(BuildPlatform):
    ...
    def build(self):
        ...
        try:
            ...
            cmd_path = os.path.join(data[0]['installationPath'], "Common7", "Tools", "VsDevCmd.bat")
        ...
        try:
            ...
            subprocess.call([
                ...
                "-B", "build",
                "-A", ARCH
            ], cwd=os.path.join(os.getcwd(), "build", "generated"), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            ...
            print(f" Out: {os.path.join(os.getcwd(), 'build', 'generated', 'build', 'Release', f'main.exe')}")
    ...
    def __package__(self):
        ...
        src_path = os.path.join(os.path.dirname(__file__), PlatformType.name(self.platform), "python", "src")
    ...
    def __create_project__(self, arch: ArchType):
        ...
        main_target.add_include_dirs([
            ...
            cmake_path(os.getcwd(), "build", "intermediate", "deps", Python(self.platform, self.py_version).path(), "include", "python3.11"),
            cmake_path(os.getcwd(), "build", "intermediate", "deps", LibZip(self.platform).path(), "include"),
            cmake_path(os.getcwd(), "build", "intermediate", "deps", ZLib(self.platform).path(), "include")
        ])
        main_target.add_link_dirs([
            cmake_path(os.getcwd(), "build", "intermediate", "deps", Python(self.platform, self.py_version).path(), "lib", ArchType.name(arch)),
            cmake_path(os.getcwd(), "build", "intermediate", "deps", LibZip(self.platform).path(), "lib", ArchType.name(arch)),
            cmake_path(os.getcwd(), "build", "intermediate", "deps", ZLib(self.platform).path(), "lib", ArchType.name(arch))
        ])
        main_target.add_libraries([
            "python311",
            "zip",
            "zlibstatic"
        ])
        main_target.add_precompile_headers([
            cmake_path("include", "precompiled.h")
        ])
        ...
        os.makedirs(os.path.join(os.getcwd(), "build", "generated", "resources"), exist_ok=True)
        shutil.copytree(
        ...
        shutil.copytree(
            os.path.join(os.path.join(os.path.dirname(__file__), PlatformType.name(self.platform), "cpp")), 
