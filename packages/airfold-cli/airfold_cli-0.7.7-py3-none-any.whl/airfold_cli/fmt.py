...
def fmt(
    ...
    if dry_run and overwrite:
        app.print_warning("--dry-run and --overwrite are mutually exclusive, ignoring --overwrite")
    ...
    for local_file in paths:
        ...
        if is_path_stream(local_file):
            app.print_warning(f"Reading from stdin is not allowed. Skipped.")
            continue
        ...
        yaml_data: str = dump_yaml([ff.data for ff in formatted_files], remove_names=len(formatted_files) == 1)
