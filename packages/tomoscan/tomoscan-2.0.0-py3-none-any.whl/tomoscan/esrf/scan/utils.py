...
def dataset_has_broken_vds(
    ...
    :param bool raise_error: if True and dataset not existing will raise an error
    :param bool return_unique_files: if True return unique files. As this step can be time consuming and reused it can sometimes be convenients
    """
    if not isinstance(url, DataUrl):
        raise TypeError(f"{url} is expected to be an instance of {DataUrl}")

    uniques_files = ()
    with HDF5File(url.file_path(), mode="r", swmr=True) as h5f:
...
def check_possible_issue_with_rlimit(
    url: Optional[DataUrl],
    raise_error=False,
    log_level=logging.WARNING,
    delta_n_file=0,
    substract_current_open=True,
    unique_files: Optional[tuple] = None,
) -> bool:
    """
