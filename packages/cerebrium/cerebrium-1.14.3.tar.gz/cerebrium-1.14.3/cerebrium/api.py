...
api_url = (
    ...
    else "https://dev-rest-api.cerebrium.ai"
    if env == "dev"
...
client_id = (
    ...
    else "207hg1caksrebuc79pcq1r3269"
    if env in ["dev", "local"]
...
auth_url = (
    ...
    else "https://dev-cerebrium.auth.eu-west-1.amazoncognito.com/oauth2/token"
    if env in ["dev", "local"]
...
def upload_cortex_files(
    ...
    file_list = [f for f in file_list if f not in files_to_remove]
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with zipfile.ZipFile(zip_path, "w") as zip_file:
            ...
            requirements_files = [
                ...
                ("pkglist.txt", pkglist),
                ("conda_pkglist.txt", conda_pkglist),
            ]

            for file_name, reqs in requirements_files:
                if reqs:
                    utils.requirements_to_file(reqs, os.path.join(temp_dir, file_name))
                    zip_file.write(os.path.join(temp_dir, file_name), arcname=file_name)
...
def log_build_status(
    ...
    # Default message
    msg = status_messages.get(build_status)
