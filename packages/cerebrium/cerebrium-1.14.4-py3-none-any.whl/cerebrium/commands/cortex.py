...
def init(
    ...
    if gpu:
        ...
        hardware = getattr(HardwareOptions, gpu).name
...
def build(
    ...
    build_status = package_app(cerebrium_config, OperationType.RUN)
    if "success" in build_status:
        console(
            Text(
...
def package_app(cerebrium_config: CerebriumConfig, type: OperationType):
    ...
    if utils.check_deployment_size(file_list, 100):
        if len(file_list) < 1000:
            ...
            cerebrium_config.local_files = sync_files.gather_hashes(file_list)
        else:
            print(
    ...
    files_hash = utils.content_hash(
        file_list, strings=[cerebrium_config.build.predict_data]
    )
    ...
    setup_response = setup_response.json()
    ...
    if build_status == "pending":
        if cerebrium_config.partial_upload:
            ...
            api.upload_cortex_files(
                ...
                file_list=file_list,
                disable_animation=cerebrium_config.build.disable_animation,
                predict_data=(
                    cerebrium_config.build.predict_data
                    if not cerebrium_config.build.disable_predict
                    else None
                ),
                requirements=cerebrium_config.dependencies.pip,
                pkglist=cerebrium_config.dependencies.apt,
                conda_pkglist=cerebrium_config.dependencies.conda,
            )
        ...
        # else:
        #     log_thread = threading.Thread(target=api.stream_logs, args=(start_event, stop_event, f'{setup_response["projectId"]}-{cerebrium_config.deployment.name}', setup_response['buildId']))
