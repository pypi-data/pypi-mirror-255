...
class DockerImage(collections.namedtuple('DockerImage', 'registry team artifact tag')):
    ...
    def parse(cls, image: str):
        ...
        '''
        # TODO support team/app/component
        parts = image.split('/')
        if len(parts) == 3:
            registry = parts[0]
        elif len(parts) < 2:
    ...
    def is_zcr_registry(self) -> bool:
        """
        Check if the registry is a ZCR registry
        """
        return self.registry == "container-registry.zalando.net" or self.registry.endswith(".container-registry.zalando.net")
