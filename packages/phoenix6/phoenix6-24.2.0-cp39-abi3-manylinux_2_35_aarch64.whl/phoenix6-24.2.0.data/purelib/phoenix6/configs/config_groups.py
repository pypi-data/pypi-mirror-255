...
class MagnetSensorConfigs:
    ...
    def __init__(self):
        ...
        """
        Direction of the sensor to determine positive facing the LED side of
        the CANcoder.
...
class FeedbackConfigs:
    ...
    def __init__(self):
        ...
        (note this requires setting FeedbackRemoteSensorID).  FusedCANcoder
        was developed for applications such as swerve-azimuth.  Choose
        SyncCANcoder (requires Phoenix Pro) and Talon FX will synchronize its
        internal rotor position against another CANcoder, then continue to use
        the rotor sensor for closed loop control (note this requires setting
        ...
        off.  Choose RemotePigeon2_Yaw, RemotePigeon2_Pitch, and
        RemotePigeon2_Roll to use another Pigeon2 on the same CAN bus (this
        also requires setting FeedbackRemoteSensorID).  Talon FX will update
        its position to match the selected value whenever Pigeon2 publishes
        its information on CAN bus. Note that the Talon FX position will be in
        rotations and not degrees.
        
        Note: When the Talon Source is changed to FusedCANcoder, the Talon
        needs a period of time to fuse before sensor-based (soft-limit, closed
        loop, etc.) features are used. This period of time is determined by
        the update frequency of the CANcoder's Position signal.
...
class DifferentialSensorsConfigs:
    ...
    def __init__(self):
        ...
        differential controller's primary targets.  Choose RemoteTalonFX_Diff
        to use another TalonFX on the same CAN bus.  Talon FX will update its
        ...
        will be in rotations and not degrees. Choose RemoteCANcoder to use
        another CANcoder on the same CAN bus (this also requires setting
        DifferentialRemoteSensorID).  Talon FX will update its differential
...
class HardwareLimitSwitchConfigs:
    ...
    def __init__(self):
        ...
        """
        If enabled, the position is auto-set to a specific value, specified by
        ...
        """
        If enabled, motor output is set to neutral when forward limit switch
        is asseted and positive output is requested.
...
class Slot0Configs:
    ...
    def __init__(self):
        ...
        voltage, or torque current. The sign is typically determined by
        reference velocity when using position and velocity closed loop modes.
        However, when using position closed loop with zero velocity reference
        (no motion profiling), application can instead use the position closed
        loop error by setting the Static Feedforward Sign configuration
        parameter.  In which case, we recommend the minimal amount of kS,
        otherwise the motor output may dither when closed loop error is near
        zero.
        ...
        type of gravity feedforward. The unit for this constant is dependent
        ...
        Elevator_Static for systems where the gravity feedforward is constant,
        such as an elevator. The gravity feedforward output will always have
        the same sign. Choose Arm_Cosine for systems where the gravity
        feedforward is dependent on the angular position of the mechanism,
        such as an arm. The gravity feedforward output will vary depending on
        the mechanism angular position. Note that the sensor offset and ratios
        must be configured so that the sensor reports a position of 0 when the
        ...
        modes. The default behavior uses the velocity feedforward sign. This
        works well with position closed loop when velocity reference is
        specified (motion profiling). However, when using position closed loop
        with zero velocity reference (no motion profiling), the application
        may want to apply static feedforward based on the closed loop error
        sign instead. In which case, we recommend the minimal amount of kS,
        otherwise the motor output may dither when closed loop error is near
        zero.
...
class Slot1Configs:
    ...
    def __init__(self):
        ...
        This determines the type of the gravity feedforward. Choose
        Elevator_Static for systems where the gravity feedforward is constant,
        such as an elevator. The gravity feedforward output will always be
        positive. Choose Arm_Cosine for systems where the gravity feedforward
        is dependent on the angular position of the mechanism, such as an arm.
        The gravity feedforward output will vary depending on the mechanism
