...
class DifferentialPositionDutyCycle:
    ...
    def __str__(self) -> str:
        ss = []
        ss.append("class: DifferentialPositionDutyCycle")
    ...
    def _send_request(self, network: str, device_hash: int, cancel_other_requests: bool) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlDifferentialPositionDutyCycle(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, cancel_other_requests, self.target_position, self.differential_position, self.enable_foc, self.target_slot, self.differential_slot, self.override_brake_dur_neutral, self.limit_forward_motion, self.limit_reverse_motion))
