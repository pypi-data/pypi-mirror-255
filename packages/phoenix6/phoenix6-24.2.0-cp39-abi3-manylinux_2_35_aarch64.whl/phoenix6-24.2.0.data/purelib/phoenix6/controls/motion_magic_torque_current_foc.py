...
class MotionMagicTorqueCurrentFOC:
    ...
    will do its best to adjust the profile.  This control mode is based on torque
    ...
    def _send_request(self, network: str, device_hash: int, cancel_other_requests: bool) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlMotionMagicTorqueCurrentFOC(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, cancel_other_requests, self.position, self.feed_forward, self.slot, self.override_coast_dur_neutral, self.limit_forward_motion, self.limit_reverse_motion))
