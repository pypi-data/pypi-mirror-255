...
class StaticFeedforwardSignValue(Enum):
    ...
    The default behavior uses the velocity feedforward sign. This works well with
    position closed loop when velocity reference is specified (motion profiling).
    ...
    closed loop error sign instead. In which case, we recommend the minimal amount
