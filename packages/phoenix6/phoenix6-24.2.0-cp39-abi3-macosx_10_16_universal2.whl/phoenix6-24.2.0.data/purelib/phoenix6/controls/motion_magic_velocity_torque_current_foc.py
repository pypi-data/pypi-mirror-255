...
class MotionMagicVelocityTorqueCurrentFOC:
    ...
    to honor the specified Acceleration and Jerk value. This control mode does not
    use the CruiseVelocity, Expo_kV, or Expo_kA configs. If the specified
    ...
    control mode is based on torque current, so relevant closed-loop gains will use
    ...
    def __init__(self, velocity: rotations_per_second, acceleration: rotations_per_second_squared = 0.0, enable_foc: bool = True, feed_forward: ampere = 0.0, slot: int = 0, override_coast_dur_neutral: bool = False, limit_forward_motion: bool = False, limit_reverse_motion: bool = False):
    ...
    def __str__(self) -> str:
        ss = []
        ss.append("class: MotionMagicVelocityTorqueCurrentFOC")
    ...
    def _send_request(self, network: str, device_hash: int, cancel_other_requests: bool) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlMotionMagicVelocityTorqueCurrentFOC(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, cancel_other_requests, self.velocity, self.acceleration, self.enable_foc, self.feed_forward, self.slot, self.override_coast_dur_neutral, self.limit_forward_motion, self.limit_reverse_motion))
