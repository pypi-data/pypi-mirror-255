...
class MotionMagicExpoVoltage:
    ...
    This control mode is voltage-based, so relevant closed-loop gains will use Volts
    for the numerator.
    ...
    def __str__(self) -> str:
        ss = []
        ss.append("class: MotionMagicExpoVoltage")
    ...
    def _send_request(self, network: str, device_hash: int, cancel_other_requests: bool) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlMotionMagicExpoVoltage(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, cancel_other_requests, self.position, self.enable_foc, self.feed_forward, self.slot, self.override_brake_dur_neutral, self.limit_forward_motion, self.limit_reverse_motion))
