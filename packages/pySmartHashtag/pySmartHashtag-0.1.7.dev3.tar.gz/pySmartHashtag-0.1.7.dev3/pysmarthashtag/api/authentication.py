...
class SmartAuthentication(httpx.Auth):
    ...
    async def _login(self):
        ...
        async with SmartLoginClient() as client:
            _LOGGER.warning(f"Aquiring access token from {CONTEXT_URL}.")
            # Get Context
            ...
            if "location" not in r_context.headers:
                raise SmartAPIError(
                    f"Could not get first redirect location from context page: {r_context.url} {r_context.status_code} {r_context.text}"
                )

            while ("location" in r_context.headers) and (
                "context" not in httpx.URL(r_context.headers["location"]).params
            ):
                _LOGGER.warning(
                    f"Another round to get context from login page: {r_context.url} {r_context.status_code} {r_context.text}"
                )
                r_context = await client.get(
                    r_context.headers["location"],
                    headers={
                        "x-app-id": "SmartAPPEU",
                        "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                        "accept-language": "de-DE,de;q=0.9,en-DE;q=0.8,en-US;q=0.7,en;q=0.6",
            ...
            context_header = httpx.URL(r_context.headers.get("location", ""))
            if "context" in context_header.params:
                context = context_header.params["context"]
            else:
                raise SmartAPIError(
                    f"Could not get context from login page: {r_context.url} {r_context.status_code} {r_context.text}"
                )
            ...
            try:
                ...
                raise SmartAPIError(f"Could not get login token from login page {r_login.text}")
            ...
            if "location" not in r_auth.headers:
                raise SmartAPIError(
                    f"Could not get location from auth page: {r_auth.url} {r_auth.status_code} {r_auth.text} {r_auth.headers}"
                )
            ...
            try:
                ...
                raise SmartAPIError(f"Could not get API access token from API: {r_api_access.text}")
