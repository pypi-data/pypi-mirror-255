...
class SmartAuthentication(httpx.Auth):
    ...
    async def _login(self):
        ...
        async with SmartLoginClient() as client:
            ...
            next_url = r_context.headers["location"]
            _LOGGER.warning(f"Aquiring access token step two from {next_url}.")
            r_context = await client.get(
                next_url,
                headers={
                    "x-app-id": "SmartAPPEU",
            ...
            try:
                context = httpx.URL(r_context.headers["location"]).params["context"]
                ...
                context_header = httpx.URL(r_context.headers["location"])
