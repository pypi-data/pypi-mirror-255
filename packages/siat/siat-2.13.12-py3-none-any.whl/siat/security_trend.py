...
def security_trend(ticker,indicator='Close', \
    ...
    if start in ['default','mrm','l1m']:  # 默认近一个月
        fromdate=date_adjust(todate,adjust=-31-15) #多几天有利于绘图坐标标示
    elif start in ['mrq','l3m']:  # 近三个月
        fromdate=date_adjust(todate,adjust=-31*4) #多一个月有利于绘图坐标标示
    elif start in ['l6m']:  # 近6个月
        fromdate=date_adjust(todate,adjust=-31*7)         
    elif start in ['mry','l12m']:  # 近一年
        fromdate=date_adjust(todate,adjust=-31*13)  
    elif start in ['l2y']:  # 近两年以来
        fromdate=date_adjust(todate,adjust=-31*25)  
    elif start in ['l3y']:  # 近三年以来
        fromdate=date_adjust(todate,adjust=-31*37)  
    elif start in ['l5y']:  # 近五年以来
        fromdate=date_adjust(todate,adjust=-31*61)  
    elif start in ['l10y']:  # 近十年以来
        fromdate=date_adjust(todate,adjust=-31*121)        
