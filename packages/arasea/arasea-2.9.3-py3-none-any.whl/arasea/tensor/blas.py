...
import arasea.scalar
from arasea.compile.mode import optdb
from arasea.configdefaults import config
from arasea.graph.basic import Apply, view_roots
from arasea.graph.features import ReplacementDidNotRemoveError, ReplaceValidate
...
from arasea.graph.rewriting.db import SequenceDB
from arasea.graph.utils import InconsistencyError, MethodNotDefined, TestValueError
from arasea.link.c.op import COp
from arasea.link.c.params_type import ParamsType
from arasea.printing import FunctionPrinter, debugprint, pprint
from arasea.scalar import bool as bool_t
from arasea.tensor import basic as at
from arasea.tensor.blas_headers import blas_header_text, blas_header_version
from arasea.tensor.elemwise import DimShuffle, Elemwise
from arasea.tensor.exceptions import NotScalarConstantError
from arasea.tensor.math import Dot, add, mul, neg, sub
from arasea.tensor.rewriting.elemwise import local_dimshuffle_lift
from arasea.tensor.shape import specify_broadcastable
...
from arasea.utils import memoize


_logger = logging.getLogger("arasea.tensor.blas")
...
class Gemm(GemmRelated):
    ...
    the context of arasea graphs. See L{BlasLapackSupport} for more
    ...
    def infer_shape(self, fgraph, node, input_shapes):
        ...
        return [
            (
                arasea.scalar.scalar_maximum(z_shape[0], x_shape[0]),
                arasea.scalar.scalar_maximum(z_shape[1], y_shape[1]),
            )
        ]
...
def _as_scalar(res, dtype=None):
    ...
    if all(s == 1 for s in res.type.shape):
        ...
        if rval.type.dtype in integer_dtypes:
            ...
            if arasea.scalar.upcast(res.dtype, dtype) == dtype:
...
def _gemm_from_factored_list(fgraph, lst):
    ...
    for sM in lst:
        ...
        if isinstance(sM, tuple):
            ...
            if arasea.scalar.upcast(sm0.dtype, sm1.dtype) == sm1.dtype:
...
class GemmOptimizer(GraphRewriter):
    ...
    def apply(self, fgraph):
        ...
        u = arasea.graph.rewriting.basic.DispatchingFeature(
        ...
        while did_something:
            nb_iter += 1
            t0 = time.perf_counter()
            nodelist = arasea.graph.basic.io_toposort(fgraph.inputs, fgraph.outputs)
...
def local_dot22_to_dot22scalar(fgraph, node):
    ...
    if not any(i_scalar):
        ...
        for i, x in enumerate(m.owner.inputs):
            if _as_scalar(x, dtype=d.dtype) and (
                arasea.scalar.upcast(x.type.dtype, d.type.dtype) == d.type.dtype
            ):
                scalar_idx = i
                break
    ...
    for i, x in enumerate(node.inputs):
        if (
            ...
            and (arasea.scalar.upcast(x.type.dtype, d.type.dtype) == d.type.dtype)
        ):
            scalar_idx = i
            break
    if scalar_idx < 0:
...
class BatchedDot(COp):
    ...
    def make_node(self, *inputs):
        ...
        dtype = arasea.scalar.upcast(*[input.type.dtype for input in inputs])
    ...
    def c_code_cache_version(self):
        from arasea.tensor.blas_headers import blas_header_version
    ...
    def R_op(self, inputs, eval_points):
        ...
        if test_values_enabled:
            try:
                iv0 = arasea.graph.op.get_test_value(inputs[0])
            except TestValueError:
                arasea.graph.op.missing_test_message(
                    "first input passed to BatchedDot.R_op has no test value"
                )
                test_values_enabled = False

            try:
                iv1 = arasea.graph.op.get_test_value(inputs[1])
            except TestValueError:
                arasea.graph.op.missing_test_message(
            ...
            if eval_points[0]:
                try:
                    ev0 = arasea.graph.op.get_test_value(eval_points[0])
                except TestValueError:
                    arasea.graph.op.missing_test_message(
            ...
            if eval_points[1]:
                try:
                    ev1 = arasea.graph.op.get_test_value(eval_points[1])
                except TestValueError:
                    arasea.graph.op.missing_test_message(
...
def batched_tensordot(x, y, axes=2):
    ...
    """
    from arasea.tensor.math import _tensordot_as_dot
