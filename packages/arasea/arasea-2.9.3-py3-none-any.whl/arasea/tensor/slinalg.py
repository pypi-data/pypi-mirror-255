...
from arasea.tensor import math as atm
from arasea.tensor.shape import reshape
from arasea.tensor.type import matrix, tensor, vector
...
class Eigvalsh(Op):
    ...
    def make_node(self, a, b):
        if b == arasea.tensor.type_other.NoneConst:
            a = as_tensor_variable(a)
            assert a.ndim == 2

            out_dtype = arasea.scalar.upcast(a.dtype)
            ...
            out_dtype = arasea.scalar.upcast(a.dtype, b.dtype)
            w = vector(dtype=out_dtype)
...
class EigvalshGrad(Op):
    ...
    # of arasea ops that is constructed directly in Eigvalsh.grad.
    ...
    def make_node(self, a, b, gw):
        ...
        out_dtype = arasea.scalar.upcast(a.dtype, b.dtype, gw.dtype)
...
class SolveContinuousLyapunov(Op):
    ...
    def make_node(self, A, B):
        ...
        out_dtype = arasea.scalar.upcast(A.dtype, B.dtype)
        X = arasea.tensor.matrix(dtype=out_dtype)

        return arasea.graph.basic.Apply(self, [A, B], [X])
...
class BilinearSolveDiscreteLyapunov(Op):
    ...
    def grad(self, inputs, output_grads):
        ...
        A_bar = arasea.tensor.linalg.matrix_dot(
            S, A, X.conj().T
        ) + arasea.tensor.linalg.matrix_dot(S.conj().T, A, X)
        Q_bar = S
        return [A_bar, Q_bar]
