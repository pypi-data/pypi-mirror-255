...
from arasea.scalar import ScalarVariable
...
from arasea.tensor.random.type import RandomGeneratorType, RandomStateType, RandomType
from arasea.tensor.random.utils import normalize_size_param, params_broadcast_shapes
from arasea.tensor.shape import shape_tuple
from arasea.tensor.type import TensorType, all_dtypes
...
def default_supp_shape_from_params(
    ...
    TODO: Consider using `arasea.compile.ops.shape_i` alongside `ShapeFeature`.
    ...
    dist_params: list of `arasea.graph.basic.Variable`
...
class RandomVariable(Op):
    ...
    def __init__(
        ...
        dtype: str (optional)
            ...
            given, then ``dtype`` is set to ``arasea.config.floatX``.  If
    ...
    def _infer_shape(
        ...
        if len(params_ind_slice) == 1:
            ...
            shape_ind = arasea.tensor.extra_ops.broadcast_shape_iter(
    ...
    def make_node(self, rng, size, dtype, *dist_params):
        ...
        dtype: str
            The dtype of the sampled output.  If the value ``"floatX"`` is
            given, then `dtype` is set to ``arasea.config.floatX``.  This value is
        ...
        if rng is None:
            rng = arasea.shared(np.random.default_rng())
    ...
    def grad(self, inputs, outputs):
        return [
            arasea.gradient.grad_undefined(
