...
from arasea.utils import apply_across_args
...
_logger = logging.getLogger("arasea.tensor.type")
...
dtype_specs_map = {
    ...
    "complex128": (complex, "arasea_complex128", "NPY_COMPLEX128"),
    "complex64": (complex, "arasea_complex64", "NPY_COMPLEX64"),
}
...
class TensorType(CType[np.ndarray], HasDataType, HasShape):
    ...
    def convert_variable(self, var):
        ...
        if (self.ndim == var.type.ndim) and (self.dtype == var.type.dtype):
            ...
            return arasea.tensor.specify_shape(var, self.shape)
...
def values_eq_approx(
    ...
    if isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
        ...
        if str(a.dtype) not in continuous_dtypes:
            ...
            cmp = arasea.tensor.math._allclose(a, b, rtol=rtol, atol=atol)
            if cmp:
...
arasea.compile.register_deep_copy_op_c_code(
    TensorType,
    """
...
def scalar(name=None, dtype=None):
    ...
    ----------
    dtype: numeric
        None means to use arasea.config.floatX.
    name
