...
def as_op(itypes, otypes, infer_shape=None):
    ...
    Examples
    --------
    @as_op(itypes=[arasea.tensor.fmatrix, arasea.tensor.fmatrix],
           otypes=[arasea.tensor.fmatrix])
    def numpy_dot(a, b):
...
class UpdatePlaceholder(Op):
    ...
    (i.e. the `updates` argument to `arasea.function` and/or updates specified via
