...
from arasea.tensor.type import discrete_dtypes, float_dtypes
...
class Binomial(Op):
    ...
    def grad(self, inputs, gout):
        ...
        comment_n = "No gradient exists for the number of samples in class\
                     Binomial of arasea/sparse/sandbox/sp2.py"
        comment_p = "No gradient exists for the prob of success in class\
                     Binomial of arasea/sparse/sandbox/sp2.py"
        return [
            arasea.gradient.grad_undefined(op=self, x_pos=0, x=n, comment=comment_n),
            arasea.gradient.grad_undefined(op=self, x_pos=1, x=p, comment=comment_p),
            arasea.gradient.disconnected_type(),
        ]
...
class Multinomial(Op):
    ...
    def grad(self, inputs, outputs_gradients):
        comment_n = "No gradient exists for the number of samples in class\
                     Multinomial of arasea/sparse/sandbox/sp2.py"
        comment_p = "No gradient exists for the prob of success in class\
                     Multinomial of arasea/sparse/sandbox/sp2.py"
        return [
            arasea.gradient.grad_undefined(
