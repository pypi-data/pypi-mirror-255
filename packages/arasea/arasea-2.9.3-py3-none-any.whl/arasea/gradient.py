...
import arasea
from arasea.compile.ops import ViewOp
from arasea.configdefaults import config
from arasea.graph import utils
from arasea.graph.basic import Apply, NominalVariable, Variable
from arasea.graph.null_type import NullType, null_type
from arasea.graph.op import get_test_values
...
def grad_not_implemented(op, x_pos, x, comment=""):
    ...
    any call to arasea.function involves this variable.
...
def Rop(
    ...
    if not isinstance(wrt, (list, tuple)):
        _wrt: List[Variable] = [arasea.tensor.as_tensor_variable(wrt)]
    else:
        _wrt = [arasea.tensor.as_tensor_variable(x) for x in wrt]

    if not isinstance(eval_points, (list, tuple)):
        _eval_points: List[Variable] = [arasea.tensor.as_tensor_variable(eval_points)]
    else:
        _eval_points = [arasea.tensor.as_tensor_variable(x) for x in eval_points]

    if not isinstance(f, (list, tuple)):
        _f: List[Variable] = [arasea.tensor.as_tensor_variable(f)]
    else:
        _f = [arasea.tensor.as_tensor_variable(x) for x in f]
        ...
        for x, y in zip(inputs, local_eval_points):
            if y is not None:
                ...
                if not isinstance(y, Variable):
                    y = arasea.tensor.as_tensor_variable(y)
                try:
                    ...
                    y = arasea.tensor.cast(y, x.type.dtype)
                    y = x.type.filter_variable(y)
    ...
    for out in _f:
        if out in _wrt:
            ...
            if return_disconnected.lower() == "zero":
                rval.append(arasea.tensor.zeros_like(out))
...
def grad(
    ...
    if cost is not None:
        ...
        if (
            ...
            and cost.type.dtype in arasea.tensor.type.continuous_dtypes
        ):
        ...
        if hasattr(g_cost.type, "dtype"):
            assert g_cost.type.dtype in arasea.tensor.type.continuous_dtypes
    ...
    for var in known_grads:
        ...
        if not hasattr(g_var, "type"):
            raise TypeError(
                "output grads must be arasea variables."
                f"Ambiguous whether {type(g_var)} should be made into tensor"
                " or sparse arasea variable"
            )
    ...
    for var in grad_dict:
        ...
        if hasattr(g.type, "dtype"):
            assert g.type.dtype in arasea.tensor.type.float_dtypes
...
def subgraph_grad(wrt, end, start=None, cost=None, details=False):
    ...
    .. code-block:: python
        ...
        x, t = arasea.tensor.fvector('x'), arasea.tensor.fvector('t')
        w1 = arasea.shared(np.random.standard_normal((3,4)))
        w2 = arasea.shared(np.random.standard_normal((4,2)))
        a1 = arasea.tensor.tanh(arasea.tensor.dot(x,w1))
        a2 = arasea.tensor.tanh(arasea.tensor.dot(a1,w2))
        cost2 = arasea.tensor.square(a2 - t).sum()
        cost2 += arasea.tensor.square(w2.sum())
        cost1 = arasea.tensor.square(w1.sum())
        ...
        param_grads = []
        for i in range(2):
            param_grad, next_grad = arasea.subgraph_grad(
    ...
    wrt : list of variables
        ...
        considered constant in arasea.grad).  For convenience, the
    ...
    cost : :class:`~arasea.graph.basic.Variable` scalar (0-dimensional) variable
...
def _populate_var_to_app_to_idx(outputs, wrt, consider_constant):
    ...
    if consider_constant is None:
        ...
        # structure with arasea variables at the leaves, only the root will
    ...
    def access_term_cache(node):
        ...
        if node not in term_dict:
            ...
            output_is_int = [
                ...
                and output.type.dtype in arasea.tensor.type.discrete_dtypes
            ...
            if True not in inputs_connected:
                ...
                for o, og in zip(node.outputs, output_grads):
                    ...
                    if (
                        o_dt not in arasea.tensor.type.discrete_dtypes
                        and og_dt
                        and o_dt != og_dt
                    ):
                ...
                for o, ng in zip(node.outputs, new_output_grads):
                    ...
                    if (
                        ng_dt is not None
                        and o_dt not in arasea.tensor.type.discrete_dtypes
                    ):
                        assert ng_dt == o_dt

                assert all(
                    getattr(ng.type, "dtype", None)
                    not in arasea.tensor.type.discrete_dtypes
            ...
            for i, term in enumerate(input_grads):
                ...
                if not isinstance(term.type, (NullType, DisconnectedType)):
                    if term.type.dtype not in arasea.tensor.type.float_dtypes:
                        raise TypeError(
                    ...
                    if only_connected_to_int[i]:
                        ...
                        if is_zero == "maybe":
                            msg = (
                                ...
                                "DisconnectedType and arasea can't "
                            ...
                            msg = (
                                ...
                                f"{arasea.get_scalar_constant_value(term)}."
                            )
...
def _float_ones_like(x):
    ...
    dtype = x.type.dtype
    if dtype not in arasea.tensor.type.float_dtypes:
        dtype = config.floatX
...
def mode_not_slow(mode):
    ...
    from arasea.compile.mode import FAST_RUN, get_mode
...
def verify_grad(
    ...
    Examples
    --------
    >>> verify_grad(arasea.tensor.tanh,
    ...
    tensor_pt = []
    for i, p in enumerate(pt):
        p_t = arasea.tensor.as_tensor_variable(p).type()
        p_t.name = f"input {i}"
    ...
    cost = arasea.tensor.sum(t_r * o_output)
...
def jacobian(expression, wrt, consider_constant=None, disconnected_inputs="raise"):
    ...
    Parameters
    ----------
    expression : Vector (1-dimensional) :class:`~arasea.graph.basic.Variable`
        Values that we are differentiating (that we want the Jacobian of)
    wrt : :class:`~arasea.graph.basic.Variable` or list of Variables
    ...
    Returns
    -------
    :class:`~arasea.graph.basic.Variable` or list/tuple of Variables (depending upon `wrt`)
    ...
    jacobs, updates = arasea.scan(
        inner_function,
        sequences=arasea.tensor.arange(expression.shape[0]),
...
def hessian(cost, wrt, consider_constant=None, disconnected_inputs="raise"):
    ...
    Returns
    -------
    :class:`~arasea.graph.basic.Variable` or list/tuple of Variables
    ...
    for input in wrt:
        ...
        hess, updates = arasea.scan(
            ...
            sequences=arasea.tensor.arange(expr.shape[0]),
...
def _is_zero(x):
    ...
    try:
        constant_value = arasea.get_scalar_constant_value(x)
        no_constant_value = False
    except arasea.tensor.exceptions.NotScalarConstantError:
        pass
...
def zero_grad(x):
    ...
    Parameters
    ----------
    x: :class:`~arasea.graph.basic.Variable`
        A Aesara expression whose gradient should be truncated.

    Returns
    -------
    :class:`~arasea.graph.basic.Variable`
...
class GradClip(ViewOp):
    ...
    def grad(self, args, g_outs):
        return [
            arasea.tensor.clip(g_out, self.clip_lower_bound, self.clip_upper_bound)
            for g_out in g_outs
        ]
...
def grad_clip(x, lower_bound, upper_bound):
    ...
    Examples
    --------
    >>> x = arasea.tensor.type.scalar()
    >>> z = arasea.gradient.grad(grad_clip(x, -1, 1)**2, x)
    >>> z2 = arasea.gradient.grad(x**2, x)
    >>> f = arasea.function([x], outputs = [z, z2])
    >>> print(f(2.0))
    [array(1.0), array(4.0)]
...
def grad_scale(x, multiplier):
    ...
    Examples
    --------
    >>> x = arasea.tensor.fscalar()
    >>> fx = arasea.tensor.sin(x)
    >>> fp = arasea.grad(fx, wrt=x)
    >>> fprime = arasea.function([x], fp)
    >>> print(fprime(2))  # doctest: +ELLIPSIS
    -0.416...
    >>> f_inverse=grad_scale(fx, -1.)
    >>> fpp = arasea.grad(f_inverse, wrt=x)
    >>> fpprime = arasea.function([x], fpp)
