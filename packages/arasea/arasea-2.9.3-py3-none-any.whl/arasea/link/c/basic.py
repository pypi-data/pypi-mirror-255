...
from arasea.compile.compilelock import lock_ctx
...
from arasea.link.basic import Container, Linker, LocalLinker, PerformLinker
from arasea.link.c.cmodule import (
    METH_VARARGS,
    DynamicModule,
    ExtFunction,
    GCC_compiler,
...
from arasea.link.c.cmodule import get_module_cache as _get_module_cache
from arasea.link.c.interface import CLinkerObject, CLinkerOp, CLinkerType
from arasea.link.utils import gc_helper, map_storage, raise_with_op, streamline
from arasea.utils import difference, uniq


if TYPE_CHECKING:
    from arasea.graph.fg import FunctionGraph
    from arasea.link.c.cmodule import ModuleCache

_logger = logging.getLogger("arasea.link.c.basic")
...
class CLinker(Linker):
    ...
    def fetch_variables(self):
        ...
        # C type constants (arasea.scalar.ScalarType). They don't request an object
        self.consts = []
        # Move c type from orphans (arasea.scalar.ScalarType) to self.consts
    ...
    def cmodule_key(self):
        ...
        of arasea.config (for all config options where "in_c_key" is True).
        ...
        def in_sig(i, topological_pos, i_idx):
            ...
            if isinstance(i, AtomicVariable):  # orphans
                if id(i) not in constant_ids:
                    ...
                    # files up to 80M.
                    if hasattr(isig[0], "arasea_hash"):
                        isig = (isig[0].arasea_hash(), topological_pos, i_idx)
                    try:
                        hash(isig)
                    except Exception:
...
class _CThunk:
    ...
    def __init__(self, cthunk, init_tasks, tasks, error_storage, module):
        # Lazy import to avoid compilation when importing arasea.
        from arasea.link.c.cutils import run_cthunk  # noqa
