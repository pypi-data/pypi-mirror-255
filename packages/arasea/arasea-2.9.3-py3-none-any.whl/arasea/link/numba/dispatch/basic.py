...
from arasea.link.utils import (
    compile_function_src,
    fgraph_to_python,
    unique_name_generator,
)
from arasea.scalar.basic import ScalarType
from arasea.scalar.math import Softplus
from arasea.tensor.blas import BatchedDot
from arasea.tensor.math import Dot
from arasea.tensor.shape import Reshape, Shape, Shape_i, SpecifyShape
from arasea.tensor.slinalg import Cholesky, Solve
...
from arasea.tensor.type_other import MakeSlice, NoneConst


if TYPE_CHECKING:
    from arasea.graph.basic import Variable
    from arasea.graph.op import StorageMapType
...
@singledispatch
def get_numba_type(arasea_type: Type, var: "Variable", **kwargs) -> numba.types.Type:
    ...
    var
        The :class:`Variable` corresponding to `arasea_type`.
    """
    return numba.types.pyobject


@get_numba_type.register(ScalarType)
def get_numba_type_ScalarType(arasea_type, var, **kwargs):
    dtype = np.dtype(arasea_type.dtype)
...
def get_numba_type_TensorType(
    ...
    """
    dtype = arasea_type.numpy_dtype
    numba_dtype = numba.from_dtype(dtype)
    if force_scalar or (reduce_to_scalar and getattr(arasea_type, "ndim", None) == 0):
        return numba_dtype

    readonly = getattr(var.tag, "indestructible", False)

    return numba.types.Array(numba_dtype, arasea_type.ndim, layout, readonly=readonly)
...
# NOTE: The remaining `arasea.tensor.blas` `Op`s appear unnecessary, because
