...
import arasea
import arasea.configparser
from arasea.configparser import (
    BoolParam,
    ConfigParam,
    DeviceParam,
    EnumStr,
...
_logger = logging.getLogger("arasea.configdefaults")
...
def _filter_mode(val):
    ...
    # arasea.compile.mode.Mode is not always available.
    # Instead of isinstance(val, arasea.compile.mode.Mode),
    # we can inspect the __mro__ of the object!
    for type_ in type(val).__mro__:
        if "arasea.compile.mode.Mode" in str(type_):
            return val
    raise ValueError(
        f"Expected one of {str_options}, or an instance of arasea.compile.mode.Mode. "
        f"Instead got: {val}."
    )
...
def add_compile_configvars():
    ...
    if rc == 0 and config.cxx != "":
        ...
        config.add(
            "linker",
            "Default linker used if the arasea flags mode is Mode",
            EnumStr(
    ...
    config.add(
        ...
        " arasea.function() is not used in the graph.",
...
def add_tensor_configvars():
    ...
    config.add(
        "tensor__cmp_sloppy",
        "Relax arasea.tensor.math._allclose (0) not at all, (1) a bit, (2) more",
...
def add_traceback_configvars():
    config.add(
        ...
        # We need to default to 8 to support arasea.tensor.type.tensor(...).
        # import arasea, numpy
...
def add_multiprocessing_configvars():
    ...
    config.add(
        ...
        "via the Aesara configuration file ~/.arasearc or with the "
...
def _filter_compiledir(path):
    ...
    if os.access(path, os.F_OK):  # Do it exist?
        ...
        try:
            ...
            # Maybe another parallel execution of arasea was trying to create
...
_compiledir_format_dict = {
    ...
    "arasea_version": arasea.__version__,
...
def add_caching_dir_configvars():
    ...
    if sys.platform == "win32" and os.getenv("LOCALAPPDATA") is not None:
        ...
        default_base_compiledir = os.path.join(_get_home_dir(), ".arasea")
...
# instruct one to use `arasea.config`, which is an alias for
# `arasea.configdefaults.config`.
config = arasea.configparser._config
