...
from arasea.graph.basic import Apply, AtomicVariable, Variable, applys_between
from arasea.graph.basic import as_string as graph_as_string
from arasea.graph.basic import clone_get_equiv, graph_inputs, io_toposort, vars_between
from arasea.graph.features import AlreadyThere, Feature, ReplaceValidate
from arasea.graph.utils import MetaObject, MissingInputError, TestValueError
from arasea.misc.ordered_set import OrderedSet
...
class FunctionGraph(MetaObject):
    ...
    def add_input(self, var: Variable, check: bool = True) -> None:
        ...
        Parameters
        ----------
        var : arasea.graph.basic.Variable
    ...
    def import_var(
        ...
        Parameters
        ----------
        variable : arasea.graph.basic.Variable
    ...
    def replace(
        ...
        if config.compute_test_value != "off":
            try:
                tval = arasea.graph.op.get_test_value(var)
                new_tval = arasea.graph.op.get_test_value(new_var)
