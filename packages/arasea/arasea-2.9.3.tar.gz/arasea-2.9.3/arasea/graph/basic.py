...
from arasea.graph.utils import (
    MetaObject,
    MethodNotDefined,
    Scratchpad,
    TestValueError,
...
class Apply(Node, Generic[OpType]):
    ...
    The function `arasea.compile.function.function` uses `Apply.inputs`
...
class Variable(Node, Generic[_TypeType, OptionalApplyType]):
    ...
    .. code-block:: python
        ...
        f = arasea.function([b], [c])   # this works because a has a value associated with it already

        assert 4.0 == f(2.5)            # bind 2.5 to an internal copy of b and evaluate an internal c

        arasea.function([a], [c])       # compilation error because b (required by c) is undefined

        arasea.function([a,b], [c])     # compilation error because a is constant, it can't be an input
