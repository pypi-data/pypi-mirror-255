...
returns a callable object.  One of arasea's most important features is
...
# arasea code, since this code may want to log some messages.
import logging
import os
import sys
...
arasea_logger = logging.getLogger("arasea")
logging_default_handler = logging.StreamHandler()
logging_default_formatter = logging.Formatter(
    fmt="%(levelname)s (%(name)s): %(message)s"
)
logging_default_handler.setFormatter(logging_default_formatter)
arasea_logger.setLevel(logging.WARNING)

if not arasea_logger.hasHandlers():
    arasea_logger.addHandler(logging_default_handler)


# Disable default log handler added to arasea_logger when the module
# is imported.
def disable_log_handler(logger=arasea_logger, handler=logging_default_handler):
    if logger.hasHandlers():
        logger.removeHandler(handler)


# Version information.
from arasea.version import version as __version__


# Raise a meaningful warning/error if the arasea directory is in the Python
# path.
rpath = os.path.realpath(__path__[0])
for p in sys.path:
    if os.path.realpath(p) != rpath:
        continue
    raise RuntimeError("You have the arasea directory in your Python path.")
...
# isort: off
from arasea.graph.basic import Variable, clone_replace
...
# isort: off
from arasea import scalar, tensor
from arasea.compile import (
    In,
    Mode,
    Out,
    ProfileStats,
...
from arasea.compile.function import function, function_dump
from arasea.compile.function.types import FunctionMaker
from arasea.gradient import Lop, Rop, grad, subgraph_grad
from arasea.printing import debugprint as dprint
from arasea.printing import pp, pprint
from arasea.updates import OrderedUpdates
...
def get_scalar_constant_value(v):
    ...
    If ``arasea.sparse`` is also there, we will look over CSM `Op`.

    If `v` is not some view of constant data, then raise a
    `NotScalarConstantError`.
    """
    # Is it necessary to test for presence of arasea.sparse at runtime?
...
# isort: off
import arasea.tensor.random.var
import arasea.sparse
from arasea.scan import checkpoints
from arasea.scan.basic import scan
from arasea.scan.views import foldl, foldr, map, reduce
...
DEPRECATED_NAMES = [
    (
        "change_flags",
        "`arasea.change_flags` is deprecated: use `arasea.config.change_flags` instead.",
        config.change_flags,
    ),
]
