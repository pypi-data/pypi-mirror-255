...
from arasea.graph.basic import Variable, _TypeType
from arasea.graph.utils import add_tag_trace
...
class SharedVariable(Variable[_TypeType, None]):
    ...
    def default_update(self) -> Optional[Variable]:
        ...
        (see `arasea.function`) for this `Variable` when no updates are
        provided through `arasea.function` and `no_default_updates` isn't
        enabled.
        """
...
def shared(value, name=None, strict=False, allow_downcast=None, **kwargs):
    ...
    `arasea.shared` is a shortcut to this function.
    ...
    try:
        ...
        e.args = e.args + ("Consider using `arasea.shared(..., borrow=True)`",)
        raise
