"""
Provides `DebugMode`, an evaluation mode for debugging arasea internals.
...
import arasea
from arasea.compile.function.types import (
    Function,
    FunctionMaker,
    infer_reuse_pattern,
    std_fgraph,
)
from arasea.compile.mode import Mode, register_mode
from arasea.compile.ops import OutputGuard, _output_guard
from arasea.configdefaults import config
from arasea.graph.basic import Variable, io_toposort
from arasea.graph.destroyhandler import DestroyHandler
from arasea.graph.features import AlreadyThere, BadOptimization, Feature
from arasea.graph.op import HasInnerGraph, Op
from arasea.graph.utils import InconsistencyError, MethodNotDefined
from arasea.link.basic import Container, LocalLinker
from arasea.link.c.op import COp
from arasea.link.utils import map_storage, raise_with_op
from arasea.printing import _debugprint
from arasea.tensor import TensorType
from arasea.utils import NoDuplicateOptWarningFilter, difference, get_unbound_function


__docformat__ = "restructuredtext en"
_logger: Logger = logging.getLogger("arasea.compile.debugmode")
_logger.addFilter(NoDuplicateOptWarningFilter())


class DebugModeError(Exception):
    """
    Generic Exception raised to indicate an internal arasea problem.
...
def str_diagnostic(expected, value, rtol, atol):
    ...
    try:
        ...
        pass
    atol_, rtol_ = arasea.tensor.math._get_atol_rtol(expected, value)
    if rtol is not None:
        rtol_ = rtol
    if atol is not None:
        atol_ = atol
...
def _check_viewmap(fgraph, node, storage_map):
    ...
    for oi, onode in enumerate(node.outputs):
        ...
        # In theory, arasea's view_map only allows for 1 output to
...
def _lessbroken_deepcopy(a):
    ...
    from arasea.link.c.type import _cdata_type
    ...
    try:
        ...
        if isinstance(getattr(node, "op", None), HasInnerGraph):
            ...
            if not fn or not hasattr(fn, "maker") or not hasattr(fn.maker, "mode"):
                _logger.warning(f"Expected arasea function not found in {node.op}.fn")
            else:
...
class _Maker(FunctionMaker):  # inheritance buys a few helper functions
    ...
    output_keys
        If the outputs argument for arasea.function was a list, then
    ...
    def __init__(
        ...
        for i in range(mode.stability_patience):
            ...
            with config.change_flags(compute_test_value=config.compute_test_value_opt):
                ...
                arasea.compile.function.types.insert_deepcopy(
...
class DebugMode(Mode):
    """
    Evaluation Mode that detects internal arasea errors.
