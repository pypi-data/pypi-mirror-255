...
from arasea.compile.function.types import Function, UnusedInputError, orig_function
...
from arasea.graph.basic import Constant, Variable, clone_node_and_cache
    ...
    if isinstance(outputs, list):
        ...
        for v in outputs:
            if isinstance(v, Variable):
                ...
                raise TypeError(
                    "Outputs must be arasea Variable or "
...
def pfunc(
    ...
    mode : string or `arasea.compile.mode.Mode` instance
        Compilation mode.
...
def construct_pfunc_ins_and_outs(
    ...
    inputs), and then shipping it off to arasea.compile.function.function
    ...
    # Transform params into arasea.compile.In objects.
    inputs = [
    ...
    for i, v in enumerate(in_variables):
        if v in in_variables[(i + 1) :]:
            ...
            raise UnusedInputError(
                f"Variable {v} is used twice in inputs to arasea.function, "
    ...
    for x, y in givens_pairs:
        if x in in_var_set:
            raise RuntimeError(
                ...
                "`arasea.function([y], f(x), givens={x: g(y)})`. Another "
                "solution consists in using `arasea.clone_replace`, e.g. like this: "
                "`arasea.function([x], "
                "arasea.clone_replace(f(x), replace={x: g(x)}))`."
            )
