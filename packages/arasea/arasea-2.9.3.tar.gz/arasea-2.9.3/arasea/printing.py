...
from arasea.compile import Function, SharedVariable
...
from arasea.graph.basic import Apply, Constant, Variable, graph_inputs, io_toposort
from arasea.graph.fg import FunctionGraph
from arasea.graph.op import HasInnerGraph, Op, StorageMapType
from arasea.graph.utils import Scratchpad
...
_logger = logging.getLogger("arasea.printing")
...
def pydotprint(
    ...
    """Print to a file the graph of a compiled arasea function's ops. Supports
    ...
    :param return_image: If True, it will create the image and return it.
        ...
        .. code-block:: python
            ...
            import arasea
            v = arasea.tensor.vector()
            from IPython.display import SVG
            SVG(arasea.printing.pydotprint(v*2, return_image=True,
                                           format='svg'))
    ...
    if outfile is None:
        outfile = os.path.join(
            config.compiledir, "arasea.pydotprint." + config.device + "." + format
        )
...
def get_node_by_id(
    ...
    """
    from arasea.printing import debugprint
