...
import arasea
import arasea.scalar.sharedvar
from arasea import compile, config, printing
from arasea import scalar as aes
from arasea.gradient import DisconnectedType, grad_not_implemented, grad_undefined
from arasea.graph.basic import Apply, Constant, Variable
...
from arasea.graph.type import HasShape, Type
from arasea.link.c.op import COp
from arasea.link.c.params_type import ParamsType
from arasea.misc.safe_asarray import _asarray
from arasea.printing import Printer, min_informative_str, pprint, set_precedence
from arasea.raise_op import CheckAndRaise, assert_op
from arasea.scalar import int32
from arasea.scalar.basic import ScalarConstant, ScalarVariable
from arasea.tensor import (
    _as_tensor_variable,
    _get_vector_length,
    as_tensor_variable,
    get_vector_length,
)
from arasea.tensor.elemwise import DimShuffle, Elemwise, scalar_elemwise
from arasea.tensor.exceptions import NotScalarConstantError
from arasea.tensor.shape import (
    Shape,
    Shape_i,
    Unbroadcast,
    shape,
...
from arasea.tensor.var import TensorConstant, TensorVariable, get_unique_value


if TYPE_CHECKING:
    from numpy.typing import NDArray

    from arasea.tensor import TensorLike
...
def _as_tensor_bool(x, name, ndim, **kwargs):
    raise TypeError(
        ...
        "use arasea.tensor.eq(v, w) instead."
    )
...
def get_scalar_constant_value(
    ...
    while True:
        ...
        if isinstance(v, Constant):
            ...
            from arasea.sparse.type import SparseTensorType
        ...
        if not only_process_constants and getattr(v, "owner", None) and max_recur > 0:
            ...
            # don't disable the check for Second.
                ...
                isinstance(v.owner.op, arasea.tensor.subtensor.Subtensor)
                and v.ndim == 0
            ):
                if isinstance(v.owner.inputs[0], TensorConstant):
                    from arasea.tensor.subtensor import get_constant_idx
                ...
                # arasea/tensor/tests/test_sharedvar.py:
...
class TensorFromScalar(COp):
    ...
    def grad(self, inp, grads):
        ...
        # Currently, arasea.grad insists that the dtype of the returned
...
def infer_static_shape(
    ...
    """
    from arasea.tensor.rewriting.basic import topo_constant_folding
    from arasea.tensor.rewriting.shape import ShapeFeature
            ...
            if client[0] == "output":
                ...
                and isinstance(
                    ...
                    (
                        ...
                        # always exists.
                        arasea.tensor.subtensor.IncSubtensor,
                        arasea.tensor.subtensor.AdvancedIncSubtensor1,
                        arasea.tensor.subtensor.AdvancedIncSubtensor,
                        arasea.tensor.blas.Gemv,
                        arasea.tensor.blas_c.CGemv,
                        arasea.tensor.blas.Ger,
                        arasea.tensor.blas_c.CGer,
                        arasea.tensor.blas_scipy.ScipyGer,
                    ),
                )
            ):
                return False
...
class MakeVector(COp):
    ...
    def perform(self, node, inputs, out_):
        (out,) = out_
        # not calling arasea._asarray as optimization
...
class Split(COp):
    ...
    def infer_shape(self, fgraph, node, in_shapes):
        ...
        for i in range(self.len_splits):
            ...
            temp = arasea.tensor.subtensor.set_subtensor(temp[axis], splits[i])
...
class Join(COp):
    ...
    def infer_shape(self, fgraph, node, ishapes):
        from arasea.tensor.math import eq, ge
...
def stack(*tensors, **kwargs):
    ...
    Examples
    --------
    >>> a = arasea.tensor.type.scalar()
    >>> b = arasea.tensor.type.scalar()
    >>> c = arasea.tensor.type.scalar()
    >>> x = arasea.tensor.stack([a, b, c])
    >>> x.ndim # x is a vector of length 3.
    1
    >>> a = arasea.tensor.type.tensor4()
    >>> b = arasea.tensor.type.tensor4()
    >>> c = arasea.tensor.type.tensor4()
    >>> x = arasea.tensor.stack([a, b, c])
    ...
    (3, 2, 2, 2, 2)
    >>> x = arasea.tensor.stack([a, b, c], axis=3)
    >>> x.ndim
    5
    >>> rval = x.eval(dict((t, np.zeros((2, 2, 2, 2))) for t in [a, b, c]))
    >>> rval.shape # 3 tensors are stacked on axis 3
    (2, 2, 2, 3, 2)
    >>> x = arasea.tensor.stack([a, b, c], axis=-2)
    >>> x.ndim
    5
    >>> rval = x.eval(dict((t, np.zeros((2, 2, 2, 2))) for t in [a, b, c]))
...
def is_flat(var, ndim=None, outdim=None):
    ...
    Parameters
    ----------
        var : arasea.tensor.var.TensorVariable
            the arasea var on which the dimensionality is checked.
...
def flatten(x, ndim=1):
    ...
    Parameters
    ----------
    x : arasea.tensor.var.TensorVariable
    ...
    Returns
    -------
    arasea.tensor.var.TensorVariable
...
def tile(x, reps, ndim=None):
    ...
    """
    from arasea.tensor.math import ge
...
class ARange(Op):
    ...
    def infer_shape(self, fgraph, node, i_shapes):
        from arasea.tensor.math import ceil, maximum
...
class PermuteRowElements(Op):
    ...
    def grad(self, inp, grads):
        from arasea.tensor.math import Sum, eq
...
class ExtractDiag(Op):
    ...
    def grad(self, inputs, gout):
        ...
        if x.ndim == 2:
            ...
            return [
                arasea.tensor.subtensor.set_subtensor(
    ...
    def infer_shape(self, fgraph, node, shapes):
        from arasea.tensor.math import clip, minimum
...
def stacklists(arg):
    ...
    Examples
    --------
    >>> from arasea.tensor import stacklists
    >>> from arasea.tensor.type import scalars, matrices
...
class Choose(Op):
    ...
    def infer_shape(self, fgraph, node, shapes):
        ...
        out_shape = arasea.tensor.extra_ops.broadcast_shape(
    ...
    def make_node(self, a, choices):
        ...
        if isinstance(choices, (tuple, list)):
            choice = arasea.typed_list.make_list(choices)
        else:
