...
import arasea
import arasea.scalar.basic as aes
from arasea.gradient import (
    DisconnectedType,
    _float_zeros_like,
    disconnected_type,
    grad_undefined,
)
from arasea.graph.basic import Apply, Constant, Variable, equal_computations
from arasea.graph.op import Op
from arasea.link.c.op import COp
from arasea.link.c.params_type import ParamsType
from arasea.link.c.type import EnumList, Generic
from arasea.misc.safe_asarray import _asarray
from arasea.raise_op import Assert
from arasea.scalar import int32 as int_t
from arasea.scalar import upcast
from arasea.scalar.basic import Composite
...
from arasea.tensor.math import ge, lt, maximum, minimum, prod
from arasea.tensor.math import sum as at_sum
from arasea.tensor.subtensor import advanced_inc_subtensor1, set_subtensor
from arasea.tensor.type import TensorType, dvector, int_dtypes, integer_dtypes, vector
...
def searchsorted(x, v, side="left", sorter=None):
    ...
    >>> from arasea.tensor import extra_ops
    >>> x = at.dvector()
    ...
    def make_node(self, a, val, offset):
        ...
        if val.dtype != a.dtype:
            ...
            raise TypeError(
                ...
                " use arasea.tensor.cast( input, 'int32/int64')"
            )
...
def to_one_hot(y, nb_class, dtype=None):
    ...
    dtype : data-type
        The dtype of the returned matrix. Default ``arasea.config.floatX``.
...
class Unique(Op):
    ...
    >>> x = arasea.tensor.vector()
    >>> f = arasea.function([x], Unique(True, True, False)(x))
    >>> f([1, 2., 3, 4, 3, 2, 1.])
    [array([ 1.,  2.,  3.,  4.]), array([0, 1, 2, 3]), array([0, 1, 2, 3, 2, 1, 0])]

    >>> y = arasea.tensor.matrix()
    >>> g = arasea.function([y], Unique(True, True, False)(y))
    >>> g([[1, 1, 1.0], (2, 3, 3.0)])
...
def broadcast_shape_iter(
    ...
    """
    one_at = arasea.scalar.ScalarConstant(arasea.scalar.int64, 1)
    ...
    for dim_shapes in zip(*array_shapes):
        ...
        if len(maybe_non_bcast_shapes) == 0:
            ...
            if len(const_nb_shapes) > 1:
                ...
                const_nt_shape_var = arasea.scalar.ScalarConstant(
                    arasea.scalar.int64, const_nb_shape
                )
...
def geomspace(start, end, steps, base=10.0):
    from arasea.tensor.math import log
