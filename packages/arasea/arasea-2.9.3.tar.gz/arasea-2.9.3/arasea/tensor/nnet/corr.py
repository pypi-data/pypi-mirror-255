...
from arasea.graph.op import _NoPythonOp
...
from arasea.scalar import int8, int64
from arasea.tensor import blas_headers
from arasea.tensor.basic import as_tensor_variable
from arasea.tensor.blas import blas_header_version, ldflags
from arasea.tensor.nnet.abstract_conv import get_conv_output_shape
...
class BaseCorrMM(OpenMPOp, _NoPythonOp):
    ...
    def as_common_dtype(in1, in2):
        ...
        """
        dtype = arasea.scalar.upcast(in1.dtype, in2.dtype)
...
class CorrMM_gradWeights(BaseCorrMM):
    ...
    def grad(self, inp, grads):
        ...
        d_height_width = (
            (arasea.gradient.DisconnectedType()(),) * 2 if len(inp) == 4 else ()
        )
