...
from arasea import config, printing
...
from arasea.scalar.basic import BinaryScalarOp
...
from arasea.tensor.elemwise import CAReduce, DimShuffle, Elemwise, scalar_elemwise
...
from arasea.tensor.utils import as_list
...
if int(config.tensor__cmp_sloppy) > 1:
    ...
    # "arasea.tensor.math.float32_atol = ..."
...
def isclose(a, b, rtol=1.0e-5, atol=1.0e-8, equal_nan=False):
    ...
    >>> arasea.tensor.isclose(a, b).eval()
    ...
    >>> a = _asarray([1.0, np.nan], dtype="float64")
    >>> b = _asarray([1.0, np.nan], dtype="float64")
    >>> arasea.tensor.isclose(a, b, equal_nan=True).eval()
    array([1, 1], dtype==int8)
...
def round(a, mode=None):
    ...
    if mode is None:
        ...
        if config.warn__round:
            warnings.warn(
                "arasea.tensor.round() changed its default from"
...
class ProdWithoutZeros(CAReduce):
    ...
    def grad(self, inp, grads):
        from arasea.gradient import grad_not_implemented
...
class MatMul(Op):
    ...
    def _get_output_shape(cls, x1, x2, shapes, validate=False):
        ...
        if x1.ndim == 1 and x2.ndim == 1:
            ...
            if validate:
                from arasea.tensor.random.basic import broadcast_shapes
...
class Convolve(Op):
    ...
    def _get_output_shape(cls, a, v, shapes, mode, validate=False):
        ...
        from arasea.tensor.math import maximum, minimum
