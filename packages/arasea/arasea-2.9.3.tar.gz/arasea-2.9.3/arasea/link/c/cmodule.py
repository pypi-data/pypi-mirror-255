...
from arasea.configdefaults import config, gcc_version_str
from arasea.configparser import BoolParam, StrParam
from arasea.graph.op import Op
from arasea.link.c.exceptions import CompileError, MissingGXX
...
_logger = logging.getLogger("arasea.link.c.cmodule")
...
class DynamicModule:
    def __init__(self, name=None):
        ...
        self.support_code = []
        self.functions = []
        self.includes = ["<Python.h>", "<iostream>", '"arasea_mod_helper.h"']
        self.init_blocks = []
...
class GCC_compiler(Compiler):
    ...
    def compile_args(march_flags=True):
        ...
        if (
            ...
            warnings.warn(
                "`arasea.config.cxx` is not an identifiable `g++` compiler. "
        ...
        if sys.platform == "darwin":
            ...
            # https://github.com/arasea-devs/arasea/issues/127
    ...
    def compile_str(
        ...
        if status:
            tf = tempfile.NamedTemporaryFile(
                mode="w", prefix="arasea_compilation_error_", delete=False
            )
