...
from arasea.graph.op import ComputeMapType, Op, StorageMapType, ThunkType
from arasea.graph.type import HasDataType
from arasea.graph.utils import MethodNotDefined
from arasea.link.c.interface import CLinkerOp
from arasea.link.c.params_type import ParamsType
from arasea.utils import hash_from_code


if TYPE_CHECKING:
    from arasea.link.c.basic import _CThunk
...
class COp(Op, CLinkerOp):
    ...
    def make_c_thunk(
        ...
        #        in arasea.link.c and dispatched onto the Op!
        import arasea.link.c.basic
        ...
        cl = arasea.link.c.basic.CLinker().accept(e, no_recycling=e_no_recycling)
...
class OpenMPOp(COp):
    ...
    def test_gxx_support():
        ...
        from arasea.link.c.cmodule import GCC_compiler
            ...
            if OpenMPOp.gxx_support_openmp is None:
                ...
                if not OpenMPOp.gxx_support_openmp:
                    ...
                    warnings.warn(
                        ...
                        " To remove this warning set the arasea flags `openmp`"
                        " to False.",
                        stacklevel=3,
                    )
...
class ExternalCOp(COp):
    ...
    def __get_op_params(self) -> List[Tuple[str, Any]]:
        ...
        If op uses a :class:`arasea.graph.params_type.ParamsType` as ``params_type``,
        it returns:
         ...
         - a macro ``DTYPE_PARAM_key`` for every ``key`` in the :class:`ParamsType` for which associated
           type implements the method :func:`arasea.graph.type.CLinkerType.c_element_type`.
        ...
        if isinstance(self.params_type, ParamsType):
            ...
            for i in range(wrapper.length):
                ...
                if c_type:
                    ...
                    # (`arasea/graph/tests/test_quadratic_function.c`), to demonstrate how we can
