...
import arasea
import arasea.link.utils as link_utils
from arasea import tensor as at
from arasea.compile.builders import construct_nominal_fgraph, infer_shape
...
from arasea.compile.profiling import register_profiler_printer
from arasea.configdefaults import config
from arasea.gradient import DisconnectedType, NullType, Rop, grad, grad_undefined
...
from arasea.graph.features import NoOutputFromInplace
from arasea.graph.op import HasInnerGraph, Op
from arasea.graph.utils import InconsistencyError, MissingInputError
from arasea.link.c.basic import CLinker
from arasea.link.c.exceptions import MissingGXX
from arasea.printing import op_debug_information
from arasea.scan.utils import ScanProfileStats, Validator, forced_replace, safe_new
from arasea.tensor.basic import as_tensor_variable
from arasea.tensor.math import minimum
from arasea.tensor.shape import Shape_i
from arasea.tensor.type import TensorType, integer_dtypes
from arasea.tensor.var import TensorVariable


# Logging function for sending warning or info
_logger = logging.getLogger("arasea.scan.op")
...
def check_broadcast(v1, v2):
    ...
    msg = (
        ...
        "them consistent, e.g. using arasea.tensor."
...
class Scan(Op, ScanMethodsMixin, HasInnerGraph):
    ...
    def __init__(
        ...
        mode
            ...
            `arasea.function` for details about possible values and their meaning).
        typeConstructor
        ...
        allow_gc
            ...
            `arasea.config.scan__allow_gc`.

            The full `Scan` behavior related to allocation is determined by this
            value and the flag `arasea.config.allow_gc`. If the flag
            ...
            flag `arasea.config.allow_gc` means.
        strict
    ...
    def prepare_fgraph(self, fgraph):
        ...
        from arasea.compile.function.types import Supervisor
    ...
    def make_thunk(self, node, storage_map, compute_map, no_recycling, impl=None):
        ...
        try:
            ...
            from arasea.scan.utils import InnerFunctionError
    ...
    def connection_pattern(self, node):
        ...
        # where calls to arasea.grad() took as much as 4h for functions
    ...
    def L_op(self, inputs, outs, dC_douts):
        ...
        for idx, Xt in enumerate(diff_outputs):
            ...
            if idx < idx_nitsot_start or idx >= idx_nitsot_end:
                ...
                if dtypes:
                    new_dtype = arasea.scalar.upcast(*dtypes)
                else:
        ...
        for (fgraph, node), v in apply_time.items():
            if isinstance(node.op, Scan) and not node.op.fn.profile:
                print(
                    ...
                    "'arasea.function(..., profile=True)', you must manually"
                    " enable the profiling for each scan too: "
                    "'arasea.scan(...,profile=True)'."
...
def _op_debug_information_Scan(op, node):
    ...
    from arasea.scan.utils import ScanArgs
