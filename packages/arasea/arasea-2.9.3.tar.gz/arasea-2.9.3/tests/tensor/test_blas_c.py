...
from arasea.tensor.basic import AllocEmpty
from arasea.tensor.blas import Ger
from arasea.tensor.blas_c import CGemv, CGer, check_force_gemv_init
from arasea.tensor.blas_scipy import ScipyGer
from arasea.tensor.type import dmatrix, dvector, matrix, scalar, tensor, vector
...
class TestCGer(OptimizationTestMixin):
    ...
    def manual_setup_method(self, dtype="float64"):
        # This tests can run even when arasea.config.blas__ldflags is empty.
        self.dtype = dtype
...
class TestCGemv(OptimizationTestMixin):
    ...
    def test_optimizations_vm(self):
        ...
        f = arasea.function([self.x, self.A], at.dot(self.x, self.A), mode=self.mode)
    ...
    def test_optimizations_mv(self):
        ...
        f = arasea.function([self.A, self.y], at.dot(self.A, self.y), mode=self.mode)
    ...
    def t_gemv1(self, m_shp):
        ...
        v1 = arasea.shared(np.array(rng.uniform(size=(m_shp[1],)), dtype="float32"))
        v2_orig = np.array(rng.uniform(size=(m_shp[0],)), dtype="float32")
        v2 = arasea.shared(v2_orig)
        m = arasea.shared(np.array(rng.uniform(size=m_shp), dtype="float32"))

        f = arasea.function([], v2 + at.dot(m, v1), mode=self.mode)
        ...
        g = arasea.function([], [], updates=[(v2, v2 + at.dot(m, v1))], mode=self.mode)
    ...
    def test_gemv_dimensions(self, dtype="float32"):
        alpha = arasea.shared(_asarray(1.0, dtype=dtype), name="alpha")
        beta = arasea.shared(_asarray(1.0, dtype=dtype), name="beta")

        z = beta * self.y + alpha * at.dot(self.A, self.x)
        f = arasea.function([self.A, self.x, self.y], z, mode=self.mode)
    ...
    def test_multiple_inplace(self):
        ...
        y = dvector("y")
        z = dvector("z")
        f = arasea.function([x, y, z], [at.dot(y, x), at.dot(z, x)], mode=mode_blas_opt)
...
class TestCGemvNoFlags:
    ...
    @arasea.config.change_flags(blas__ldflags="")
