...
import arasea.tensor.basic as at
import arasea.tensor.math as tm
from arasea import compile, config, function, shared
from arasea.compile.io import In, Out
from arasea.compile.mode import get_default_mode
from arasea.compile.ops import DeepCopyOp
from arasea.gradient import grad, hessian
from arasea.graph.basic import Apply
from arasea.graph.op import Op
from arasea.misc.safe_asarray import _asarray
from arasea.raise_op import Assert
from arasea.scalar import autocast_float, autocast_float_as
from arasea.tensor import NoneConst
from arasea.tensor.basic import (
    Alloc,
    AllocDiag,
    AllocEmpty,
    ARange,
...
from arasea.tensor.elemwise import DimShuffle
from arasea.tensor.exceptions import NotScalarConstantError
from arasea.tensor.math import dense_dot
from arasea.tensor.math import sum as at_sum
from arasea.tensor.shape import Reshape, Shape, Shape_i, shape_padright, specify_shape
...
from arasea.utils import PYTHON_INT_BITWIDTH
...
class TestMakeVector(utt.InferShapeTester):
    ...
    def test_make_vector(self, dtype, inputs):
        ...
        s = mv.sum()
        gb = arasea.gradient.grad(s, b, disconnected_inputs="ignore")
        gi = arasea.gradient.grad(s, i, disconnected_inputs="ignore")
        gd = arasea.gradient.grad(s, d, disconnected_inputs="ignore")
...
class TestAlloc:
    dtype = config.floatX
    mode = mode_opt
    shared = staticmethod(arasea.shared)
    allocs = [Alloc()] * 3
    ...
    def test_alloc_constant_folding(self):
        ...
        for alloc_, (subtensor, n_alloc) in zip(
            ...
            fobj = arasea.function([some_vector], derp, mode=self.mode)
            grad_derp = arasea.grad(derp, some_vector)
            fgrad = arasea.function([some_vector], grad_derp, mode=self.mode)
    ...
    def test_alloc_output(self):
        ...
        for alloc_ in self.allocs:
            ...
            f = arasea.function([], out, mode=self.mode)
    ...
    def test_ones(self):
        for shp in [[], 1, [1], [1, 2], [1, 2, 3], np.r_[1, 2, 3]]:
            ones = arasea.function([], [at.ones(shp)], mode=self.mode)
            assert np.allclose(ones(), np.ones(shp))
            # When shape is a TensorConstant
            ones_const = arasea.function(
    ...
    def test_zeros(self):
        ...
        x = scalar()
        shp = []
        zeros_scalar = arasea.function([], [at.zeros(x.shape)], mode=self.mode)
        assert np.allclose(zeros_scalar(), np.zeros(shp))

        for typ, shp in [(vector, [3]), (matrix, [3, 4])]:
            x = typ()
            zeros_tensor = arasea.function([x], [at.zeros(x.shape)], mode=self.mode)
            inp = np.zeros(shp, dtype=config.floatX)
            assert np.allclose(zeros_tensor(inp), np.zeros(shp))

    def test_full(self):
        full_at = at.full((2, 3), 3, dtype="int64")
        res = arasea.function([], full_at, mode=self.mode)()
...
def test_get_vector_length():
    ...
    a, b, c = triple
    mode = arasea.compile.get_default_mode().excluding("constant_folding")
...
class TestJoinAndSplit:
    ...
    def setup_method(self):
        ...
        self.mode = arasea.compile.get_default_mode().excluding("constant_folding")
        self.join_op = Join()
    ...
    def eval_outputs_and_check_join(self, outputs):
        f = arasea.function([], outputs, self.mode)
        topo = f.maker.fgraph.toposort()
    ...
    def test_stack_mixed_type_constants(self):
        ...
        c = arasea.shared(np.asarray(3.0, dtype=self.floatX))
        s = stack([a, b, c])
        want = np.array([1, 2, 3])
    ...
    def test_join_matrixC_negative_axis(self):
        ...
        s = join(-1, a, b)
        f = arasea.function([], [s], mode=self.mode)
        topo = f.maker.fgraph.toposort()
        assert [True for node in topo if isinstance(node.op, type(self.join_op))]
    ...
    def test_infer_shape_join(self):
        ...
        f = arasea.function([], z.shape, mode=self.mode)
        topo = f.maker.fgraph.toposort()
...
def test_join_inplace():
    ...
    f = arasea.function([In(x, borrow=True), s], Out(c, borrow=True))
...
class TestNdGrid:
    ...
    def test_mgrid_arasea_variable_numpy_equiv(self):
        ...
        ff = arasea.function([i, j, k], tfmgrid)
        fi = arasea.function([l, m, n], timgrid)
        for n, t in zip((nfmgrid, nimgrid), (ff(0, 10, 10.0), fi(0, 10, 10))):
            for ng, tg in zip(n, t):
                utt.assert_allclose(ng, tg)

    def test_ogrid_arasea_variable_numpy_equiv(self):
        ...
        ff = arasea.function([i, j, k], tfogrid)
        fi = arasea.function([l, m, n], tiogrid)
...
@pytest.mark.skipif(
    isinstance(get_default_mode(), arasea.compile.debugmode.DebugMode),
...
def test_unalign():
    ...
    av, bv = vectors("ab")
    f = arasea.function([av, bv], 2 * av + 3 * bv)
...
class TestGetScalarConstantValue:
    ...
    def test_shape_i(self):
        ...
        d = arasea.shared(np.random.standard_normal((1, 1)), shape=(1, 1))
...
class TestAllocDiag:
    def setup_method(self):
        ...
        self.mode = arasea.compile.mode.get_default_mode()
    ...
    def test_alloc_diag_values(self):
        for x, test_val in self._generator():
            for offset, axis1, axis2 in [
                ...
                f = arasea.function([x], adiag_op(x))
                ...
                # Test infer_shape
                f_shape = arasea.function([x], adiag_op(x).shape, mode="FAST_RUN")

                # arasea.printing.debugprint(f_shape.maker.fgraph.outputs[0])
                ...
                grad_x = arasea.grad(sum_diag_x, x)
                grad_diag_x = arasea.grad(sum_diag_x, diag_x)
                f_grad_x = arasea.function([x], grad_x, mode=self.mode)
                f_grad_diag_x = arasea.function([x], grad_diag_x, mode=self.mode)
...
class TestChoose(utt.InferShapeTester):
    ...
    def test_infer_shape(self):
        for shp1, shp2 in [
            ...
            self._compile_and_check(
                [a, c],  # arasea.function inputs
                [self.op(a, c)],  # arasea.function outputs
    ...
    def test_infer_shape_tuple(self):
        ...
        self._compile_and_check(
            [a, b, c],  # arasea.function inputs
            [self.op(a, (b, c))],  # arasea.function outputs
            # Always use not square matrix!
            # inputs data
            [A, B, C],
            # Op that should be removed from the graph.
...
def test_empty():
    ...
    f = arasea.function([], AllocEmpty("float32")(2, 3))
...
def test_atleast_Nd():
    ...
    for n in range(1, 3):
        ...
        res_ary1_val, res_ary2_val = arasea.function(
...
def test_expand_dims():
    ...
    res_val = arasea.function([x_at], res_at)(x_val)
...
class TestTakeAlongAxis:
    ...
    def test_take_along_axis(self, shape, axis, samples):
        ...
        func = arasea.function([arr_in, indices_in], out)
