...
from arasea import config, function
from arasea.compile.mode import Mode
from arasea.graph.rewriting.db import RewriteDatabaseQuery
from arasea.tensor.random.utils import RandomStream, broadcast_params
from arasea.tensor.type import matrix, tensor
...
class TestSharedRandomStream:
    ...
    def test_multiple_rng_aliasing(self, rng_ctor):
        ...
        # copy the (random) state between two similar arasea graphs. The test is
    ...
    def test_random_state_transfer(self, rng_ctor):
        # Test that random state can be transferred from one arasea graph to another.
