...
from arasea.graph.basic import Apply, Variable
from arasea.graph.fg import FunctionGraph
from arasea.link.basic import PerformLinker
from arasea.link.c.basic import CLinker, OpWiseCLinker
from arasea.tensor import as_tensor_variable
from arasea.tensor.basic import second
from arasea.tensor.elemwise import CAReduce, DimShuffle, Elemwise
from arasea.tensor.exceptions import ShapeError
from arasea.tensor.math import all as at_all
from arasea.tensor.math import any as at_any
from arasea.tensor.math import exp
...
class TestDimShuffle(unittest_tools.InferShapeTester):
    ...
    def with_linker(self, linker):
        for xsh, shuffle, zsh in [
            ...
            f = arasea.function([x], e, mode=Mode(linker=linker))
        ...
        e = self.op(ib, (1, 2))(x)
        f = arasea.function([x], e.shape, mode=Mode(linker=linker))
    ...
    def test_memory_leak(self, inplace):
        ...
        x = arasea.shared(np.ones(n, dtype=np.float64))

        y = x.dimshuffle([0, "x"])
        y.owner.op.inplace = inplace

        f = arasea.function([], y, mode=Mode(optimizer=None))
...
class TestBroadcast:
    ...
    def rand_val(self, shp):
        return np.asarray(np.random.random(shp), dtype=arasea.config.floatX)
        ...
        for shape_info in ("complete", "only_broadcastable", "none"):
            for xsh, ysh in [
                ...
                if shape_info == "complete":
                    x_type = type(arasea.config.floatX, shape=xsh)
                    y_type = type(arasea.config.floatX, shape=ysh)
                    ...
                    x_type = type(arasea.config.floatX, shape=[None for _ in xsh])
                    y_type = type(arasea.config.floatX, shape=[None for _ in ysh])
    ...
    def test_fill(self):
        for linker, op, t, rval in zip(
            ...
            x = t(arasea.config.floatX, shape=(None, None))("x")
            y = t(arasea.config.floatX, shape=(1, 1))("y")
    ...
    def test_weird_strides(self):
        for linker, op, t, rval in zip(
            ...
            x = t(arasea.config.floatX, shape=(None,) * 5)("x")
            y = t(arasea.config.floatX, shape=(None,) * 5)("y")
            e = op(aes.add)(x, y)
            f = make_function(linker().accept(FunctionGraph([x, y], [e])))
            xv = rval((2, 2, 2, 2, 2))
    ...
    def test_same_inputs(self):
        for linker, op, t, rval in zip(
            ...
            x = t(arasea.config.floatX, shape=(None,) * 2)("x")
            e = op(aes.add)(x, x)
...
class TestCAReduce(unittest_tools.InferShapeTester):
    ...
    def with_mode(
        ...
        for xsh, tosum in self.cases:
            ...
            f = arasea.function([x], e, mode=mode, on_unused_input="ignore")
            ...
            if tosum is None:
                tosum = list(range(len(xsh)))
            f = arasea.function([x], e.shape, mode=mode, on_unused_input="ignore")
            if not (
...
class TestBitOpReduceGrad:
    ...
    def test_all_grad(self):
        x = bmatrix("x")
        x_all = x.all()
        gx = arasea.grad(x_all, x)
        f = arasea.function([x], gx)
...
class TestElemwise(unittest_tools.InferShapeTester):
    ...
    def test_input_dimensions_overflow(self):
        ...
        s = a + b + c + d + e + f
        g = arasea.function([a, b, c, d, e, f], s, mode=Mode(linker="py"))
    ...
    def check_input_dimensions_match(self, mode):
        ...
        z_v = x_v - m_v
        f = arasea.function([x_v, m_v], z_v, mode=mode)
    ...
    def test_partial_static_shape_info(self):
        ...
        assert arasea.get_scalar_constant_value(res_shape[0][0]) == 1
        assert arasea.get_scalar_constant_value(res_shape[0][1]) == 1
...
def test_not_implemented_elemwise_grad():
    ...
    class TestOp(aes.ScalarOp):
        ...
        def grad(self, inputs, gout):
            ...
            (gz,) = gout
            dy_dx = n
            return [arasea.gradient.grad_not_implemented(self, 0, n), gz * dy_dx]

    test_op = Elemwise(TestOp())
    x = scalar()
    assert isinstance(arasea.gradient.grad(test_op(2, x), x), Variable)

    # Verify that trying to use the not implemented gradient fails.
    with pytest.raises(arasea.gradient.NullTypeGradError):
        arasea.gradient.grad(test_op(x, 2), x)
