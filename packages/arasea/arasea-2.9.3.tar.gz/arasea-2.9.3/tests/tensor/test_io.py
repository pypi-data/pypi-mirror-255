...
from arasea.tensor.io import load


class TestLoadTensor:
    def setup_method(self):
        self.data = np.arange(5, dtype=np.int32)
        self.filename = os.path.join(arasea.config.compiledir, "_test.npy")
    ...
    def teardown_method(self):
        os.remove(os.path.join(arasea.config.compiledir, "_test.npy"))
