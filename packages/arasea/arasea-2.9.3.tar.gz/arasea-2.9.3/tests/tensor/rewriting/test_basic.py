...
from arasea import shared
from arasea.compile import optdb
from arasea.compile.function import function
from arasea.compile.mode import get_default_mode, get_mode
...
from arasea.graph.rewriting.basic import check_stack_trace, out2in
...
from arasea.raise_op import Assert, CheckAndRaise
...
from arasea.tensor.math import softplus, sqrt, sub
from arasea.tensor.math import sum as at_sum
from arasea.tensor.math import true_divide
from arasea.tensor.rewriting.basic import (
    assert_op,
...
from arasea.tensor.rewriting.math import local_lift_transpose_through_dot
...
def test_local_upcast_elemwise_constant_inputs():
    s = dvector("s")
    x = at_sum(log(10**s))
    f = function([s], [arasea.gradient.grad(x, s)])
...
def test_assert_op_gradient():
    ...
    grad = arasea.gradient.grad(cost, x)
...
def test_deprecations():
    ...
    with pytest.deprecated_call():
        from arasea.tensor.basic_opt import register_useless  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor.rewriting.basic import ShapeFeature  # noqa: F401
