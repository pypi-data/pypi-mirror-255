...
from arasea.graph.basic import Constant, Variable, ancestors
...
from arasea.tensor.basic import Alloc, MakeVector, _convert_to_int8, make_vector
from arasea.tensor.elemwise import DimShuffle, Elemwise
from arasea.tensor.math import Dot, add, dot, exp, square
from arasea.tensor.rewriting.subtensor import (
    local_replace_AdvancedSubtensor,
    local_subtensor_shape_constant,
)
from arasea.tensor.shape import SpecifyShape, Unbroadcast, _shape, shape, specify_shape
...
from arasea.tensor.type_other import make_slice, slicetype
from tests import unittest_tools as utt
from tests.unittest_tools import create_arasea_param
...
y = create_arasea_param(np.random.default_rng().integers(0, 4, size=(2,)))
z = create_arasea_param(np.random.default_rng().integers(0, 4, size=(2, 2)))
...
class TestLocalUselessSubtensor:
    ...
    def test_local_useless_subtensor_1(self, idx):
        ...
        x_val = np.array([[0, 1, 2], [3, 4, 5]], dtype=arasea.config.floatX)
        idx_val = idx
...
def test_deprecations():
    ...
    with pytest.deprecated_call():
        from arasea.tensor.subtensor_opt import get_advsubtensor_axis  # noqa: F401 F811
...
def test_local_uint_constant_indices():
    ...
    z_fn = arasea.function([x], z, mode=mode)
    ...
    z_fn = arasea.function([x, y], z, mode=mode)
