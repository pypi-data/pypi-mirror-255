...
import arasea.tensor.blas_scipy
...
from arasea.graph.rewriting.basic import in2out
from arasea.graph.utils import InconsistencyError
...
from arasea.tensor.blas import (
    Dot22,
    Dot22Scalar,
    Gemm,
    Gemv,
...
from arasea.tensor.math import Dot, dot, mean, mul, neg, outer, sigmoid, sqrt
...
mode_blas_opt = arasea.compile.get_default_mode().including(
...
def test_gemm_unrolled():
    ...
    for num_rounds in range(1, 10):
        ...
        unrolled_arasea = function(
            [], updates=[(V, cur_V), (H, cur_H)], name="unrolled_arasea"
        )
        nb_dot = sum(
            1
            for node in unrolled_arasea.maker.fgraph.toposort()
            if isinstance(
                node.op,
                (
                    Dot,
...
def test_gemm_broadcasting(inplace, linker):
    ...
    if inplace:
        ...
        f = arasea.function([z, x, y, a, b], out, accept_inplace=True, mode=mode)
        assert [node.op for node in f.maker.fgraph.toposort()] == [gemm_inplace]
    else:
        out = gemm_no_inplace(z, a, x, y, b)
        f = arasea.function([z, x, y, a, b], out, mode=mode)
...
def test_dot22scalar():
    ...
    # m = arasea.compile.get_default_mode().including('local_dot_to_dot22',
    #                           'local_dot22_to_dot22scalar','specialize')
    # m = arasea.compile.get_default_mode().including('BlasOpt', 'specialize')
    rng = np.random.default_rng(unittest_tools.fetch_seed())
            ...
            for dtype3 in ["complex64", "complex128"]:
                ...
                for dtype4 in ["complex64", "complex128"]:
                    ...
                    def check_dot22scalar(func, len_topo_scalar=-1):
                        ...
                        dtype4_upcast = arasea.scalar.upcast(dtype4, dtype1, dtype2)
...
class TestGemv(unittest_tools.OptimizationTestMixin):
    def test_dot_vv(self):
        ...
        f = function([], arasea.tensor.dot(v, w), mode=mode_blas_opt)
    ...
    def test_dot_vm(self):
        ...
        f = function([], arasea.tensor.dot(v, m), mode=mode_blas_opt)
    ...
    def test_dot_mv(self):
        ...
        f = function([], arasea.tensor.dot(m, v), mode=mode_blas_opt)
    ...
    def t_gemv1(m_shp):
        ...
        f = function([], v2 + arasea.tensor.dot(m, v1), mode=mode_blas_opt)
        ...
        g = function(
            [], [], updates=[(v2, v2 + arasea.tensor.dot(m, v1))], mode=mode_blas_opt
        )
    ...
    def test_gemv2(self):
        ...
        f = function([], v2 + arasea.tensor.dot(v1, m), mode=mode_blas_opt)
        ...
        # test the inplace version
        g = function(
            [], [], updates=[(v2, v2 + arasea.tensor.dot(v1, m))], mode=mode_blas_opt
        )
...
# NOTE: At the time these tests were written, arasea did not have a
...
class TestGer(unittest_tools.OptimizationTestMixin):
    ...
    def setup_method(self):
        self.mode = arasea.compile.get_default_mode().including("fast_run")
...
class TestBlasStrides:
    dtype = "float64"
    mode = arasea.compile.get_default_mode()
    ...
    def cmp_dot22(self, b_shp, c_shp, rng):
        ...
        # arasea.printing.debugprint(f_nn)
