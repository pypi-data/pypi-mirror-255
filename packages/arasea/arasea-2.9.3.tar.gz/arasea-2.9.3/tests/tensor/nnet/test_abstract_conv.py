...
from arasea.compile.mode import Mode
from arasea.configdefaults import config
from arasea.graph.rewriting.basic import check_stack_trace
from arasea.tensor.nnet import abstract_conv as conv
from arasea.tensor.nnet import conv2d_transpose, corr, corr3d
from arasea.tensor.nnet.abstract_conv import (
    AbstractConv2d,
...
from arasea.tensor.nnet.corr import CorrMM, CorrMM_gradInputs, CorrMM_gradWeights
from arasea.tensor.nnet.corr3d import Corr3dMM, Corr3dMMGradInputs, Corr3dMMGradWeights
...
class TestAssertConvShape:
    def test_basic(self):
        ...
        f = arasea.function(shape, assert_conv_shape(shape))
...
class TestAssertShape:
    ...
    def test_basic(self):
        ...
        f = arasea.function([x, s1, s2], assert_shape(x, expected_shape))
    ...
    def test_shape_check_conv2d(self):
        ...
        f = arasea.function([input, filters], out)
    ...
    def test_shape_check_conv2d_grad_wrt_inputs(self):
        ...
        f = arasea.function([output_grad, filters], out)
        # mismatched filter_shape
        with pytest.raises(AssertionError):
            f(
    ...
    def test_shape_check_conv2d_grad_wrt_weights(self):
        ...
        f = arasea.function([input, output_grad], out)
        # mismatched filter_shape
        with pytest.raises(AssertionError):
            f(
...
class BaseTestConv:
    ...
    def run_fwd(
        ...
        f_ref = arasea.function([], c_ref, mode="FAST_RUN")
        f = arasea.function([], c, mode=mode)
...
class BaseTestConv2d(BaseTestConv):
    ...
    def setup_class(cls):
        ...
        cls.shared = staticmethod(arasea.compile.shared)
...
class TestConvTypes:
    ...
    def test_grad_types(self):
        ...
        grad_input, grad_filters = arasea.grad(output.sum(), wrt=(input, filters))
        ...
        grad_input, grad_topgrad = arasea.grad(grad_filters.sum(), wrt=(input, topgrad))
        ...
        grad_filters, grad_topgrad = arasea.grad(
    ...
    def test_constant_input(self):
        ...
        grad_filters = arasea.grad(output.sum(), wrt=filters)
        ...
        grad_input = arasea.grad(output.sum(), wrt=input)
        ...
        grad_topgrad = arasea.grad(grad_filters.sum(), wrt=topgrad)
        ...
        grad_input = arasea.grad(grad_filters.sum(), wrt=input)
        assert grad_input.type == input.type, (
            grad_input,
            grad_input.type,
            input,
        ...
        grad_topgrad = arasea.grad(grad_input.sum(), wrt=topgrad)
        assert topgrad.type.is_super(grad_topgrad.type), (
            grad_topgrad,
            grad_topgrad.type,
            topgrad,
        ...
        grad_filters = arasea.grad(grad_input.sum(), wrt=filters)
        assert grad_filters.type == filters.type, (
            grad_filters,
            grad_filters.type,
            filters,
...
class TestBilinearUpsampling:
    ...
    compile_mode = arasea.compile.mode.get_default_mode()
    ...
    def test_bilinear_kernel_1D(self):
        ...
        f_ten = arasea.function([rat], kernel_ten)

        kernel_ten_norm = bilinear_kernel_1D(ratio=rat, normalize=True)
        f_ten_norm = arasea.function([rat], kernel_ten_norm)
    ...
    def test_bilinear_upsampling_1D(self):
        ...
        for ratio in [2, 3, 4, 5, 6, 7, 8, 9]:
            ...
            f = arasea.function([], bilin_mat, mode=self.compile_mode)
    ...
    def test_compare_1D_and_2D_upsampling_values(self):
        ...
        f_1D = arasea.function([], mat_1D, mode=self.compile_mode)
        f_2D = arasea.function([], mat_2D, mode=self.compile_mode)
    ...
    def test_fractional_bilinear_upsampling(self):
        ...
        f_up_x = arasea.function([], up_x, mode=self.compile_mode)
    ...
    def test_fractional_bilinear_upsampling_shape(self):
        ...
        out = arasea.function([], z.shape, mode="FAST_RUN")()
...
class TestConv2dTranspose:
    ...
    def test_interface(self):
        ...
        if config.mode == "FAST_COMPILE":
            mode = (
                arasea.compile.get_mode(mode)
                .excluding("conv_gemm")
                .excluding("AbstractConvCheck")
            )

        output = arasea.function(
            inputs=[],
...
class TestConv2dGrads:
    ...
    def test_conv2d_grad_wrt_inputs(self):
        ...
        # the outputs of `arasea.tensor.nnet.conv` forward grads to make sure the
            ...
            for bm in self.border_modes:
                for ss in self.subsamples:
                    for ff in self.filter_flip:
                        ...
                        out_grad_shape = (
                            arasea.tensor.nnet.abstract_conv.get_conv_output_shape(
                        ...
                        conv_out = arasea.tensor.nnet.conv2d(
                            self.x,
                            filters=self.w,
                            border_mode=bm,
                            subsample=ss,
                        ...
                        conv_grad = arasea.grad(
                            conv_out.sum(),
                            wrt=self.x,
                            known_grads={conv_out: self.output_grad},
                        )
                        f_old = arasea.function(
                            [self.x, self.w, self.output_grad], conv_grad
                        )

                        conv_wrt_i_out = (
                            arasea.tensor.nnet.abstract_conv.conv2d_grad_wrt_inputs(
            ...
            for bm in self.border_modes:
                for ss in self.subsamples:
                    for ff in self.filter_flip:
                        ...
                        conv_wrt_w_out = (
                            arasea.tensor.nnet.abstract_conv.conv2d_grad_wrt_weights(
                                self.x,
                                output_grad=self.output_grad_wrt,
                                border_mode=bm,
                                subsample=ss,
...
class TestGroupedConvNoOptim:
    conv = arasea.tensor.nnet.abstract_conv.AbstractConv2d
    conv_gradw = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradWeights
    conv_gradi = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradInputs
    conv_op = arasea.tensor.nnet.abstract_conv.AbstractConv2d
    conv_gradw_op = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradWeights
    conv_gradi_op = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradInputs
    mode = Mode(optimizer=None)
    is_dnn = False
...
class TestGroupedConv3dNoOptim(TestGroupedConvNoOptim):
    conv = arasea.tensor.nnet.abstract_conv.AbstractConv3d
    conv_gradw = arasea.tensor.nnet.abstract_conv.AbstractConv3d_gradWeights
    conv_gradi = arasea.tensor.nnet.abstract_conv.AbstractConv3d_gradInputs
    conv_op = arasea.tensor.nnet.abstract_conv.AbstractConv3d
    conv_gradw_op = arasea.tensor.nnet.abstract_conv.AbstractConv3d_gradWeights
    conv_gradi_op = arasea.tensor.nnet.abstract_conv.AbstractConv3d_gradInputs
    mode = Mode(optimizer=None)
...
class TestUnsharedConv:
    conv2d = arasea.tensor.nnet.abstract_conv.AbstractConv2d
    conv2d_gradw = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradWeights
    conv2d_gradi = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradInputs
    conv2d_op = arasea.tensor.nnet.abstract_conv.AbstractConv2d
    conv2d_gradw_op = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradWeights
    conv2d_gradi_op = arasea.tensor.nnet.abstract_conv.AbstractConv2d_gradInputs
    ...
    def test_gradweight(self):
        ...
        for imshp, kshp, topshp, mode, sub, groups, verify in zip(
            ...
            ref_func = arasea.function([img_sym, top_sym], ref_out_sym, mode=self.mode)
...
class TestAsymmetricPadding:
    ...
    def test_fwd(self):
        ...
        for imshp, kshp, pad in zip(self.img_shape, self.kern_shape, self.border_mode):
            ...
            asymmetric_func = arasea.function(
            ...
            ref_func = arasea.function([img_sym, kern_sym], ref_out_sym, mode=self.mode)
    ...
    def test_gradinput(self):
        ...
        for imshp, kshp, topshp, pad in zip(
            ...
            ref_func = arasea.function([kern_sym, top_sym], ref_out_sym, mode=self.mode)
...
class TestCausalConv:
    ...
    def test_interface(self):
        ...
        causal_func = arasea.function([img_sym, kern_sym], sym_out, mode=self.mode)
