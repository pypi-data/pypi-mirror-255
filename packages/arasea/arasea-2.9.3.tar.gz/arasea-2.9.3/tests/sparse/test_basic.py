...
from arasea.gradient import GradientError
from arasea.graph.basic import Apply, Constant, applys_between
from arasea.graph.op import Op
from arasea.misc.safe_asarray import _asarray
from arasea.sparse import (
    CSC,
    CSM,
    CSR,
    AddSD,
...
from arasea.sparse.basic import (
    SparseConstant,
    _is_dense_variable,
    _is_sparse,
    _is_sparse_variable,
...
from arasea.sparse.rewriting import (
    AddSD_ccode,
    CSMGradC,
    StructuredDotCSC,
    UsmmCscDense,
)
from arasea.tensor.basic import MakeVector
from arasea.tensor.elemwise import DimShuffle, Elemwise
from arasea.tensor.math import sum as at_sum
from arasea.tensor.shape import Shape_i
from arasea.tensor.subtensor import AdvancedIncSubtensor, AdvancedSubtensor1, Subtensor
from arasea.tensor.type import (
    TensorType,
    float_dtypes,
    fscalar,
    iscalar,
...
def sparse_random_inputs(
    ...
    If `out_dtype` is `None`, arasea.config.floatX is used.
    ...
    if out_dtype is None:
        out_dtype = arasea.config.floatX
    ...
    variable = [
        getattr(arasea.sparse, format + "_matrix")(dtype=out_dtype) for k in range(n)
    ]
    data = [
...
def verify_grad_sparse(op, pt, structured=False, *args, **kwargs):
    """
    Wrapper for arasea.test.unittest_tools.py:verify_grad which
...
class TestSparseInferShape(utt.InferShapeTester):
    ...
    def test_csm_grad(self):
        for sparsetype in ("csr", "csc"):
            ...
            call = getattr(sp.sparse, sparsetype + "_matrix")
            spm = call(random_lil((300, 400), config.floatX, 5))
            out = arasea.grad(dense_from_sparse(CSM(sparsetype)(x, y, z, s)).sum(), x)
            self._compile_and_check(
                [x, y, z, s],
                [out],
    ...
    def test_structured_dot_grad(self):
        ...
        for format, op in [
            ...
            grads = arasea.grad(dense_from_sparse(structured_dot(x, y)).sum(), [x, y])
            self._compile_and_check(
                [x, y],
                [grads[0]],
                [
...
class TestConstructSparseFromList:
    def test_adv_sub1_sparse_grad(self):
        ...
        with pytest.raises(TypeError):
            arasea.sparse.sparse_grad(v)

        with pytest.raises(TypeError):
            sub = m[v, v]
            arasea.sparse.sparse_grad(sub)

        # Assert we don't create a sparse grad by default
        sub = m[v]
        g = arasea.grad(sub.sum(), m)
        ...
        m = matrix()
        v = ivector()
        sub = arasea.sparse.sparse_grad(m[v])
        g = arasea.grad(sub.sum(), m)
        ...
        m = matrix()
        shared_v = arasea.shared(valv)

        def fn(m):
            return arasea.sparse.sparse_grad(m[shared_v])
...
class TestComparison:
    ...
    def __generalized_ss_test(self, araseap, symbolicType, testOp, scipyType):
        x = symbolicType()
        y = symbolicType()

        op = araseap(x, y)

        f = arasea.function([x, y], op)
    ...
    def __generalized_sd_test(self, araseap, symbolicType, testOp, scipyType):
        x = symbolicType()
        y = matrix()
    ...
    def __generalized_ds_test(self, araseap, symbolicType, testOp, scipyType):
        x = symbolicType()
        y = matrix()

        op = araseap(y, x)

        f = arasea.function([y, x], op)
    ...
    def test_equality_case(self):
        ...
        m1 = sp.sparse.csc_matrix((2, 2), dtype=arasea.config.floatX)
        m2 = np.asarray([[0, 0], [0, 0]], dtype=arasea.config.floatX)
...
class TestConversion:
    def test_basic(self):
        ...
        a = sp.sparse.csr_matrix(test_val)
        s = as_sparse_or_tensor_variable(a)
        from arasea.tensor.exceptions import NotScalarConstantError
    ...
    def test_csm_properties(self):
        ...
        for format in ("csc", "csr"):
            for dtype in ("float32", "float64"):
                ...
                f = arasea.function([x], csm_properties(x))
    ...
    def test_csm(self):
        ...
        for format in ("csc", "csr"):
            for dtype in ("float32", "float64"):
                ...
                z = ivector()
                s = ivector()
                f = arasea.function([x, y, z, s], CSM(format)(x, y, z, s))
...
class TestStructuredDot:
    ...
    def test_upcast(self):
        ...
        for dense_dtype in typenames:
            for sparse_dtype in typenames:
                correct_dtype = arasea.scalar.upcast(sparse_dtype, dense_dtype)
                ...
                arasea_result = f(spmat, mat)
                scipy_result = spmat * mat
                assert arasea_result.shape == scipy_result.shape
                assert arasea_result.dtype == scipy_result.dtype
                utt.assert_allclose(scipy_result, arasea_result)
    ...
    def test_opt_unpack(self):
        ...
        f = arasea.function([kerns, images], structured_dot(cscmat, images.T))
    ...
    def test_dot_sparse_sparse(self):
        ...
        for sparse_format_a in ["csc", "csr", "bsr"]:
            for sparse_format_b in ["csc", "csr", "bsr"]:
                ...
                d = at.dot(a, b)
                f = arasea.function([a, b], Out(d, borrow=True))
                for M, N, K, nnz in [
                    (4, 3, 2, 3),
                    (40, 30, 20, 3),
                    (40, 30, 20, 30),
    ...
    def test_csc_correct_output_faster_than_scipy(self):
        ...
        for M, N, K, nnz in [
            ...
            for i in range(5):
                ...
                arasea_times.append(t1 - t0)
                scipy_times.append(t2 - t1)

            arasea_time = np.min(arasea_times)
            scipy_time = np.min(scipy_times)

            # speedup = scipy_time / arasea_time
            # print scipy_times
            # print arasea_times
            # print ('M=%(M)s N=%(N)s K=%(K)s nnz=%(nnz)s arasea_time'
            #       '=%(arasea_time)s speedup=%(speedup)s') % locals()
            ...
            utt.assert_allclose(scipy_result, arasea_result)
            if arasea.config.mode == "FAST_RUN" and arasea.config.cxx:
                assert arasea_time <= overhead_rtol * scipy_time + overhead_tol
...
class TestDots(utt.InferShapeTester):
    def setup_method(self):
        ...
        self.x_csr = sp.sparse.csr_matrix(
            np.random.binomial(1, 0.5, x_size), dtype=arasea.config.floatX
        )
        self.x_csc = sp.sparse.csc_matrix(
            np.random.binomial(1, 0.5, x_size), dtype=arasea.config.floatX
        )
        self.y = np.asarray(
            np.random.uniform(-1, 1, y_size), dtype=arasea.config.floatX
        )
        self.y_csr = sp.sparse.csr_matrix(
            np.random.binomial(1, 0.5, y_size), dtype=arasea.config.floatX
        )
        self.y_csc = sp.sparse.csc_matrix(
            np.random.binomial(1, 0.5, y_size), dtype=arasea.config.floatX
        )
        self.v_10 = np.asarray(np.random.uniform(-1, 1, 10), dtype=arasea.config.floatX)
        self.v_100 = np.asarray(
            np.random.uniform(-1, 1, 100), dtype=arasea.config.floatX
        )
    ...
    def test_csr_dense(self):
        ...
        for x, y, x_v, y_v in [
            ...
            f_a = arasea.function([x, y], sparse.dot(x, y))
    ...
    def test_sparse_sparse(self):
        for d1, d2 in [
            ...
            for x_f, y_f in [
                ...
                f_b = arasea.function([x, y], sparse.dot(x, y))
                ...
                # Test infer_shape
                f_a = arasea.function([x, y], sparse.dot(x, y).shape)
    ...
    def test_int32_dtype(self):
        ...
        f = arasea.function(inputs=[I, C], outputs=y)
...
class TestUsmm:
    ...
    def setup_method(self):
        ...
        self.x = np.asarray(
            self.rng.binomial(1, 0.5, x_size), dtype=arasea.config.floatX
        )
        self.y = np.asarray(self.rng.uniform(-1, 1, y_size), dtype=arasea.config.floatX)
        self.z = np.asarray(self.rng.uniform(-1, 1, z_size), dtype=arasea.config.floatX)
    ...
    def test_basic(self):
        ...
        for dtype1, dtype2, dtype3, dtype4, format1, format2 in params:
            ...
            z = arasea.shared(np.asarray(self.z, dtype=dtype4).copy())
            ...
            # Can it work inplace?
            inplace = dtype4 == arasea.scalar.upcast(dtype1, dtype2, dtype3)

            # To make it easier to check the toposort
            mode = arasea.compile.mode.get_default_mode().excluding("fusion")

            if inplace:
                updates = [(z, z - a * sparse.dot(x, y))]
                f_a = arasea.function([a, x, y], [], updates=updates, mode=mode)
                f_a(a_data, x_data, y_data)
                f_a_out = z.get_value(borrow=True)
            else:
                f_a = arasea.function([a, x, y], z - a * sparse.dot(x, y), mode=mode)
                # In DebugMode there is a strange difference with complex
                # So we raise a little the threshold a little.
                try:
                    orig_atol = arasea.tensor.math.float64_atol
                    orig_rtol = arasea.tensor.math.float64_rtol
                    arasea.tensor.math.float64_atol = 1e-7
                    arasea.tensor.math.float64_rtol = 1e-6
                    f_a_out = f_a(a_data, x_data, y_data)
                finally:
                    arasea.tensor.math.float64_atol = orig_atol
                    arasea.tensor.math.float64_rtol = orig_rtol
            ...
            topo = f_a.maker.fgraph.toposort()
            up = arasea.scalar.upcast(dtype1, dtype2, dtype3, dtype4)

            fast_compile = arasea.config.mode == "FAST_COMPILE"

            if not arasea.config.blas__ldflags:
                ...
                assert isinstance(topo[2].op, Elemwise) and isinstance(
                    topo[2].op.scalar_op, arasea.scalar.Mul
                )
                assert isinstance(topo[3].op, Elemwise) and isinstance(
                    topo[3].op.scalar_op, arasea.scalar.Sub
                )
            elif (
                y.type.dtype == up
                and format1 == "csc"
                ...
                assert (
                    sum(
                        ...
                        and isinstance(node.op.scalar_op, arasea.scalar.basic.Cast)
                        for node in topo
                    )
                    == len(topo) - 5
                )
                ...
                assert topo[1].op == arasea.tensor.neg
...
class TestZerosLike:
    def test(self):
        x = sparse.csr_matrix()
        f = arasea.function([x], sparse.sp_zeros_like(x))
        vx = sp.sparse.csr_matrix(
            np.asarray(
                np.random.binomial(1, 0.5, (100, 100)), dtype=arasea.config.floatX
            )
        )
...
def test_sparse_shared_memory():
    ...
    f = arasea.function([In(x, mutable=True), In(y, mutable=True)], z, mode="FAST_RUN")
...
def test_size():
    ...
    for sparse_type in ("csc_matrix", "csr_matrix"):
        x = getattr(arasea.sparse, sparse_type)()
        y = getattr(sp.sparse, sparse_type)((5, 7)).astype(config.floatX)
        get_size = arasea.function([x], x.size)
...
class TestColScaleCSC(utt.InferShapeTester):
    ...
    def test_op(self):
        for format in sparse.sparse_formats:
            ...
            f = arasea.function(variable, self.op(*variable))
    ...
    def test_op(self, op_type):
        for format in sparse.sparse_formats:
            for axis in self.possible_axis:
                ...
                f = arasea.function(variable, self.op(variable[0], axis=axis))
                tested = f(*data)
...
class TestRemove0(utt.InferShapeTester):
    ...
    def test_remove0(self):
        ...
        for format, matrix_class in configs:
            for zero, unsor in [
                ...
                # the In thingy has to be there because arasea has as rule not
                # to optimize inputs
                f = arasea.function([In(x, borrow=True, mutable=True)], Remove0()(x))

                # assert optimization local_inplace_remove0 is applied in
                # modes with optimization
                if arasea.config.mode not in ["FAST_COMPILE"]:
    ...
    def test_infer_shape(self):
        ...
        x_csc = sparse.csc_matrix(dtype=arasea.config.floatX)
        mat_csc = sp.sparse.csc_matrix(mat, dtype=arasea.config.floatX)
        self._compile_and_check([x_csc], [Remove0()(x_csc)], [mat_csc], self.op_class)

        x_csr = sparse.csr_matrix(dtype=arasea.config.floatX)
        mat_csr = sp.sparse.csr_matrix(mat, dtype=arasea.config.floatX)
...
class TestGetItem:
    ...
    def test_GetItemList(self):
        ...
        fa = arasea.function([a[0]], y)
        fb = arasea.function([b[0]], z)
    ...
    def test_GetItem2Lists_wrong_index(self):
        ...
        f1 = arasea.function([a[0]], y1)
        f2 = arasea.function([a[0]], y2)
    ...
    def test_GetItem2D(self):
        ...
        for format in sparse_formats:
            ...
            vx = as_sparse_format(self.rng.binomial(1, 0.5, (100, 97)), format).astype(
                arasea.config.floatX
            )

            # mode_no_debug = arasea.compile.mode.get_default_mode()
            # if isinstance(mode_no_debug, arasea.compile.debugmode.DebugMode):
            #    mode_no_debug = 'FAST_RUN'
            if is_supported_version:
                f1 = arasea.function([x, a, b, c, d, e, f], x[a:b:e, c:d:f])
                r1 = f1(vx, m, n, p, q, j, k)
                t1 = vx[m:n:j, p:q:k]
            else:
                f1 = arasea.function([x, a, b, c, d], x[a:b, c:d])
                r1 = f1(vx, m, n, p, q)
                t1 = vx[m:n, p:q]
            ...
            """
            if is_supported_version:
                f4 = arasea.function([x, a, b, e], x[a:b:e])
                r4 = f4(vx, m, n, j)
                t4 = vx[m:n:j]
            else:
                f4 = arasea.function([x, a, b], x[a:b])
                r4 = f4(vx, m, n)
                t4 = vx[m:n]
            ...
            # -----------------------------------------------------------
            # test cases using int indexing instead of arasea variable
            f6 = arasea.function([x], x[1:10:j, 10:20:k])
            r6 = f6(vx)
            ...
            # ----------------------------------------------------------
            # test cases with indexing both with arasea variable and int
            if is_supported_version:
                f8 = arasea.function([x, a, b, e], x[a:b:e, 10:20:1])
                r8 = f8(vx, m, n, j)
                t8 = vx[m:n:j, 10:20:1]
            else:
                f8 = arasea.function([x, a, b], x[a:b, 10:20])
                r8 = f8(vx, m, n)
                t8 = vx[m:n, 10:20]
            assert r8.shape == t8.shape
            assert np.all(r8.toarray() == t8.toarray())

            f9 = arasea.function([x, a, b], x[1:a:j, 1:b:k])
            r9 = f9(vx, p, q)
            t9 = vx[1:p:j, 1:q:k]
    ...
    def test_GetItemScalar(self):
        ...
        for format in sparse_formats:
            ...
            f1 = arasea.function([x, a, b], x[a, b])
            r1 = f1(vx, 10, 10)
            t1 = vx[10, 10]
            assert r1.shape == t1.shape
            assert np.all(t1 == r1)

            f2 = arasea.function([x, a], x[50, a])
            r2 = f2(vx, m)
            t2 = vx[50, m]
            assert r2.shape == t2.shape
            assert np.all(t2 == r2)

            f3 = arasea.function([x, a], x[a, 50])
            r3 = f3(vx, m)
            t3 = vx[m, 50]
            assert r3.shape == t3.shape
            assert np.all(t3 == r3)

            f4 = arasea.function([x], x[50, 42])
            r4 = f4(vx)
            t4 = vx[m, n]
            assert r3.shape == t3.shape
            assert np.all(t4 == r4)
        ...
        for format in sparse.sparse_formats:
            for i_dtype in sparse.all_dtypes:
                for o_dtype in sparse.all_dtypes:
                    ...
                    func = arasea.function([variable], cast(variable, o_dtype))
                    cls = arasea.function([variable], Cast(o_dtype)(variable))
                    prop = arasea.function([variable], variable.astype(o_dtype))
...
def _format_info(nb):
    ...
    for format in sparse.sparse_formats:
        variable = getattr(arasea.sparse, format + "_matrix")
        spa = getattr(sp.sparse, format + "_matrix")

        x[format] = [variable() for t in range(nb)]
        mat[format] = [
            spa(random_lil((3, 4), arasea.config.floatX, 8)) for t in range(nb)
        ]
    return x, mat
...
class TestAddSSData(utt.InferShapeTester):
    ...
    def test_op(self):
        for format in sparse.sparse_formats:
            f = arasea.function(self.x[format], add_s_s_data(*self.x[format]))
...
def test_hstack_vstack():
    ...
    def get_expected_dtype(blocks, to_dtype):
        if to_dtype is None:
            ...
            return arasea.scalar.upcast(*block_dtypes)
        else:
            return to_dtype

    # a deliberately weird mix of dtypes to stack
    dtypes = ("complex128", arasea.config.floatX)
    ...
    def test_mul_s_v(self):
        ...
        for format in ("csr", "csc"):
            for dtype in ("float32", "float64"):
                ...
                f = arasea.function([x, y], mul_s_v(x, y))
    ...
    def test_structured_add_s_v(self):
        ...
        for format in ("csr", "csc"):
            for dtype in ("float32", "float64"):
                x = sparse.SparseTensorType(format, dtype=dtype)()
                y = vector(dtype=dtype)
                f = arasea.function([x, y], structured_add_s_v(x, y))
...
class TestSamplingDot(utt.InferShapeTester):
    ...
    def test_op(self):
        f = arasea.function(self.x, sampling_dot(*self.x))
...
@makeSharedTester(
    ...
    arasea_fct_=lambda a: dense_from_sparse(a * 2.0),
