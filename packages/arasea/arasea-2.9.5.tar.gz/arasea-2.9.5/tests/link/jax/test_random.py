...
import arasea.tensor.random as aer
...
from arasea.tensor.random.basic import RandomVariable
...
def test_random_updates(rng_ctor):
    ...
    with pytest.warns(
        ...
        f = arasea.function([], [x], updates={rng: next_rng}, mode=jax_mode)
    assert f() != f()
...
def test_random_custom_implementation():
    ...
    from arasea.link.jax.dispatch.random import jax_sample_fn
...
def test_random_concrete_shape_subtensor():
    ...
    This test ensures that the `DimShuffle` `Op` used by Arasea to turn scalar
...
def test_random_concrete_shape_subtensor_tuple():
    ...
    This test ensures that the `MakeVector` `Op` used by Arasea to turn tuple
