...
===========================================
:mod:`io` - defines arasea.function [TODO]
===========================================

.. module:: arasea.compile.io
...
The ``inputs`` argument to ``arasea.function`` is a list, containing the ``Variable`` instances for which values will be specified at the time of the function call.  But inputs can be more than just Variables.
...
The following restrictions apply to the inputs to ``arasea.function``:
...
For each input, ``arasea.function`` will create a ``Container`` if
...
>>> from arasea import tensor as at, function
...
>>> fn = arasea.function(inputs=[x, y, In(z, value=42), ((w, w+x), 0)],
...                      outputs=x + y + z)
...
>>> import numpy
>>> from arasea.compile.io import Out
>>> x, y, s = at.matrices('xys')

>>> # print a list of 2 ndarrays
>>> fn1 = arasea.function([x], [x+x, Out((x+x).T, borrow=True)])
...
>>> fn2 = arasea.function([x], [x+x])
>>> fn2(numpy.asarray([[1,0],[0,1]]))
[array([[ 2.,  0.],
       [ 0.,  2.]])]

>>> # print an ndarray
>>> fn3 = arasea.function([x], outputs=x+x)
