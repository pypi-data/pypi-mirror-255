...
on Arasea's loop mechanism. This mechanism is called `Scan` and its internals
...
Arasea. Under the hood, this function will perform some processing on its
inputs and instantiate the `Scan` `Op` class which implements the looping
mechanism. It achieves this by compiling its own Arasea function representing
...
1. Arasea's :ref:`graph structure <graphstructures>` (`Apply` nodes, `Variable` nodes and `Op`\s)

2. The interface and usage of Arasea's :ref:`scan <reference_scan>` function

Additionally, the :ref:`scan_internals_rewrites` section below assumes
knowledge of:

3. Arasea's :ref:`graph rewriting <graph_rewriting>`
...
* ``opt.py`` contains the list of all Arasea graph rewrites for the
  `Scan` operator.
...
A `Scan` `Op` contains an Arasea function representing the computation
...
function** refers to the Arasea function that *contains* the `Scan` `Op` whereas the
**inner function** refers to the Arasea function that is *contained* inside the
`Scan` `Op`.
...
**Sequence** : A sequence is an Arasea variable which `Scan` will iterate
...
**Non-sequences** : A non-sequence is an Arasea variable which Scan
...
but they cannot be directly created by the user. They can appear in an Arasea
