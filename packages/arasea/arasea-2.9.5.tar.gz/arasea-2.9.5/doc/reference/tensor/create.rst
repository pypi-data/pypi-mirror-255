...
Arasea provides a list of predefined tensor types that can be used
...
(with dtype determined by `arasea.config.floatX`) so if you use
...
The following `TensorType` instances are provided in the `arasea.tensor` module.
...
.. #COMMENT
    table generated by
    $ python Arasea/doc/generate_dtype_tensor_table.py
...
One can convert python objects by calling either :func:`arasea.tensor.as_tensor_variable` or :func:`arasea.shared`.


``arasea.tensor.as_tensor_variable``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Arasea requires that the inputs to all expressions be `Variable` instances, so
Arasea automatically wraps them in a :class:`TensorConstant`.
...
.. note::
    ...
    Arasea makes a copy of any `ndarray` that is used in an expression, so
    subsequent changes to that `ndarray` will not have any effect on the Arasea
...
precise) is by calling :func:`arasea.shared`

.. testcode::

    x = arasea.shared(np.random.standard_normal((3, 4)))
...
.. note::
    ...
    `arasea.tensor.basic.autocast_float`.
