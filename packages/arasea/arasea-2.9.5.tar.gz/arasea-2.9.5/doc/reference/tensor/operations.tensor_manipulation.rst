...
.. attention::
   ...
   Index assignment is **not** supported in Arasea. If you want to do the equivalent of ``a[5] = b`` or ``a[5]+=b`` you will need to use the ``set_subtensor`` or ``inc_subtensor`` operator respectively.
...
.. admonition:: Specific to Arasea

   The ``specify_shape`` operator is specific to Arasea.
...
.. function:: stacklists(tensor_list)
    ...
    >>> from arasea.tensor import stacklists, scalars, matrices
