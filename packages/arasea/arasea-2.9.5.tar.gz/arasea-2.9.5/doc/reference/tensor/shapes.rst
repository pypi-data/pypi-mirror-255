...
How Shape Information is Handled by Arasea
------------------------------------------

Currently, information regarding shape is used in the following ways by Arasea:
...
power computations; Arasea has removed them to compute the shape of the output
directly.

Arasea propagates information about shapes within a graph using specialized
...
- You can use the :class:`SpecifyShape`\ :class:`Op` to add shape information anywhere in the
  ...
  this makes it possible to precompute the Arasea function to a constant.
...
based on the first input Arasea variable, which leads to an error.
...
Arasea assumes that the computation is correct and consistent
in the first place, as it does here.

You can detect those problems by running the code without this optimization,
using the Arasea flag ``optimizer_excluding=local_shape_to_shape_i``. You can
...
Unlike numpy which does broadcasting dynamically, Arasea needs
