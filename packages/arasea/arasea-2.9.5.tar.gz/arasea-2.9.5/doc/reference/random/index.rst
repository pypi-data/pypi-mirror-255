...
Because in Arasea you first express everything symbolically and
...
The way to think about putting randomness into Arasea's computations is
to put random variables in your graph. Arasea will allocate a NumPy
...
hold here as well. Arasea's random objects are defined and implemented in
...
Copying Random State Between Arasea Graphs
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In some use cases, a user might want to transfer the "state" of all random
number generators associated with a given Arasea graph (e.g. ``g1``, with compiled
...
:class:`arasea.tensor.random.utils.RandomStream` and
:class:`arasea.sandbox.rng_mrg.MRG_RandomStream`
...
*particular* variable, while the second represents the Arasea graph
...
.. testcode::
    ...
    w = arasea.shared(rng.standard_normal(feats), name="w")

    # initialize the bias term
    b = arasea.shared(0., name="b")
    ...
    predict = arasea.function(inputs=[x], outputs=prediction)
...
The :mod:`arasea.tensor.random` module provides random-number drawing functionality
...
Arasea assignes NumPy RNG states (e.g. `Generator` or `RandomState` objects) to
...
.. currentmodule:: arasea.tensor.random

Create a new :class:`RandomStream` instance, then call its methods to generate :class:`RandomVariable` with different distributions. The :class:`RandomStream` interface follows that of NumPy's ``Generator``; the implementation details depend on the backend to which the Arasea graph is compiled.
