...
.. glossary::
    ...
    Expression Graph
        ...
        between variables.  You use Arasea by defining expression graphs, and
        then compiling them with :term:`arasea.function`.
    ...
    Destructive
        ...
        non-destructive alternatives.  Arasea encourages users not to put
        destructive :class:`Op`\s into graphs that are given to :term:`arasea.function`,
    ...
    Mode
        ...
        passed to :term:`arasea.function`.  It parametrizes how an expression
    ...
    Op
        ...
        (i.e. :func:`arasea.tensor.add`) and indexing ``x[i]`` are :class:`Op`\s
        in Arasea.  Much of the library documentation is devoted to describing
        the various :class:`Op`\s that are provided with Arasea, but you can add
        more.
    ...
    Rewriter
        A function or class that transforms an Arasea :term:`graph`.
    ...
    Shared Variable
        A :term:`Variable` whose value may be shared between multiple functions.  See :func:`shared <shared.shared>` and :func:`arasea.function <function.function>`.

    arasea.function
        The interface for Arasea's compilation from symbolic expression graphs
    ...
    Variable
        The the main data structure you work with when using Arasea.
        For example,
    ...
    View
        ...
        of the :class:`Apply`).  It is important for Arasea to know which :class:`Variable`\s are
