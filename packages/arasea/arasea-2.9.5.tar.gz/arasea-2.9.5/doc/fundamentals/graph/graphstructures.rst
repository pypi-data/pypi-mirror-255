...
Arasea works by modeling mathematical operations and their outputs using
symbolic placeholders, or :term:`variables <variable>`, which inherit from the class
:class:`Variable`.  When writing expressions in Arasea one uses operations like
...
Arasea graphs are composed of interconnected :term:`Apply`, :term:`Variable` and
...
>>> import arasea
>>> x = arasea.tensor.dmatrix('x')
>>> y = x * 2.

If you enter ``type(y.owner)`` you get ``<class 'arasea.graph.basic.Apply'>``,
...
<class 'arasea.graph.basic.Apply'>
>>> y.owner.inputs[1].owner.op # doctest: +SKIP
<arasea.tensor.elemwise.DimShuffle object at 0x106fcaf10>
>>> y.owner.inputs[1].owner.inputs
[TensorConstant{2.0}]

All of the above can be succinctly summarized with the :func:`arasea.dprint`
function:
...
in an Arasea-built computation graph.
...
:term:`computation graph <graph>` in Arasea. Unlike
...
Arasea's version of a function call whereas an :class:`Op` is
Arasea's version of a function definition.
...
An :class:`Op` in Arasea defines a certain computation on some types of
...
function). If you were to interpret the Python language using Arasea's
...
A :class:`Type` in Arasea provides static information (or constraints) about
data objects in a graph. The information provided by :class:`Type`\s allows
Arasea to perform rewrites and produce more efficient compiled code.

Every symbolic :class:`Variable` in an Arasea graph has an associated
...
For example, :ref:`arasea.tensor.irow <reference_tensor_create>` is an instance of a
:class:`Type` and it can be used to construct variables as follows:

>>> from arasea.tensor import irow
>>> irow()
<TensorType(int32, (1, ?))>
...
Note that Arasea :class:`Type`\s are not necessarily equivalent to Python types or
classes. Arasea's :class:`TensorType`'s, like `irow`, use :class:`numpy.ndarray`
...
In other words, Arasea :class:`Type`'s try to be more specific.

For more information see :ref:`arasea_type`.
...
Arasea. The symbolic inputs that you operate on are :class:`Variable`\s and what
...
More specifically, a :class:`Variable` is a basic structure in Arasea that
...
simple. The only thing :func:`arasea.grad` has to do is to traverse the
...
When compiling an Arasea graph using :func:`arasea.function`, a graph is
...
computation is carried out. The way rewrites work in Arasea is by
...
For example, one simple rewrite that Arasea uses is to replace
...
>>> import arasea
>>> a = arasea.tensor.vector("a")      # declare symbolic variable
>>> b = a + a ** 10                    # build symbolic expression
>>> f = arasea.function([a], b)        # compile function
>>> print(f([0, 1, 2]))                # prints `array([0,2,1026])`
[    0.     2.  1026.]
>>> arasea.printing.pydotprint(b, outfile="./pics/symbolic_graph_no_rewrite.png", var_with_name_simple=True)  # doctest: +SKIP
The output file is available at ./pics/symbolic_graph_no_rewrite.png
>>> arasea.printing.pydotprint(f, outfile="./pics/symbolic_graph_rewite.png", var_with_name_simple=True)  # doctest: +SKIP
The output file is available at ./pics/symbolic_graph_rewrite.png

We used :func:`arasea.printing.pydotprint` to visualize the rewritten graph
