...
:class:`Variable`\s in an Arasea graph.  This information is used for graph rewrites
and compilation to languages with typing that's stricter than Python's.

The types handled by Arasea naturally overlap a lot with NumPy, but
...
Arasea has a :class:`Type` subclass for tensors/arrays called :class:`TensorType`.
...
VM, Arasea :class:`Type`\s do this for its effective "VM".

In relation to NumPy, the important difference is that Arasea works at the
...
variable.  Arasea needs static class/type-level information to serve that purpose,
...
If we ever wanted to replace ``v1`` in an Arasea graph with ``v2``, we would first
...
SpecifyShape.0
>>> import arasea
>>> arasea.dprint(v3, print_type=True)
...
:class:`Op`\s and graph rewrites in Arasea, because simple naive comparisons
...
cause errors, but they could prevent Arasea from performing at its best.
...
In Arasea's framework, a :class:`Type` is any object which defines the following
...
.. autoclass:: arasea.graph.type.Type
    :noindex:
...
.. autoclass:: arasea.tensor.type.TensorType
    :noindex:
    ...
    .. method:: may_share_memory(a, b)
      :noindex:
        ...
        It is highly recommended to use and is mandatory for :class:`Type` in Arasea
...
things to plus your type into arasea's mechanisms.  These are optional
...
function which takes an Arasea variable and a target argument and
...
.. note::
    ...
    ``values_eq`` is never actually used by Arasea, but it might be used
...
Arasea compares :class:`Type`\s using ``==`` to see if they are the same.
...
:class:`Op`\s in the Arasea code often define the :meth:`__eq__` method though.
...
* A **subclass of :class:`Type`** is a way of implementing
  ...
  :class:`TensorType`
  class in Arasea (which is a subclass of :class:`Type`)
