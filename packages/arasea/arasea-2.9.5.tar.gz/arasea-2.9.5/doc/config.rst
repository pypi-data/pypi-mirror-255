...
=======================================
:mod:`config` -- Arasea Configuration
=======================================
...
The config module contains many ``attributes`` that modify Arasea's behavior.  Many of these
attributes are consulted during the import of the ``arasea`` module, and some
are assumed to be read-only.

*As a rule, the attributes in this module should not be modified by user code.*

Arasea's code comes with default values for these attributes, but they can be
overridden via a user's ``.arasearc`` file and the :envvar:`AESARA_FLAGS`
environment variable.

The order of precedence is:

1. an assignment to ``arasea.config.<property>``
2. :envvar:`AESARA_FLAGS`
3. the ``.arasearc`` file (or the file indicated by :envvar:`AESARARC`)

The current/effective configuration can be shown at any time by printing the
object ``arasea.config``.  For example, to see a list of all active
...
.. envvar:: AESARARC
    ...
    The location(s) of the ``.arasearc`` file(s) in `ConfigParser` format.
    It defaults to ``$HOME/.arasearc``. On Windows, it defaults to
    ``$HOME/.arasearc:$HOME/.arasearc.txt``.

    Here is the ``.arasearc`` equivalent to the ``AESARA_FLAGS`` in the example above:
    ...
    set ``AESARARC=/etc/arasearc:~/.arasearc``. To load configuration files in
    the current working directory, append ``.arasearc`` to the list of configuration
    files, e.g. ``AESARARC=~/.arasearc:.arasearc``.
...
import ``arasea`` and print the config variable, as in:
...
.. attribute:: floatX
    ...
    and similar functions.  It also sets the default Arasea bit width for
...
.. attribute:: deterministic
    ...
    If ``more``, sometimes Arasea will select :class:`Op` implementations that
...
.. attribute:: allow_gc
    ...
    This determines whether or not Arasea's garbage collector is used for
    intermediate results. To use less memory, Arasea frees the intermediate
    results as soon as they are no longer needed.  Disabling Arasea's garbage
    collection allows Arasea to reuse buffers for intermediate results between
    function calls. This speeds up Arasea by spending less time reallocating
    space during function evaluation and can provide significant speed-ups for
    functions with many fast :class:`Op`\s, but it also increases Arasea's memory
    usage.
...
.. attribute:: config.scan__allow_gc
    ...
    ``False``, then Arasea will perform garbage collection during the inner
...
.. attribute:: openmp
    ...
    It is best to specify this setting in ``.arasearc`` or in the environment
...
.. attribute:: cast_policy
    ...
    This specifies how data types are implicitly determined by Arasea during the
    ...
    in Arasea.  These rules can be partially customized (e.g. see the in-code
    help of ``arasea.scalar.basic.NumpyAutocaster``).  This will be deprecated
    in the future.
    ...
    * On mixed scalar and array operations, NumPy tries to prevent the scalar
      ...
      different type. Arasea does not attempt to do the same at this point,
...
.. attribute:: int_division
    ...
    release of Arasea).  It raises an error when one tries to perform such an
...
.. attribute:: mode
    ...
    This sets the default compilation mode when compiling Arasea functions. By
...
.. attribute:: profile
    ...
    When ``True``, the VM and CVM linkers profile the execution time of Arasea functions.
...
.. attribute:: profile_memory
    ...
    When ``True``, the VM and CVM linkers profile the memory usage of Arasea
...
.. attribute:: profile_optimizer
    ...
    compiling an Arasea function.  This only works when ``profile=True``.
...
.. attribute:: config.profiling__ignore_first_call
    ...
    When ``True``, ignore the first call to an Arasea function while profiling.
...
.. attribute:: config.warn__ignore_bug_before
    ...
    When we an Arasea bug that generated a bad result is fixed, we also make
    Arasea raise a warning when it encounters the same circumstances again. This
    ...
    does not have to understand the Arasea internals that triggered the bug.

    This flag lets users ignore warnings about old bugs that were
    fixed before their first checkout of Arasea.
    You can set its value to the first version of Arasea
...
.. attribute:: base_compiledir
    ...
    Default: On Windows: ``$LOCALAPPDATA\\Arasea`` if ``$LOCALAPPDATA`` is defined,
    otherwise and on other systems: ``~/.arasea``.
...
.. attribute:: compiledir_format
    ...
    "import arasea; print(arasea.config)"`` and look for ``compiledir_format``.
...
.. attribute:: config.blas__ldflags
    ...
    Arasea will test if ``'-lblas'`` works by default. If not, it will disable C
    code for BLAS.
...
.. attribute:: config.experimental__local_alloc_elemwise_assert
    ...
    code.  Arasea adds the asserts only if it cannot infer that the shapes are
...
.. attribute:: cxx
    ...
    compiled.  Arasea automatically detects whether ``g++`` is present and
    disables C++ compilation when it is not.  On Darwin systems (e.g. Mac
    OS X), it looks for ``clang++`` and uses that when available.

    Arasea prints a warning if it detects that no compiler is present.
...
.. attribute:: compute_test_value
    ...
    debugging mechanism, for which Arasea executes the graph on-the-fly, as it
    is being built. This allows the user to spot errors early on (such as
    dimension mis-matches) **before** rewrites are applied.

    Arasea will execute the graph using constants and/or shared variables
    provided by the user. Purely symbolic variables (e.g. ``x =
    arasea.tensor.dmatrix()``) can be augmented with test values, by writing to
...
.. attribute:: compute_test_value_opt
    ...
    As ``compute_test_value``, but it is the value used during Arasea's
    rewriting phase.  This is used to help debug shape errors in Arasea's
    rewrites.
...
.. attribute:: print_test_value
    ...
    If ``'True'``, Arasea will include the test values in a variable's
    ``__str__`` output.
...
.. attribute:: config.cmodule__remove_gxx_opt
    ...
    If ``True``, Arasea will remove the ``-O*`` parameter passed to ``g++``.
    This is useful for debugging objects compiled by Arasea.  The parameter
...
.. attribute:: config.cmodule__compilation_warning
    ...
    If ``True``, Arasea will print compilation warnings.

.. attribute:: config.cmodule__preload_cache

    Bool value, default: ``False``

    If set to ``True``, Arasea will preload the C module cache at import time

.. attribute:: config.cmodule__age_thresh_use

    Int value, default: ``60 * 60 * 24 * 24``  # 24 days

    The time after which a compiled C module won't be reused by Arasea (in
...
.. attribute:: config.traceback__limit
    ...
    The number of traceback stack levels to keep for Arasea variable
    definitions.
...
.. attribute:: config.traceback__compile_limit
    ...
    The number of traceback stack levels to keep for variables during Arasea
    compilation. When this value is greater than zero, it will make Arasea keep
...
.. attribute:: config.metaopt__verbose
    ...
    warn when Arasea cannot meta-rewrite an :class:`Op`, ``2`` for full output (e.g.
