...
Implementing an Arasea scalar Op allows that scalar operation to be reused
...
``Arasea.sparse.as_sparse_variable(x)`` on sparse input variables,
...
See the sparse :class:`Arasea.sparse.basic.Cast` `Op` code for a good example of
...
.. note::
   ...
   <Arasea.sparse.basic.EnsureSortedIndices>` if your code does not
   support it.

   Also, there can be explicit zeros in your inputs. Use
   :class:`Remove0 <Arasea.sparse.basic.Remove0>` or ``remove0`` to
   ...
   :func:`clean <Arasea.sparse.basic.clean>`.
...
Arasea does not have a native C code interface for sparse matrices. The
...
:func:`Arasea.sparse.basic.csm_properties`. You can use
:func:`Arasea.sparse.basic.csm_data`,
:func:`Arasea.sparse.basic.csm_indices`,
:func:`Arasea.sparse.basic.csm_indptr` and
:func:`Arasea.sparse.basic.csm_shape` to extract the individual
fields.
...
2. Extend MRG implementation by reusing existing Arasea Op. Look into
   the ``Arasea/sandbox/rng_mrg.py`` file and grep for all code about
   binomial(). This distribution uses the output of the uniform
   distribution and converts it to a binomial distribution with
   existing Arasea operations. The tests go in
   ``Arasea/sandbox/test_rng_mrg.py``

3. Extend MRG implementation with a new Op that takes a uniform sample as
   input. Look in the ``Arasea/sandbox/{rng_mrg,multinomial}.py`` file
   and its test in ``Arasea/sandbox/test_multinomal.py``. This is
   recommended when current Arasea ops aren't well suited to modify
...
Your Op needs to inherit from ``arasea.link.c.op.OpenMPOp``. If it overrides
...
The Arasea flag ``openmp`` is currently False by default as we do not
...
variable OMP_NUM_THREADS is not 1. This allows Arasea to respect the
current convention.
...
.. note:
   ...
   computation in an Arasea graph. If we have 2 Apply nodes with the
...
.. _alternate_Arasea_types:

Alternate Arasea Types
======================

Most ops in Arasea are used to manipulate tensors. However, Arasea also
supports many other variable types. The supported types are listed below,
along with pointers to the relevant documentation.

*       :class:`TensorType <tensor.type.TensorType>` : Arasea type that represents
        a multidimensional array containing elements that all have the same
        type. Variables of this Arasea type are represented in C as objects of
        class
        `PyArrayObject <http://docs.scipy.org/doc/numpy/reference/c-api.types-and-structures.html#PyArrayObject>`_.

*       :ref:`SparseTensorType <sparse_ops>` : Arasea `Type` used to represent sparse
        tensors. There is no equivalent C type for this Arasea `Type` but you
        can split a sparse variable into its parts as TensorVariables. Those
        can then be used as inputs to an op with C code.

*       :class:`Generic <arasea.link.c.type.Generic>` : Arasea type that
        represents a simple Python Object. Variables of this Arasea type are
        represented in C as objects of class `PyObject
        <https://docs.python.org/2/c-api/structures.html#c.PyObject>`_.

*       :class:`CDataType <arasea.link.c.type.CDataType>` :  Arasea type that
        represents a C data type. The C type associated with this Arasea type
