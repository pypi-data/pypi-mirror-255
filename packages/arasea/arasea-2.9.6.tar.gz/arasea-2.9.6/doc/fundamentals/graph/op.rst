...
.. function:: make_node(*inputs)
  ...
  using Arasea's symbolic language to compute the symbolic output
...
.. function:: __call__(*inputs, **kwargs)
   ...
   subclasses to do anything else, but must return either an Arasea
...
.. function:: infer_shape(fgraph, node, shapes)
   ...
   Implementing this method will allow Arasea to compute the output's
...
.. function:: do_constant_folding(fgraph, node)
   ...
   We replace the :class:`Apply` node with an Arasea constant variable.
...
.. function:: grad(inputs, output_gradients)
  ...
  ``output_gradients`` are lists of symbolic Arasea :class:`Variable`\s and
  ...
  If the :meth:`Op.grad` method is not defined, then Arasea assumes it has been
  ...
  more complex setting: Arasea provides in the list
  ...
  Arasea currently imposes the following constraints on the values
  ...
  functions you're likely to implement in Arasea, this means your
  ...
  Arasea. Most of the time in machine learning we mean ":math:`f` is a
  ...
  undefined.  Finally, many times in Arasea, integer valued inputs
...
.. function:: connection_pattern(node):
  ...
  1) Which of the :class:`Op`'s inputs are truly ancestors of each of the
     ...
     it appears to be so in the Arasea graph.
