...
rewrites can be constructed in Arasea.
...
.. class:: GraphRewriter
    ...
    .. method:: rewrite(fgraph)
      ...
      This is the interface function called by Arasea.  It calls
...
.. testcode::
   ...
   import arasea
   from arasea.graph.rewriting.basic import GraphRewriter
   from arasea.graph.features import ReplaceValidate
...
>>> from arasea.scalar import float64, add, mul, true_divide
>>> x = float64('x')
>>> y = float64('y')
>>> z = float64('z')
>>> a = add(z, mul(true_divide(mul(y, x), y), true_divide(z, x)))
>>> e = arasea.graph.fg.FunctionGraph([x, y, z], [a])
>>> e
...
:mod:`arasea.graph.rewriting.basic`.

>>> from arasea.graph.rewriting.basic import MergeOptimizer
...
to be different because Arasea has no clue that ``add`` is
...
arithmetic that your Ops implement. Arasea might provide facilities
for this somewhere in the future.

.. note::

   :class:`FunctionGraph` is an Arasea structure intended for the rewrite
   phase. It is used internally by :func:`arasea.function` and is rarely
...
.. testcode::
   ...
   from arasea.graph.rewriting.basic import NodeRewriter
...
>>> e
FunctionGraph(add(z, mul(true_divide(mul(y, x), y), true_divide(z, x))))
>>> simplify = arasea.graph.rewriting.basic.WalkingGraphRewriter(local_simplify)
>>> simplify.rewrite(e)
(<arasea.graph.rewriting.basic.WalkingGraphRewriter object at 0x...>, 1, 5, 3, ..., ..., ...)
>>> e
FunctionGraph(add(z, mul(x, true_divide(z, x))))
...
Arasea defines some shortcuts to make :class:`NodeRewriter`\s:
...
.. code::
   ...
   from arasea.scalar import identity
   from arasea.graph.rewriting.basic import SubstitutionNodeRewriter, RemovalNodeRewriter, PatternNodeRewriter
...
Arasea's unification and reification tools are provided by the
...
In order to use :func:`unify` and :func:`reify` with Arasea graphs, we need an intermediate
structure that will allow us to represent Arasea graphs that contain :class:`var`\s, because
Arasea :class:`Op`\s and :class:`Apply` nodes will not accept these foreign objects as inputs.
...
In this example, :func:`unify` matched the Arasea graph in the first argument with the "pattern"
...
>>> res
e(<arasea.scalar.basic.Add at 0x7f54dfa5a350>, y, y)

Since :class:`ExpressionTuple`\s can be evaluated, we can produce a complete Arasea graph from these
results as follows:

>>> res.evaled_obj
add.0
>>> arasea.dprint(res.evaled_obj)
add [id A] ''
 |y [id B]
 |y [id B]
...
>>> s
{~_2: <arasea.scalar.basic.Add at 0x7f54dfa5a350>, ~_3: e(x, y)}
>>> s = unify(cons(op_lv, args_lv), add(x, y, z))
>>> s
{~_2: <arasea.scalar.basic.Add at 0x7f54dfa5a350>, ~_3: e(x, y, z)}
...
>>> res
e(<arasea.scalar.basic.Mul at 0x7f54dfa5ae10>, x, y, z)
>>> arasea.dprint(res.evaled_obj)
mul [id A] ''
 |x [id B]
 |y [id C]
...
Given that unification and reification are fully implemented for Arasea objects via the :mod:`unificiation` package,
the `kanren <https://github.com/pythological/kanren>`_ package can be used with Arasea graphs, as well.
...
.. code::
    ...
    from arasea.graph.rewriting.kanren import KanrenRelationSub
    from arasea.graph.rewriting.basic import EquilibriumGraphRewriter
    from arasea.graph.rewriting.utils import rewrite_graph
    from arasea.tensor.math import _dot
    from etuples import etuple
    ...
    arasea.pprint.assign(
        _dot, arasea.printing.OperatorPrinter("@", -1, "left")
    )
...
Arasea exports a symbol called :obj:`optdb` which acts as a sort of ordered
...
Arasea contains one principal :class:`RewriteDatabase` object, :class:`optdb`, which
contains all of Arasea's rewriters with proper tags. It is
recommended to insert new :class:`Rewriter`\s in it. As mentioned previously,
:obj:`optdb` is a :class:`SequenceDB`, so, at the top level, Arasea applies a sequence
...
.. code-block:: python
   ...
   arasea.graph.rewriting.db.RewriteDatabaseQuery(include, require=None, exclude=None, subquery=None)
...
.. testcode::
   ...
   from arasea.compile import optdb
...
Arasea defines two :class:`EquilibriumDB`\s in which one can put node
rewrites:
...
.. autoclass:: arasea.graph.rewriting.basic.NodeProcessingGraphRewriter
    :noindex:


.. _profiling_rewrite:

Profiling Arasea Function Compilation
=====================================

If one finds that compiling an Arasea function is taking too much time,
profiling information about each Arasea rewrite can be obtained. The normal
:ref:`Arasea profiler <tut_profiling>` provides some
...
.. code-block:: none
    ...
    ==================
      ...
      Total compile time: 1.131874e+01s
        ...
        Arasea rewriter time: 1.152431e+00s
           Arasea validate time: 2.790451e-02s
        Arasea Linker time (includes C, CUDA code generation/compiling): 7.893991e-02s
           Import time 1.153541e-02s
      Time in all call to arasea.grad() 4.732513e-02s

Explanations:

* ``Total compile time: 1.131874e+01s`` gives the total time spent inside `arasea.function`.
* ``Number of Apply nodes: 50`` means that after rewriting, there are 50 apply node in the graph.
* ``Arasea rewrite time: 1.152431e+00s`` means that we spend 1.15s in the rewriting phase of `arasea.function`.
* ``Arasea validate time: 2.790451e-02s`` means that we spent 2.8e-2s in the validation phase of rewriting.
* ``Arasea Linker time (includes C code generation/compiling): 7.893991e-02s`` means that we spent 7.9e-2s in linker phase of `arasea.function`.
* ``Import time 1.153541e-02s`` is a subset of the linker time where we import the compiled module.
* ``Time in all call to arasea.grad() 4.732513e-02s`` tells that we spent a total of 4.7e-2s in all calls to `arasea.grad`. This is outside of the calls to `arasea.function`.

The *linker* phase includes the generation of the C code, the time spent
by g++ to compile and the time needed by Arasea to build the object we
...
Detailed Profiling of Arasea Rewrites
-------------------------------------

You can get more detailed profiling information about the Arasea
rewriting phase by setting to ``True`` the Arasea flags
