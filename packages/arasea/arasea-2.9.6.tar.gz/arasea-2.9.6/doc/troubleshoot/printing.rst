...
the normal python way with the print statement, because Arasea has no *statements*.
Instead there is the :class:`Print` Op.

>>> from arasea import tensor as at, function, printing
>>> x = at.dvector()
...
Arasea also provides :func:`arasea.printing.pydotprint` that creates a png image of the function.

1) The first is :func:`arasea.pp`.

>>> from arasea import pp, grad,
...
2) The second function to print a graph is :func:`arasea.printing.debugprint`

>>> arasea.printing.debugprint(f.maker.fgraph.outputs[0])  # doctest: +NORMALIZE_WHITESPACE
...
>>> arasea.printing.debugprint(gy)  # doctest: +NORMALIZE_WHITESPACE
Elemwise{mul} [id A] ''
 |Elemwise{mul} [id B] ''
...
>>> arasea.printing.debugprint(gy, depth=2)  # doctest: +NORMALIZE_WHITESPACE
Elemwise{mul} [id A] ''
 |Elemwise{mul} [id B] ''
 |Elemwise{pow} [id C] ''
...
3) The function :func:`arasea.printing.pydotprint` will print a compiled arasea function to a png file.
...
.. class:: Print(Op)
    ...
    .. method:: __call__(x)
        ...
        When you use the return-value from this function in an Arasea function,
        running the function will print the value that `x` takes in the graph.


.. autofunction:: arasea.printing.debugprint

.. function:: arasea.pp(*args)

   Just a shortcut to :func:`arasea.printing.pp`

.. autofunction:: arasea.printing.pp(*args)

.. autofunction:: arasea.printing.pydotprint
