...
1. Use the Arasea flag :attr:`config.profile` to enable profiling.
    - To enable the memory profiler use the Arasea flag:
      :attr:`config.profile_memory` in addition to :attr:`config.profile`.
    - Moreover, to enable the profiling of Arasea rewrite phases,
      use the Arasea flag: :attr:`config.profile_optimizer` in addition
      to :attr:`config.profile`.
    - You can also use the Arasea flags :attr:`profiling__n_apply`,
      :attr:`profiling__n_ops` and :attr:`profiling__min_memory_size`
      to modify the quantity of information printed.

2. Pass the argument :attr:`profile=True` to the function :func:`arasea.function
   ...
   function.
    ...
    - You can also combine the profile results of many functions:
      ...
      .. doctest::
          ...
          profile = arasea.compile.ProfileStats()
          f = arasea.function(..., profile=profile)  # doctest: +SKIP
          g = arasea.function(..., profile=profile)  # doctest: +SKIP
          ...  # doctest: +SKIP
          profile.summary()



The profiler will output one profile per Arasea function and produce a profile
...
In the global section, the "Message" is the name of the Arasea
function. :func:`arasea.function` has an optional parameter ``name`` that
defaults to ``None``. Change it to something else to help profile many
Arasea functions. In that section, we also see the number of times the
...
to understand Arasea's overhead.
...
Here is some example output when Arasea's rewrites are disabled. With all
