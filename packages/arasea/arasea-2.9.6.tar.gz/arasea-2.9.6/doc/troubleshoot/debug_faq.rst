...
=========================================
Debugging Arasea: FAQ and Troubleshooting
=========================================
...
own Arasea code, and even (it happens) in Arasea's internals, in
:ref:`using_debugmode`.

Isolating the Problem/Testing Arasea Compiler
---------------------------------------------

You can run your Arasea function in a :ref:`DebugMode<using_debugmode>`.
This tests the Arasea rewrites and helps to find where NaN, inf and other problems come from.

Interpreting Error Messages
---------------------------

Even in its default configuration, Arasea tries to display useful error
...
.. testoutput::
   ...
   HINT: Re-running with most Arasea optimizations disabled could give you a back-traces when this node was created. This can be done with by setting the Arasea flags 'optimizer=fast_compile'. If that does not work, Arasea optimizations can be disabled with 'optimizer=None'.
   HINT: Use the Arasea flag 'exception_verbosity=high' for a debugprint of this apply node.
...
The two hints can also be helpful when debugging. Using the Arasea flag
...
As of v.0.4.0, Arasea has a new mechanism by which graphs are executed
...
.. testcode:: testvalue
    ...
    x  = at.matrix('x')
    # provide Arasea with a default test-value
    #x.tag.test_value = np.random.random((5, 10))

    # transform the shared variable in some way. Arasea does not
...
.. testoutput:: testvalue
    ...
    HINT: Re-running with most Arasea optimization disabled could give you a back-traces when this node was created. This can be done with by setting the Arasea flags 'optimizer=fast_compile'. If that does not work, Arasea optimization can be disabled with 'optimizer=None'.

If the above is not informative enough, by instrumenting the code ever
so slightly, we can get Arasea to reveal the exact source of the error.
...
value. This allows Arasea to evaluate symbolic expressions on-the-fly (by
...
* Arasea ``constants`` and ``shared`` variables are used as is. No need to instrument them.
* A Arasea *variable* (i.e. ``dmatrix``, ``vector``, etc.) should be
  given a special test value through the attribute ``tag.test_value``.
* Arasea automatically instruments intermediate results. As such, any quantity
...
Arasea provides a :class:`Print`\ :class:`Op` to do this.
...
Since Arasea runs your program in a topological order, you won't have precise
...
.. warning::
    ...
    Using this :class:`Print`\ `Op` can prevent some Arasea rewrites from being
...
Arasea provides two functions, :func:`arasea.pp` and
...
Arasea also provides :func:`arasea.printing.pydotprint` that creates a PNG image of the graph.
...
Second, try the Arasea :ref:`profiling <tut_profiling>`.  This will tell you which
...
* Use the flags ``floatX=float32`` to require type float32 instead of float64.
  Use the Arasea constructors `matrix`, `vector`, etc., instead of `dmatrix`, `dvector`, etc.,
...
.. note::
    ...
    The Arasea flags ``optimizer_including``, ``optimizer_excluding``
...
needed anymore by the Arasea function. This can be done with the Arasea
flag:
...
.. testoutput::
   ...
   HINT: Use the Arasea flag 'exception_verbosity=high' for a debugprint and storage map footprint of this apply node.
...
.. code-block:: none
    ...
    File "ex.py", line 14, in <module>
      f(mat1, mat2)
    File "/u/username/Arasea/arasea/compile/function/types.py", line 451, in __call__
    File "/u/username/Arasea/arasea/graph/link.py", line 271, in streamline_default_f
    File "/u/username/Arasea/arasea/graph/link.py", line 267, in streamline_default_f
    File "/u/username/Arasea/arasea/graph/cc.py", line 1049, in execute ValueError: ('Input dimension mismatch. (input[0].shape[0] = 3, input[1].shape[0] = 5)', Elemwise{mul,no_inplace}(a, b), Elemwise{mul,no_inplace}(a, b))
...
After learning a few things about the graph structure in Arasea, we can use
...
Breakpoint during Arasea function execution
-------------------------------------------

You can set a breakpoint during the execution of an Arasea function with
