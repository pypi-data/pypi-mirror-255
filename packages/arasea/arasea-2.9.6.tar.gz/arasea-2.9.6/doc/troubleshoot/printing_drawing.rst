...
==============================
Printing/Drawing Arasea graphs
==============================


Arasea provides the functions :func:`arasea.printing.pprint` and
:func:`arasea.printing.debugprint` to print a graph to the terminal before or
after compilation. :func:`pprint` is more compact and math-like,
:func:`debugprint` is more verbose. Arasea also provides :func:`pydotprint`
that creates an image of the function. You can read about them in
:ref:`libdoc_printing`.

.. note::

    When printing Arasea functions, they can sometimes be hard to
    read.  To help with this, you can disable some Arasea rewrites
    by using the Arasea flag:
...
>>> N = 400
>>> feats = 784
>>> D = (rng.standard_normal(N, feats).astype(arasea.config.floatX), rng.integers(size=N,low=0, high=2).astype(arasea.config.floatX))
...
>>> train = arasea.function(inputs=[x,y], outputs=[prediction, xent], updates=[[w, w-0.01*gw], [b, b-0.01*gb]], name = "train")
>>> predict = arasea.function(inputs=[x], outputs=prediction, name = "predict")


Pretty Printing
===============

>>> arasea.printing.pprint(prediction) # doctest: +NORMALIZE_WHITESPACE
...
>>> arasea.printing.debugprint(prediction) # doctest: +NORMALIZE_WHITESPACE, +ELLIPSIS
...
>>> arasea.printing.debugprint(predict)  # doctest: +NORMALIZE_WHITESPACE, +ELLIPSIS
...
>>> arasea.printing.pydotprint(prediction, outfile="logreg_pydotprint_prediction.png", var_with_name_simple=True)  # doctest: +SKIP
...
>>> arasea.printing.pydotprint(predict, outfile="logreg_pydotprint_predict.png", var_with_name_simple=True)  # doctest: +SKIP
...
>>> arasea.printing.pydotprint(train, outfile="logreg_pydotprint_train.png", var_with_name_simple=True)  # doctest: +SKIP
...
The new :mod:`d3viz` module complements :func:`arasea.printing.pydotprint` to
