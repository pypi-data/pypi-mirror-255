...
.. class:: CLinkerType
    ...
    .. method:: c_element_type()
       ...
       for the variables handled by this Arasea type. For example,
...
.. warning::
   ...
   leaks because Arasea defines its own cleanup system and assumes
...
``py_<name>`` or ``storage_<name>`` because Arasea already defines
them.
...
we have a reference to a Python object which Arasea has placed in
``py_%(name)s`` where ``%(name)s`` must be substituted for the name
given in the inputs. This special variable is declared by Arasea as
...
variable which Arasea recognizes (this is the same pointer we get in
...
``py_%(name)s``. From there Arasea will do the rest and the end user
...
``NULL`` is a pretty bad reference to have and neither Python nor Arasea
...
``%(fail)s``. Furthermore, because of the way Arasea blocks code together,
...
respect the user vs. Arasea memory region as described in the :ref:`tutorial
<aliasing>`. Arasea has a Python implementation that calls the object's
``copy`` or ``deepcopy`` method for Arasea types for which it does not
...
.. code-block:: python
   ...
   arasea.compile.ops.register_deep_copy_op_c_code(YOUR_TYPE_CLASS, THE_C_CODE, version=())
...
.. code-block:: python
   ...
   arasea.compile.ops.register_view_op_c_code(YOUR_TYPE_CLASS, THE_C_CODE, version=())
...
Arasea :class:`Variable` that has a shape attribute (:class:`Shape_i` returns only one of
the elements of the shape).


.. code-block:: python

   from arasea.tensor.shape import register_shape_c_code, register_shape_i_c_code
