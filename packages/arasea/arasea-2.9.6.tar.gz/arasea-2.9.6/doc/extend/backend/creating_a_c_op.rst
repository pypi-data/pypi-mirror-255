...
=====================================
Extending Arasea with a C :Class:`Op`
=====================================

This tutorial covers how to extend Arasea with an :class:`Op` that offers a C
implementation.  This tutorial is aimed at individuals who already know how to
extend Arasea (see tutorial :ref:`creating_an_op`) by adding a new :class:`Op`
...
Providing an Arasea :class:`Op` with a C implementation requires to interact with
...
two macros most often used in Arasea C :class:`Op`\s.
...
are used extensively inside Arasea and so extending Arasea with a C :class:`Op` will
require interaction with the NumPy C-API.

This sections covers the API's elements that are often required to write code
for an Arasea C :class:`Op`. The full documentation for the API can be found here :
...
time an Arasea function was called, this overhead would occur as many times
as the number of :class:`Op`\s with C implementations in the function's computational
graph.

To maximize performance, Arasea instead requires the C :class:`Op`\s to simply return
...
Arasea is able to minimize the number of times C code is called from Python
code.
...
Moreover, the fact that Arasea itself takes care of compiling the C code,
instead of the individual :class:`Op`\s, allows Arasea to easily cache the compiled C
...
.. method:: c_support_code_apply(node, name)
    ...
    because an Arasea `Op` can be used more than once in a given Arasea
    function. For example, an `Op` that adds two matrices could be used at some
    point in the Arasea function to add matrices of integers and, at another
...
.. method:: c_code_cache_version()
    ...
    This tuple is used by Arasea to cache the compiled C code for this `Op`. As
    such, the return value **MUST BE CHANGED** every time the C code is altered
    or else Arasea will disregard the change in the code and simply load a
    ...
    :note:
        Arasea can handle tuples of any hashable objects as return values
...
and outputs storage. Arasea guarantees that the inputs exist and have the
...
For the outputs, things are a little bit more subtle. Arasea does not
guarantee that they have been allocated but it does guarantee that, if they
have been allocated, they have the right number of dimension. Again, Arasea
...
in Arasea by inheriting from the class :class:`Op`. This process is mostly
...
To help with this, Arasea defines a class, `ExternalCOp`, from which new C :class:`Op`\s
...
*       It must return an int. The value of that int indicates whether
        ...
        of the Arasea function.  When returning non-zero the function
...
For this, you must enable this Arasea: `cmodule__remove_gxx_opt=True`.
...
Arasea tensors. For more information about other Arasea types, you can refer
to the section :ref:`Alternate Arasea Types <alternate_arasea_types>`.
