...
Arasea is able to convert its graphs into JAX and Numba compiled functions. In order to do
this, each :class:`Op` in an Arasea graph must have an equivalent JAX/Numba implementation function.
...
Step 1: Identify the Arasea :class:`Op` you’d like to implement in JAX
----------------------------------------------------------------------

Find the source for the Arasea :class:`Op` you’d like to be supported in JAX, and
...
with Arasea :class:`Op`\s in order to provide a conversion implementation, so first read
...
automatically converted to Arasea variables via :func:`as_tensor_variable`, and
...
attribute of the Arasea :class:`Eye`\ :class:`Op` it's converting.
...
With a precise idea of what the Arasea :class:`Op` does we need to figure out how
...
With the Arasea `Op` replicated in JAX, we’ll need to register the
function with the Arasea JAX `Linker`. This is done through the use of
...
The relevant dispatch functions created by `singledispatch` are :func:`arasea.link.numba.dispatch.basic._numba_funcify` and
:func:`arasea.link.jax.dispatch.jax_funcify`.
...
.. code:: python
   ...
   from arasea.tensor.basic import Eye
   from arasea.link.jax.dispatch import jax_funcify
...
appropriate test suites in Arasea (e.g. in ``tests.link.test_jax`` and one of
...
.. code:: python
   ...
   def test_jax_Eye():
       ...
       # Create an Arasea `FunctionGraph`
