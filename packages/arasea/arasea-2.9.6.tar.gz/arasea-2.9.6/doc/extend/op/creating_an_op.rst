...
Odds are your function that uses existing Arasea expressions is short,
...
.. note::
    ...
    without doing as prescribed in that page, Arasea will run, but will
...
Arasea Graphs refresher
-----------------------

.. image:: apply.png
    :width: 500 px

Arasea represents symbolic mathematical computations as graphs. Those graphs
...
intermediary values. As such, inputs and outputs of a graph are lists of Arasea
...
or :meth:`Op.make_thunk`.
  ...
  of the :class:`Op` on the inputs provided. This method is responsible for three things:
    ...
    - it operates on the :class:`Variable`\s found in
      ``*inputs`` in Arasea's symbolic language to infer the type of
  ...
  If :meth:`Op.make_thunk` is defined by an :class:`Op`, it will be used by Arasea
...
There are other methods that can be optionally defined by the :class:`Op`:
  ...
  those must be operated on using Arasea's symbolic language. The :meth:`Op.grad`
...
.. and ``__hash__``.
.. When two :class:`Op`\s are equal, Arasea will merge their outputs if they are applied to the same inputs.
...
.. function so that he two ``Fibby`` :class:`Op`\s would *not be equal*.  The reason why: Arasea's merge
...
is important:  Arasea will call ``make_node(*inputs)`` to copy the graph,
...
Arasea has some functionalities to simplify testing. These help test the
...
the default tolerance can be changed with the Arasea flags
...
:meth:`InferShapeTester._compile_and_check` compiles an Arasea function. It takes as
parameters the lists of input and output Arasea variables, as would be
...
The function ``verify_grad`` verifies the gradient of an :class:`Op` or Arasea
...
(Notice that Arasea's current *elemwise fusion* rewrite is
...
basic Arasea :class:`Op` that will call the supplied function during execution.
...
.. note::
    ...
    inputs Arasea variables that were declared.
