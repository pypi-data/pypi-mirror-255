...
This page describes :class:`arasea.compile.builders.OpFromGraph
<arasea.compile.builders.OpFromGraph>`, an `Op` constructor that allows one to
encapsulate an Arasea graph in a single `Op`.

This can be used to encapsulate some functionality in one block. It is
useful to scale Arasea compilation for regular bigger graphs when we
reuse that encapsulated functionality with different inputs many
times. Due to this encapsulation, it can make Arasea's compilation phase
...
.. autoclass:: arasea.compile.builders.OpFromGraph
