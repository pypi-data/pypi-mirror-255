...
.. module:: arasea.compile.nanguardmode
...
.. testcode::
    ...
    from arasea.compile.nanguardmode import NanGuardMode

    x = at.matrix()
    w = arasea.shared(np.random.standard_normal((5, 7)).astype(arasea.config.floatX))
    y = at.dot(x, w)
    fun = arasea.function(
        [x], y,
...
While using the Arasea function ``fun``, it will monitor the values of each
...
.. testcode::
    ...
    infa = np.tile((np.asarray(100.) ** 1000000).astype(arasea.config.floatX), (3, 5))
    fun(infa)
...
.. autoclass:: arasea.compile.nanguardmode.NanGuardMode
