...
Derivatives in Arasea
=====================

Computing Gradients
===================

Now let's use Arasea for a slightly more sophisticated task: create a
...
.. note::
    Arasea's rewrites simplify the symbolic gradient expression.  You can see
...
>>> gs = at.grad(s, x)
>>> dlogistic = arasea.function([x], gs)
>>> dlogistic([[0, 1], [-1, -2]])
array([[ 0.25      ,  0.19661193],
       [ 0.19661193,  0.10499359]])

In general, for any **scalar** expression ``s``, ``at.grad(s, w)`` provides
the Arasea expression for computing :math:`\frac{\partial s}{\partial w}`. In
this way Arasea can be used for doing **efficient** symbolic differentiation
...
In Arasea's parlance, the term **Jacobian** designates the tensor comprising the
...
Arasea implements the :func:`arasea.gradient.jacobian` macro that does all
...
.. note::
    ...
    `scan` is a generic op in Arasea that allows writing in a symbolic
...
>>> x = at.dvector('x')
>>> y = x ** 2
>>> J, updates = arasea.scan(lambda i, y, x : at.grad(y[i], x), sequences=at.arange(y.shape[0]), non_sequences=[y, x])
>>> f = arasea.function([x], J, updates=updates)
>>> f([4, 4])
array([[ 8.,  0.],
       [ 0.,  8.]])
...
.. note::
    ...
    ``arasea.scan(lambda y_i,x: at.grad(y_i,x), sequences=y, non_sequences=x)``,
...
In Arasea, the term **Hessian** has the usual mathematical meaning: It is the
matrix comprising the second order partial derivative of a function with scalar
output and vector input. Arasea implements :func:`arasea.gradient.hessian` macro that does all
...
>>> cost = y.sum()
>>> gy = at.grad(cost, x)
>>> H, updates = arasea.scan(lambda i, gy,x : at.grad(gy[i], x), sequences=at.arange(gy.shape[0]), non_sequences=[gy, x])
>>> f = arasea.function([x], H, updates=updates)
>>> f([4, 4])
array([[ 2.,  0.],
       [ 0.,  2.]])
...
While in principle we would want Arasea to identify these patterns automatically for us,
...
expressions in terms of weight matrices, Arasea supports this more generic
...
>>> x = at.dvector('x')
>>> y = at.dot(x, W)
>>> JV = arasea.gradient.Rop(y, W, V)
>>> f = arasea.function([W, V, x], JV)
...
>>> x = at.dvector('x')
>>> y = at.dot(x, W)
>>> VJ = arasea.gradient.Lop(y, W, v)
>>> f = arasea.function([v,x], VJ)
>>> f([2, 2], [0, 1])
array([[ 0.,  0.],
       [ 2.,  2.]])
...
>>> y = at.sum(x ** 2)
>>> gy = at.grad(y, x)
>>> Hv = arasea.gradient.Rop(gy, x, v)
>>> f = arasea.function([x, v], Hv)
>>> f([4, 4], [2, 2])
array([ 4.,  4.])
...
- The `grad` function works symbolically: it receives and returns Arasea variables.
