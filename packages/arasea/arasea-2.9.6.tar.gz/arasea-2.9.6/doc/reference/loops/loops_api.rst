...
in Arasea. Scan comes with many whistles and bells, which we will introduce
by way of examples.
...
The equivalent Arasea code would be:
...
This demonstrates that you can introduce new Arasea variables into a scan function.
...
.. testoutput:: scan1
    ...
    not given a value.Use the Arasea flag exception_verbosity='high',for
...
the Arasea variables needed we construct our RNN as follows :
...
Arasea needs to keep in memory some intermediate computations of scan. This
...
This section covers some ways to improve performance of an Arasea function
using Scan.
...
There are two ways to achieve this, using the Arasea flag
...
This one is simple but still worth pointing out. Arasea is able to
automatically recognize and rewrite many computation patterns. However, there
are patterns that Arasea doesn't rewrite because doing so would change the
user interface (such as merging shared variables together into a single one,
for instance). Additionally, Arasea doesn't catch every case that it could
...
many times for every execution of the Arasea function that contains it.
...
rewrite that Arasea cannot perform. Instead of performing many matrix
