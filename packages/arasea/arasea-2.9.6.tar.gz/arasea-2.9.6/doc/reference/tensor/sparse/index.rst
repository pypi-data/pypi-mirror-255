...
Arasea's sparse package provides efficient algorithms, but its use is not recommended
...
of the matrix. Since Arasea's sparse matrix package is based on the SciPy
sparse package, complete information about sparse matrices can be found
in the SciPy documentation. Like SciPy, Arasea does not implement sparse formats for
arrays with a number of dimensions different from two.

So far, Arasea implements two ``formats`` of sparse matrix: ``csc`` and ``csr``.
...
Arasea supports two *compressed sparse formats*: ``csc`` and ``csr``, respectively based on columns
...
Handling Sparse in Arasea
-------------------------

Most of the ops in Arasea depend on the ``format`` of the sparse matrix.
...
To move back and forth from a dense matrix to a sparse matrix representation, Arasea
