...
The aggressive reuse of memory is one of the ways through which Arasea makes code fast, and
it is important for the correctness and speed of your program that you understand
how Arasea might alias buffers.

This section describes the principles based on which Arasea handles memory, and explains
...
There are some simple principles that guide Arasea's handling of memory.  The
main idea is that there is a pool of memory managed by Arasea, and Arasea tracks
changes to values in that pool.

- Arasea manages its own memory space, which typically does not overlap with
  the memory of normal Python variables that non-Arasea code creates.

- Arasea functions only modify buffers that are in Arasea's memory space.

- Arasea's memory space includes the buffers allocated to store ``shared``
  variables and the temporaries used to evaluate functions.

- Physically, Arasea's memory space may be spread across the host, a GPU
...
- Arasea's managed memory is constant while Arasea functions are not running
  and Arasea's library code is not running.

- The default behaviour of a function is to return user-space values for
  outputs, and to expect user-space values for inputs.

The distinction between Arasea-managed memory and user-managed memory can be
broken down by some Arasea functions (e.g. ``shared``, ``get_value`` and the
...
It is not guaranteed to occur because if Arasea is using a GPU device, then the
``borrow`` flag has no effect. It may occur only temporarily because
if we call an Arasea function that updates the value of ``s_true`` the aliasing
...
may not be aliased to any part of Arasea's internal memory.
When ``borrow=True`` is passed to ``get_value``, it means that the return value
might be aliased to some of Arasea's internal memory.
...
must return a NumPy array too.  That's how Arasea can make the GPU use
...
If you really want Arasea to return its internal representation and never copy it
...
that Arasea may reuse the buffer you provide as the internal storage for the variable.
...
makes it possible to expose Arasea's internal variables without a copy, then it
...
..
   ...
   * Prior to Arasea 0.3.1, ``set_value`` did not work in-place on the GPU. This meant that, sometimes,
     ...
     *Solution*: update to a newer version of Arasea.
   ...
   * If you are going to swap several chunks of data in and out of a ``shared`` variable repeatedly,
     ...
     *Solution*: upgrade to a recent version of Arasea (>0.3.0) and consider padding your source
     data to make sure that every chunk is the same size.

   * It is also worth mentioning that, current GPU copying routines
     support only contiguous memory.  So Arasea must make the value you
...
Borrowing an input means that Arasea will treat the argument you provide as if
it were part of Arasea's pool of temporaries.  Consequently, your input
...
Borrowing an output means that Arasea will not insist on allocating a fresh
...
value right away before calling any more Arasea functions.
...
returns and you would like to make it available to Arasea as
