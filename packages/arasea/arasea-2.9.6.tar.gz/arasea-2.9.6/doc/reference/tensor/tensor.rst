...
.. admonition:: Example
    ...
    Note that Arasea returns `None` when the shape of the tensor is unknown.
...
.. danger::
   ...
   The Python operators ``==`` and ``!=`` do not work as a comparison operator in the usual sense in Arasea. Use :func:`eq` and :func:`neq` respectively instead.
