...
(function() {
  ...
  var arasea_dir = 'arasea'; // directory containing arasea doc
  // Default arasea version: release and development.
  var versions_dir = {"release": "arasea", "dev": "arasea_versions/dev"};
  ...
  var root_url = url.substring(0, url.search('/' + arasea_dir)) + '/';

  // Regular expression to find arasea version directory in URL.
  var version_regex = new RegExp("\\/" + arasea_dir + "(_versions\\/)?([_a-zA-Z.0-9]*)\\/");
  ...
  if (current_version != "arasea" && current_version != "html") {
    ver = current_version.replace("arasea_versions/", "")
  ...
  // Build HTML string for version selector, based on ReadTheDocs theme's versions.html
    ...
    var vlabel = current_version.replace("arasea_versions/", "");
    if (vlabel == 'arasea') {
      vlabel = 'release';
    }
    ...
    var git_master = "https://github.com/arasea-devs/arasea"
...
// Create HTML for version switcher and assign to placeholder in layout.html.
  $(document).ready(function() {
    ...
    if (url.startsWith('http')) {
      $.getJSON(root_url + 'arasea_versions/versions.json', function(data){
