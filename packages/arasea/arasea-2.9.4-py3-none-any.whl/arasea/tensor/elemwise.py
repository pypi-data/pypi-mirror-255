...
import arasea.tensor.basic
from arasea.configdefaults import config
from arasea.gradient import DisconnectedType
from arasea.graph.basic import Apply
from arasea.graph.null_type import NullType
from arasea.graph.utils import MethodNotDefined
from arasea.link.c.basic import failure_code
from arasea.link.c.op import COp, ExternalCOp, OpenMPOp
from arasea.link.c.params_type import ParamsType
from arasea.misc.frozendict import frozendict
from arasea.misc.safe_asarray import _asarray
from arasea.printing import FunctionPrinter, Printer, pprint
from arasea.scalar import get_scalar_type
from arasea.scalar.basic import bool as scalar_bool
from arasea.scalar.basic import identity as scalar_identity
from arasea.scalar.basic import transfer_type, upcast
from arasea.tensor import _get_vector_length, as_tensor_variable
from arasea.tensor import elemwise_cgen as cgen
...
class DimShuffle(ExternalCOp):
    ...
    new_order
        ...
        as python integers, not arasea symbolic integers.
...
class Elemwise(OpenMPOp):
    ...
    def R_op(self, inputs, eval_points):
        ...
        for idx, out in enumerate(outs):
            ...
            ograds[idx] = arasea.tensor.basic.ones_like(out)
    ...
    def _bgrad(self, inputs, outputs, ograds):
        ...
        def transform(r):
            ...
            if node is None:
                ...
                # dimensions
                res = arasea.tensor.basic.constant(
    ...
    def perform(self, node, inputs, output_storage):
        ...
        for i, (variable, storage, nout) in enumerate(
            ...
            # trunc, eq, ...
                ...
                # Normally we would call arasea.misc._asarray, but it
    ...
    def infer_shape(self, fgraph, node, i_shapes) -> List[Tuple[TensorVariable, ...]]:
        ...
        out_shape = arasea.tensor.broadcast_shape(*i_shapes, arrays_are_shapes=True)
...
class CAReduce(COp):
    ...
    def _c_all(self, node, name, inames, onames, sub):
        ...
        if len(axis) == 0:
            ...
            var = arasea.tensor.basic.cast(input, node.outputs[0].dtype)
            if var is input:
...
def scalar_elemwise(*symbol, nfunc=None, nin=None, nout=None, symbolname=None):
    ...
    """
    import arasea.scalar as scalar
