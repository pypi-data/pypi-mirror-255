...
import arasea
from arasea.gradient import DisconnectedType
from arasea.graph.basic import Apply, Variable
from arasea.graph.type import HasShape
...
from arasea.tensor import _get_vector_length, as_tensor_variable
...
from arasea.tensor.type import DenseTensorType, TensorType, int_dtypes, tensor
from arasea.tensor.type_other import NoneConst
from arasea.tensor.var import TensorConstant, TensorVariable
...
class Shape(COp):
    ...
    def make_node(self, x):
        ...
        if isinstance(x.type, TensorType):
            ...
            out_var = arasea.tensor.type.lvector()
    ...
    def grad(self, inp, grads):
        ...
        # in the computation of the shape, so they are not really
        # part of the graph
        return [arasea.gradient.DisconnectedType()()]
...
def shape_tuple(x: TensorVariable) -> Tuple[Variable, ...]:
    ...
    for i in range(x.type.ndim):
        ...
        if shape_val is not None:
            ...
            res += (arasea.scalar.ScalarConstant(arasea.scalar.int64, shape_val),)
        else:
...
class Shape_i(COp):
    ...
    def __init__(self, i):
        ...
        if isinstance(i, np.ndarray):
            assert i.dtype in arasea.tensor.type.integer_dtypes
        assert i == int(i)
        i = int(i)
        self.i = i
    ...
    #    loop in Python import caused by importing arasea.scalar below
    ...
    def params_type(self):
        return ParamsType(i=arasea.scalar.basic.int64)
    ...
    def make_node(self, x):
        ...
        return Apply(self, [x], [arasea.tensor.type.lscalar()])
    ...
    def grad(self, inp, grads):
        return [
            arasea.gradient.grad_not_implemented(
                op=self,
                x_pos=0,
                x=inp[0],
...
class SpecifyShape(COp):
    ...
    def make_node(self, x, *shape):
        from arasea.tensor.basic import get_scalar_constant_value
        ...
        if any(
            s.dtype not in arasea.tensor.type.integer_dtypes
            for s in shape
            if hasattr(s, "dtype")
        ):
    ...
    def grad(self, inp, grads):
        ...
        return [specify_shape(gz, shape)] + [
            arasea.gradient.DisconnectedType()() for _ in range(len(shape))
        ]
...
class Reshape(COp):
    ...
    def infer_shape(self, fgraph, node, ishapes):
        from arasea.tensor.math import eq, maximum, mul
...
def shape_padaxis(t, axis):
    ...
    Examples
    --------
    >>> tensor = arasea.tensor.type.tensor3()
    >>> arasea.tensor.shape_padaxis(tensor, axis=0)
    DimShuffle{x,0,1,2}.0
    >>> arasea.tensor.shape_padaxis(tensor, axis=1)
    DimShuffle{0,x,1,2}.0
    >>> arasea.tensor.shape_padaxis(tensor, axis=3)
...
def specify_broadcastable(x, *axes):
    ...
    -------
    tensor
        A arasea tensor, which is broadcastable along the specified dimensions.
...
class Unbroadcast(COp):
    ...
    See Also
    --------
    unbroadcast <arasea.tensor.shape.unbroadcast>
...
def unbroadcast(x, *axes):
    ...
    -------
    tensor
        A arasea tensor, with static broadcastable dimensions masked as `None`
