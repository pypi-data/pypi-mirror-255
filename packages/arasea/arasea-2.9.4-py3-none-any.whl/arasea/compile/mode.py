...
from arasea.graph.rewriting.db import (
    EquilibriumDB,
    LocalGroupDB,
    RewriteDatabase,
...
from arasea.link.basic import Linker, PerformLinker
from arasea.link.c.basic import CLinker, OpWiseCLinker
from arasea.link.jax.linker import JAXLinker
from arasea.link.numba.linker import NumbaLinker
from arasea.link.vm import VMLinker


_logger = logging.getLogger("arasea.compile.mode")
...
class AddDestroyHandler(GraphRewriter):
    ...
    there is a bug in arasea. It should not be possible to destroy outputs.
...
class PrintCurrentFunctionGraph(GraphRewriter):
    ...
    def apply(self, fgraph):
        import arasea.printing

        print("PrintCurrentFunctionGraph:", self.header)
        arasea.printing.debugprint(fgraph.outputs)
