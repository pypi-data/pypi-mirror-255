...
from arasea.tensor.type import discrete_dtypes


logger = logging.getLogger("arasea.compile.nanguardmode")
...
def _is_numeric_value(arr, var):
    ...
    from arasea.tensor.random.type import RandomType
...
class NanGuardMode(Mode):
    ...
    def __init__(
        ...
        def do_check_on(value, nd, var=None):
            ...
            if error:
                if nd:
                    ...
                    print(arasea.printing.debugprint(nd, file="str"), file=sio)
                else:
                    print(
                ...
                if nd:
                    var = nd.outputs[0]
                print(arasea.graph.utils.get_variable_trace_string(var), file=sio)
                msg = sio.getvalue()
        ...
        wrap_linker = arasea.link.vm.VMLinker(
