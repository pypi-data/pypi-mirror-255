...
from arasea.compile.ops import update_placeholder
from arasea.compile.sharedvalue import SharedVariable
from arasea.configdefaults import config
from arasea.gradient import DisconnectedType, Rop, grad
...
from arasea.graph.rewriting.basic import in2out, node_rewriter
...
class OpFromGraph(Op, HasInnerGraph):
    ...
    The signature is similar to :func:`arasea.function <arasea.function>`
    ...
    .. code-block:: python
        ...
        from arasea import function, tensor as at
        from arasea.compile.builders import OpFromGraph
        x, y, z = at.scalars('xyz')
        e = x + y * z
        op = OpFromGraph([x, y, z], [e])
        # op behaves like a normal arasea op
    ...
    .. code-block:: python
        ...
        import numpy as np
        import arasea
        from arasea import config, function, tensor as at
        from arasea.compile.builders import OpFromGraph

        x, y, z = at.scalars('xyz')
        s = arasea.shared(np.random.random((2, 2)).astype(config.floatX))
        e = x + y * z + s
        op = OpFromGraph([x, y, z], [e])
        # op behaves like a normal arasea op
        e2 = op(x, y, z) + op(z, y, x)
    ...
    .. code-block:: python
        ...
        from arasea import function, tensor as at, grad
        from arasea.compile.builders import OpFromGraph
    ...
    def __init__(
        ...
        **kwargs,
        ...
        rop_overrides
            ...
            :class:`Variable <arasea.graph.basic.Variable>`. Each list element
        ...
        for v in shared_inputs:
            # This is needed so that `arasea.function` will create an update
            ...
            # removed at the `arasea.function` level.
    ...
    def infer_shape(self, fgraph, node, shapes):
        ...
        # `ret = [arasea.clone_replace(shp, replace=repl) for shp in out_shp]`
