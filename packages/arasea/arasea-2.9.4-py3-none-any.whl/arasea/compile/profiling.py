...
from arasea.link.utils import get_destroy_dependencies
...
logger = logging.getLogger("arasea.compile.profiling")

arasea_imported_time: float = time.perf_counter()
...
def print_global_stats():
    ...
    with extended_open(destination_file, mode="w"):
        ...
        print(
            (
                "Global stats: ",
                f"Time elasped since Aesara import = {time.perf_counter() - arasea_imported_time:6.3f}s, "
    ...
    **kwargs : misc initializers
    ...
    def summary_class(self, file=sys.stderr, N=None):
        ...
        for f, t, a, impl, nb_call, nb_apply in otimes[:N]:
            ...
            # "<class 'arasea.backend.blas.Dot22'>" ->
            #  "arasea.backend.blas.Dot22"
    ...
    def summary_globals(self, file):
        print(
            f"Time in all call to arasea.grad() {arasea.gradient.grad_time:e}s",
            file=file,
        )
        total_time = time.perf_counter() - arasea_imported_time
        print(f"Time since arasea import {total_time:.3f}s", file=file)
    ...
    def summary(self, file=sys.stderr, n_ops_to_print=20, n_apply_to_print=20):
        ...
        if config.profiling__debugprint:
            ...
            arasea.printing.debugprint(fcts, print_type=True)
    ...
    def print_tips(self, file):
        ...
        if not config.lib__amblibm and any(
            ...
            print(
                ...
                "install amdlibm and set the arasea flags lib__amblibm=True",
                file=file,
            )
            printed_tip = True
        ...
        for fgraph, a in self.apply_time:
            ...
            if isinstance(node.op, RandomVariable):
                ...
                print(
                    ...
                    "'from arasea.sandbox.rng_mrg import MRG_RandomStream "
