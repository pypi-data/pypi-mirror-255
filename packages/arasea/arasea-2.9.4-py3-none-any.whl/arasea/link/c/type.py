...
from arasea.utils import Singleton
...
class CType(Type[D], CLinkerType):
    ...
    Whenever you create a symbolic variable in arasea (technically,
    ...
    In this sense, arasea is like a strongly-typed language because
...
class CDataType(CType[D]):
    ...
    The constructor builds a type made to represent a C pointer in arasea.
...
class EnumType(CType, dict):
    ...
    **Example with aliases**
    ...
    .. code-block:: python
        ...
        from arasea.link.c.type import EnumType
    ...
    def c_to_string(self):
        ...
        .. code-block:: c
            ...
            int arasea_enum_to_string_<cname>(<ctype> value, char* output_string);
        ...
        It is available in C code when arasea flag ``config.cmodule__debug`` is set to ``True``.
        """
        return """
        #ifdef DEBUG
        int arasea_enum_to_string_%(cname)s(%(ctype)s in, char* out) {
            int ret = 0;
            switch(in) {
                %(cases)s
                default:
