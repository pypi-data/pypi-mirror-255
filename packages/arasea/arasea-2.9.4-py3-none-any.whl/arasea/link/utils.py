...
def raise_with_op(
    ...
    storage_map: dict, optional
        storage map of the arasea function that resulted in the
        raised exception.
    ...
    if verbosity == "high":
        ...
        f = io.StringIO()
        arasea.printing.debugprint(node, file=f, stop_on_name=True, print_type=True)
    ...
    if verbosity == "high" and storage_map is not None:
        ...
        shared_input_list = [
            ...
            if isinstance(item, arasea.compile.SharedVariable)
        ]
        nonshared_input_list = [
            item
            for item in fgraph.inputs
            if not isinstance(item, arasea.compile.SharedVariable)
        ]
...
def register_thunk_trace_excepthook(handler: TextIO = sys.stdout) -> None:
    """Adds the `__log_thunk_trace` except hook to the collection in `arasea.utils`.
