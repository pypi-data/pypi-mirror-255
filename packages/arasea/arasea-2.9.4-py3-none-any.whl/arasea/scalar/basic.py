...
you probably want to use arasea.tensor.[c,z,f,d,b,w,i,l,]scalar!
"""
...
import arasea
from arasea import printing
from arasea.configdefaults import config
from arasea.gradient import DisconnectedType, grad_undefined
from arasea.graph.basic import Apply, Constant, Variable, clone, list_of_nodes
...
from arasea.graph.type import HasDataType, HasShape
from arasea.graph.utils import MetaObject, MethodNotDefined
from arasea.link.c.op import COp
from arasea.link.c.type import CType
from arasea.misc.safe_asarray import _asarray
from arasea.printing import pprint
from arasea.utils import (
    apply_across_args,
    difference,
    from_return_values,
    to_return_values,
...
def as_common_dtype(*vars):
    """
    For for arasea.scalar.ScalarType and TensorVariable.
    """
...
class ScalarType(CType, HasDataType, HasShape):
    ...
    def dtype_specs(self):
        try:
            ...
            return {  # dtype: (py_type, c_type, cls_name)
                ...
                "complex128": (np.complex128, "arasea_complex128", "Complex128"),
                "complex64": (np.complex64, "arasea_complex64", "Complex64"),
    ...
    def c_support_code(self, **kwargs):
        if self.dtype.startswith("complex"):
            cplx_types = ["arasea_complex64", "arasea_complex128"]
            real_types = [
                "npy_int8",
                "npy_int16",
                "npy_int32",
            ...
            template = """
            struct arasea_complex%(nbits)s : public npy_complex%(nbits)s
            {
                typedef arasea_complex%(nbits)s complex_type;
                ...
                arasea_complex%(nbits)s() {}

                template <typename T>
                arasea_complex%(nbits)s(const T& y) { *this = y; }

                template <typename TR, typename TI>
                arasea_complex%(nbits)s(const TR& r, const TI& i) { this->real=r; this->imag=i; }
            };
            """
...
arasea.compile.register_view_op_c_code(
    ScalarType,
    """
    %(oname)s = %(iname)s;
    """,
...
def as_scalar(x: Any, name: Optional[str] = None) -> ScalarVariable:
    from arasea.tensor.basic import scalar_from_tensor
...
class Composite(ScalarOp, HasInnerGraph):
    ...
    def __init__(self, inputs, outputs):
        ...
        if len(outputs) > 1 or not any(
            ...
            # Composite
            res = arasea.compile.rebuild_collect_shared(
            ...
            # the inner Composite
            res2 = arasea.compile.rebuild_collect_shared(
    ...
    def py_perform(self):
        ...
        from arasea.link.utils import fgraph_to_python
    ...
    def c_code_template(self):
        from arasea.link.c.interface import CLinkerType
