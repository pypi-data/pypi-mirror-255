...
from arasea.compile.function.pfunc import construct_pfunc_ins_and_outs
from arasea.compile.sharedvalue import SharedVariable, collect_new_shareds
from arasea.configdefaults import config
from arasea.graph.basic import Constant, Variable, clone_replace, graph_inputs
from arasea.graph.op import get_test_value
from arasea.graph.utils import MissingInputError, TestValueError
from arasea.scan.op import Scan, ScanInfo
from arasea.scan.utils import expand_empty, safe_new, until
...
from arasea.tensor.shape import shape_padleft, unbroadcast
...
def get_updates_and_outputs(ls):
    ...
    def _filter(x):
        ...
        variables or `arasea.scan.utils.until` objects.
    ...
    if not _filter(ls):
        raise ValueError(
            ...
            "variables (or `arasea.scan.utils.until` objects for "
...
def scan(
    ...
    tuple
        ...
        This ``dict`` should be passed to `arasea.function` when you compile
        your function.
    ...
    # create arasea inputs for the recursive function
