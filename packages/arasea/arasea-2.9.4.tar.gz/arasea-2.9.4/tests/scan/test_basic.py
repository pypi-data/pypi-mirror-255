"""
   ...
   * Scan seems to do copies of every input variable. Is that needed?
   ...
   arasea variables, which just cary information about the type / dimension
   of the data)
...
from arasea.compile.function.pfunc import rebuild_collect_shared
from arasea.compile.mode import Mode, get_default_mode, get_mode
from arasea.compile.monitormode import MonitorMode
from arasea.compile.sharedvalue import shared
from arasea.configdefaults import config
from arasea.gradient import NullTypeGradError, Rop, disconnected_grad, grad, hessian
from arasea.graph.basic import Apply, ancestors, equal_computations
...
from arasea.graph.utils import MissingInputError
from arasea.misc.safe_asarray import _asarray
from arasea.raise_op import assert_op
from arasea.scan.basic import scan
from arasea.scan.op import Scan
from arasea.scan.utils import until
from arasea.tensor.math import all as at_all
from arasea.tensor.math import dot, exp, mean, sigmoid
from arasea.tensor.math import sum as at_sum
from arasea.tensor.math import tanh
from arasea.tensor.random import normal
from arasea.tensor.random.utils import RandomStream
from arasea.tensor.shape import Shape_i, reshape, specify_shape
from arasea.tensor.sharedvar import SharedVariable
from arasea.tensor.subtensor import Subtensor
...
class TestScan:
    ...
    def test_pickling(self):
        ...
        arasea_values = my_f(state, steps)
        utt.assert_allclose(numpy_values, arasea_values)
    ...
    def test_one_sequence_one_output_weights(self):
        ...
        arasea_values = f2(v_u, v_x0, W_in, W)
        utt.assert_allclose(arasea_values, v_out)
    ...
    def test_one_sequence_one_output_weights_shared(self):
        ...
        arasea_values = f3(v_u, v_x0)
        assert np.allclose(arasea_values, v_out)
    ...
    def test_output_only(self):
        ...
        numpy_result = v_u + 3
        arasea_result = f2(v_u)
        utt.assert_allclose(arasea_result, numpy_result)
    ...
    def test_shared_arguments_with_updates(self):
        ...
        arasea_y0, arasea_y1, arasea_y2 = allstuff
        ...
        utt.assert_allclose(arasea_y0, numpy_y0[3:])
        utt.assert_allclose(arasea_y1, numpy_y1[1:])
        utt.assert_allclose(arasea_y2, numpy_y2)
        utt.assert_allclose(W1.get_value(), numpy_W1)
        utt.assert_allclose(W2.get_value(), numpy_W2)

    def test_simple_shared_random(self):
        arasea_rng = RandomStream(utt.fetch_seed())

        values, updates = scan(
            lambda: arasea_rng.uniform(-1, 1, size=(2,)),
            [],
            [],
            [],
            n_steps=5,
        ...
        rng = arasea_rng.rng_ctor(rng_seed)
        ...
        arasea_v = my_f()
        utt.assert_allclose(arasea_v, numpy_v[:5, :])
        arasea_v = my_f()
        utt.assert_allclose(arasea_v, numpy_v[5:, :])
    ...
    def test_grad_duplicate_outputs(self):
        ...
        multiple outputs are the same arasea variable, works.
        """
...
def test_cython_performance(benchmark):
    ...
    from arasea.scan import scan_perform_ext  # noqa: F401
...
class TestGradUntil:
    ...
    def test_grad_until(self):
        ...
        arasea_output, arasea_gradient = f(self.seq, 5)

        utt.assert_allclose(arasea_output, self.numpy_output)
        utt.assert_allclose(arasea_gradient, self.numpy_gradient)
    ...
    def test_grad_until_ndim_greater_one(self):
        ...
        arasea_output, arasea_gradient = f(arr, 5)

        utt.assert_allclose(arasea_output, tile_array(self.numpy_output))
        utt.assert_allclose(arasea_gradient, tile_array(self.numpy_gradient))
...
def test_profile_info():
    from arasea.scan.utils import ScanProfileStats
...
class TestExamples:
    ...
    def test_gibbs_chain(self):
        ...
        arasea_vsamples, updates = scan(
            f, [], vsample, [], n_steps=10, truncate_gradient=-1, go_backwards=False
        )

        my_f = function(
            [vsample], arasea_vsamples[-1], updates=updates, allow_input_downcast=True
        )
    ...
    def test_reordering(self, benchmark):
        ...
        (arasea_dump1, arasea_dump2, arasea_x, arasea_y) = benchmark(
            f4, v_u1, v_u2, v_x0, v_y0, vW_in1
        )

        utt.assert_allclose(arasea_x, v_x)
        utt.assert_allclose(arasea_y, v_y)
    ...
    def test_using_taps_input_output(self):
        ...
        utt.assert_allclose(numpy_out, arasea_out)
    ...
    def test_generator_one_output_scalar(self):
        ...
        numpy_values = np.array([state * (2 ** (k + 1)) for k in range(steps)])
        arasea_values = my_f(state, steps)
        utt.assert_allclose(numpy_values, arasea_values[0])
    ...
    def test_return_steps(self):
        ...
        (arasea_dump, arasea_x, arasea_y) = f4(v_u1, v_u2, v_x0, v_y0, vW_in1)

        utt.assert_allclose(arasea_x, v_x[-2:])
        utt.assert_allclose(arasea_y, v_y[-4:])
    ...
    def test_multiple_inputs_multiple_outputs(self):
        ...
        (arasea_x, arasea_y) = f4(v_u1, v_u2, v_x0, v_y0, vW_in1)
        utt.assert_allclose(arasea_x, v_x)
        utt.assert_allclose(arasea_y, v_y)
