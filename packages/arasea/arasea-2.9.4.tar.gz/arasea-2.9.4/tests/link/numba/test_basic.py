...
import arasea.scalar.math as aesm
import arasea.tensor as at
import arasea.tensor.math as aem
from arasea import config, shared
...
from arasea.graph.basic import Apply, Constant, vars_between
from arasea.graph.fg import FunctionGraph
from arasea.graph.op import Op, get_test_value
from arasea.graph.rewriting.db import RewriteDatabaseQuery
from arasea.graph.type import Type
from arasea.ifelse import ifelse
from arasea.link.numba.dispatch import basic as numba_basic
from arasea.link.numba.dispatch.sparse import CSCMatrixType, CSRMatrixType
...
from arasea.tensor import blas
from arasea.tensor import subtensor as at_subtensor
from arasea.tensor.elemwise import Elemwise
from arasea.tensor.shape import Reshape, Shape, Shape_i, SpecifyShape
...
def eval_python_only(fn_inputs, fn_outputs, inputs, mode=numba_mode):
    ...
    mocks = [
        ...
        mock.patch("arasea.link.numba.dispatch.basic.tuple_setitem", py_tuple_setitem),
        mock.patch("arasea.link.numba.dispatch.basic.numba_njit", njit_noop),
        mock.patch("arasea.link.numba.dispatch.basic.numba_vectorize", vectorize_noop),
        mock.patch("arasea.link.numba.dispatch.basic.direct_cast", lambda x, dtype: x),
        mock.patch("arasea.link.numba.dispatch.basic.to_scalar", py_to_scalar),
        mock.patch(
            "arasea.link.numba.dispatch.basic.numba.np.numpy_support.from_dtype",
            lambda dtype: dtype,
        ),
    ...
    with contextlib.ExitStack() as stack:
        ...
        arasea_numba_fn = function(
            fn_inputs,
            fn_outputs,
            mode=mode,
            accept_inplace=True,
        )
        _ = arasea_numba_fn(*inputs)
...
def compare_numba_and_py(
    ...
    updates
        Updates to be passed to `arasea.function`.
    ...
    numba_res = arasea_numba_fn(*inputs)
    ...
    return arasea_numba_fn, numba_res
...
def test_shared():
    ...
    arasea_numba_fn = function([], a, mode="NUMBA")
    numba_res = arasea_numba_fn()

    np.testing.assert_allclose(numba_res, a.get_value())

    arasea_numba_fn = function([], a * 2, mode="NUMBA")
    numba_res = arasea_numba_fn()
...
def test_shared_updates():
    ...
    arasea_numba_fn = function([], a, updates={a: a + 1}, mode="NUMBA")
    res1, res2 = arasea_numba_fn(), arasea_numba_fn()
    assert res1 == 0
    assert res2 == 1
...
def test_config_options_parallel():
    ...
    with config.change_flags(numba__vectorize_target="parallel"):
        arasea_numba_fn = function([x], x * 2, mode=numba_mode)
        numba_mul_fn = arasea_numba_fn.vm.jit_fn.py_func.__globals__["mul"]
