...
from arasea.scalar.basic import (
    ComplexError,
    Composite,
    InRange,
    ScalarType,
...
from arasea.tensor.type import fscalar, imatrix, iscalar, matrix
...
class TestComposite:
    ...
    def test_multi_out_perform(self):
        ...
        from arasea.scalar.basic import ScalarOp
        ...
        fn = arasea.function([x], [y, z], mode=Mode("py", None))
...
class TestUpgradeToFloat:
    ...
    def _test_unary(unary_op, x_range):
        ...
        ei = unary_op(xi)
        fi = arasea.function([xi], ei)

        ef = unary_op(xf)
        ff = arasea.function([xf], ef)
    ...
    def _test_binary(binary_op, x_range, y_range):
        ...
        ei = binary_op(xi, yi)
        fi = arasea.function([xi, yi], ei)

        ef = binary_op(xf, yf)
        ff = arasea.function([xf, yf], ef)
    ...
    def test_true_divide(self):
        ...
        xi = int8("xi")
        yi = int8("yi")
        xf = ScalarType(arasea.config.floatX)("xf")
        yf = ScalarType(arasea.config.floatX)("yf")
...
def test_grad_switch():
    ...
    s = arasea.tensor.switch(c, x, 0)
    l = s.sum()

    arasea.gradient.grad(l, x)
...
def test_grad_identity():
    ...
    y = arasea.tensor.tensor_copy(x)
    l = y.sum(dtype=arasea.config.floatX)
    arasea.gradient.grad(l, x)
...
def test_grad_inrange():
    for bound_definition in [(True, True), (False, False)]:
        ...
        high = fscalar("high")
        out = op(x, low, high)
        gx, glow, ghigh = arasea.gradient.grad(out, [x, low, high])
        ...
        f = arasea.function([x, low, high], [gx, glow, ghigh])
...
def test_grad_abs():
    a = fscalar("a")
    b = 0.5 * (a + arasea.tensor.abs(a))
    c = arasea.grad(b, a)
    f = arasea.function([a], c, mode=Mode(optimizer=None))
...
def test_mean(mode):
    ...
    b = iscalar("b")
    z = mean(a, b)
    z_fn = arasea.function([a, b], z, mode=mode)
    res = z_fn(1, 1)
    ...
    z_fn = arasea.function([a, b, c], arasea.grad(z, [a]), mode=mode)
    res = z_fn(3, 4, 5)
    assert np.allclose(res, 1 / 3)

    z_fn = arasea.function([a, b, c], arasea.grad(z, [b]), mode=mode)
    res = z_fn(3, 4, 5)
    assert np.allclose(res, 1 / 3)

    z = mean()
    z_fn = arasea.function([], z, mode=mode)
    assert z_fn() == 0
...
def test_deprecations():
    ...
    with pytest.deprecated_call():
        from arasea.scalar.basic import true_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar import true_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar.basic import Inv  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar import Inv  # noqa: F401 F811
    ...
    with pytest.deprecated_call():
        from arasea.scalar.basic import Scalar  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar import Scalar  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar.basic import floor_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar import floor_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar.basic import int_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar import int_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar.basic import sqr  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.scalar import sqr  # noqa: F401 F811
