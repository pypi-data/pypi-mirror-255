...
from arasea import configdefaults, configparser
from arasea.configdefaults import short_platform
from arasea.configparser import ConfigParam


def _create_test_config():
    return configparser.AesaraConfigParser(
        flags_dict={},
        arasea_cfg=stdlib_configparser.ConfigParser(),
        arasea_raw_cfg=stdlib_configparser.RawConfigParser(),
    )
...
def test_mode_apply():
    ...
    import arasea.compile.mode

    assert (
        configdefaults._filter_mode(arasea.compile.mode.FAST_COMPILE)
        == arasea.compile.mode.FAST_COMPILE
    )
