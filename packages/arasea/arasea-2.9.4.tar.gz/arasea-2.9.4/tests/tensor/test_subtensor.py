...
import arasea
import arasea.scalar as scal
import arasea.tensor.basic as at
from arasea.compile import DeepCopyOp, shared
from arasea.compile.io import In
from arasea.configdefaults import config
from arasea.graph.op import get_test_value
from arasea.graph.rewriting.utils import is_same_graph
from arasea.printing import pprint
from arasea.scalar.basic import as_scalar
from arasea.tensor import get_vector_length
from arasea.tensor.elemwise import DimShuffle
from arasea.tensor.math import exp, isinf
from arasea.tensor.math import sum as at_sum
from arasea.tensor.subtensor import (
...
from arasea.tensor.type_other import NoneConst, SliceConstant, make_slice, slicetype
...
class TestSubtensor(utt.OptimizationTestMixin):
    ...
    def function(
        ...
        """
        wrapper around arasea.function that also check the output
        ...
        f = arasea.function(inputs, outputs, mode=mode, accept_inplace=accept_inplace)
    ...
    def test_err_bounds(self):
        ...
        _logger = logging.getLogger("arasea.graph.rewriting.basic")
    ...
    def test_slice_symbol(self):
        ...
        o = x[: y.shape[0], None, :]
        f = arasea.function([], o, mode=self.mode)
        ret = f()
    ...
    def test_boolean(self):
        ...
        with config.change_flags(compute_test_value="off"):
            ...
            with pytest.raises(TypeError):
                test_array.__getitem__(([0, 1], [0, arasea.shared(True)]))
    ...
    def test_grad_2d_inc_set_subtensor(self):
        for n_shape, m_shape in [
            ...
            for op in [inc_subtensor, set_subtensor]:
                ...
                t = op(n[:z, :z], m)
                gn, gm = arasea.grad(at_sum(t), [n, m])
    ...
    def grad_list_(self, idxs, data):
        ...
        for idx in idxs:
            ...
            def fct2(t):
                return arasea.grad(at_sum(t[idx_]), t)
    ...
    def test_adv1_inc_sub_notlastdim(self):
        ...
        f = arasea.function([m, i], [m1, m2], mode=self.mode)
...
def test_take_cases(a_shape, index, axis, mode):
    ...
    f = arasea.function([], a.take(index, axis=axis, mode=mode))
    f_res = f()
...
class TestIncSubtensor:
    ...
    def test_simple_2d(self):
        ...
        for do_set in [False, True]:
            ...
            f = arasea.function([a, increment, sl2_end], resut)
    ...
    def test_wrong_broadcast(self):
        ...
        def rng_randX(*shape):
            return rng.random(shape).astype(arasea.config.floatX)

        for op in (set_subtensor, inc_subtensor):
            for base in (a[:], a[0]):
                out = op(base, increment)
                f = arasea.function([a, increment], out)
...
class TestIncSubtensor1:
    ...
    def test_1d_set_adv_selection(self):
        ...
        f = arasea.function([self.v, self.adv1q], a, allow_input_downcast=True)
    ...
    def test_matrix_idx(self):
        ...
        f = arasea.function([self.m, idx], a2)
    ...
    def test_inc_bcastableidx(self):
        ...
        f = arasea.function([self.m, c_inc, m_inc], [out1, out2])
...
class TestAdvancedSubtensor:
    ...
    def test_adv_subtensor_w_int_and_matrix(self):
        ...
        f = arasea.function([self.ft4, self.ix2], subt, mode=self.mode)
    ...
    def test_2d_3d_tensors(self):
        ...
        j = iscalar()
        z = b[[i, j], :]
        f1 = arasea.function([i, j], z, mode=self.mode)
        cmd = f1(0, 1) == a[[0, 1], :]
        assert cmd.all()
        ...
        k = iscalar()
        z = bb[[i, j, k], :, i:k]
        f2 = arasea.function([i, j, k], z, mode=self.mode)
    ...
    def test_adv_sub_3d(self):
        ...
        f = arasea.function([X], X[b_idx, r_idx, c_idx], mode=self.mode)
        out = f(xx)
    ...
    def test_adv_sub_slice(self):
        ...
        slc = slicetype()
        f = arasea.function([slc], var[slc], mode=self.mode)
        s = slice(1, 3)
        f(s)
