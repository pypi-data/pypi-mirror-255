...
import arasea
import arasea.scalar as aes
import arasea.tensor as at
from arasea import pprint, shared
...
from arasea.compile.mode import Mode, get_default_mode, get_mode
from arasea.compile.ops import DeepCopyOp, deep_copy_op
from arasea.configdefaults import config
from arasea.graph.basic import Apply, Constant, equal_computations
from arasea.graph.fg import FunctionGraph
from arasea.graph.rewriting.basic import (
...
from arasea.graph.rewriting.utils import is_same_graph, rewrite_graph
from arasea.misc.safe_asarray import _asarray
from arasea.tensor import inplace
from arasea.tensor.basic import Alloc, join, switch
from arasea.tensor.blas import Dot22, Gemv
from arasea.tensor.blas_c import CGemv
from arasea.tensor.elemwise import CAReduce, DimShuffle, Elemwise
from arasea.tensor.math import Dot, MaxAndArgmax, Prod, Sum, _conj
from arasea.tensor.math import abs as at_abs
from arasea.tensor.math import add
from arasea.tensor.math import all as at_all
from arasea.tensor.math import any as at_any
from arasea.tensor.math import (
    arccosh,
    arcsinh,
    arctanh,
    bitwise_and,
...
from arasea.tensor.math import max as at_max
from arasea.tensor.math import maximum
from arasea.tensor.math import min as at_min
from arasea.tensor.math import minimum, mul, neg, neq
from arasea.tensor.math import pow as at_pow
from arasea.tensor.math import prod, rad2deg, reciprocal
from arasea.tensor.math import round as at_round
from arasea.tensor.math import sgn, sigmoid, sin, sinh, softplus, sqrt, square, sub
from arasea.tensor.math import sum as at_sum
from arasea.tensor.math import tan, tanh, true_divide, xor
from arasea.tensor.rewriting.elemwise import local_dimshuffle_lift
from arasea.tensor.rewriting.math import (
    compute_mul,
    is_1pexp,
...
from arasea.tensor.shape import Reshape, Shape_i
...
from arasea.tensor.var import TensorConstant
...
class TestAlgebraicCanonizer:
    ...
    def test_canonicalize_nan(self):
        ...
        logging.getLogger("arasea.graph.rewriting.basic").addHandler(handler)
        try:
            x = vector()
            function([x], x + np.nan)
        finally:
            logging.getLogger("arasea.graph.rewriting.basic").removeHandler(handler)
...
def test_merge_abs_bugfix():
    ...
    function([input], arasea.gradient.grad(l1_norm, input))
...
def test_local_subtensor_of_dot():
    ...
    # [cst]
    f = function([m1, m2], arasea.tensor.dot(m1, m2)[1], mode=mode)
    topo = f.maker.fgraph.toposort()
    ...
    # slice
    f = function([m1, m2], arasea.tensor.dot(m1, m2)[1:2], mode=mode)
    topo = f.maker.fgraph.toposort()
    ...
    f = function([m1, m2, idx], arasea.tensor.dot(m1, m2)[idx, 1:4, :, idx:], mode=mode)
    assert test_equality(f(d1, d2, 1), np.dot(d1, d2)[1, 1:4, :, 1:])
    # if we return the gradients. We need to use same mode as before.
    assert check_stack_trace(f, ops_to_check="last")

    f = function([m1, m2, idx], arasea.tensor.dot(m1, m2)[1:4, :, idx:, idx], mode=mode)
...
class TestLocalUselessElemwiseComparison:
    ...
    def test_local_useless_elemwise_comparison(self):
        ...
        # arasea.printing.debugprint(f, print_type=True)
        # here is the output for the debug print:
        """
...
class TestLocalSwitchSink:
    ...
    def test_local_mul_switch_sink(self):
        ...
        f = self.function_remove_nan([x], arasea.gradient.grad(y, x), self.mode)
        assert f(5) == 1, f(5)
...
class TestLocalErfc:
    ...
    def test_local_grad_log_erfc_neg(self):
        ...
        # produced
        log_erfc_grad = arasea.gradient.grad(log(erfc(x)).sum(), x)
        ...
        f = function([x], arasea.gradient.grad(log(erfc(2 * x)).sum(), x), mode=mode)
        ...
        f = function([x], arasea.gradient.grad(log(erfc(x)).sum(), x), mode=mode_fusion)
...
class TestSigmoidRewrites:
    def get_mode(self, excluding=None):
        ...
        m = config.mode
        if m == "FAST_COMPILE":
            mode = arasea.compile.mode.get_mode("FAST_RUN")
        else:
            mode = arasea.compile.mode.get_default_mode()
        if excluding:
    ...
    def test_exp_over_1_plus_exp(self):
        ...
        f = arasea.function([x], exp(x) / (1 + exp(x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] == [sigmoid]
        f(data)
        f = arasea.function([x], exp(x) / (2 + exp(x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] != [sigmoid]
        f(data)
        f = arasea.function([x], exp(x) / (1 - exp(x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] != [sigmoid]
        f(data)
        f = arasea.function([x], exp(x + 1) / (1 + exp(x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] != [sigmoid]
        f(data)

        # tests inv_1_plus_exp
        f = arasea.function([x], at.fill(x, 1.0) / (1 + exp(-x)), mode=m)
        # todo: solve issue #4589 first
        # assert check_stack_trace(f, ops_to_check=sigmoid)
        assert [node.op for node in f.maker.fgraph.toposort()] == [sigmoid]
        f(data)
        f = arasea.function([x], at.fill(x, 1.0) / (2 + exp(-x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] != [sigmoid]
        f(data)
        f = arasea.function([x], at.fill(x, 1.0) / (1 - exp(-x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] != [sigmoid]
        f(data)
        f = arasea.function([x], at.fill(x, 1.1) / (1 + exp(-x)), mode=m)
        assert [node.op for node in f.maker.fgraph.toposort()] != [sigmoid]
        f(data)
    ...
    def test_local_1msigmoid(self):
        ...
        f = arasea.function([x], 1 - exp(x) / (1 + exp(x)), mode=m)
        ...
        # Test `inv_1_plus_exp`
        f = arasea.function([x], 1 - at.fill(x, 1.0) / (1 + exp(-x)), mode=m)
        # assert check_stack_trace(f, ops_to_check=[neg, sigmoid])
        assert [node.op for node in f.maker.fgraph.toposort()] == [neg, sigmoid]
    ...
    def test_local_sigm_times_exp(self):
        ...
        f = arasea.function([x], sigmoid(-x) * exp(x), mode=m)
        match(f, [sigmoid])
        assert check_stack_trace(f, ops_to_check=sigmoid)
    ...
    def test_perform_sigm_times_exp(self):
        ...
        def check(expr1, expr2):
            ...
            if not good:
                ...
                print(trees[1])
                print("***")
                arasea.printing.debugprint(compute_mul(trees[0]))
                print("***")
                arasea.printing.debugprint(compute_mul(trees[1]))
            assert good
...
class TestSoftplusRewrites:
    def setup_method(self):
        if arasea.config.mode == "FAST_COMPILE":
            m = arasea.compile.mode.get_mode("FAST_RUN").excluding(
                "local_elemwise_fusion"
            )
        else:
            m = arasea.compile.mode.get_default_mode().excluding(
                "local_elemwise_fusion"
            )
        self.m = m
    ...
    def test_logsigm_to_softplus(self):
        ...
        out = log(sigmoid(x))
        f = arasea.function([x], out, mode=self.m)

        # Fix ticket #4581 first
        # assert check_stack_trace(
        #     f, ops_to_check=(arasea.scalar.Neg,
        #                      ScalarSoftplus))
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 3
        assert isinstance(topo[0].op.scalar_op, arasea.scalar.Neg)
        assert isinstance(topo[1].op.scalar_op, arasea.scalar.Softplus)
        assert isinstance(topo[2].op.scalar_op, arasea.scalar.Neg)
    ...
    def test_log1msigm_to_softplus(self):
        ...
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 2
        assert isinstance(topo[0].op.scalar_op, arasea.scalar.Softplus)
        assert isinstance(topo[1].op.scalar_op, arasea.scalar.Neg)
        # assert check_stack_trace(f, ops_to_check='all')
...
def test_deprecations():
    ...
    with pytest.deprecated_call():
        from arasea.tensor.math_opt import AlgebraicCanonizer  # noqa: F401 F811
