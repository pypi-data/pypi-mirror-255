...
from arasea.graph.rewriting.basic import out2in
from arasea.link.basic import PerformLinker
from arasea.tensor.elemwise import CAReduce, DimShuffle, Elemwise
from arasea.tensor.math import MaxAndArgmax
from arasea.tensor.math import max as at_max
from arasea.tensor.math import max_and_argmax
from arasea.tensor.math import min as at_min
from arasea.tensor.rewriting.uncanonicalize import (
    local_alloc_dimshuffle,
    local_dimshuffle_alloc,
    local_dimshuffle_subtensor,
    local_reshape_dimshuffle,
)
from arasea.tensor.shape import reshape, specify_shape
from arasea.tensor.type import dtensor4, iscalar, matrix, tensor, vector
...
class TestMaxAndArgmax:
    def test_optimization(self):
        ...
        # a faster one.
        mode = arasea.compile.mode.get_default_mode().including(
...
class TestMinMax:
    def setup_method(self):
        self.mode = arasea.compile.mode.get_default_mode().including(
            "canonicalize", "fast_run"
        )
...
def test_deprecations():
    ...
    with pytest.deprecated_call():
        from arasea.tensor.opt_uncanonicalize import (  # noqa: F401 F811
            local_reshape_dimshuffle,
        )
