...
from arasea.gradient import NullTypeGradError, grad, numeric_grad
from arasea.graph.basic import Variable, applys_between
from arasea.graph.fg import FunctionGraph
from arasea.link.c.basic import DualLinker
from arasea.misc.safe_asarray import _asarray
from arasea.printing import pprint
from arasea.tensor import blas, blas_c
...
from arasea.tensor.elemwise import CAReduce, Elemwise
...
from arasea.tensor.math import tan, tanh, tensordot, true_divide, trunc, var
...
from arasea.tensor.type_other import NoneConst
...
class TestMinMax:
    ...
    def _grad_list(self):
        ...
        # http://www.assembla.com/spaces/arasea/tickets/511
        data = random(2, 3)
...
class TestArithmeticCast:
    ...
    def test_arithmetic_cast(self, op, a_type, b_type, combo):
        ...
        with config.change_flags(cast_policy="numpy+floatX"):
            ...
            arasea_args = list(map(eval, [f"arasea_{c}" for c in combo]))
            numpy_args = list(map(eval, [f"numpy_{c}" for c in combo]))
            arasea_arg_1 = arasea_args[0](a_type)
            arasea_arg_2 = arasea_args[1](b_type)
            arasea_dtype = op(
                arasea_arg_1,
                arasea_arg_2,
            ).type.dtype
...
def test_logsumexp(shape, axis, keepdims):
    ...
    arasea_inp = as_tensor_variable(scipy_inp)
    f = function([], logsumexp(arasea_inp, axis=axis, keepdims=keepdims))
    arasea_out = f()
...
class TestMatMul(utt.InferShapeTester):
    ...
    def _validate_output(self, a, b):
        arasea_sol = self.op(a, b).eval()
        numpy_sol = np.matmul(a, b)
        assert _allclose(numpy_sol, arasea_sol)
...
def test_deprecations():
    ...
    with pytest.deprecated_call():
        from arasea.tensor.math import abs_  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor import abs_  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor import inv  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor.math import inv  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor import true_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor.math import true_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor import floor_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor.math import floor_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor import int_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor.math import int_div  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor import sqr  # noqa: F401 F811

    with pytest.deprecated_call():
        from arasea.tensor.math import sqr  # noqa: F401 F811
...
class TestConvolve(utt.InferShapeTester):
    ...
    def test_op(self, a, v, mode):
        arasea_sol = convolve(a, v, mode=mode).eval()
        numpy_sol = np.convolve(a, v, mode=mode)
        assert np.allclose(numpy_sol, arasea_sol)
