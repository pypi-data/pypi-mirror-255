...
from arasea.graph.utils import MethodNotDefined
...
from arasea.printing import Printer, pprint, set_precedence
from arasea.scalar.basic import ScalarConstant
from arasea.tensor import _get_vector_length, as_tensor_variable, get_vector_length
from arasea.tensor.basic import alloc, get_scalar_constant_value
from arasea.tensor.elemwise import DimShuffle
from arasea.tensor.exceptions import (
    AdvancedIndexingError,
    NotScalarConstantError,
    ShapeError,
)
from arasea.tensor.math import clip
from arasea.tensor.shape import Reshape, specify_broadcastable
...
from arasea.tensor.type_other import NoneConst, NoneTypeT, SliceType, make_slice


_logger = logging.getLogger("arasea.tensor.subtensor")
...
def get_canonical_form_slice(
    ...
    """
    from arasea.tensor import ge, lt, sgn, switch
...
def range_len(slc):
    ...
    """
    from arasea.tensor import and_, gt, lt, switch
...
def indexed_result_shape(array_shape, indices, indices_are_shapes=False):
    ...
    remaining_dims = range(arasea.tensor.basic.get_vector_length(array_shape))
    ...
    for basic, grp_dim_indices in idx_groups:
        ...
        if basic:
            ...
            from arasea.tensor.extra_ops import broadcast_shape
...
def as_nontensor_scalar(a: Variable) -> aes.ScalarVariable:
    ...
    if isinstance(a, Variable) and isinstance(a.type, TensorType):
        return arasea.tensor.scalar_from_tensor(a)
    else:
...
class Subtensor(COp):
    ...
    def grad(self, inputs, grads):
        ...
        if x.dtype in discrete_dtypes:
            ...
            # arasea/tensor/opt.py:local_incsubtensor_of_zeros_to_setsubtensor()
...
class AdvancedSubtensor1(COp):
    ...
    def grad(self, inputs, grads):
        ...
        if self.sparse_grad:
            ...
            rval1 = [arasea.sparse.construct_sparse_from_list(x, gz, ilist)]
        else:
            if x.dtype in discrete_dtypes:
                # The output dtype is the same as x
                gx = x.zeros_like(dtype=config.floatX)
...
class AdvancedSubtensor(Op):
    ...
    def make_node(self, x, *index):
        ...
        with config.change_flags(compute_test_value="off"):
            ...
            fake_index = tuple(
                chain.from_iterable(
                    arasea.tensor.basic.nonzero(idx)
...
def _get_vector_length_Subtensor(op, var):
    ...
    try:
        indices = arasea.tensor.subtensor.get_idx_list(
