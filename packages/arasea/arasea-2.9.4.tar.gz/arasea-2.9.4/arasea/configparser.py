...
from arasea.utils import hash_from_code


_logger = logging.getLogger("arasea.configparser")
...
class AesaraConfigParser:
    ...
    def __init__(self, flags_dict: dict, arasea_cfg, arasea_raw_cfg):
        self._flags_dict = flags_dict
        self._arasea_cfg = arasea_cfg
        self._arasea_raw_cfg = arasea_raw_cfg
    ...
    def add(self, name, doc, configparam, in_c_key=True):
        ...
        if not callable(configparam.default):
            ...
            try:
                ...
                # `self._arasea_cfg` does not contain an entry for the given
    ...
    def fetch_val_for_key(self, key, delete_key=False):
        ...
        try:
            try:
                return self._arasea_cfg.get(section, option)
            except InterpolationError:
                return self._arasea_raw_cfg.get(section, option)
...
def _create_default_config():
    ...
    config_files = config_files_from_arasearc()
    arasea_cfg = ConfigParser(
        {
    ...
    arasea_raw_cfg = RawConfigParser()
    arasea_raw_cfg.read(config_files)
    ...
    config = AesaraConfigParser(
        ...
        arasea_cfg=arasea_cfg,
        arasea_raw_cfg=arasea_raw_cfg,
    )
    return config
...
class _ConfigProxy:
    ...
    config settings, but raises DeprecationWarnings with instructions to use `arasea.config`.
    """
    ...
    def __getattr__(self, attr):
        ...
        warnings.warn(
            "`arasea.configparser.config` is deprecated; use `arasea.config` instead.",
            DeprecationWarning,
            stacklevel=2,
        )
...
# These imports/accesses should be replaced with `arasea.config`, so this wraps
# it with warnings:
...
DEPRECATED_NAMES = [
    (
        "change_flags",
        "`change_flags` is deprecated; use `arasea.config.change_flags` instead.",
        _config.change_flags,
    ),
    (
        "_change_flags",
        "`_change_flags` is deprecated; use `arasea.config.change_flags` instead.",
        _config.change_flags,
    ),
    (
        "_config_print",
        "`_config_print` is deprecated; use `arasea.config.config_print` instead.",
