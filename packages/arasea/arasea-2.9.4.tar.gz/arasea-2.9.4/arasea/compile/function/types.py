...
import arasea
import arasea.compile.profiling
from arasea.compile.io import In, Out, SymbolicInput, SymbolicOutput
from arasea.compile.ops import deep_copy_op, update_placeholder, view_op
from arasea.compile.profiling import ProfileStats
...
from arasea.graph.destroyhandler import DestroyHandler
from arasea.graph.features import AlreadyThere, Feature, PreserveVariableAttributes
from arasea.graph.fg import FunctionGraph
from arasea.graph.op import HasInnerGraph
from arasea.graph.utils import InconsistencyError, get_variable_trace_string
from arasea.link.basic import Container
from arasea.link.utils import raise_with_op


if TYPE_CHECKING:
    from arasea.compile.mode import Mode
    from arasea.link.vm import VM


_logger = logging.getLogger("arasea.compile.function.types")
...
class Function:
    ...
    def copy(
        ...
        profile :
            as arasea.function profile parameter

        Returns
        -------
        arasea.Function
            Copied arasea.Function
        """
        ...
        def checkSV(sv_ori, sv_rpl):
            ...
            Assert two SharedVariable follow some restirctions:
                ...
                2. same shape or dim?
            """
            SharedVariable = arasea.tensor.sharedvar.SharedVariable
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.trust_input:
            ...
            if len(args) + len(kwargs) > len(self.input_storage):
                raise TypeError("Too many parameter passed to arasea function")
        ...
        arasea.compile.profiling.total_fct_exec_time += dt_call
...
class FunctionMaker:
    ...
    def check_unused_inputs(inputs, outputs, on_unused_input):
        ...
        msg = (
            "arasea.function was asked to create a function computing "
        ...
        for i in inputs:
            if (i.variable not in used_inputs) and (i.update is None):
                if on_unused_input == "warn":
                    ...
                    raise ValueError(
                        "Invalid value for keyword on_unused_input of arasea.function: "
                        f"'{on_unused_input}'.\n"
    ...
    def prepare_fgraph(
        ...
        try:
            ...
            arasea.compile.profiling.total_graph_rewrite_time += rewrite_time
        ...
        if not hasattr(mode.linker, "accept"):
            raise ValueError(
                ...
                f"a Linker with an accept method or one of {list(arasea.compile.mode.predefined_linkers.keys())}"
            )
    ...
    def __init__(
        ...
        self.mode = mode
        mode = arasea.compile.mode.get_mode(mode)
        ...
        if profile:
            ...
            #    module.
            arasea.link.c.basic.get_module_cache().refresh()
    ...
    def create(self, input_storage=None, trustme=False, storage_map=None):
        ...
        start_import_time = arasea.link.c.cmodule.import_time
        ...
        arasea.compile.profiling.total_time_linker += linker_time
        ...
        if self.profile:
            ...
            import_time = arasea.link.c.cmodule.import_time - start_import_time
...
def orig_function(
    ...
    on_unused_input : {'raise', 'warn', 'ignore', None}
        What to do if a variable in the 'inputs' list is not used in the graph.
    output_keys
        If the outputs were provided to arasea.function as a list, then
