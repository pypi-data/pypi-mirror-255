...
import arasea
from arasea import _as_symbolic, as_symbolic
...
from arasea.link.c.type import generic
from arasea.misc.safe_asarray import _asarray
from arasea.sparse.type import SparseTensorType, _is_sparse
from arasea.sparse.utils import hash_from_sparse
from arasea.tensor import basic as at
from arasea.tensor.basic import Split
from arasea.tensor.math import _conj
from arasea.tensor.math import add as at_add
from arasea.tensor.math import arcsin, arcsinh, arctan, arctanh, ceil, deg2rad
from arasea.tensor.math import dot as at_dot
from arasea.tensor.math import exp, expm1, floor, log, log1p, maximum, minimum
...
from arasea.tensor.shape import shape, specify_broadcastable
from arasea.tensor.type import TensorType
from arasea.tensor.type import continuous_dtypes as tensor_continuous_dtypes
from arasea.tensor.type import discrete_dtypes as tensor_discrete_dtypes
from arasea.tensor.type import iscalar, ivector, scalar, tensor, vector
from arasea.tensor.var import TensorConstant, TensorVariable, _tensor_py_operators
...
class GetItem2d(Op):
    ...
    to preserve consistency between numpy and arasea. This may be revised
    ...
    def make_node(self, x, index):
        ...
        for ind in index:
            if isinstance(ind, slice):
                # in case of slice is written in arasea variable
                start = ind.start
                stop = ind.stop
                step = ind.step
...
class GetItemScalar(Op):
    ...
    def make_node(self, x, index):
        ...
        for ind in index:
            ...
            # in case of indexing using int instead of arasea variable
            elif isinstance(ind, int):
                ind = at.constant(ind)
                input_op += [ind]

            # in case of indexing using arasea variable
            elif ind.ndim == 0:
                input_op += [ind]
            else:
                raise NotImplementedError
...
class SpSum(Op):
    ...
    *do* want Sums with different structured values to be merged
    ...
    def grad(self, inputs, gout):
        ...
        if self.structured:
            if self.axis is None:
                r = gz * arasea.sparse.sp_ones_like(x)
            elif self.axis == 0:
                r = col_scale(arasea.sparse.sp_ones_like(x), gz)
            elif self.axis == 1:
                r = row_scale(arasea.sparse.sp_ones_like(x), gz)
            else:
                raise ValueError("Illegal value for self.axis.")
        else:
            o_format = x.format
...
class ConstructSparseFromList(Op):
    ...
    def grad(self, inputs, grads):
        ...
        gx = g_output
        gy = arasea.tensor.subtensor.advanced_subtensor1(g_output, *idx_list)
