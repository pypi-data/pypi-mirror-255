		...
		for i, e in enumerate(self.entries):
			...
			if e.lookup_list:
				if e.entry_type == 'listbox':
					...
					e.entryctrl.grid(column=1, row=0, sticky=tk.W+tk.NS, padx=(3,0), pady=3)
			...
			elif e.entry_type is not None and e.entry_type == "textarea":
				has_textarea = True
			...
			elif e.entry_type is not None and e.entry_type == "inputfile":
				...
				entry_gridded = True
				e.entryctrl.bind("<Return>", self.do_continue)
			elif e.entry_type is not None and e.entry_type == "outputfile":
				getfile_entryframe = ttk.Frame(master=itementryframe)
				e.entryctrl = ttk.Entry(getfile_entryframe, width=e.width if e.width else 20, textvariable=e.entryvar)
			...
			if e.entry_type is None or (e.entry_type != "textarea" and e.entry_type != "listbox"):
