...
def generate_aliases(hub, run_dir, run_dir_root=None, target_os="linux"):
    """
    Generate the alias data for the Salt Proxy
    """
    if not run_dir_root:
    ...
    aliases = {
        "salt-proxy": {"file": artifacts_dir / "salt-proxy", "cmd": "salt-proxy"},
    }
...
def get_salt_opts(
    ...
    proxy_opts = roster.get("proxy_opts")
    if proxy_opts:
        for key, value in proxy_opts.items():
...
def raw_run_cmd(
    ...
    **kwargs,
    ...
    proxy when using the raw service plugin
    """
    if not run_dir_root:
        run_dir_root = hub.heist.init.default(target_os, "run_dir_root")
