...
def plot_irl_world(S, s_lst_lst=[], titles=["States", "Classes", "Features", "Rewards"],
    ...
    plt.subplot(2, 2, 4)
    p.plot_array(
        np.asarray([r.detach() for r in S.rewards(numpyize=False, key=r_key)], dtype=np.float32).round(r_round_to),
...
class NavGridViewPlotter:
    def __init__(self, S, R=None, cartesian=False, r_key=None, phi_key=None):
        self.S = S
        self.R = np.asarray([r.detach() for r in S.rewards(key=r_key, numpyize=False)], dtype=np.float32) if R is None else R
