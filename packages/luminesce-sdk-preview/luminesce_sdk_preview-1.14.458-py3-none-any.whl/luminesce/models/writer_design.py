...
class WriterDesign(object):
    ...
    def sql(self, sql):
        ...
        if (self.local_vars_configuration.client_side_validation and
                sql is not None and len(sql) > 256):
            raise ValueError("Invalid value for `sql`, length must be less than or equal to `256`")  # noqa: E501
        if (self.local_vars_configuration.client_side_validation and
                sql is not None and len(sql) < 0):
            raise ValueError("Invalid value for `sql`, length must be greater than or equal to `0`")  # noqa: E501
