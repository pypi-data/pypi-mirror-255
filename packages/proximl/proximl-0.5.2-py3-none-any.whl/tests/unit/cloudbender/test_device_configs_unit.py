...
import proximl.cloudbender.device_configs as specimen
...
@fixture
def device_configs(mock_proximl):
    yield specimen.DeviceConfigs(mock_proximl)


@fixture
def device_config(mock_proximl):
...
class RegionsTests:
    ...
    @mark.asyncio
    async def test_create_device_config(self, device_configs, mock_proximl):
...
class device_configTests:
    ...
    def test_device_config_repr(self, device_config):
        ...
        regex = (
            r"^DeviceConfig\( proximl , \*\*{.*'config_id': '"
            + device_config.id
            + r"'.*}\)$"
        )
        assert isinstance(string, str)
    ...
    def test_device_config_bool(self, device_config, mock_proximl):
        empty_device_config = specimen.DeviceConfig(mock_proximl)
        assert bool(device_config)
        assert not bool(empty_device_config)

    @mark.asyncio
    async def test_device_config_remove(self, device_config, mock_proximl):
        api_response = dict()
        mock_proximl._query = AsyncMock(return_value=api_response)
