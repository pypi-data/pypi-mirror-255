...
import proximl.cloudbender.regions as specimen
...
@fixture
def regions(mock_proximl):
    yield specimen.Regions(mock_proximl)


@fixture
def region(mock_proximl):
...
class RegionsTests:
    ...
    @mark.asyncio
    async def test_create_region(self, regions, mock_proximl):
...
class regionTests:
    ...
    def test_region_bool(self, region, mock_proximl):
        empty_region = specimen.Region(mock_proximl)
        assert bool(region)
        assert not bool(empty_region)

    @mark.asyncio
    async def test_region_remove(self, region, mock_proximl):
        api_response = dict()
        mock_proximl._query = AsyncMock(return_value=api_response)
        await region.remove()
        mock_proximl._query.assert_called_once_with(
    ...
    @mark.asyncio
    async def test_region_stage_dataset(self, region, mock_proximl):
        api_response = dict()
        expected_payload = dict(
    ...
    @mark.asyncio
    async def test_region_stage_model(self, region, mock_proximl):
        api_response = dict()
        expected_payload = dict(
            project_uuid="proj-id-1",
    ...
    @mark.asyncio
    async def test_region_stage_checkpoint(self, region, mock_proximl):
        api_response = dict()
        expected_payload = dict(
            project_uuid="proj-id-1",
