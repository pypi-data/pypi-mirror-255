...
def list_public(config):
    ...
    datasets = config.proximl.run(config.proximl.client.datasets.list_public())
...
def remove(config, dataset, force):
    ...
    if None is found:
        if force:
            config.proximl.run(config.proximl.client.datasets.remove(dataset))
        else:
            raise click.UsageError("Cannot find specified dataset.")
...
def rename(config, dataset, name):
    ...
    return config.proximl.run(dataset.rename(name=name))
