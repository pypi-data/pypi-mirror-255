...
class Auth(object):
    def __init__(self, domain_suffix="proximl.ai", **kwargs):
        ...
        self.region = (
            kwargs.get("region")
            or os.environ.get("PROXIML_REGION")
            or env.get("region")
            or auth_defaults.get("region")
        )
        self.client_id = (
            kwargs.get("client_id")
            or os.environ.get("PROXIML_CLIENT_ID")
            or env.get("client_id")
            or auth_defaults.get("userPoolSDKClientId")
        )
        self.pool_id = (
            kwargs.get("pool_id")
            or os.environ.get("PROXIML_POOL_ID")
            or env.get("pool_id")
        ...
        self.username = (
            kwargs.get("user")
            or os.environ.get("PROXIML_USER")
            or keys.get("user")
        )
        self.password = (
            kwargs.get("key")
            or os.environ.get("PROXIML_KEY")
            or keys.get("key")
        )
        if not self.username or not self.password:
            raise ProxiMLException("proxiML credentials not found.")
