...
class Regions(object):
    ...
    async def get(self, provider_uuid, id, **kwargs):
        ...
        return Region(self.proximl, **resp)

    async def list(self, provider_uuid, **kwargs):
        resp = await self.proximl._query(
            f"/provider/{provider_uuid}/region", "GET", kwargs
        )
        regions = [Region(self.proximl, **region) for region in resp]
        return regions
    ...
    async def create(self, provider_uuid, name, public, storage, **kwargs):
        ...
        region = Region(self.proximl, **resp)
...
class Region:
    ...
    def __repr__(self):
        return f"Region( proximl , **{self._region.__repr__()})"
