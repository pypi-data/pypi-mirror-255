...
class MssqlAdapter(DbAdapter[Connection, Cursor, str, str]):
    ...
    def execute_file(self, path: str|Path, params: list|tuple|dict = None, *, cursor: Cursor = None, results: bool|TextIOWrapper = False, offset: int = None, limit: int = None, encoding: str = 'utf-8') -> None:
        ...
        with nullcontext(cursor) if cursor else self.cursor() as _cursor:
            for query_index, query in enumerate(queries):
                ...
                if query_num < query_count:
                    ...
                    self.execute_query(query, params, cursor=_cursor, results='warning', query_id=query_id, offset=offset, limit=limit)

                else:
                    # Last query
                    return self.execute_query(query, params, cursor=_cursor, results=results, query_id=query_id, offset=offset, limit=limit)
    ...
    def _paginate_parsed_query(self, selectpart: str, orderpart: str, *, offset: int, limit: int) -> str:
        return f"{selectpart} {orderpart} OFFSET {offset} ROWS FETCH NEXT {limit} ROWS ONLY"
