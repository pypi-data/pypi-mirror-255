...
class Command(base.BaseCommand):
    REMAKEMIGRATIONS_SEALED: dict[str,int] = getattr(settings, "REMAKEMIGRATIONS_SEALED", {})
    REMAKEMIGRATIONS_ORDER: list[str] = getattr(settings, "REMAKEMIGRATIONS_ORDER", [])
    ...
    def handle(self, apps: list[str] = [], exclude_apps: list[str] = None, **kwargs):
        ...
        for label in self.REMAKEMIGRATIONS_ORDER:
            if label in app_configs:        
                logger.info("makemigrations %s", label)
                call_command("makemigrations", label)

        if any(label not in self.REMAKEMIGRATIONS_ORDER for label in app_configs):
    ...
    def is_sealed_migration(self, app_label: str, path: Path):
        if not self.REMAKEMIGRATIONS_SEALED:
            return False
        if not app_label in self.REMAKEMIGRATIONS_SEALED:
            return False
        ...
        if migration_number is None:
            ...
            return True
        elif migration_number <= self.REMAKEMIGRATIONS_SEALED[app_label]:
            logger.info(f"preserve (sealed: app {app_label} migration <= {self.REMAKEMIGRATIONS_SEALED[app_label]}): {path}")
            return True
        else:
            return False
