...
LATEST_UBUNTU_LTS = "22.04"


class OsVersionCheck(TestOnce):
    @classmethod
    def evaluate(cls, c: Connection):
        if LATEST_UBUNTU_LTS not in (version := c.run("lsb_release -a", hide=True, warn=True).stdout):
            print(
                f"ERROR: Ubuntu version {version} other than {LATEST_UBUNTU_LTS} is detected, aborting. ",
                file=sys.stderr,
            )
            exit(1)
...
def install_python(c):
    ...
    if execution_fails(c, "virtualenv --version"):
        # c.sudo("apt install  python3-virtualenv")
        c.sudo("pip3 install virtualenv")
    c.run("mkdir .virtualenvs", warn=True, hide=True)
    if execution_fails(c, "~/.local/bin/pipx --version"):
        c.run("pip install --user 'pipx<1.3'")

    c.run("pip install --user --upgrade invoke pyyaml tabulate humanize httpx")
...
def clone_git_repo_with_new_key_file_for_edwh_repos(c, repo, rw, target_directory="", branch=None):
    ...
    if rw == "r" and executes_correctly(c, f"git clone https://github.com/{git_user}/{git_repo}"):
    ...
    if execution_fails(c, f"cat .ssh/{filename}.pub"):
        ...
        c.put(StringIO(config), ".ssh/config")
...
def install_edwh(c):
    ...
    Args:
        c (Connection)
    """
    if execution_fails(c, "~/.local/bin/pipx install edwh[omgeving,plugins]"):
        c.run("~/.local/bin/pipx upgrade edwh")


def _prepare_generic_server(c, silent: bool):
...
def prepare_generic_server(
    ...
    """
    _prepare_generic_server(c, silent)
