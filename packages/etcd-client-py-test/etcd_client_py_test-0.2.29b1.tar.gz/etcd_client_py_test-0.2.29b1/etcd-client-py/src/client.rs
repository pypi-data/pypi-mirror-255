...
impl PyClient {
    ...
    fn __repr__(&self) -> String {
        format!(
            "Client(endpoints={:?}, options={:?}, lock_options={:?})",
            self.endpoints, self.options, self.lock_options
        )
    }

    pub fn connect(&self, options: Option<PyConnectOptions>) -> Self {
        let mut result = self.clone();
        result.options = options.unwrap_or(self.options.clone());
        result
    }

    pub fn with_lock(&self, lock_options: PyEtcdLockOption, options: Option<PyConnectOptions>) -> Self {
        let mut result = self.clone();
        result.options = options.unwrap_or(self.options.clone());
    ...
    fn __aenter__<'a>(&'a mut self, py: Python<'a>) -> PyResult<&'a PyAny> {
        ...
        self.lock_manager = Some(Arc::new(Mutex::new(EtcdLockManager::new(self.clone(), with_lock.clone().unwrap()))));

        let lock_manager = if with_lock.is_some() {
            ...
            match client_result {
                Ok(client) => {
                    if with_lock.is_some() {
                        Ok(lock_manager.unwrap().lock().await.handle_aenter().await?)
                    } else {
    ...
    fn __aexit__<'a>(&'a self, py: Python<'a>) -> PyResult<&'a PyAny> {
        future_into_py(py, async move {
            Ok(())
        })
    }

    fn cleanup<'a>(&'a mut self, py: Python<'a>) -> PyResult<&'a PyAny> {
        println!("__aexit__!!!");
        let endpoints = self.endpoints.clone();
        let options = self.options.clone();
        let with_lock = self.lock_options.clone();
        ...
        future_into_py(py, async move {
            if with_lock.is_some() {
                ...
                match client_result {
                    Ok(client) => {
                        println!("__aexit__!!! 2");
                        return Ok(lock_manager.unwrap().lock().await.handle_aexit().await?);
                    }
