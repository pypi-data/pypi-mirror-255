...
from simba.utils.checks import check_str, check_all_file_names_are_represented_in_video_log
...
class UserDefinedFeatureExtractor(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        stdout_success(f'Feature extraction complete for {str(len(self.files_found))} video(s). Results are saved inside the project_folder/csv/features_extracted directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
