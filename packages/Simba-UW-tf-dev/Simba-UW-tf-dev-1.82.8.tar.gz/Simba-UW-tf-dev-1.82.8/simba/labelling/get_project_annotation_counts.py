...
def get_project_annotation_counts(config_path: Union[str, os.PathLike]) -> None:
    """
    Retrieve and save annotation counts for annotated videos within the SimBA project.

    Will iterate over all files in the project_folder/csv/targets_inserted directory and count the number of frames, for
    each classifier, that are annotated as present and absent. Will also present results in seconds. Annotation counts are saved in a CSV within
    the `project_folder/logs` directory in with the annotation_counts_20240110185525.csv filename format.
    ...
    :example:
    >>> get_project_annotation_counts(config_path='/Users/simon/Desktop/envs/troubleshooting/two_black_animals_14bp/project_folder/project_config.ini')
    """
    ...
    column_names = ['VIDEO', 'VIDEO LENGTH (SECONDS)']
    ...
    for file_cnt, file_path in enumerate(config_obj.target_file_paths):
        ...
        print(f'Reading annotations for {file_name} ({file_cnt+1}/{len(config_obj.target_file_paths)})...')
        ...
        video_results = {'VIDEO': file_name, "VIDEO LENGTH (SECONDS)": len(df) / fps}
