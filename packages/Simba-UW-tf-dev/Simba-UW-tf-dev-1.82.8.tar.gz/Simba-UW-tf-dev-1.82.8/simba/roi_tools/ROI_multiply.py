...
def multiply_ROIs(config_path, filename):
    ...
    if not os.path.isfile(ROIcoordinatesPath):
        raise NoROIDataError(msg='Cannot multiply ROI definitions: no ROI definitions exist in SimBA project', source=multiply_ROIs.__name__)
    ...
    if (len(r_df) == 0 and len(c_df) == 0 and len(p_df) == 0):
        ...
        store['polygons'] = p_df
        store.close()
        stdout_success(msg=f'ROI(s) for {CurrVidName} applied to all videos', source=multiply_ROIs.__name__)
        print()
