...
def reset_video_ROIs(config_path, filename) -> None:
    """
    Helper to remove all ROI definitions from a specific video.

    :param config_path: Path to SimBA project config file in Configparser format
    :param filename: Video name to remove ROIs from.

    :example:
    >>> reset_video_ROIs(config_path='project_folder/project_config.ini', filename='Video_1')
    """
    ...
    if not os.path.isfile(ROIcoordinatesPath):
        raise NoROIDataError(msg='Cannot delete ROI definitions: no definitions exist to delete', source=delete_all_ROIs.__name__)
...
def delete_all_ROIs(config_path: str) -> None:
    """
    Helper to delete all ROI definitions from all videos in the SimBA project

    :param config_path: Path to SimBA project config file in Configparser format

    :example:
    >>> delete_all_ROIs(config_path='project_folder/project_config.ini')
    """
    ...
    def delete_file(config_path):
        ...
        if not os.path.isfile(roi_data_path):
            raise NoROIDataError(msg=f'No ROI definitions exist in this SimBA project. Expected file at path {roi_data_path}', source=delete_all_ROIs.__name__)
        else:
            os.remove(roi_data_path)
            close_window()
            stdout_trash(msg=f'SIMBA COMPLETE: All ROI definitions deleted in this SimBA project ({roi_data_path})', source=delete_all_ROIs.__name__)
