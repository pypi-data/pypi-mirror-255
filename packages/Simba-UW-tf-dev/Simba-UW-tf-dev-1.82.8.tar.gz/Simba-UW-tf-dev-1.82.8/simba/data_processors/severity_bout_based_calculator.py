...
class SeverityBoutCalculator(ConfigReader, FeatureExtractionMixin):
    ...
    def __calculate_movements(self, data_paths: List[str]):
        ...
        for file_cnt, file_path in enumerate(data_paths):
            ...
            if self.settings['clf'] not in df.columns:
                NoDataFoundWarning(msg=f'Skipping file {video_name} - {self.settings["clf"]} data not present in file', source=self.__class__.__name__)
                continue
    ...
    def save(self):
        ...
        stdout_success(msg=f'Bout severity data saved at {self.save_path}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def save_bin_definitions(self, data=dict):
        ...
        stdout_success(msg=f'Severity bracket definitions saved at {save_path}', source=self.__class__.__name__)
    ...
    def visualize(self):
        ...
        for idx, r in viz_df.iterrows():
            ...
            while current_frm < end_frame:
                ...
                if not ret:
                    raise InvalidVideoFileError(msg=f'Could not find frame {current_frm} in video {video_path}. Video {video_path} contains {video_meta_data["frame_count"]} frames.', source=self.__class__.__name__)
                writer.write(img)
                frm_cnt += 1
        ...
        stdout_success(msg=f'Videos complete: saved in {save_dir}', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
