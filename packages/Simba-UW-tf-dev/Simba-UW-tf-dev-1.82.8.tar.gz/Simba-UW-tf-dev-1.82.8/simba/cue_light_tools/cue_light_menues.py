...
from simba.utils.enums import Keys, Links, TagNames
...
class CueLightAnalyzerMenu(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        if len(self.shape_names) == 0:
            raise CountError(msg='SIMBA ERROR: Cue light analysis require ROI definitions. Please define ROIs before doing cue light analysis', source=self.__class__.__name__)
    ...
    def __visualize_cue_light_data(self):
        ...
        if len(self.cue_light_data_files) == 0:
            raise NoFilesFoundError(msg='SIMBA ERROR: Zero data files found. Please analyze cue light data prior to visualizing cue light data', source=self.__class__.__name__)
        else:
            self.__get_cue_light_names()
    ...
    def start_movement_analysis(self):
        ...
        if len(self.light_lst) == 0:
            raise CountError(msg='Zero user-specified cue lights. Please specify cue lights before analyzing movements', source=self.__class__.__name__)
