...
class CueLightClfAnalyzer(ConfigReader):
    ...
    def __init__(self,
        ...
        if len(self.files_found_cue_light) == 0:
            raise NoFilesFoundError(msg='SIMBA ERROR: No cue light data found. Please analyze cue light data before analyzing classifications based on cue light data', source=self.__class__.__name__)
    ...
    def save_data(self):
        ...
        stdout_success(msg='SIMBA COMPLETE: Cue light classifier statistics saved in project_folder/logs directory.', source=self.__class__.__name__)
