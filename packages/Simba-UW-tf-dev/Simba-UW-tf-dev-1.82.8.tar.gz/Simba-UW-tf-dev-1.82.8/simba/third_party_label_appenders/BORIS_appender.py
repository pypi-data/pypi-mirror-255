...
class BorisAppender(ConfigReader):
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.feature_file_paths):
            ...
            for clf in self.clf_names:
                ...
                if len(clf_annotations_start) != len(clf_annotations_stop):
                    raise ThirdPartyAnnotationEventCountError(video_name=self.video_name, clf_name=self.clf, start_event_cnt=len(clf_annotations_start), stop_event_cnt=len(clf_annotations_stop), source=self.__class__.__name__)
                ...
                if len(results) > 0:
                    raise ThirdPartyAnnotationOverlapError(video_name=self.video_name, clf_name=self.clf, source=self.__class__.__name__)
        ...
        stdout_success(msg='BORIS annotations appended to dataset and saved in project_folder/csv/targets_inserted directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
