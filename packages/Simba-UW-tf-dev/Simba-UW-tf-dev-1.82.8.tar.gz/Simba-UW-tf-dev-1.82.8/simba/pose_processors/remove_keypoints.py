...
class KeypointRemover(object):
    ...
    .. image:: _static/img/remove_keypoints.png
       :width: 300
       :align: center
    ...
    def __init__(self,
        ...
        if not os.path.isdir(data_folder):
            raise NotDirectoryError(msg='{} is not a valid directory.'.format(str(data_folder)), source=self.__class__.__name__)
    ...
    def run(self, animal_names: list, bp_to_remove_list: list):
        ...
        if (self.pose_tool == 'DLC') or (self.pose_tool == 'maDLC'):
            for file_cnt, file_path in enumerate(self.files_found):
                ...
                if self.file_format == 'csv':
                    ...
                    for body_part in bp_to_remove_list:
                        if body_part not in self.df.columns._levels[1]:
                            raise BodypartColumnNotFoundError(msg=f'{body_part} key point is not present in file {file_path}', source=self.__class__.__name__)
                ...
                if self.file_format == 'h5':
                    ...
                    try:
                        ...
                        raise InvalidFileTypeError(msg=f'{file_path} is not a valid maDLC pose-estimation file', source=self.__class__.__name__)
                    for (body_part, animal_name) in zip(bp_to_remove_list, animal_names):
                        for cord in ['x', 'y', 'likelihood']:
                            try:
                                ...
                                raise BodypartColumnNotFoundError(msg=f'Could not find body part {body_part} in {file_path}', source=self.__class__.__name__)
            ...
            stdout_success(msg=f'{str(len(self.files_found))} new data with {str(len(bp_to_remove_list))} body-parts removed saved in {save_directory} directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)