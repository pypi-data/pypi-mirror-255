...
from simba.ui.tkinter_functions import DropDownMenu, CreateLabelFrameWithIcon, Entry_Box, LabelFrame, Label, FolderSelect
...
class BooleanConditionalSlicerPopUp(PopUpMixin, ConfigReader):
    def __init__(self,
        ...
        self.select_input_dir_frm = CreateLabelFrameWithIcon(parent=self.main_frm, header='SELECT INPUT DIRECTORY', icon_name=Keys.DOCUMENTATION.value, icon_link=Links.AGGREGATE_BOOL_STATS.value)
        self.input_dir_select = FolderSelect(self.select_input_dir_frm, 'INPUT DIRECTORY:', lblwidth=15, initialdir=self.project_path, )
        self.select_folder_btn = Button(self.select_input_dir_frm, text='Confirm', fg='blue', command=lambda: self.show_table())
        self.select_input_dir_frm.grid(row=0, column=0, sticky='NW')
        self.input_dir_select.grid(row=0, column=0, sticky='NW')
        self.select_folder_btn.grid(row=1, column=0, sticky='NW')
    ...
    def show_table(self):
        self.input_dir = self.input_dir_select.folderPath.get()
        self.file_paths = glob.glob(self.input_dir + f'/*.{self.file_type}')
        check_if_filepath_list_is_empty(filepaths=self.file_paths, error_msg=f'No data found in format {self.file_type} in {self.input_dir}')
        data_df = read_df(file_path=self.file_paths[0], file_type=self.file_type)
        self.bool_cols = data_df.columns[data_df.apply(self._is_bool)]
        if len(self.bool_cols) < 2:
            raise CountError(msg=f'The data file {self.file_paths[0]} contains less than 2 boolean columns', source=self.__class__.__name__)
        ...
        self.rule_cnt_frm.grid(row=1, column=0, sticky='NW')
    ...
    def create_rules_frames(self, rules_cnt: int):
        ...
        self.rule_definitions_frame.grid(row=2, column=0, sticky='NW')
...
# BooleanConditionalSlicerPopUp(config_path='/Users/simon/Desktop/envs/troubleshooting/two_animals_16bp_032023/project_folder/project_config.ini')
