    ...
    def run(self, chunk_size=50):
        if self.include_key_points:
            ...
            if not os.path.isfile(self.data_df_path):
                raise NoFilesFoundError(msg=f'SIMBA ERROR: No keypoint data found in {self.data_df_path}. Untick key-point checkbox or import pose-estimation data.', source=self.__class__.__name__)
        ...
        stdout_success(msg=f'Anchored ROI video created at {self.save_video_path}', source=self.__class__.__name__)
