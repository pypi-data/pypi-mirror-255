...
class ProjectCreatorPopUp(PopUpMixin):
    ...
    def extract_frames(self):
        if not hasattr(self, 'config_path'):
            raise MissingProjectConfigEntryError(msg='Create PROJECT CONFIG before extracting frames', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        if len(list(set(target_list))) != len(self.clf_name_entries):
            raise DuplicationError(msg='All classifier names have to be unique.', source=self.__class__.__name__)
        for target in target_list:
            if ('/' in target) or ("\\" in target):
                raise InvalidInputError(msg=f'Classifier name {target} contains the slash character. Slash character not allowed. Remove the slash character to proceed.', source=self.__class__.__name__)
