...
class HeatmapLocationPopup(PopUpMixin, ConfigReader):
    ...
    def __create_heatmap_plots(self,
        ...
        if not self.multiprocessing_var.get():
            heatmapper_loc = HeatmapperLocationSingleCore(config_path=self.config_path,
            ...
            heatmapper_loc = HeatMapperLocationMultiprocess(config_path=self.config_path,
                                                            style_attr=style_attr,
                                                            final_img_setting=self.heatmap_last_frm_var.get(),
                                                            video_setting=self.heatmap_videos_var.get(),
                                                            frame_setting=self.heatmap_frames_var.get(),
        ...
        threading.Thread(target=heatmapper_loc.run).start()
