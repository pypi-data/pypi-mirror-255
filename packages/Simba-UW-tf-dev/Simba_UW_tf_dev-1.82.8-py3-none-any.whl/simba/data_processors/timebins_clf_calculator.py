...
class TimeBinsClfCalculator(ConfigReader):
    ...
    def __init__(self,
        ...
        if len(set(measurements).intersection(set(Options.TIMEBINS_MEASURMENT_OPTIONS.value))) == 0:
            raise InvalidInputError(msg=f'No valid input data measures. OPTIONS: {Options.TIMEBINS_MEASURMENT_OPTIONS.value}', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        stdout_success(msg=f'Classification time-bins results saved at project_folder/logs/output/{str("Time_bins_ML_results_" + self.datetime + ".csv")}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
