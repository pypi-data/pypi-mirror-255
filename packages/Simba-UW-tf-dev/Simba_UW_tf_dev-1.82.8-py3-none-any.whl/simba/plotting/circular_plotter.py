...
class CircularFeaturePlotter(ConfigReader, PlottingMixin, FeatureExtractionMixin):
    ...
    def _create_palette(self):
        c = self.create_single_color_lst(pallete_name='bwr', increments=360)
        self.colors = {}
    ...
    def run(self):
        ...
        while (self.cap.isOpened()):
            ...
            for animal_name, animal_bps in self.animal_bp_dict.items():
                ...
                print(self.loc_dict[animal_name]['degree_txt'], self.frame.shape)
        ...
        #     for animal_name, animal_bps in self.animal_bp_dict.items():
        #         animal_bp_cols = [item for pair in zip(animal_bps['X_bps'], animal_bps['Y_bps']) for item in pair]
        #         animal_bp_data = frm_data[animal_bp_cols].values.reshape(-1, 2).astype(np.float32)
        #         print(animal_bp_data)
        #
        #         minimum_bounding_rectangle = self.minimum_bounding_rectangle(points=animal_bp_data)
        #         print(minimum_bounding_rectangle)

                # hull_cords = jitted_hull(points=animal_bp_data, target='hull')
                # print(animal_bp_data.shape)
...
# settings = {'center': {'Animal_1': 'SwimBladder'}, 'text_settings': False}
#
