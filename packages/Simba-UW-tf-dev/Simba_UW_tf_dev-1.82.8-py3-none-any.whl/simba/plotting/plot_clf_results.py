...
class PlotSklearnResultsSingleCore(ConfigReader, TrainModelMixin, PlottingMixin):
    ...
    def create_visualizations(self):
        ...
        if len(self.data_df) != self.video_meta_data['frame_count']:
            FrameRangeWarning(msg=f'The video {self.video_path} contains {self.video_meta_data["frame_count"]} frames, while the data {self.file_path} contains {len(self.data_df)} frames.')
            ...
            try:
                if ret:
                    ...
                    if self.rotate:
                        ...
                        self.frame = PlottingMixin().rotate_img(img=self.frame).astype(np.uint8)
