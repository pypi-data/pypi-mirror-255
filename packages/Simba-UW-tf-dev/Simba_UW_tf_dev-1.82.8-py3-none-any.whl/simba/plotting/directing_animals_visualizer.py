...
class DirectingOtherAnimalsVisualizer(ConfigReader, PlottingMixin):
    ...
    def run(self):
        ...
        if self.video_name in list(self.video_data['Video']):
            ...
            NoDataFoundWarning(msg=f'SimBA skipping video {self.video_name}: No animals are directing each other in the video.', source=self.__class__.__name__)
    ...
    def __create_video(self):
        ...
        stdout_success(msg=f'Directionality video {self.video_name} saved in {self.directing_animals_video_output_path} directory', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
