...
def import_dlc_csv(config_path: Union[str, os.PathLike],
    ...
    for file_path in csv_files:
        ...
        if new_file_name_wo_ext in imported_file_names:
            raise FileExistError('SIMBA IMPORT ERROR: {} already exist in project. Remove file from project or rename imported video file name before importing.'.format(new_file_name), source=import_dlc_csv.__name__)
...
def import_single_dlc_tracking_csv_file(config_path: str,
                                        ...
                                        smoothing_time: int,
                                        file_path: str):
    log_event(logger_name=str(__name__), log_type=TagNames.CLASS_INIT.value, msg=ConfigReader(config_path=config_path, read_video_info=False).create_log_msg_from_init_args(locals=locals()))
    ...
    timer.stop_timer()
    stdout_success(msg=f'Imported {str(len(imported_file_paths))} pose estimation file(s)', elapsed_time=timer.elapsed_time_str, source=import_single_dlc_tracking_csv_file.__name__)
...
def import_multiple_dlc_tracking_csv_file(config_path: str,
    ...
    timer.stop_timer()
    stdout_success(msg=f'Imported {str(len(imported_file_paths))} pose estimation file(s)', elapsed_time=timer.elapsed_time_str, source=import_multiple_dlc_tracking_csv_file.__name__)
