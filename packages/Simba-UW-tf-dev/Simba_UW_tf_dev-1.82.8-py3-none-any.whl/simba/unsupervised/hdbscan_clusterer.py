...
class HDBSCANClusterer(UnsupervisedMixin):
    ...
    def fit(self,
        ...
        stdout_success(msg=f'{str(len(self.search_space) * len(self.embeddings.keys()))} saved in {self.save_dir}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def __save(self, data: dict) -> None:
        ...
        stdout_success(msg=f'Model {self.model_counter}/{len(self.search_space) * len(list(self.embeddings.keys()))} ({self.model[Unsupervised.HASHED_NAME.value]}) saved...', elapsed_time=self.model_timer.elapsed_time, source=self.__class__.__name__)
