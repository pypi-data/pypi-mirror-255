...
class ROIClfCalculator(ConfigReader):
    ...
    def __init__(self,
                 config_ini: Union[str, os.PathLike]):
    ...
    def __print_missing_roi_warning(self,
        ...
        """
        names = 'None'
        ROIWarning(msg=f'ROI named "{roi_name}" of shape type "{roi_type}" not found for video {self.video_name}. Skipping shape...', source=self.__class__.__name__)
        ...
        ROIWarning(msg=f'NOTE: Video {self.video_name} has the following {roi_type} shape names: {names}', source=self.__class__.__name__)
    ...
    def run(self,
        ...
        for file_cnt, file_path in enumerate(self.machine_results_paths):
            ...
            if shapes_in_video == 0:
                NoDataFoundWarning(msg='Skipping {self.video_name}: Video {self.video_name} has 0 user-defined ROI shapes.', source=self.__class__.__name__)
                continue
    ...
    def __organize_output_data(self):
        ...
        if len(self.results_dict.keys()) == 0:
            raise NoROIDataError(msg='ZERO ROIs found the videos represented in the project_folder/csv/machine_results directory', source=self.__class__.__name__)
        ...
        stdout_success(msg=f'Classification data by ROIs saved in {out_path}.', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
#
