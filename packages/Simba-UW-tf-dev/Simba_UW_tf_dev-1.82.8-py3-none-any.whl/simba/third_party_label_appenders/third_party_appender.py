...
class ThirdPartyLabelAppender(ConfigReader):
    ...
    def __init__(self,
        ...
        if self.settings['log']:
            self.create_third_part_append_logger(path=os.path.join(self.logs_path, f'BORIS_append_{self.datetime}.log'))
    ...
    def __check_annotation_clf_df_integrity(self, df=pd.DataFrame):
        ...
        if len(df[df['EVENT'] == 'START']) != len(df[df['EVENT'] == 'STOP']):
            if self.err_settings[Methods.THIRD_PARTY_EVENT_COUNT_CONFLICT.value] == Methods.WARNING.value:
                ...
                raise ThirdPartyAnnotationEventCountError(video_name=self.video_name, clf_name=clf_name,
                                                          start_event_cnt=len(df[df['EVENT'] == 'START']),
                                                          stop_event_cnt=len(df[df['EVENT'] == 'STOP']), source=self.__class__.__name__)
        ...
        if overlaps_idx:
            if self.err_settings[Methods.THIRD_PARTY_EVENT_OVERLAP.value] == Methods.WARNING.value:
                ...
                raise ThirdPartyAnnotationOverlapError(video_name=self.video_name, clf_name=clf_name, source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.feature_file_paths):
            ...
            print(f'Processing annotations for video: {self.video_name} ...')
            if self.video_name not in data.keys():
                ...
                if self.err_settings[Methods.THIRD_PARTY_ANNOTATION_FILE_NOT_FOUND.value] == Methods.ERROR.value:
                    raise ThirdPartyAnnotationFileNotFoundError(video_name=self.video_name, source=self.__class__.__name__)
            ...
            if additional_clfs and self.err_settings[Methods.ADDITIONAL_THIRD_PARTY_CLFS.value] == Methods.WARNING.value:
                ...
                raise ThirdPartyAnnotationsAdditionalClfError(video_name=self.video_name, clf_names=additional_clfs, source=self.__class__.__name__)
            ...
            for clf in self.clf_names:
                ...
                if len(clf_annot) == 0:
                    if self.err_settings[Methods.ZERO_THIRD_PARTY_VIDEO_BEHAVIOR_ANNOTATIONS.value] == Methods.WARNING.value:
                        print('s')
                        ThirdPartyAnnotationsMissingAnnotationsWarning(video_name=self.video_name, clf_names=[clf], log_status=self.settings['log'])
                        out_df[clf] = 0
                        continue
                    elif self.err_settings[Methods.ZERO_THIRD_PARTY_VIDEO_BEHAVIOR_ANNOTATIONS.value] == Methods.ERROR.value:
                        raise ThirdPartyAnnotationsMissingAnnotationsError(video_name=self.video_name, clf_names=[clf], source=self.__class__.__name__)
                ...
                if len(idx_diff) > 0:
                    if self.err_settings[Methods.THIRD_PARTY_FRAME_COUNT_CONFLICT.value] == Methods.WARNING.value:
                        ...
                        raise ThirdPartyAnnotationsOutsidePoseEstimationDataError(video_name=self.video_name,
                                                                                  ...
                                                                                  frm_cnt=out_df.index[-1],
                                                                                  first_error_frm=idx_diff[0],
                                                                                  ambiguous_cnt=len(idx_diff), source=self.__class__.__name__)
        ...
        stdout_success(msg=f'{self.app} annotations appended to dataset and saved in project_folder/csv/targets_inserted directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
...
#
#
# test = ThirdPartyLabelAppender(config_path='/Users/simon/Desktop/envs/troubleshooting/RatsSimba/project_folder/project_config.ini',
#                                data_dir='/Users/simon/Desktop/envs/troubleshooting/RatsSimba/Ethovision Scoring',
#                                settings=settings,
#                                app='ETHOVISION')
# test.run()
...
#                                data_dir='/Users/simon/Desktop/envs/troubleshooting/two_black_animals_14bp/BORIS',
#                                settings=settings,
#                                app='BORIS')
# test.run()
