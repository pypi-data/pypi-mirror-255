...
class FeatureExtractionCircularMixin(object):
    ...
    def rolling_kuipers_test(data: np.ndarray,
        ...
        >>> kuipers_statistics = FeatureExtractionCircularMixin().rolling_kuipers_test(data=data, time_windows=np.array([0.5, 5]), fps=2)
        """
        data = np.deg2rad(data)
        results = np.full((data.shape[0], time_windows.shape[0]), -1.0)
        for time_window_cnt in prange(time_windows.shape[0]):
...
# start = time.time()
# data = np.random.randint(low=0, high=360, size=(100000,)).astype(np.float64)
# result = FeatureExtractionCircularMixin().sliding_rao_spacing(data=data, time_window=0.5, fps=10)
...
#data = [0, 90, 180, 270]
data_1 = np.random.normal(loc=90, scale=2, size=20)
data_2 = np.random.normal(loc=180, scale=2, size=20)
data = np.hstack([data_1, data_2])
angle_data = FeatureExtractionCircularMixin().sliding_hodges_ajne(data=data, time_window=np.array([1]), fps=15)



#data = np.random.normal(loc=90, scale=2, size=20)
# data = np.random.randint(low=0, high=360, size=(10,)).astype(np.float64)
# angle_data = FeatureExtractionCircularMixin().rolling_kuipers_test(data=data, time_windows=np.array([0.5]), fps=2)
