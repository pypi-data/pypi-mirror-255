...
from simba.utils.warnings import ThirdPartyAnnotationsInvalidFileFormatWarning, BorisPointEventsWarning
...
def read_boris_annotation_files(data_paths: List[str],
    ...
    for file_cnt, file_path in enumerate(data_paths):
        ...
        try:
            ...
            point_event_cnt = len(df[df[STATUS] == POINT])
            if point_event_cnt > 0:
                BorisPointEventsWarning(msg=f'SimBA found {point_event_cnt} BORIS POINT events. SimBA is incompatible with POINT events and will remove these {point_event_cnt} entries from file {file_path}', source=read_boris_annotation_files.__name__)
                df = df[df[STATUS] != POINT].reset_index(drop=True)
            ...
            if error_setting == Methods.WARNING.value:
                ...
                raise InvalidFileTypeError(msg=f'{file_path} is not a valid BORIS file. See the docs for expected file format.', source=read_boris_annotation_files.__name__)
            else:
                pass
...
def read_ethovision_files(data_paths: List[str],
    ...
    for file_cnt, file_path in enumerate(data_paths):
        ...
        try:
            ...
            video_dir, video_name, ext = get_fn_ext(video_path)
            ...
            if error_setting == Methods.WARNING.value:
                ...
                raise InvalidFileTypeError(msg=f'{file_path} is not a valid ETHOVISION file. See the docs for expected file format.', source=read_ethovision_files.__name__)
            else:
                pass
...
def read_observer_files(data_paths: List[str],
    ...
    for file_path in data_paths:
        try:
            ...
            raise ColumnNotFoundError(file_name=file_path, column_name=', '.join(EXPECTED_FIELDS), source=read_observer_files.__name__)
        try:
            ...
            if error_setting == Methods.WARNING.value:
                ...
                raise InvalidFileTypeError(msg=f'{file_path} is not a valid OBSERVER file. See the docs for expected file format.', source=read_observer_files.__name__)
            else:
                pass
    ...
    for file_cnt, file_path in enumerate(data_paths):
        ...
        try:
            ...
            if error_setting == Methods.WARNING.value:
                ...
                raise InvalidFileTypeError(msg=f'{file_path} is not a valid SOLOMON file. See the docs for expected file format.', source=read_solomon_files.__name__)
            else:
                pass
    ...
    for file_cnt, file_path in enumerate(data_paths):
        ...
        try:
            ...
            if error_setting == Methods.WARNING.value:
                ...
                raise InvalidFileTypeError(msg=f'{file_path} is not a valid BENTO file. See the docs for expected file format.', source=read_bento_files.__name__)
            else:
                pass
    return dfs
    ...
    for file_cnt, file_path in enumerate(data_paths):
        ...
        try:
            ...
            if error_setting == Methods.WARNING.value:
                ...
                raise InvalidFileTypeError(msg=f'{file_path} is not a valid BORIS file. See the docs for expected file format.', source=read_deepethogram_files.__name__)
            else:
                pass
