...
class BentoAppender(ConfigReader):
    ...
    def run(self):
        check_all_file_names_are_represented_in_video_log(video_info_df=self.video_info_df, data_paths=self.feature_files)
        for file_cnt, file_path in enumerate(self.feature_files):
            ...
            if bento_path not in self.bento_files:
                raise AnnotationFileNotFoundError(video_name=self.video_name, source=self.__class__.__name__)
            ...
            self.__save()
        stdout_success(msg=f'Annotations for {str(len(self.saved_files))} video(s) and saved in project_folder/csv/targets_inserted directory.', source=self.__class__.__name__)
