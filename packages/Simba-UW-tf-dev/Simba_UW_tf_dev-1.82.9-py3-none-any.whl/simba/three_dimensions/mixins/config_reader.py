...
class ConfigReader(object):
    ...
    def __init__(self,
        ...
        self.frames_out_dir = os.path.join(self.project_path, 'frames', 'output')
        ...
        self.get_bp_headers()
        self.bp_col_names = self.x_cols + self.y_cols + self.z_cols + self.p_cols
        ...
        self.animal_bp_dict = self.create_body_part_dictionary(self.multi_animal_status, self.multi_animal_id_list, self.animal_cnt, self.x_cols, self.y_cols, self.z_cols, self.p_cols, self.clr_lst)
    ...
    def get_number_of_header_columns_in_df(self,
        ...
        for i in range(len(df)):
            try:
                ...
                pass
        raise DataHeaderError(msg='Could find the count of header columns in dataframe', source=self.__class__.__name__)
    ...
    def create_body_part_dictionary(self,
        ...
        if multi_animal_status:
            for animal in range(animal_cnt):
                ...
                animal_bp_dict[animal_id_lst[animal]]['Z_bps'] = [i for i in z_cols if animal_id_lst[animal] in i]
                if colors:
        ...
        if not multi_animal_status:
            if animal_cnt > 1:
                for animal in range(animal_cnt):
                    ...
                    search_string_z = f'_{str(animal + 1)}_z'
                    ...
                    animal_bp_dict[currAnimalName]['Z_bps'] = [i for i in z_cols if i.endswith(search_string_z)]
                    if colors:
                ...
                animal_bp_dict['Animal_1']['Z_bps'] = [i for i in z_cols]
                if colors:
    ...
    def get_body_part_names(self):
        ...
        """
        self.x_cols, self.y_cols, self.z_cols, self.p_cols = [], [], [], []
        for bp in self.body_parts_lst:
            self.x_cols.append(f'{bp}_x'); self.y_cols.append(f'{bp}_y'); self.z_cols.append(f'{bp}_z'); self.p_cols.append(f'{bp}_p')
    ...
    def get_bp_headers(self) -> None:
        ...
        self.bp_headers = []
        for bp in self.body_parts_lst:
            c1, c2, c3, c4 = (f'{bp}_x', f'{bp}_y', f'{bp}_z', f'{bp}_p')
            self.bp_headers.extend((c1, c2, c3, c4))
