...
class DrawPathPlot(object):
    def __init__(self,
        ...
        if self.line_clr_bgr == self.bg_clr_bgr:
            raise DuplicationError(msg='The line color and background color are identical', source=self.__class__.__name__)
        ...
        if ext.lower() == '.h5':
            ...
            self.data = read_df(file_path=data_path, file_type='csv', check_multiindex=True, multi_index_headers_to_keep=0)

        else:
            raise InvalidFileTypeError(msg=f'File type {str(ext)} is not supported (OPTIONS: h5 or csv)', source=self.__class__.__name__)
    ...
    def draw_video(self):
        ...
        while (self.cap.isOpened()):
            ...
            if ret == True:
                if frm_counter not in list(self.data.index):
                    print(f'SIMBA ERROR: The video contains {self.video_meta_data["frame_count"]} frames, but the data file contains data for {len(self.data)} frames.')
                    break
        ...
        stdout_success(msg=f'Path plot saved at {self.save_name}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
