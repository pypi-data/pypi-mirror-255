...
class ValidateModelOneVideoMultiprocess(ConfigReader,
    ...
    def __init__(self,
        ...
        self.clf_name = get_fn_ext(filepath=model_path)[1]
        ...
        self.temp_dir = os.path.join(self.single_validation_video_save_dir, self.clf_name, 'temp')
    ...
    def __create_video(self):
        ...
        print(f'Multiprocessing video using {self.cores} cores (follow progress in operating system terminal)...')
        ...
        pool.terminate()
        pool.join()
        stdout_success(msg=f'Video {self.feature_filename} complete', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
...
#
# test = ValidateModelOneVideoMultiprocess(config_path=r'/Users/simon/Desktop/envs/troubleshooting/dam_nest-c-only_ryan/project_folder/project_config.ini',
