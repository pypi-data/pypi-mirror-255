...
class TresholdPlotCreatorMultiprocess(ConfigReader, PlottingMixin):
    ...
    def __init__(self,
        ...
        if (not self.frame_setting) and (not self.video_setting) and (not self.last_frame):
            raise NoSpecifiedOutputError('SIMBA ERROR: Please choose to create either probability videos, frames, and/or last frame.', show_window=True, source=self.__class__.__name__)
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if self.probability_col not in data_df.columns:
                raise ColumnNotFoundError(column_name=self.probability_col, file_name=file_path, source=self.__class__.__name__)
        ...
        stdout_success(msg=f'Probability visualizations for {str(len(self.files_found))} videos created in project_folder/frames/output/gantt_plots directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
