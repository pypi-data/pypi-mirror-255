...
class CueLightAnalyzer(ConfigReader):
    ...
    def __init__(self,
        ...
        if len(cue_light_names) == 0:
            raise CountError(msg='SIMBA ERROR: Please select one or more cue lights', source=self.__class__.__name__)
        ...
        if len(self.files_found) == 0:
            raise NoFilesFoundError(msg='SIMBA ERROR: Zero tracking files detected in the "project_folder/csv/outlier_corrected_movement_location" directory', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if len(self.video_recs) + len(self.video_circs) + len(self.video_polys) == 0:
                NoDataFoundWarning(msg=f'No roi data found for video {self.video_name}. Skipping analysis of {self.video_name}...', source=self.__class__.__name__)
                continue
        ...
        stdout_success(msg=f'Analysed {str(len(self.files_found))} files. Data stored in project_folder/csv/cue_lights', elapsed_time=elapsed_time, source=self.__class__.__name__)
