        ...
        for file_path in self.files_found:
            ...
            if self.initial_import_multi_index:
                if len(df.columns) != len(self.bp_headers):
                    raise DataHeaderError(msg=f'SimBA found {len(df.columns)} data columns but expected {len(self.bp_headers)} in file {file_path}')
...
class AdvancedSmoother(ConfigReader):
    ...
    .. note::
       `Example notebook <https://simba-uw-tf-dev.readthedocs.io/en/latest/nb/advanced_smoothing_interpolation.html>`__.
