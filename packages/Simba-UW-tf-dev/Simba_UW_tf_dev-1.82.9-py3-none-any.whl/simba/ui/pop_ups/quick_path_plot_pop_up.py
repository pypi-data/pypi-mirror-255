...
from simba.utils.read_write import find_all_videos_in_directory, web_callback
...
class QuickLineplotPopup(PopUpMixin, ConfigReader):
    def __init__(self,
        ...
        if len(self.video_files) == 0: raise NoFilesFoundError(msg='SIMBA ERROR: No files detected in the project_folder/videos directory.', source=self.__class__.__name__)
        ...
        lbl = Label(self.main_frm, text='For more advanced path plots, \n click here for documentation', cursor="hand2", fg="blue")
        lbl.bind("<Button-1>", lambda e: web_callback(Links.PATH_PLOTS.value))
        ...
        self.video_lbl.grid(row=1, column=0, sticky=NW)
        self.chosen_video_dropdown.grid(row=1, column=1, sticky=NW)
        ...
        self.body_part_lbl.grid(row=2, column=0, sticky=NW)
        self.choose_bp_dropdown.grid(row=2, column=1, sticky=NW)
