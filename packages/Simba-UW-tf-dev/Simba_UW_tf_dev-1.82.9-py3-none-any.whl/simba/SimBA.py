...
from simba.utils.read_write import get_video_meta_data, find_all_videos_in_project
...
from simba.ui.pop_ups.annotation_cnt_pop_up import AnnotationCountPopUp
...
from simba.labelling.targeted_annotations_bouts import select_labelling_video_targeted_clips
from simba.labelling.targeted_annotations_frames import select_labelling_video_targeted_frames
...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        extract_frm_btn = Button(further_methods_frm, text='EXTRACT FRAMES FOR ALL VIDEOS IN SIMBA PROJECT', fg='blue', command= lambda: self.extract_frames_from_all_videos())
        ...
        self.scriptfile = FileSelect(labelframe_usrdef, 'Python script path:', file_types=[('Python .py file', '*.py')], initialdir=self.project_path)
        self.scriptfile.btnFind.configure(state='disabled')
        self.user_defined_var = BooleanVar(value=False)


        user_defined_script_lbl = Label(labelframe_usrdef, text='Click here for more info on user-defined feature extraction.', cursor="hand2", fg="blue")
        user_defined_script_lbl.bind("<Button-1>", lambda e: self.callback(Links.USER_DEFINED_FEATURE_EXTRACTION.value))
        ...
        label_adv_note_2.bind("<Button-1>", lambda e: self.callback(Links.ADVANCED_LBL.value))
        ...
        targeted_clip_annotator_note = Label(targeted_clip_annotator_frm,text='An annotator that creates annotated clips from videos associated with ML results.')
        targeted_clip_annotator_btn = Button(targeted_clip_annotator_frm, text='Select video',command= lambda: select_labelling_video_targeted_frames(config_path=self.config_path))
        ...
        count_annotations_btn = Button(lbl_tools_frm, text='Count annotations in project', fg='darkgreen', command=lambda: self._count_annotations_confirm())
        ...
        self.csvfile = FileSelect(label_model_validation, 'SELECT DATA FEATURE FILE', color='blue', lblwidth=30, file_types=[('SimBA CSV or PARQUET', Options.WORKFLOW_FILE_TYPE_STR_OPTIONS.value)], initialdir=self.features_dir)
        self.modelfile = FileSelect(label_model_validation, 'SELECT MODEL FILE', color='blue', lblwidth=30, file_types=[('SimBA MODEL SAV', '*.sav')], initialdir=self.project_path)
        ...
        labelframe_usrdef.grid(row=0, column=0, sticky=NW)
        user_defined_script_lbl.grid(row=0, column=0, sticky=NW)
        ...
        third_party_annotations_btn.grid(row=0, column=0, sticky=NW)
        remove_roi_features_from_annotation_set.grid(row=1, column=0, sticky=NW)
        visualize_annotation_img_btn.grid(row=0, column=1, sticky=NW)
        count_annotations_btn.grid(row=1, column=1, sticky=NW)
    ...
    def _count_annotations_confirm(self):
        option = TwoOptionQuestionPopUp(question=f'Are you sure you want to \n count your annotations in your SimBA project?', title='COUNT ANNOTATIONS IN SIMBA PROJECT', option_one='YES', option_two='NO', link=Links.COUNT_ANNOTATIONS_IN_PROJECT.value)
        if hasattr(option, 'selected_option') and option.selected_option.lower() in Options.PERFORM_FLAGS.value:
            _ = get_project_annotation_counts(config_path=self.config_path)
        else:
            pass

    def extract_frames_from_all_videos(self):
        video_paths = find_all_videos_in_project(videos_dir=self.video_dir, basename=False)
        option = TwoOptionQuestionPopUp( question=f'Are you sure you want to \n extract frames for all {len(video_paths)} video(s) in the SimBA project \n (potentially expensive for disk space and time) ?', title='EXTRACT FRAMES FOR ALL VIDOES', option_one='YES', option_two='NO')
        if hasattr(option, 'selected_option') and option.selected_option.lower() in Options.PERFORM_FLAGS.value:
            extract_frames_from_all_videos_in_directory(config_path=self.config_path, directory=self.video_dir)
        else:
            pass
    ...
    def train_single_model(self, config_path: Union[str, os.PathLike]):
        model_trainer = TrainRandomForestClassifier(config_path=config_path)
        model_trainer.perform_sampling()
        model_trainer.train_model()
        model_trainer.save_model()

    def train_multiple_models_from_meta(self, config_path: Union[str, os.PathLike]):
...
class App(object):
    def __init__(self):
        ...
        video_process_menu.add_cascade(label='Count labels in project', compound='left', image=self.menu_icons['count']['img'], command=lambda:AnnotationCountPopUp())
        ...
        links_menu.add_command(label='SimBA GitHub Repository', command=lambda: webbrowser.open_new(Links.GITHUB_REPO.value))
        links_menu.add_command(label='SimBA on OSF', command=lambda: webbrowser.open_new(Links.OSF_REPO.value))
        links_menu.add_command(label='Gitter Chatroom', command=lambda: webbrowser.open_new(Links.GITTER.value))
        ...
        self.txt = Text(self.frame, bg='white', insertborderwidth=2, height=32, width=100, yscrollcommand=y_sb)
        if currentPlatform == OS.WINDOWS.value:
...
class SplashMovie():
    def __init__(self):
        ...
        self.frames = self.preload_frames()
        half_width = int((self.parent.winfo_screenwidth() - width) // 2)
        half_height = int((self.parent.winfo_screenheight() - height) // 2)
        self.parent.geometry("%ix%i+%i+%i" % (width, height, half_width, half_height))
    ...
    def preload_frames(self):
        ...
        for frm_cnt in range(self.meta_['frame_count'] - 1):
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, frm_cnt)
            ret, frame = self.cap.read()
            if not ret: break
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGBA)
            frames.append(ImageTk.PhotoImage(image=PIL.Image.fromarray(frame)))

        return frames

    def show_animation(self):
        for frm_cnt, frame in enumerate(self.frames):
