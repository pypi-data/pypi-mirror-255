...
from simba.utils.enums import Dtypes, TagNames
...
class AnnotationFrameExtractor(ConfigReader):
    ...
    def __init__(self,
        ...
        self.clfs, self.settings = clfs, settings
    ...
    def run(self):
        ...
        for file_path in self.target_file_paths:
            ...
            for clf in self.clfs:
                ...
                for frm_cnt, frm in enumerate(annot_idx):
                    ...
                    ret, img = cap.read()
                    if not ret:
                        raise FrameRangeError(msg=f'Frame {str(frm+1)} is annotated as {clf} present. But frame {str(frm+1)} does not exist in video file {video_path}. The video file contains {video_meta_data["frame_count"]} frames.', source=self.__class__.__name__)
        ...
        stdout_success(msg=f'Annotated frames saved in {self.annotated_frm_dir} directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
