...
class UmapEmbedder(UnsupervisedMixin):
    ...
    def fit(self,
        ...
        stdout_success(msg=f'{len(self.search_space)} models saved in {self.save_dir} directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def __save(self, data: dict) -> None:
        ...
        stdout_success(msg=f'Model {self.model_count+1}/{len(self.search_space)} ({self.model[Unsupervised.HASHED_NAME.value]}) saved...', elapsed_time=self.model_timer.elapsed_time, source=self.__class__.__name__)
