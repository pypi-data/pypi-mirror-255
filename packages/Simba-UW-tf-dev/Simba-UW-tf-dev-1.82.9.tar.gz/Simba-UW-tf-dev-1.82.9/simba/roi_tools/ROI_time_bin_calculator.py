...
from simba.utils.enums import DirNames, TagNames
from simba.mixins.config_reader import ConfigReader
from simba.utils.read_write import get_fn_ext, read_df, write_df
from simba.utils.errors import ROICoordinatesNotFoundError

ENTRIES_COL_NAMES = ['VIDEO', 'SHAPE', 'ANIMAL', 'BODY-PART', 'TIME BIN', 'ENTRY COUNT']
TIME_COL_NAMES = ['VIDEO', 'SHAPE', 'ANIMAL', 'BODY-PART', 'TIME BIN', 'TIME INSIDE SHAPE (S)']
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            for animal_name, shape_name in list(itertools.product(self.animal_names, self.shape_names)):
                results_entries = pd.DataFrame(columns=ENTRIES_COL_NAMES)
                results_time = pd.DataFrame(columns=TIME_COL_NAMES)
                ...
                body_part_name = self.roi_analyzer.bp_dict[animal_name][0][:-2]
                ...
                for bin_cnt, bin_frms in enumerate(frame_bins):
                    ...
                    results_time.loc[len(results_time)] = [self.video_name, shape_name, animal_name, body_part_name, bin_cnt, len(frms_inside_roi_in_timebin) / fps]
                    results_entries.loc[len(results_entries)] = [self.video_name, shape_name, animal_name, body_part_name, bin_cnt, len(entry_roi_in_timebin)]
        ...
        self.out_time = pd.concat(self.out_time_lst, axis=0).sort_values(by=TIME_COL_NAMES[:-1])
        self.out_entries = pd.concat(self.out_entries_lst, axis=0).sort_values(by=ENTRIES_COL_NAMES[:-1])

    def save(self):
        self.out_entries.to_csv(self.save_path_entries)
        self.out_time.to_csv(self.save_path_time)
        self.timer.stop_timer()
        stdout_success(msg=f'ROI time bin entry data saved at {self.save_path_entries}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
        stdout_success(msg=f'ROI time bin time data saved at {self.save_path_time}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
...
#                             bin_length=50, body_parts=['Simon_Nose_1_1', 'JJ_Nose_1_2'], threshold=0.00)
# test.run()
# test.save()