...
class ExtractFeaturesFrom4bps(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            print(f'Feature extraction complete for {self.video_name} ({file_cnt + 1}/{len(self.files_found)}, elapsed time: {video_timer.elapsed_time_str})...')

        self.timer.stop_timer()
        stdout_success(msg='All features extracted. Results are stored in the project_folder/csv/features_extracted directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)

# test = ExtractFeaturesFrom4bps(config_path='/Users/simon/Desktop/envs/simba_dev/tests/data/test_projects/mouse_open_field/project_folder/project_config.ini')
# test.run()
