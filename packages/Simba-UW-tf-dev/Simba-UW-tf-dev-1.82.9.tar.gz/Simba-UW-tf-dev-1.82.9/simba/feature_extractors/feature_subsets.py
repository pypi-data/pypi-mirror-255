...
class FeatureSubsetsCalculator(ConfigReader,
    ...
    def run(self):
        ...
        stdout_success(msg='Feature sub-sets calculations complete!', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def append_to_data(self):
        for file_path in glob.glob(self.general_temp_folder + f'/*.{self.file_type}'):
            ...
            if self.append_to_features:
                ...
                if not check_same_number_of_rows_in_dfs(dfs=[data_df, new_df]):
                    ...
                    raise CountError(f'Cannot append feature subsets to features file of {video_name}. The files contain an unequal number of rows ({len(new_df)} vs {len(data_df)})', source=self.__class__.__name__)
                if len(check_if_headers_in_dfs_are_unique(dfs=[data_df, new_df])) > 0:
                    self.remove_multiple_folders(folders=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise DataHeaderError(msg=f'Attempted to append new feature data to features extracted file {features_path}. At least one of the new feature names aleady exist in the file.', source=self.__class__.__name__)
            ...
            if self.append_to_targets:
                ...
                if not check_same_number_of_rows_in_dfs(dfs=[data_df, new_df]):
                    self.perform_clean_up(directories=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise CountError(f'Cannot append feature subsets to targets file of {video_name}. The files contain an unequal number of rows ({len(new_df)} vs {len(data_df)})', source=self.__class__.__name__)
                if len(check_if_headers_in_dfs_are_unique(dfs=[data_df, new_df])) > 0:
                    self.perform_clean_up(directories=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise DataHeaderError(msg=f'Attempted to append new feature data to targets file of {target_path}. At least one of the nes feature names aleady exist in the file.', source=self.__class__.__name__)
        ...
        if self.checks_files:
            if self.append_to_features:
                ...
                self.data_df, _ = self.read_all_files_in_folder_mp_futures(annotations_file_paths=file_paths, file_type=self.file_type)
