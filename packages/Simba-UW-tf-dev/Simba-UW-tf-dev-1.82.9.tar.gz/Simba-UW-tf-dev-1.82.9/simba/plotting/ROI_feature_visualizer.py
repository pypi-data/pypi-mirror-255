    ...
    def run(self):
        ...
        while (self.cap.isOpened()):
            ...
            if ret:
                ...
                for _, row in self.video_polys.iterrows():
                    ...
                    if self.style_attr['ROI_ear_tags']:
                        for tag_data in row['vertices']:
                            cv2.circle(self.img_w_border, tuple(tag_data), self.circle_scale, row['Color BGR'], -1)
                for animal_name, shape_name in itertools.product(self.animal_ids, self.video_shapes):
                ...
                self.cap.release()
                self.writer.release()
                print(f'Feature video {self.video_name} saved in {self.save_path} directory ...(elapsed time: {self.timer.elapsed_time_str})')

        self.timer.stop_timer()
        self.cap.release()
        self.writer.release()
        stdout_success(f'Feature video {self.video_name} saved in {self.roi_features_save_dir} directory ...', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)

# style_attr = {'ROI_centers': True, 'ROI_ear_tags': True, 'Directionality': True, 'Border_color': (0, 128, 0), 'Pose_estimation': True}
# test = ROIfeatureVisualizer(config_path='/Users/simon/Desktop/envs/troubleshooting/two_black_animals_14bp/project_folder/project_config.ini',
