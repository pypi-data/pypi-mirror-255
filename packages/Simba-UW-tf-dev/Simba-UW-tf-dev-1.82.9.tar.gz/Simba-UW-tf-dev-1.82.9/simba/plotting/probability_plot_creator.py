...
class TresholdPlotCreatorSingleProcess(ConfigReader, PlottingMixin):
    ...
    def __init__(self,
        ...
        if (not self.frame_setting) and (not self.video_setting) and (not self.last_image):
            raise NoSpecifiedOutputError(msg='Please choose to either probability videos, probability frames, or both probability frames and videos.', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        stdout_success(msg=f'All probability visualizations created in project_folder/frames/output/probability_plots directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
