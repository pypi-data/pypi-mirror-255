...
class ShapAggregateStatisticsVisualizer(ConfigReader):
    ...
    def __init__(self,
        ...
        if (self.pose_setting != '14') and (self.pose_setting != '16'):
            ShapWarning('SHAP visualizations/aggregate stats skipped (only viable for projects with two animals and default 7 or 8 body-parts per animal) ...', source=self.__class__.__name__)
        else:
    ...
    def __calculate_agg_shap_scores(self):
        ...
        stdout_success(msg=f'Aggregate SHAP statistics saved in {self.shap_logs_path} directory', elapsed_time=self.agg_stats_timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def __insert_data_in_img(self):
        ...
        stdout_success(msg=f'SHAP summary graph saved at {self.img_save_path}', elapsed_time=self.visualization_timer.elapsed_time_str, source=self.__class__.__name__)
