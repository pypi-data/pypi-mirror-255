...
from simba.utils.printing import SimbaTimer, stdout_success, log_event
...
class MutualExclusivityCorrector(ConfigReader):
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.machine_results_paths):
            ...
            stdout_success(msg=f'Mutual exclusivity complete video {self.video_name}. ({file_cnt+1}/{len(self.machine_results_paths)})', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)

        self.timer.stop_timer()
        stdout_success(msg=f'Mutual exclusivity performed on {len(self.machine_results_paths)} file(s). Results are saved in the project_folder/csv/machine_results directory. Copies of files PRIOR to applying mutual exclusivity rules are saved in {self.save_dir}', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def highest_probability_determinator(self):
        ...
        if len(overlap_df) > 0:
            ...
            if len(identical_rows) > 0 and self.rule_data[SKIP_FILES_WITH_IDENTICAL]:
                IdenticalInputWarning(msg=f'Skipping rule {self.rule_cnt} for video {self.video_name} in frames {identical_rows}. Identical probability rows found and "SKIP ON EQUAL" selected...', source=self.__class__.__name__)
...
# rules = {1: {'rule_type': 'highest_probability', 'subordinates': ['Sniffing', 'Attack'], 'tie_breaker': 'Attack', 'skip_files_with_identical': True}}
