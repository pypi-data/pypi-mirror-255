...
class PiotrFeatureExtractor(ConfigReader,
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            white_animal_df = white_animal_df.replace(0.0, np.nan).interpolate(method='nearest', axis=0).ffill().bfill().astype(int)
            black_animal_df = black_animal_df.replace(0.0, np.nan).interpolate(method='nearest', axis=0).ffill().bfill().astype(int)
