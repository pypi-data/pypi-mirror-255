...
class DeepEthogramImporter(ConfigReader):
    ...
    def run(self):
        ...
        stdout_success(msg=f'Annotations for {str(len(list(self.clf_names)))} behaviors added to {len(self.matches_dict.keys())} videos and saved in the project_folder/csv/targets_inserted directory.', source=self.__class__.__name__)
