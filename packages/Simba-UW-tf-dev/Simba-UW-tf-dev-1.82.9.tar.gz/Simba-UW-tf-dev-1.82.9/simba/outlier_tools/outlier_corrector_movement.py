...
class OutlierCorrecterMovement(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        check_if_filepath_list_is_empty(filepaths=self.input_csv_paths, error_msg=f'No data found in {self.input_csv_dir} directory of your project. Import data to SimBA project before performing outlier correction.')
    ...
    def __save_log_file(self):
        ...
        stdout_success(msg='Log for corrected "movement outliers" saved in project_folder/logs', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
#
