...
def extract_frame_range(file_path: Union[str, os.PathLike],
    ...
    for frm_cnt, frm_number in enumerate(frame_range):
        ...
        print(f'Frame {frm_number} saved (Frame: {frm_cnt}/{len(frame_range)}, Video: {file_name})')
    stdout_success(msg=f'{str(len(frame_range))} frames extracted for video {file_name}. Saved in directory {file_dir}', source=extract_frame_range.__name__)
...
def clip_video_in_range(file_path: Union[str, os.PathLike],
                        ...
                        include_frame_numbers_in_filename: Optional[bool] = False,
                        gpu: Optional[bool] = False) -> None:
    """
    ...
    .. note::
       If both include_clip_time_in_filename and include_frame_numbers_in_filename, then the time will be included in the filename
       followed by the frame numbers. E.g., ``Together_1_00-00-00.000_00-00-00.033_0_1.mp4``.
    ...
    :parameter Optional[bool] include_clip_time_in_filename: If True, include the clip start and end in HH-MM-SS format as suffix in the filename. If False, then only ``_clipped`` suffix. Default False.
    :parameter Optional[bool] include_frame_numbers_in_filename: If True, include the clip start and end as suffix in the filename. If False, then only ``_clipped`` suffix. Default False.
    ...
    if (not include_clip_time_in_filename) and (not include_frame_numbers_in_filename):
        save_name = os.path.join(dir, file_name + '_clipped.mp4')
    elif include_clip_time_in_filename and (not include_frame_numbers_in_filename):
        save_name = os.path.join(dir, file_name + f'_{start_time.replace(":", "-")}_{end_time.replace(":", "-")}.mp4')
    else:
        frm_numbers = find_frame_numbers_from_time_stamp(start_time=start_time, end_time=end_time, fps=get_video_meta_data(video_path=file_path)['fps'])
        start, end = min(frm_numbers), max(frm_numbers)
        if include_clip_time_in_filename:
            save_name = os.path.join(dir, file_name + f'_{start_time.replace(":", "-")}_{end_time.replace(":", "-")}_{start}_{end}.mp4')
        else:
            save_name = os.path.join(dir, file_name + f'_{start}_{end}.mp4')
