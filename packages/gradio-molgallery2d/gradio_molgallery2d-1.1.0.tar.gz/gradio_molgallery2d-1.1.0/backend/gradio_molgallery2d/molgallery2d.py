...
@document()
class MolGallery2D(Component):
    """
    Used to display a list of 2D molecular structures as a gallery that can be scrolled through.
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            preview: If True, MolGallery2D will start in preview mode, which shows all of the images as thumbnails and allows the user to click on them to view them in full size. Only works if allow_preview is True.
            selected_index: The index of the image that should be initially selected. If None, no image will be selected at start. If provided, will set MolGallery2D to preview mode unless allow_preview is set to False.
    ...
    def postprocess(
        ...
        for img in value:
            ...
            if isinstance(img, (tuple, list)):
                img, caption = img
            if isinstance(img, Mol):
                img = self.draw_2D_SMILES(img)
    ...
    def draw_2D_SMILES(self, current_mol):
        d2d = rdMolDraw2D.MolDraw2DCairo(1200, 1200)
        d2d.drawOptions().padding = 0.0
        d2d.DrawMolecule(current_mol)
        d2d.FinishDrawing()
        current_img = Image.open(BytesIO(d2d.GetDrawingText()))
        #current_img = Draw.MolToImage(current_mol, size=(1200, 1200))
