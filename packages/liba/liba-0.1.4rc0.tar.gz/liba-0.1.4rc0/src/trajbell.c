...
{
    a_float v02, v12, tj, _2v0, _2v1, _2am, v0pv1, am2_jm, delta;
    ...
    if (ctx->ta * jm < _2am)
    {
        ctx->taj = a_float_sqrt(ctx->ta / jm);
        ctx->ta = 2 * ctx->taj;
        ctx->am = +jm * ctx->taj;
    }
    else
    {
        ctx->taj = am / jm;
        ctx->ta = ctx->taj + ctx->ta / am;
        ctx->am = +am;
    }
    ctx->td = vm - v1;
    if (ctx->td * jm < _2am)
    {
        ctx->tdj = a_float_sqrt(ctx->td / jm);
        ctx->td = 2 * ctx->tdj;
        ctx->dm = -jm * ctx->tdj;
    }
    else
    ...
    do {
        ...
        ctx->tdj = tj;
        am2_jm = am * am / jm;
        delta = am2_jm * am2_jm + 2 * (v02 + v12) + am * (4 * q - 2 * tj * v0pv1);
        ctx->ta = (am2_jm - _2v0 + a_float_sqrt(delta)) / _2am;
        ctx->td = (am2_jm - _2v1 + a_float_sqrt(delta)) / _2am;
        if (ctx->ta < 0)
        {
            ctx->ta = 0;
            ctx->taj = 0;
