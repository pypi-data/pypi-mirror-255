...
from Orange.data import StringVariable, Domain, Table, ContinuousVariable, DiscreteVariable
from Orange.widgets.data.owsave import OWSave
from Orange.widgets.data.owfile import OWFile
from Orange.widgets.hkh_pre_release.Chatbot import OWPythonScript
...
def get_widgets():
    ...
    for obj in gc.get_objects():
        if isinstance(obj, OWFile) and obj.captionTitle == "Pointer":
            widgets["File"] = obj
        if isinstance(obj, OWSave) and obj.captionTitle == "Sauvegarde":
            widgets["Save Data"] = obj
        if isinstance(obj, OWPythonScript) and obj.captionTitle == "Chatbot":
...
class Thread(QThread):
    # Send responses as Orange Table
    signal_output = pyqtSignal(Table, OWPythonScript)
    
    def __init__(self, parameters, widgets, parent=None):
    ...
    def build_prompt(self):
        prompts = []
        question = self.parameters["question"]
        preprompt = self.parameters["preprompt"]
        postprompt = self.parameters["postprompt"]
        n_sources = self.parameters["n_sources"]
        if self.parameters["database"] == "Oui":
            # Embeddings model
            embeddings = HuggingFaceEmbeddings(model_name=self.parameters["embeddings_model"])
            # Load database and get relevant documents
            db = FAISS.load_local(folder_path=self.parameters["data_path"], embeddings=embeddings)
            question_embedded = embeddings.embed_documents([question])[0]
            documents = db.similarity_search_with_score_by_vector(question_embedded, k=n_sources)
            # Split the distinct parts
            prompt = preprompt + "Voici des informations :\\n\\n"
            references = ""
            for i in range(len(documents)):
                document = documents[i]
                context = document[0].page_content
                source = document[0].metadata["source"]
                references += "SOURCE {}:".format(i+1) + source + "\\n\\n" + context + "\\n\\n\\n"
                prompt += "Info {}: ".format(i+1) + context + "\\n\\n"
            prompt += "\\nQuestion : " + question
            prompt +=  "\\n\\n" + postprompt
            # Build a prompt depending on the number of sources
            prompt = preprompt + "\\n\\n"
        else:
            references = ""
            prompt = preprompt + " " + question + "\\n" + postprompt
        prompts.append(prompt)
        # Add all the data to the Parameters dictionnary
        self.parameters["references"] = references
        prompts = [preprompt + question + "\\n" + postprompt]
        self.parameters["prompts"] = prompts
    
    def run_llm(self):
        answers = []
        # Init model
        print("Running...")
        llm = GPT4All(model_name=self.parameters["model"],
                      model_path=self.parameters["model"],
                      allow_download=False)
        for prompt in self.parameters["prompts"]:
            # Run LLM
        ...
        self.parameters["answers"] = answers
    
    def build_references(self):
        problems = self.parameters["problems"]
        solutions = self.parameters["solutions"]
        referencesFT = self.parameters["referencesFT"]
        answers = self.parameters["answers"]
        embeddings_model = SentenceTransformer(model_name_or_path=self.parameters["embeddings_model"])
        answer_3s = answers[0]
        answer_2s = answers[1]
        confidences_3s = []
        confidences_2s = []
        ...
        for i in range(len(problems)):
            references += "Fait Technique {0} : ".format(i+1) + problems[i] + "\\n"
            references += "Action GIAT {0} : ".format(i+1) + solutions[i] + "\\n"
            references += "Référence {0} : ".format(i+1) + referencesFT[i] + "\\n"
            embd_generated_3s = embeddings_model.encode(answer_3s, show_progress_bar=False)
            embd_generated_2s = embeddings_model.encode(answer_2s, show_progress_bar=False)
            embd_action_giat = embeddings_model.encode(solutions[i], show_progress_bar=False)
            confidence_3s = util.cos_sim(embd_generated_3s, embd_action_giat).item()*100
            confidence_2s = util.cos_sim(embd_generated_2s, embd_action_giat).item()*100
            confidences_3s.append(confidence_3s)
            confidences_2s.append(confidence_2s)
            references += "\\n\\n\\n"
        references += "\\n\\nRuntime: {:02f}s".format(time.time() - self.parameters["time"])
        references += "\\n\\nModel:" + self.parameters["model"]
        references += "\\n\\nFULL PROMPT:\\n--------------------------------------------------------------"
        references += self.parameters["prompts"][0]
        references += "\\n--------------------------------------------------------------"
        
        score_3s = max(confidences_3s)
        score_2s = max(confidences_2s)
        def categorize_score(score):
            if score < 40:
                ...
                confidence = "High"
            return confidence
        confidence_3s = categorize_score(score_3s)
        confidence_2s = categorize_score(score_2s)
        
        self.parameters["references"] = references
        self.parameters["confidence_3s"] = confidence_3s
        self.parameters["confidence_2s"] = confidence_2s

    def build_table(self):
        widgetSaveData = self.widgets["Save Data"]
        data = load_pkl(widgetSaveData.filename)
        question_fr_dom = StringVariable(name="Question FR")
        response_fr_dom_3s = StringVariable(name="Réponse FR")
        response_fr_dom_2s = StringVariable(name="Réponse FR - 2 sources")
        references_dom = StringVariable(name="References")
        confidence_dom_3s = DiscreteVariable(name="Confidence 3S", values=["Low", "Medium", "High"])
        confidence_dom_2s = DiscreteVariable(name="Confidence 2S", values=["Low", "Medium", "High"])
        domain = Domain([], metas=[question_fr_dom,
                                   response_fr_dom_3s,
                                   references_dom,])
        corp = Corpus.from_list(domain=domain,
                                rows=[[self.parameters["question"],
                                       self.parameters["answers"][0],
                                       self.parameters["references"],]])
        try:
            out_data = Corpus.concatenate([data, corp], axis=0)
        except (AttributeError, ValueError):
            out_data = corp
            print("concat error")
        titles = [line["Question FR"].value for line in out_data]
        out_data._titles = np.array(titles)
        out_data.text_features = [question_fr_dom,
                                  response_fr_dom_3s,
                                  references_dom]
        return out_data
    ...
    def run(self):
        ...
        self.parameters["references"] += "\\n\\nTIME: {:.2f}".format(self.t)
        out_data = self.build_table()
        self.signal_output.emit(out_data, widgets["Python Script"])

def send_table(table, widgetPythonScript):
    getattr(widgetPythonScript.Outputs, "data").send(table)


### Paths and directories management
widgets = get_widgets()
widgetFile = widgets["File"]
working_dir = os.path.split(widgetFile.loaded_file)[0]
model_dir = join(working_dir, "Models")
data_dir = join(working_dir, "Data")
db_dir = join(data_dir, "db")

model_path = join(model_dir, "Mistral-HKH.gguf")
embeddings_model_path = join(model_dir, "all-MiniLM-L6-v2")

parameters = dict()
parameters["data_path"] = db_dir
parameters["model"] = model_path
parameters["embeddings_model"] = embeddings_model_path


### Identification of entries : metaparameters | question | pre-prompt | post-prompt
for entry in in_datas:
    if "START BUTTON" in [str(var) for var in entry.domain]:
        metaparameters = entry
    else:
        questions = entry

for obj in in_objects:
    if obj.label == "Question":
        question = obj.to_string().split("content:")[-1].split("str_index")[0].strip()[1:-1]
    if obj.label == "Pre-prompt":
        preprompt = obj.to_string().split("content:")[-1].split("str_index")[0].strip()[1:-1]
    if obj.label == "Post-prompt":
        postprompt = obj.to_string().split("content:")[-1].split("str_index")[0].strip()[1:-1]


parameters["preprompt"] = preprompt
parameters["question"] = question
parameters["postprompt"] = postprompt


### Metaparameters preparation
try:
    max_length = int(metaparameters[0][metaparameters.domain.index("Longueur max")])
except:
    max_length = 4096
try:
    temperature = metaparameters[0][metaparameters.domain.index("Température")].value
except:
    temperature = 0.1
try:
    top_p = metaparameters[0][metaparameters.domain.index("Top P")].value
except:
    top_p = 0.1
try:
    top_k = int(metaparameters[0][metaparameters.domain.index("Top K")])
except:
    top_k = 40
try:
    prompt_batch = int(metaparameters[0][metaparameters.domain.index("Prompt batch")])
except:
    prompt_batch = 512
try:
    repeat_penalty = metaparameters[0][metaparameters.domain.index("Pénalité de répétition")].value
except:
    repeat_penalty = 1.1
try:
    token_repeat_penalty = int(metaparameters[0][metaparameters.domain.index("tokens pénalisés pour répétition")])
except:
    token_repeat_penalty = 64
try:
    n_sources = int(metaparameters[0][metaparameters.domain.index("Sources")])
except:
    n_sources = 2
try:
    database = metaparameters[0][metaparameters.domain.index("Database")]
except:
    database = "Non"
try:
    start_button = metaparameters[0][metaparameters.domain.index("START BUTTON")]
except:
    start_button = "PLAY"
...
parameters["prompt_batch"] = prompt_batch
parameters["repeat_penalty"] = repeat_penalty
parameters["token_repeat_penalty"] = token_repeat_penalty
parameters["n_sources"] = n_sources
parameters["database"] = database
parameters["database"] = "Non"


### Main loop
if start_button == "PLAY" and question is not None:
    llm_thread = Thread(parameters, widgets)
    llm_thread.signal_output.connect(send_table)
    llm_thread.start()
