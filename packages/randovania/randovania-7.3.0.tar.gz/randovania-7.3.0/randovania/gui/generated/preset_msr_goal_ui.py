...
class Ui_PresetMSRGoal(object):
    ...
    def retranslateUi(self, PresetMSRGoal):
        ...
        self.description_label.setText(QCoreApplication.translate("PresetMSRGoal", u"<html><head/><body><p>In addition to just collecting the Baby, it is now necessary to collect Metroid DNA in order to reach Ridley. The minimum and maximum are limited to 0 and 39 DNA. The game starts with 39 DNA minus the shuffled amount.</p></body></html>", None))
        ...
        self.restrict_placement_label.setText(QCoreApplication.translate("PresetMSRGoal", u"<html><head/><body><p>The following options limit where Metroid DNA will be placed. There can only be as many DNA shuffled as there are preferred locations enabled. The first option adds 25 preferred locations, the second adds 14, and the third adds 4. In Multiworlds, DNA is guaranteed to be in your World.</p></body></html>", None))
