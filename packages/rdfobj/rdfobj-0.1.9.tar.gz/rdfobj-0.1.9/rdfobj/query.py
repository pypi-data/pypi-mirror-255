...
# fix order /union => DONE
# add connectedWith partie droite +OR []. ex : left or right from a conversion => OK
# tb.use_subclassof=True ==> error => OK
## TODO:
# add "or" for EntityNode
# partie gauche de connectedWith (list) . ex: 2 proteines mb d un meme complexe
...
select ?s ?right ?o ?participant ?s2
where {
?s ?right ?o .
?o ?participant ?s2 .
{ ?o a bi:PhysicalEntity }  .
{ ?s bi:displayName ?displayName }.
{ ?s a bi:Control }  { ?s2 a bi:Catalysis }
FILTER (  ?left__p_e0 =  bi:left  ) .
FILTER (  ?right =  bi:right  ) .
FILTER ( STRSTARTS(STR(?displayName),'ADPGK')  ) .
FILTER ( ?displayName  =  'ADPGK2'  || ?displayName  =  'ADPGK3'   ) .
FILTER (  ?participant =  bi:participant  ) .
}
...
class QueryBuilder():
  ...
  def defineVarTagMap(self,tb,g):
    ...
    for nid in nodes:
      ...
      # instance type extraction from futur query
         ...
         if isinstance(element,EntityNode):
          ...
          #  assol = element.association
           for key, value in element.associationDict.items():
              # print(value)
              assol = value
          #  assol = element.associationDict.get(element.label)
