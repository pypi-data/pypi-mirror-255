...
def build_parser() -> argparse.ArgumentParser:
    ...
    classify_parser = subparsers.add_parser(
        "classify",
        help="""Deprecated alias for dux4r""",
        add_help=False,
    )
    populate_classify_parser(dux4r_parser)
    populate_classify_parser(classify_parser)
    return parser
...
class CliController:
    ...
    def dispatch(self, args: List[str]) -> None:
        ...
        if parsed_args.action == "version":
            ...
            if parsed_args.action == "classify":
                default_formatting = warnings.formatwarning
                warnings.formatwarning = custom_warning_formatting  # type: ignore
                warnings.warn(
                    message="Command `classify` is deprecated. Please use `dux4r` instead",
                    category=FutureWarning,
                )
                warnings.formatwarning = default_formatting
