...
class DoctorMaster(BaseModel):
    ...
    @classmethod
    def db_update_or_create(cls, data, pharmacy_doctor_id, company_id, add_fields=None, remove_fields=None,
        ...
        """ Takes data from the file and updates the data if exists else creates new record.
                ...
                Args:
                    ...
                    pharmacy_doctor_id: Value against which record will be identified uniquely.
        ...
        try:
            record, created = DoctorMaster.get_or_create(
                pharmacy_doctor_id=pharmacy_doctor_id,
                company_id=company_id,
                defaults=data
            )
            if not created:
                if record.pharmacy_doctor_id == data['pharmacy_doctor_id'] and record.workphone == data['workphone'] and record.first_name == data['first_name'] and record.last_name == data['last_name'] and record.created_by == data['created_by'] and record.modified_by == data['modified_by']:
                    return record
                else:
                    DoctorMaster.update(**data) \
                        .where(DoctorMaster.pharmacy_doctor_id == pharmacy_doctor_id,
                               DoctorMaster.company_id == company_id).execute()
    ...
    @classmethod
    def concated_doctor_name_field(cls, sep=', '):
