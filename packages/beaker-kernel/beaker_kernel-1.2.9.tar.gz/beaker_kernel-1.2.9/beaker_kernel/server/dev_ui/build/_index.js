...
window.__webpack_public_path__ = URLExt.join(PageConfig.getBaseUrl(), 'example/');
...
import '../index.css';
import { Toolbar as AppToolbar, CommandToolbarButton, SessionContextDialogs } from '@jupyterlab/apputils';
...
import { ExecutionIndicator, NotebookModelFactory, NotebookPanel, NotebookWidgetFactory, ToolbarItems } from '@jupyterlab/notebook';
...
async function createApp(manager) {
    ...
    const handleMessage = (context, msg) => {
        ...
        if (msg_type === "stream" && ((_a = msg.parent_header) === null || _a === void 0 ? void 0 : _a.msg_type) == "llm_request") {
    ...
    void sessionContext.ready.then(() => {
        ...
        session === null || session === void 0 ? void 0 : session.iopubMessage.connect(handleMessage);
    });
    ...
    const setKernelContext = (context_info) => {
        ...
        const messageBody = {
            session: (session === null || session === void 0 ? void 0 : session.name) || '',
            channel: 'shell',
            content: context_info,
            msgType: 'context_setup_request',
            msgId: `${kernel.id}-setcontext`
        };
        const message = createMessage(messageBody);
        kernel === null || kernel === void 0 ? void 0 : kernel.sendShellMessage(message);
    };
    const sendLLMQuery = (query) => {
        const session = sessionContext.session;
        const kernel = session === null || session === void 0 ? void 0 : session.kernel;
        if (kernel) {
    ...
    const sendCustomMessage = (channel, msgType, content) => {
        const session = sessionContext.session;
        const kernel = session === null || session === void 0 ? void 0 : session.kernel;
        if (kernel) {
