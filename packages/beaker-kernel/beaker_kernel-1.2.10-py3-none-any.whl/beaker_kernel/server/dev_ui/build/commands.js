...
import { addIcon, copyIcon, cutIcon, deleteIcon, fastForwardIcon, pasteIcon, refreshIcon, runIcon, saveIcon, stopIcon } from '@jupyterlab/ui-components';
...
export const setupCommands = (commands, palette, nbWidget, handler, sessionContextDialogs) => {
    // Add commands.
    ...
    commands.addCommand(COMMAND_IDS.runAndAdvance, {
        ...
        execute: () => {
            return NotebookActions.runAndAdvance(nbWidget.content, nbWidget.context.sessionContext, sessionContextDialogs);
        }
    });
    commands.addCommand(COMMAND_IDS.run, {
        label: 'Run',
        execute: () => {
            return NotebookActions.run(nbWidget.content, nbWidget.context.sessionContext, sessionContextDialogs);
        }
    });
    ...
    commands.addCommand(COMMAND_IDS.restartAndRun, {
        ...
        execute: () => {
            void sessionContextDialogs
                ...
                .then(restarted => {
                if (restarted) {
                    void NotebookActions.runAll(nbWidget.content, nbWidget.sessionContext, sessionContextDialogs);
                }
                return restarted;
            });
        }
