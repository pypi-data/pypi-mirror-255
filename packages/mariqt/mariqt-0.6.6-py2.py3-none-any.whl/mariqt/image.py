...
def writeUUIDtoPhoto(path, overwrite_original=False):
	...
	if overwrite_original:
		command = [miqtv.exiftool_path+"exiftool", "-overwrite_original", "-imageuniqueid=" + uuid, path]
	else:
		if os.path.isfile(path + '_original'):
			raise Exception("writeUUIDtoPhoto: original file <file>_original already exists.")
		command = [miqtv.exiftool_path+"exiftool", "-imageuniqueid=" + uuid, path]
...
def writeUUIDtoVideo(path,overwrite_original=False,test_val=""):
        """ Uses XMP Toolkit to write UUID to copy of file in XMP namespace Darvin Core metadata field 'identifier'. 
