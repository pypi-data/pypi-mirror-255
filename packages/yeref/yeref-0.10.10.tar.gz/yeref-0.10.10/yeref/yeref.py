...
async def pst_gen_ent(bot, chat_id, state, lc, lz, CONTENT_TYPE, ENT_TID, ENT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D, EXTRA_D, prompt='', ENT_TOKEN_=None):
    result = None
    try:
        ...
        if CONTENT_TYPE == 'text':
        ...
        elif CONTENT_TYPE in ['photo', 'gif', 'sticker', 'video', 'telescope']:
            ...
            print(f"{POST_LNK=}")
            await state.update_data(POST_TYPE=CONTENT_TYPE, POST_LNK=POST_LNK, POST_FILENAME=os.path.basename(dst),
                                    POST_FID=POST_FID, POST_FIDNOTE=POST_FIDNOTE)
        else:
            lst = await outsource_handle({'type': 'txt', 'prompt': prompt_txt}, openai_keys)
            if not len(lst): return
            ...
            await state.update_data(POST_TYPE=CONTENT_TYPE, POST_FILENAME=os.path.basename(dst), POST_FID=POST_FID)
