...
tids_not_for_trans = [5134596871, 5152320320, 5200969162, 5240041587, 5277505344, 5298580533, 5300792695, 5304873693, 5318482236, 5372068352, 5380838469, 5456863561, 5588781655, 5621199443, 5805890320, 5846446387, 5850795580, 5868482417, 5951232918, 5962132892, 5969907012]
...
async def bots_by_inline(chat_id, message, BASE_D):
    ...
    try:
        ...
        data = [['👩🏽‍💻 @FereyDemoBot', l_inline_demo[lz], "<a href='https://t.me/FereyDemoBot?start=inline'>@FereyDemoBot</a>"],
                ['👩🏽‍💻 @FereyBotBot', l_inline_bot[lz], "<a href='https://t.me/FereyBotBot?start=inline'>@FereyBotBot</a>"],
                ['👩🏽‍💻 @FereyPostBot', l_inline_post[lz], "<a href='https://t.me/FereyPostBot?start=inline'>@FereyPostBot</a>"],
                ['👩🏽‍💻 @FereyMediaBot', l_inline_media[lz], "<a href='https://t.me/FereyMediaBot?start=inline'>@FereyMediaBot</a>"],
                ['👩🏽‍💻 @FereyChannelBot', l_inline_channel[lz], "<a href='https://t.me/FereyChannelBot?start=inline'>@FereyChannelBot</a>"],
                ['👩🏽‍💻 @FereyGroupBot', l_inline_group[lz], "<a href='https://t.me/FereyGroupBot?start=inline'>@FereyGroupBot</a>"],
                ['👩🏽‍💻 @FereyFindBot', l_inline_find[lz], "<a href='https://t.me/FereyFindBot?start=inline'>@FereyFindBot</a>"],
                ['👩🏽‍💻 @FereyAIBot', l_inline_ai[lz], "<a href='https://t.me/FereyAIBot?start=inline'>@FereyAIBot</a>"],
                ['👩🏽‍💻 @FereyStickerBot', l_inline_sticker[lz], "<a href='https://t.me/FereyStickerBot?start=inline'>@FereyStickerBot</a>"],
                ['👩🏽‍💻 @FereyTargetBot', l_inline_target[lz], "<a href='https://t.me/FereyTargetBot?start=inline'>@FereyTargetBot</a>"],
                ['👩🏽‍💻 @FereyUserBot', l_inline_user[lz], "<a href='https://t.me/FereyUserBot?start=inline'>@FereyUserBot</a>"],
                ['👩🏽‍💻 @FereyToolsBot', l_inline_tools[lz], "<a href='https://t.me/FereyToolsBot?start=inline'>@FereyToolsBot</a>"],
                ['👩🏽‍💻 @FereyWorkBot', l_inline_work[lz], "<a href='https://t.me/FereyWorkBot?start=inline'>@FereyWorkBot</a>"], ]
...
async def pst_gen_ent(bot, chat_id, state, lc, lz, POST_TYPE, ENT_TID, ENT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D, EXTRA_D, prompt='', ENT_TOKEN_=None):
    result = None
    try:
        if prompt in ['➡️️ Next', '/Next']: prompt = ''
        BASE_ENT = os.path.join(MEDIA_D, str(ENT_TID), f"{str(ENT_TID)}.db")
        prompt_txt, prompt_img = await train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, BASE_ENT, lz, prompt, ENT_TOKEN_)
...
# region blocks
async def recursion_blocks(app, tgr_cmd, BOT_TID_TMP, BOT_USERNAME, BASE_ENT, msg_lst, lc, deep_cnt=0, next_items=None,
                           target_click=None):
    ...
    try:
        ...
        msg_lst, next_items, clicks, messages, out_vid = await bypass_messages(app, lc, BOT_USERNAME, BASE_ENT, msg_lst,
                                                                               next_items, clicks, out_vid)
        msg_lst, next_items, clicks, messages, out_vid = await bypass_messages(app, lc, BOT_USERNAME, BASE_ENT, msg_lst,
                                                                               next_items, clicks, out_vid)
                ...
                for click in clicks.get(message.id, []):
                    ...
                    while True:
                        try:
                            ...
                            await recursion_blocks(app, tgr_cmd, BOT_TID_TMP, BOT_USERNAME, BASE_ENT, msg_lst, lc,
                                                   deep_cnt, next_items)
                            deep_cnt -= 1
                            ...
                            if 'Request timed out' in str(e) and cnt >= 0:
                                ...
                                try:
                                    ...
                                    await recursion_blocks(app, tgr_cmd, BOT_TID_TMP, BOT_USERNAME, BASE_ENT, msg_lst,
                                                           lc, deep_cnt, next_items, click)
                                    deep_cnt -= 1
                                    print('end2')
                                    break
                                except Exception as e:
...
async def bypass_messages(app, lc, BOT_USERNAME, BASE_ENT, msg_lst, next_items, clicks, out_vid):
    messages = []
    try:
...
async def export_template(bot, chat_id, lz, BOT_TID, BOT_USERNAME, is_premium_owner_=False):
    result = None
    try:
    ...
    try:
        ...
        for item in d_trgs:
            ...
            if len([it for it in TRG_CONTENT if
                    it.get('name') == 'cmd' and it.get('value') == '/start' and it.get('utm') == '' and it.get(
                        'link') == '']):
...
async def get_x_y_for_vid(state_file, width, height, from_x=0, from_y=0, from_w=0, from_h=0, to_vid=0, to_w=0, to_h=0):
    ...
    try:
        try:
            ...
            vids = {}
        if not len(vids) or not from_x: return
            ...
            try:
                ...
                if not result or os.path.getsize(FILE_NAME) < 5242880:
                    ...
                    res = await extra_bot.send_audio(chat_id=OWNER_TID, audio=types.FSInputFile(file_name_video),
                                                     thumbnail=thumb, title=title, performer=performer, disable_notification=True)
                    await extra_bot.delete_message(OWNER_TID, res.message_id)
                    # if os.path.exists(file_name_video): os.remove(file_name_video)
...
async def item_to_static_sticker(bot, chat_id, input_file, PACK_TYPE, PACK_KIND, is_upload=True, is_circle=False, is_rbg=False):
    ...
    try:
        ...
        if input_file.lower().endswith('.webp') and os.path.getsize(input_file) <= size_kb:
            ...
            if PACK_TYPE == 'emoji' and width == 100 and height == 100:
                upl = await bot.upload_sticker_file(user_id=chat_id, sticker=types.FSInputFile(input_file), sticker_format=sticker_format)
        ...
        if is_upload:
            upl = await bot.upload_sticker_file(user_id=chat_id, sticker=types.FSInputFile(file_png), sticker_format=sticker_format)
            result_upl = upl.file_id
        else:
            result = await bot.send_sticker(chat_id, types.FSInputFile(file_png))
            result = result.sticker.file_id
        print(f"result_upl ok")
        # res = await bot.send_sticker(chat_id, types.FSInputFile(file_png))
        # await bot.delete_message(chat_id, res.message_id)
...
async def item_to_dynamic_sticker(bot, chat_id, input_file, bot_username, MEDIA_D, PACK_TYPE, PACK_KIND, is_upload=True, is_circle=False, is_rbg=False):
    result = result_upl = None
    file_jpg = input_file[:input_file.rfind('.')] + '_.jpg'
    ...
    try:
        ...
        # sticker = types.InputSticker(sticker=result, emoji_list=['👩🏽‍💻'])
        # await bot.add_sticker_to_set(user_id=chat_id, name=name, sticker=sticker)
        # get_sticker_set2_ = await bot.get_sticker_set(name=name)
        # res2 = get_sticker_set2_.stickers[-1].file_id
        # await bot.send_sticker(chat_id, res2)
    except TelegramRetryAfter as e:
        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
        await asyncio.sleep(e.retry_after + 1)
    except Exception as e:
...
async def squared_video_ffmpeg(file_name, MEDIA_D):
    ...
    try:
        ...
        ffmpeg_command = [
            ...
            '-i', file_name,
            '-filter_complex_threads', '2',
            '-vf', f'scale={min_side}:{min_side},setsar=1:1',
            '-r', '30',
            '-s', f'{min_side}x{min_side}',
            '-vf', f'crop={min_side}:{min_side}:{center_x}:{center_y}',
            '-t', '59',
            '-y',
            file_video_note
        ]
...
async def photo_to_video_sticker(bot, user_id, input_file, bot_username):
    ...
    try:
        ...
        await bot.create_new_sticker_set(user_id=user_id, name=name, title=f"{user_id}", stickers=[sticker], sticker_format='video')
                        ...
                        while cnt >= 0:
                            ...
                            try:
                                ...
                                if item['prompt'] == 'variation':
                                    ...
                                    res = await client.images.generate(prompt=item['prompt'], model=f"dall-e-{model_num}",
