...
def list_accounts(
    ...
    """List active AWS accounts in the organization."""
...
def list_ou_accounts(
    ...
    while len(current_ous) > 0:
        for ou in current_ous:
            for page in paginator.paginate(
                ...
                current_ous = [child['Id'] for child in page['Children']]
    ...
    print(f'Found {len(accounts)} accounts in {ou_id}...')
...
def make_account_list(
    ...
    """Make a list of AWS accounts from a comma-separated string of IDs."""
...
def main():
    ...
    myparser.add_argument(
        ...
        '--accounts',
        action='store',
        help='[OPTIONAL: default = None] Comma-separated list of account IDs',
        required=False,
        default=None,
        type=str
    )
    ...
    if accounts is not None:
        account_ids = make_account_list(accounts)
    elif ou_id is not None:
        account_ids = list_ou_accounts(session, ou_id)
    else:
        account_ids = list_accounts(session)
