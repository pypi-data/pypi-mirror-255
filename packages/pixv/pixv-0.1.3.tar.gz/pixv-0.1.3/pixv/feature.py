...
def revert_by_one_save_imageGraph(image_path:str):
    ...
    _img_to_be_saved=_imageGraph_instance.return_np_image_at_head()
...
def revert(image_path):
    '''
    shifts head location and modifies the image at image path
    '''
...
def original(image_path):
    '''
    doesnt shift the head, just shows the original image
    '''
...
def shift(image_path,id):
    ...
    if image_path is None or not os.path.isfile(image_path):
       ...
       return
        ...
        _name_graph=image_path.split('/')[-1].split('.')[0]
        _gr=load_graph(_name_graph)
        _img_to_be_saved=_gr.return_np_image_at_head()
